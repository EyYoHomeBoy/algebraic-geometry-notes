...
\begin{example}\label{SubMonoidsExample}
    \begin{enumerate}
        ...
        \item Once constructed, $(\Z,+),(\Z,\cdot),(\Q,+),(\Q,\cdot),(\R,+),(\R,\cdot),(\C,+),(\C,\cdot)$ are monoids.
        \item Let $A$ be a set. Consider $\mathrm{Set}(A,A) := \{f:A\rightarrow A\}$. This a monoid under function composition.
        \item Given a non-empty set $X$ and a monoid $M$ the set 
        $$
            \mathrm{Set}(X,M)
        $$
        with the binary operation
        \begin{gather*}
            \mathrm{Set}(X,M)\times \mathrm{Set}(X,M) \rightarrow \mathrm{Set}(X,M)\\
            (f,g)\mapsto fg
        \end{gather*}
        where for $f,g\in \mathrm{Set}(X,M)$
        \begin{gather*}
            fg : X\rightarrow M\\
            x \mapsto f(x)g(x)
        \end{gather*}
        Indeed, for each $x\in X$ the diagram
        $$
            \begin{tikzcd}
                \mathrm{Set}(X,M)\times \mathrm{Set}(X,M) \times \mathrm{Set}(X,M)  \arrow[r,"\ev_x\times \ev_x\times \ev_x"] &M\times M \times M \arrow[r,"\fone_M\times \circ"] \arrow[d,"\circ \times \fone_M",swap] & M\times M \arrow[d,"\circ"]\\
                 & M\times M \arrow[r,"\circ"] & M
            \end{tikzcd}
        $$
        commutes
        Moreover, for the function $e : X \rightarrow M$, mapping every element in $X$ to $e_M$ we have that 
        $$
...
\end{remark}
\begin{example}
    By a prior example (cf. Example~\ref{SubMonoidsExample}) we have seen that $\mathrm{Set}(M,N)$ is a monoid. $\mathrm{Mon}(M,N)\subset \mathrm{Set}(M,N)$ is a submonoid if $N$ is commutative. Indeed, for $f,g\in \mathrm{Mon}(M,N)$ and $x,y\in M$. Then 
...
$$
    \prod_{x\in E} := \left(\prod_{m\in E\setminus \{m_n\}} m \right)m_n
$$
...
\begin{definition}
    ...
    $$gg^{-1}=g^{-1}g = e.$$
    For $g\in G$ we refer to $g^{-1}$ as the \textit{inverse of $g$}. The data specifying a group is also often written as the tuple $(G,\circ,e)$ or $(G,\circ)$.
\end{definition}
\begin{remark}
    Not that for $g\in G$ the inverse $g^{-1}$ is the mutually inverse morphism of $g$, when we interpret $G$ as the groupoid $\mathrm{B}G = (\{G\},G)$. For the same reason we have that 
    $$
...
\end{remark}
\begin{definition}
    A group $(G,+)$ is called \textit{abelian} or \textit{additive}, if it is also a commutative monoid with respect to $+$. We denote the inverse of $g\in G$ with respect to addition by $-g$, and for $g_1,g_2\in G$ we define 
    $$g_1-g_2 := g_1+(-g_2).$$
...
\begin{lemma}
    If $K$ is a field of characteristic $0$, or of characteristic $p>0$ and perfect, then every non-constant $f\in K[x]$ is seperable.  
\end{lemma}
\begin{proof}
