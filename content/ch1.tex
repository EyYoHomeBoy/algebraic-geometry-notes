...
\end{proof}
\begin{corollary}
    If $\pazocal{V},\pazocal{V}'$ are finite bases of a vector space, then 
    $$
        \#\pazocal{V}=\#\pazocal{V}'.
    $$
\end{corollary}
\begin{definition}
    ...
    $$
    Such a vector space is called a \emph{finite dimensional vector space}. When $V$ does not have a finite basis we define
    $$
        \dim\ V := \infty
    $$
\end{definition}
...
\begin{lemma}
    ...
    is a covariant endofunctor on $\mathrm{ComRing}$ for every $n\geq 1$.  
\end{lemma}
\begin{proof}
...
\begin{corollary}\label{MaximalIdealsOfPolynomialRings}
    Let $(r_1,\dots,r_n)\in R^n$. If $\ev_{r_1,\dots,r_n}$ is surjective, then 
    $$
        R[x_1,\dots,x_n]/\langle x_1-r_1,\dots,x_n-r_n\rangle \simeq R.
    $$ 
    Hence if $R$ is a field, then $\langle x_1-r_1,\dots,x_n-r_n\rangle$ is maximal. 
\end{corollary}
