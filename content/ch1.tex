...
\begin{definition}
    ...
    $$ 
    with the convention that $X^0$ is a singleton set. We call the element of this set \emph{the empty string} in $\N[X]$. Given 
    $$
...
$$
    \sum_{x\in X} a_x x.
$$
We now continue under the assumption that $R$ is commutative. We want to give an analogous construction of a free algebra over a ring, which when $R=\Z$ leads to the notion of the free ring generated by a set.
\begin{definition}
    \emph{The free $R$-algebra generated by $X$} is the free $R$-module generated by $X_\N$. 
\end{definition}
...
$$
for some $n$ and some $x_1,\dots,x_n\in X$. In fact, any finite selection of elements in $R[X_\N]$ can be written in this way for a sufficiently large $n$ and a list of elements $x_1,\dots,x_n\in X$. We define multiplication in $R[X_\N]$ in the following way: Given a pair, 
$$
    (a,b) = \left(\sum_{k}\sum_{i_1,\dots,i_k} a_{i_1,\dots,i_k} x_{i_1}\cdots x_{i_k},\sum_{k}\sum_{i_1,\dots,i_k} b_{i_1,\dots,i_k} x_{i_1}\cdots x_{i_k})\right) 
$$
define for a given $k$ and $i_1,\dots,i_k\in \{1,\dots,n\}$
$$
    c_{i_1,\dots, i_k} := \sum_{p,q : p+q=k} a_{i_1,\dots,i_p}b_{i_{p+1},\dots,i_k}.
$$
Then 
$$
    ab = \sum_k \sum_{i_1,\dots,i_k} c_{i_1,\dots,i_k} x_{i_1}\cdots x_{i_k}
$$
Note that for sufficiently large $P$ and $Q$, for $p\geq P$ and $q\geq Q$, $a_{i_1,\dots,i_p}=0$ and $b_{j_1,\dots,j_q}=0$. Then for all $k\geq P+Q$, 
$$
    a_{i_{1},\dots,i_p}=0 \text{ or } b_{i_{p+1},\dots,i_k}=0
$$ 
when $p+q=k$, thus for large $k$, $c_{i_1,\dots,i_k}=0$, hence this multiplication is well-defined. We take $1$ to be the function that takes the empty string to $1$ and every other string to $0$. This is clearly neutral with respect to the above binary operation. One also verifies that this is associative and distributes over addition: Let $a,b,c\in R[X_\N]$. Write
\begin{gather*}
    a = \sum_{k,i_1,\dots,i_k} a_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k},\\
    b = \sum_{k,i_1,\dots,i_k} b_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k},\\
    c = \sum_{k,i_1,\dots,i_k} c_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k},\\
    ab  = \sum_{k,i_1,\dots,i_k} d_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k},\\
    bc = \sum_{k,i_1,\dots,i_k} e_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k}.
\end{gather*}
Then 
\begin{align*}
    (ab)c &= \left(\sum_{k,i_1,\dots,i_k} d_{i_1,\dots,i_k} x_{i_1}\cdots x_{i_k}\right) c\\
    &= \sum_{k,i_1,\dots,i_k} \left(\sum_{p+q = k} d_{i_1,\dots,i_p}c_{i_{p+1},\dots,i_k} \right) x_{i_1}\cdots x_{i_k}\\
    &=\sum_{k,i_1,\dots,i_k}\left(\sum_{p+q=k}\left(\sum_{r+s=p} a_{i_1,\dots,i_r}b_{i_{r+1},\dots,i_p} \right)c_{i_{p+1},\dots,i_k} \right)x_{i_1}\cdots x_{i_k}\\
    &= \sum_{k,i_1,\dots,i_k} \left(\sum_{r+s+q = k} a_{i_1,\dots,i_r}b_{i_{r+1},\dots,i_{s}}c_{i_{s+1},\dots,i_{k}} \right) x_{i_1}\cdots x_{i_k}\\
    &= \sum_{k,i_1,\dots,i_k}\left(\sum_{p+q = k} a_{i_1,\dots,i_p}\sum_{r+s=q} b_{i_{p+1,\dots,i_{p+r}}}c_{i_{p+r+1},\dots,i_k}\right)x_{i_1}\cdots x_{i_k}\\
    &= \sum_{k,i_1,\dots,i_k}\left(\sum_{p + q = k} a_{i_1,\dots i_p}e_{i_{p+1},\dots,i_k} \right) x_{i_1}\cdots x_{i_k}\\
    &= a \left(\sum_{k,i_1,\dots,i_k} e_{i_1,\dots,i_k}x_{i_1,\dots,i_k}\right) = a(bc).
\end{align*}
Moreover,
\begin{align*}
    a(b+c) &= a \left( \sum_{k,i_1,\dots,i_k} \left( b_{i_1,\dots,i_k}+c_{i_1,\dots,i_k}\right) x_{i_1}\cdots x_{i_k}\right)\\
    &= \sum_{k,i_1,\dots,i_k} \left( \sum_{p+q = k} a_{i_1,\dots,i_p}\left(b_{i_{p+1},\dots,i_k}+c_{i_{p+1},\dots,i_k}\right)\right) x_{i_1}\cdots x_{i_k}\\
    &= \left(\sum_{k,i_1,\dots,i_k} \left(\sum_{p+q=k} a_{i_1,\dots,i_p}b_{i_{p+1},\dots,i_k}\right) x_{i_1}\cdots x_{i_k}\right)\\ 
    &+ \left(\sum_{k,i_1,\dots,i_k} \left(\sum_{p+q=k} a_{i_1,\dots,i_p}c_{i_{p+1},\dots,i_k}\right) x_{i_1}\cdots x_{i_k}\right)\\
    &= ab + ac.  
\end{align*}
We map $a\in R$ to the function that takes the empty string to $a$ and all other strings to $0$, which we will simply denote by $a$. The map $a\mapsto a$ is an injective ring map. It thus follows that $R[X_\N]$ is indeed an algebra over $R$ generated by $X\subset R[X_\N]$.
\subsubsection{The Free Commutative Algebra}
We can endow this construction with more interesting structure by taking the quotient of an ideal. Of interest here is the ideal in $R[X_\N]$ generated by the set 
$$
    \{ xy - yx : x,y\in X\},
$$
which for this discussion will be denoted by $I$. This leads to the following definition. 
\begin{definition}
    \emph{The free commutative $R$-algebra generated by $X$} is the commutative $R$-algebra
    $$
        R[X] := R[X_\N]/I
    $$
\end{definition}
There is another way to go about constructing the free $R$-algebra in the case where $X$ itself is an additive monoid, by explicitly defining a suitable multiplication on $R[X]$ to begin with.
\begin{lemma}
    Suppose $(X,+, e)$ is a commutative monoid. For $\sum_{x} a_x x, \sum_x b_x x\in R[X]$, we define 
    $$
        \sum_{x} \left(\sum_{y,z : y+z = x} a_ybz\right) x. 
    $$ 
    With $1 := e \in R[X]$, this is a commutative $R$-algebra 
\end{lemma}
\begin{proof}
    One readily verifies by explicit computation that this 
\end{proof}

We note the following result 
\begin{proposition}
    $R[X]$ has a basis over $R$ given by
    $$
        \left\{ x_1^{v_1}\cdots x_n^{v_n} : n\geq 0, x_1,\dots,x_n\in X, (v_1,\dots,v_n)\in \N^n \right\}.
    $$
    It is thus, as expected, generated by $X\subset R[X]$ as an $R$-algebra. 
\end{proposition}
\begin{proposition}
    Suppose $X$ is an additive monoid such that 
    $$
        X = \sum_{\alpha\in A} \N e_\alpha
    $$
    for some $\{e_\alpha\}_{\alpha\in A}$. Then Letting $e_\alpha$ denote the image of $e_\alpha$ in $R[X]$, 
    $$
        R[X] = R[x_\alpha : \alpha\in A]. 
    $$
    I.e. $R[x]$ is generated as an $R$-algebra by $\{x_\alpha\}$.
\end{proposition}
\begin{proof}
    
\end{proof}
\begin{remark}
    The elements of $\{x_\alpha\}$ defined above are called the variables of $R[X]$. These are algebraically independent over $R$. 
\end{remark}
\subsubsection{Polynomial Rings}
    We can now define, what we mean by a polynomial ring over $R$. For this we will fix $X := \N^n$ for an $n\geq 1$. Note that 
    $$
        \N^n = \N e_1+\dots+\N e_n
    $$ 
    where $e_i$ is the $n$-tuple of natural numbers, with the $i$'th entry being $1$ and all remaining entries being $0$.
    \begin{definition}
        \emph{The polynomial ring over $R$ in $n$-variables} is defined to be 
        $$
            R[\N^n].
        $$ 
    \end{definition}
    The following result simply spells out prior results in the special case of $X=\N^n$ and is thus given without proof.  
    \begin{lemma}
        The polynomial ring is generated by the variables $x_1,\dots,x_n$ as an $R$-algebra where $x_i$ is the element in $R[\N^n]$ induced by $e_i$. 
    \end{lemma}
    With the above lemma in mind we will this point on take $R[x_1,\dots,x_n]$ to mean the polynomial ring $R[\N^n]$.
    \begin{definition}
...
\subsubsection{An Example of a Non-noetherian Ring}
    ...
    \begin{definition}
        Given a commutative ring $R$, \emph{the polynomial ring in countably many variables} is the ring $R[\bigoplus_{n\in \N} \N]$. We denote it by $R[x_1,x_2,\dots]$. 
    \end{definition}
    \begin{proposition}
