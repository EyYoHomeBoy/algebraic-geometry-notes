...
\begin{definition}
    ...
    \text{ and }
    \begin{tikzcd}
        M \arrow[r, "(\fone_M{,} e)"] \arrow[rd,"\fone_M",swap] & M \times  M \arrow[d,"\circ"] &  M\arrow[l,"(e{,}\fone_M)",swap] \arrow[ld, "\fone_M"]\\
        & M
    \end{tikzcd}
    $$ 
...
\begin{remark}
    \begin{enumerate}
        ...
        \item As we saw in Example~\ref{MonoidIsACategoryGroupIsGroupoid} $\mathrm{B}M := (\{M\}, M)$ is a one object category, where composition is given by evaluation of the binary operation on $M$ and the neutral element is the identity morphism. As an immediate consequence, $e$ is unique.
...
\begin{example}\label{SubMonoidsExample}
    \begin{enumerate}
        ...
        \item The pairs, $(\Z,+),(\Z,\cdot),(\Q,+),(\Q,\cdot),(\R,+),(\R,\cdot),(\C,+),(\C,\cdot)$ are all monoids.
        \item Let $A$ be a set. Consider $\mathrm{End}(A)$. This a monoid under function composition.
...
\begin{definition}
    Let $M,N$ be monoids. A \emph{monoid homomorphism/map of monoids/morphism of monoids} is a map $\rho : M \rightarrow N$ such that 
    \begin{enumerate}
...
\end{definition}
\begin{remark}
    The collection of monoids and the collection of monoid homomorphisms form a subcategory of $\mathrm{Set}$ denoted $\mathrm{Mon}$. Indeed, given monoid homomorphisms $f: A\rightarrow B$, $g:B\rightarrow C$, then for each $x,y\in A$,
    $$
    ...
    $$
        gf(e_A)=g(e_B)=e_C,
    $$
    hence $gf$ is a monoid homomorphism. 
\end{remark}
...
\end{proof}
\subsubsection{Product Monoids \& Finitely Supported Product of Monoids}
...
\end{proof}
\begin{corollary}
    Given any non-empty set $X$ and monoid $M$, 
    $$
        \mathrm{Set}(X,M)
    $$
    is a monoid.
\end{corollary}
\begin{example}
    For monoids $M$ and $N$. $\mathrm{Mon}(M,N)\subset \mathrm{Set}(M,N)$ is a submonoid if $N$ is commutative. Indeed, for $f,g\in \mathrm{Mon}(M,N)$ and $x,y\in M$. Then 
    $$fg(xy)=f(xy)g(xy) = f(x)f(y)g(x)g(y) = f(x)g(x)f(y)g(y)=fg(x)fg(y),$$
    hence $fg\in \mathrm{Mon}(M,N)$
    Furthermore, we have 
    $$e(xy)=e_M(xy)=xy=(e_Nx)(e_Ny)=e(x)e(y)\implies e\in \mathrm{Mon}(M,N).$$
\end{example}
...
\begin{lemma}\label{ProductMonoidIsAProductInTheCategoryOfMonoids}
    ...
    is monoid homomorphism. Given a monoid $N$ and a family monoid homomorphisms $\{f_\alpha : N\rightarrow M_\alpha\}_{\alpha\in A}$ then the unique map $f: N\rightarrow \prod_{\alpha\in A}$ (cf. Proposition~\ref{DirectProductIsADirectProductInTheCategoryOfSets}) such that $\pi_\alpha \circ f = f_\alpha$ for every $\alpha \in A$ is monoid homomorphism. Hence, the cartesian product is the product in the category of monoids. 
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{example}
    Not every submonoid of a product monoid arises in this fashion. For instance consider $N=\{(n,n)\in\N\times \N \}$ which is a proper submonoid of $(\N\times \N,+)$. Indeed, $0_{\N\times \N} = (0,0)\in N$ and if $(n_1,n_1),(n_2,n_2)\in N$, then $(n_1,n_1)+(n_2,n_2)=(n_1+n_2,n_1+n_2)\in N$. Let any two submonoids $A,B\subset \N$ be given and suppose neither are trivial. Take any pair of elements $a\in A$, $b\in B$. It is clear that we can add $a$ to itself to the point where the outcome is not equal to $b$. I.e. there is an $n\in\N$ such that $na\neq b$. Then $(na,b)\notin N$, hence $N\neq A\times B$. This means $N$ cannot be the product monoid of submonoids of $\N$.
\end{example}
\begin{definition}
    Let $A$ be a set and $\{M_\alpha\}_{\alpha\in A}$ a family of monoids. We define the \textit{finitely supported direct product} of $M_\alpha$ over $A$ as the set 
    $$\prod_{\alpha\in A,\mathrm{fin}} M_\alpha := \left\{(m_\alpha) \in \prod_{\alpha\in A} M_\alpha : m_\alpha = e_\alpha \text{ for all but finitely many } \alpha \in A \right\}$$
\end{definition}
\begin{lemma}\label{RestrictedProductMonoidIsAMonoid}
    Let $A$ be a set and $\{M_\alpha\}_{\alpha\in A}$ a family of monoids.  $\prod_{\alpha\in A,\mathrm{fin}} M_\alpha$ is a submonoid of $\prod_{\alpha \in A} M_\alpha$.
\end{lemma}
\begin{proof}
    Let $(m_\alpha),(m_\alpha')\in \prod_{\alpha\in A,\mathrm{fin}} M_\alpha$. For some distinct $\alpha_1,\dots,\alpha_r\in A$ and $\beta_1,\dots,\beta_p\in A$, $m_\alpha = e_\alpha$ for every $\alpha \in A\setminus \{\alpha_1,\dots,\alpha_r\}$ and $m_\alpha' = e_\alpha$ for every $\alpha \in A \setminus \{\beta_1,\dots,\beta_p\}$. Then $m_\alpha m'_\alpha = e_\alpha$ for every $\alpha \in A\setminus \{\alpha_1,\dots,\alpha_r,\beta_1,\dots,\beta_p\}$ hence $(m_\alpha)(m'_\alpha)=(m_\alpha m'_\alpha) \in \prod_{\alpha\in A,\mathrm{fin}} M_\alpha$. Clearly $e=(e_\alpha)\in \prod_{\alpha\in A,\mathrm{fin}} M_\alpha$. 
\end{proof}
Here is a note on notation: Given a finite set of elements $E \subset M$ in a monoid $(M,e)$, we define, when $E$ is empty,
$$
    \prod_{m\in E} m := e.
$$
When $E=\{m_1,\dots,m_n\}$, we define 
$$
    \prod_{m \in E} m := \left(\prod_{m\in E\setminus \{m_n\}} m \right)
$$
In the non-empty case we often use the notation
$$
    \prod_1^n m_i := \prod_{m\in E} m.
$$
When $(M,+)$ is commutative we opt to write 
$$
    \sum_{m\in E} m
$$ 
instead of 
...
$$
\begin{definition}
    Given a monoid $M$ and a set $X$, for a function $f: X\rightarrow M$, we define \emph{the support of $f$} to be the set
    $$
        \mathrm{supp}\ f := \{x\in X : f(x)\neq e\}
    $$
\end{definition}
\begin{lemma}\label{SupportLemma}
    Given a monoid $M$ and a set $X$, for functions $f,g: X\rightarrow M$,
    $$
        \mathrm{supp} \ fg \subset \mathrm{supp} \ f \cup \mathrm{supp} \ g 
    $$
\end{lemma}
\begin{proof}
    If $x\in X$ is such that $f(x)=e=g(x)$, then $fg(x) = e$.
\end{proof}
\begin{lemma}\label{SumOfCommutativeSubmonoids}
    Given a family of submonoids $\{M_\alpha\}_{\alpha\in A}$ of a commutative monoid $(M,+)$, there is a monoid map
    \begin{gather*}
        \prod_{\alpha\in A, \mathrm{fin}} M_\alpha \rightarrow M\\
        (m_\alpha) \mapsto \sum_{\alpha\in A} m_\alpha:= \sum_{\alpha \in \mathrm{supp}\ f} m_\alpha
    \end{gather*}
    We denote the image of this monoid map by 
    $$
        \sum_{\alpha} M_\alpha.
    $$
\end{lemma}
\begin{proof}
    Indeed,
    \begin{align*}
        (m_\alpha)+(n_\alpha)\mapsto &\sum_{\alpha\in \mathrm{supp} \ (m_\alpha+n_\alpha)} m_\alpha+n_\alpha = \sum_{\alpha\in (\mathrm{supp} \ (m_\alpha)) \cup( \mathrm{supp} \ (n_\alpha))} m_\alpha + n_\alpha \\
        &= \sum_{\alpha \in \mathrm{supp} \ (m_\alpha)} m_\alpha + \sum_{\alpha\in \mathrm{supp} \ (n_\alpha)} n_\alpha,
    \end{align*}
    using Lemma~\ref{SupportLemma} in the first equality. It is clear that $(0)$ is mapped to $0$. 
\end{proof}
\begin{definition}
    Let $X$ be a set and $M$ a monoid. We define \emph{the monoid over $M$ generated by $X$} to be the set 
    $$
        M[X] := \{(m_x)\in \mathrm{Set}(X,M) : \mathrm{supp} \ (m_x) \text{ is finite}\}.
    $$
    When $(M,+,0)$ is a commutative monoid, we denote an element of $M[X]$ by 
    $$
        \sum_{x\in X}m_x x
    $$
    In this case, we call $M[X]$ \emph{the free commutative monoid generated by $X$ over $M$}
\end{definition}
\begin{remark}
    From Lemma~\ref{SupportLemma} it follows that $M[X]$ is submonoid of $\mathrm{Set}(X,M)$. Alternatively, note that $\mathrm{Set}(X,M) = \prod_{x\in X} M$ and that $M[X]$ just the finitely supported product. 
\end{remark}
\subsubsection{The Free Monoid Generated by a Set}
\begin{definition}
    Given a set $X$, we define \emph{the free monoid generated by $X$} to be the set
    $$
        X_\N := \bigsqcup_{n\in \N} X^n
    $$ 
    with the convention that $X^0 := \prod_{n\in \emptyset} X$, which is just the set containing the empty function. In this context we call this function \emph{the empty string} in $X_\N$. Given 
    $$
        x=(x_1,\dots,x_n)\in X^n \text{ and } y=(y_1,\dots,y_m)\in X^m,
    $$
    we define \emph{concatenation}
    $$
        xy := (x_1,\dots,x_n,y_1,\dots,y_m)\in X^{n+m}
    $$
\end{definition}
\begin{remark}
    Note that any element is $X_\N$ is of the form 
    $$
        x_1\cdots x_n
    $$
    for $x_1,\dots,x_n\in X$
\end{remark}
\begin{lemma}
    The free monoid generated by a set $X$ with concatenation is a monoid with the neutral element being the empty string.
\end{lemma}
\begin{proof}
    the operation of concatenation is clearly associative and the empty string is also clearly seen to be neutral element with respect to concatenation.
\end{proof}
\begin{definition}
    Consider a pair of monoids $M$ and $N$. Consider the set 
    $$
        (M\cup N)_\N.
    $$
    An element in this monoid is the empty string or can be written as 
    $$
        a_1\cdots a_n
    $$
    where $a_{2k+1} M_\N$ and $a_{2k}\in N_\N$ or vice versa. Such an element, $a_1\cdots a_n$ is \emph{reduced} if it is the empty string or if each $a_i\in (M\setminus\{e_M\})\cup (N\setminus\{e_N\})$. We denote the set of reduced elements by $M\ast N$. This is called the \emph{free product of $M$ and $N$}
\end{definition}
For any monoid $A$ there is a function
\begin{gather*}
    \alpha_A: A_\N \rightarrow A\cup\{\ast\}\\
\end{gather*}
...
and 
$$
    \alpha_A(a_1\cdots a_{n+1}) = 
    \begin{cases}
        \alpha_A(a_1\cdots a_n) & \text{if } a_{n+1} = e\\
        \alpha_A(a_1\cdots a_n)a_{n+1} & \text{otherwise}    
    \end{cases}
$$
for $a_1,\dots,a_{n+1}\in A$
Let $M$ and $N$ be monoids. There is a function 
\begin{gather*}
    \mathrm{red}: (M\cup N)_\N \rightarrow \mathrm{red}(M,N)\\
    a_1\cdots a_n \mapsto \alpha(a_1)\alpha(a_2)\cdots \alpha(a_n) 
\end{gather*}
where $\alpha$ denotes the appropriate choice of function in $\{\alpha_M,\alpha_N\}$. The image under this operation is called \emph{reduction}. Note that this function restricts to the identity on $M\ast N $.
We concatenate elements in this set in the following way,
    $$
        (a_1\cdots a_n)(b_1\cdots b_m) := \mathrm{red}(a_1\cdots a_nb_1\cdots b_m)
    $$

\begin{lemma}
    The above operation of concatenation is well-defined and with it $M\ast N$ is a monoid with neutral element being the empty string.
\end{lemma}
\begin{proof}
    To check associativity, the following is sufficient. For monoids $(A,\cdot)$ and $(B,\cdot)$, 
    $$
        \mathrm{red}(\mathrm{red}(ab)b')=\mathrm{red}(a\mathrm{red}(bb'))
    $$ 
    for $a\in A$ and $b,b'\in B$ and 
    $$
        \mathrm{red}(\mathrm{red}(ab)a') = \mathrm{red}(a\mathrm{ba'})
    $$
    for $a,a'\in A$ and $b\in B$. This is just a matter of case checking. Clearly the empty string is the neutral element.  
\end{proof}
\subsubsection{The Free Commutative Monoid Generated by a Set}
Fix a set $X$. For strings $ x_1\cdots x_n, y_1\cdots y_m \in X_\N$, we write $x_1\cdots x_n \equiv y_1\cdots y_m$ if $n=m$ and $x_1\cdots x_n = y_{\pi(1)}\cdots y_{\pi(n)}$ for some permutation 
$$
    \pi : \{1,\dots,n\}\rightarrow \{1,\dots,n\}.
$$
Suppose $x_1\cdots x_n = x_{\pi(1)}'\cdots x_{\pi(n)}'$ and $y_1\cdots y_m \sim y_{\tau(1)}' \cdots y_{\tau(m)}'$ for permutations $\pi$ and $\tau$. We define 
$$
    \omega : \{1,\dots,n+m\} \rightarrow \{1,\dots,n+m\}
$$
by 
$$
    \omega(i) := 
    \begin{cases}
        \pi(i) & \text{if } i\in \{1,\dots,n\}\\
        \tau(i-n) & \text{otherwise}
    \end{cases}
$$
Then 
$$
    x_1\cdots x_ny_1\cdots y_m = x_{\pi(1)}'\cdots x_{\pi(n)}'y_{\tau(1)}'\cdots y_{\tau(m)}' = x_{\omega(1)}'\cdots x_{\omega(n)}'y_{\omega(n+1)}'\cdots y_{\omega(n+1)}'.
$$
We thus give the following definition. 
\begin{definition}
    \emph{The free commutative monoid generated by $X$} is the set 
    $$
        X_\N^\mathrm{Comm} := X_\N/\equiv
    $$
\end{definition}
One readily verifies that with 
$$
    [x_1\cdots x_n]_\equiv [y_1\cdots y_m]_\equiv := [x_1\cdots x_n y_1\cdots y_m]_\equiv
$$
and neutral element $[e]$ (here $e$ denotes the empty string) that $X_\N^\mathrm{comm}$ is a commutative monoid. 
\subsubsection{The Opposite Monoid}
    \begin{definition}
        Consider a monoid $(M,\circ)$. The opposite category is then $M\op:=\mathrm{End}(\mathrm{B}M\op)$.
    \end{definition}
    \begin{remark}
        In other words, $M\op$ is the monoid $(M,\star)$, where for $x,y\in M$
        $$
            x\star y:= y\circ x.
        $$
    \end{remark}
    The upshot of having such a construction is in many cases we are interested in statements that involve some sort of action from the left and a notion of an action from the right. The statement about the right action can be reduced to a statement about the left action in the opposite monoid, allowing us prove only one statement.\\
    Note that in the case of a commutative monoid $M$, 
    $$
        (M\op,\star) = (M,\circ)
    $$
\subsection{Groups}
...
\begin{definition}
    ...
    $$gg^{-1}=g^{-1}g = e.$$
    For $g\in G$ we refer to $g^{-1}$ as the \textit{inverse of $g$}. In other words a group is a monoid where $\mathrm{B}M$ is a groupoid. The data specifying a group is also often written as a tuple $(G,\circ,e)$ or $(G,\circ)$.
\end{definition}
\begin{remark}
    Note that for $g\in G$ the inverse $g^{-1}$ is the mutually inverse morphism of $g$, when we interpret $G$ as the groupoid $\mathrm{B}G = (\{G\},G)$. For the same reason we have that 
    $$
        \left(g_1g_2\right)^{-1} = g_2^{-1}g_1^{-1}
    $$
    for each $g_1,g_2\in G$ and that 
    $$
        \left(g^{-1}\right)^{-1} = g
    $$
\end{remark}
\begin{remark}
    For an element $g\in G$ and a non-negative integer $n$, we define 
    $$
        g^{-n} := \left(g^{-1}\right)^n.
    $$ 
    It is easy to check that $\left( g^n\right)^{-1}=g^{-n}$. 
\end{remark}
\begin{definition}
    A group $(G,+)$ is called \textit{abelian} or \textit{additive}, if it is a commutative monoid with respect to $+$. We denote the inverse of $g\in G$ with respect to addition by $-g$, and for $g_1,g_2\in G$ we define 
    $$g_1-g_2 := g_1+(-g_2).$$
    and $ng := \sum_1^n g$.
\end{definition}
\begin{example}
    Consider any locally small category $\pazocal{C}$. Then $\Aut(\pazocal{C})$ forms a group under composition. 
\end{example}
\begin{example}
    Consider any monoid $M$ and its associated category $\mathrm{B}M$. By considering the automorphisms of this category, we get ample examples of groups (cf. Example~\ref{SubMonoidsExample}): 
    \begin{enumerate}
        \item $(\Z,+),(\{-1,1\}\subset \Z,\cdot),(\Q,+),(\Q\setminus\{0\}),(\R,+),(\R\setminus\{0\},\cdot),(\C,+),(\C\setminus\{0\},\cdot)$.
        \item $\Aut(A)$ is a group for each set $A$. 
    \end{enumerate}
\end{example}
\begin{lemma}\label{LeftAndRightMultiplicationIsInjective}
    Let $(G,\circ)$ be a group. Let $g,g',a\in G$. If $ag = ag'$, then $g=g'$. Similarly, if $ga=g'a$, then $g=g'$.
\end{lemma}
\begin{proof}
    This follows from each $g\in G$ being an isomorphism in $\mathrm{B}G$, which in particular implies that they are epimorphisms and monomorphisms. 
\end{proof}
\begin{definition}
...
\begin{example}
    \begin{enumerate}
        ...
        \item Given a set $A$. The set of invertible maps $A\rightarrow A$, denoted $\mathrm{Aut}(A)$ forms a submonoid of $\mathrm{Set}(A,A)$ under composition, furthermore it is a group, when picking the inverse elements to be inverse maps and the neutral to be the identity on $A$. 
        \item Given a non-empty set $X$ and a group $G$ the monoid $\mathrm{Set}(X,G)$ forms a group. Indeed for $f\in \mathrm{Set}(X,G)$, define $f^{-1}:X\rightarrow G$ by $f^{-1}(x) = \left(f(x)\right)^{-1}$. Then 
...
\begin{remark}\label{GroupHomomorphismSendsInversesToInversesAndNeutralElementsToNeutralElements}
    ...
    hence by Lemma~\ref{LeftAndRightMultiplicationIsInjective}, $e_H= \rho(e_G)$. Thus a group homomorphism is a monoid homomorphism. It follows that the collection of all groups together with group homomorphisms form a subcategory of $\mathrm{Mon}$, denoted $\mathrm{Grp}$.\\ 
    ...
    hence by uniqueness of inverse elements $\rho\left(g^{-1}\right) = \rho(g)^{-1}$.
    \item The class of all groups together with group homomorphisms being morphisms is a subcategory of $\mathrm{Set}$, denoted $\mathrm{Grp}$. As a subcategory of $\mathrm{Grp}$, we have the category of abelian groups, denoted $\mathrm{ab}$. The zero object in these categories is the trivial group. 
    \item Suppose $H$ is commutative. Let $\rho \in \mathrm{Grp}(G,H)$ and $x\in M$. Then 
    $$\rho(x)\left(\rho(x)\right)^{-1}=e_N = e(x),$$
    implying that $\mathrm{Grp}(G,H)\subset \mathrm{Set}(G,H)$ is a subgroup. Let $f\in \mathrm{Grp}(G,H)$ and $x,y\in G$, then 
    \begin{align*} 
        f^{-1}(xy) = \left(f(xy)\right)^{-1} &= \left(f(x)f(y)\right)^{-1} = \left(f(x)\right)^{-1}\left(f(y)\right)^{-1} = f^{-1}(x)f^{-1}(y)\\ 
        &\implies f^{-1}\in \mathrm{Grp}(G,H)
    \end{align*}
    \end{enumerate}
\end{remark}
...
\begin{lemma}
    Given a family of groups $\{G_\alpha\}$. The product monoid 
    $$
        \prod_{\alpha} G_\alpha
    $$
    is a group where the inverse of an element $(g_\alpha)$ is given by $(g_\alpha^{-1})$.
\end{lemma}
\begin{proof}
    Indeed, given $(g_\alpha)\in \prod_{\alpha} G_\alpha$ we have that
    $$
        (g_\alpha)(g_\alpha)^{-1} = (g_\alpha)(g_\alpha^{-1}) = (g_\alpha g_\alpha^{-1}) = (e_\alpha) = e
    $$
    proving that $(g_\alpha)^{-1}$ is the left inverse of $(g_\alpha)$ follows from a dual argument.
\end{proof}
\begin{definition}
    Let $A$ be a set and $\{G_\alpha\}$ a family of additive groups. We define the \textit{direct sum of $G_\alpha$ over $A$} denoted
    $$\bigoplus_{\alpha\in A} G_\alpha$$
    to be the finitely supported product monoid of $\{G_\alpha\}$.
\end{definition}
One readily verifies that if an element $(g_\alpha)$ is finitely supported then $(g_{\alpha})^{-1}$ is finitely supported, so the above construction defines an additive group.  
\begin{remark}\label{SumsOfInfinitelyOftenZeroGroupElements}
    Let $\{G_\alpha\}$ a family of subgroups of an additive group $G$. Then 
    $$\sum_{\alpha\in A} G_\alpha \in G,$$ Lemma~\ref{SumOfCommutativeSubmonoids} defines a subgroup of $G$, since $(g_\alpha)\mapsto \sum_\alpha g_\alpha$ defines a group homomorphism, $\bigoplus_{\alpha} G_\alpha\rightarrow G$. 
\end{remark}
...
\end{proof}
\begin{example}
    Not all subgroups of $\prod_\alpha G_\alpha$ arise in this fashion. For instance $\{(n,n)\in \Z\times \Z : n\in \Z\}$, which is a subgroup of $(\Z\times \Z,+)$, is not the product of pair of subgroups of $\Z$.
\end{example}
\begin{lemma}
...
\begin{proposition}
    ...
    $$\sim_l := \left\{ (g_1,g_2)\in G\times G : g_1H = g_2H \right\} \ \text{and} \ \sim_r := \left\{ (g_1,g_2)\in G\times G : Hg_1 = Hg_2 \right\}$$
...
\begin{remark}\label{ReformulationOfNormalSubgroupAndAllSubgroupsOfAbelianGroupsAreNormal}
    ...
    Hence any subgroup of an abelian group is normal. Furthermore $\sim_l$ and $\sim_r$ define the same set. Thus we may define $\sim$ to be $\sim_l$.
...
\end{proof}
It may be that a subgroup is not normal, but we may construct a normal subgroup containing it.
\begin{lemma}
    The intersection of a collection of normal subgroups of some group $G$ is a normal subgroup of $G$.
\end{lemma}
\begin{proof}
    Consider a collection of normal subgroups $G$, $\{N_\alpha\}$. Let $g\in G$. Let $ ghg^{-1}\in g\left(\bigcap_{alpha}N\right)g^{-1}$. Then $ghg^{-1} \in N_\alpha$ for each $\alpha$, hence $ghg^{-1}\in N_\alpha$ for each $\alpha$. This means $ghg^{-1}\in \bigcap_\alpha N_\alpha$. 
\end{proof}
\begin{lemma}
    Let $H\subset G$ be a subgroup of a group $G$. We define the normal closure of $H$ to be the intersection of all normal subgroups containing $H$. This is the smallest normal subgroup containing $H$, which we denote $\overline{H}$.
\end{lemma}
\begin{proof}
    By the prior lemma $\overline{H}$ is a normal subgroup of $G$ and it trivially contains $H$. If $N$ is a normal subgroup of $G$ containing $H$, then trivially it contains the intersection of all normal subgroups of $G$ containing $H$. 
\end{proof}
\begin{proposition}
...
\end{proof}
\begin{proposition}
    Let $G$ be a group and $N\subset G$ a normal subgroup. The surjection $\pi : G\rightarrow G/N, g\mapsto gN$ is a group map with kernel $N$.
\end{proposition}
\begin{proof}
    Let $g_1,g_2\in G$. Then $\pi(g_1g_2)=g_1g_2N= g_1Ng_2N=\pi(g_1)\pi(g_2)$. Suppose $\pi(g) = eN$. Then $g\in N$ hence $\ker \ \pi = N$. 
\end{proof}   
\begin{corollary}
    Since the kernel of a group map is a normal subgroup, it follows that every normal subgroup arises as the kernel of a group map
\end{corollary} 
\begin{theorem}
    Given a normal subgroup $N$ of a group $G$, consider the diagram
    $$
        \begin{tikzcd}
            N \arrow[r, hookrightarrow, shift right=0.5ex, "\iota", swap] \arrow[r, shift left=0.5ex,"0"] & G 
        \end{tikzcd}  
    $$
    $G/N$ with $\pi : G \rightarrow G/N$ is the coequaliser of this diagram
\end{theorem}
\begin{proof}
    Let $\alpha : G\rightarrow H$ be a group map such that $\alpha\iota = 0$. The second condition is equivalent to $\ker\ \alpha \subset N$. We define 
    \begin{gather*}
        \beta : G/N \rightarrow H\\
        gN \mapsto \alpha(g)
    \end{gather*}
    If $gN = g'N$, then $g(g')^{-1}\in N$, hence 
    $$
        \alpha(g)\alpha(g')^{-1}=\alpha(gg'^{-1}) = e,
    $$
    hence 
    $$
        \beta(gN)=\alpha(g)=\alpha(g')=\beta(g'N).
    $$
    It is a group map. Indeed, 
    $$
        \beta(gNg'N)=\beta(gg'N)= \alpha(gg')=\alpha(g)\alpha(g')=\beta(gN)\beta(g'N)
    $$
    Clearly the coequaliser diagram
    $$
        \begin{tikzcd}
            G \arrow[rd,"\alpha"] \arrow[d, "\pi"] \\
            G/N \arrow[r,"\beta"] & H
        \end{tikzcd}
    $$
    commutes. If $\gamma: G/N\rightarrow H$ is a group map such that $\alpha = \gamma\pi$, then for each $gN\in G/N$,
    $$
        \gamma(gN)=\gamma\pi(g) = \alpha(g)= \beta\pi(g)=\beta(gN),
    $$
    hence $\gamma=\beta$.
\end{proof}

\begin{corollary}\label{WellDefinednessOfQuotientGroupMaps}
    Let $\alpha : G \rightarrow H$ be a group homomorphism and $I\subset G$ $J\subset H$ be normal subgroups. Then 
    \begin{gather*}
        \beta : G/I \rightarrow H/J\\
        gI \mapsto \alpha(g)J    
    \end{gather*}
     is a well-defined group homomorphism if and only if $\alpha(I)\subset J$. If so $\beta$ is the unique group map such that the square
     $$
        \begin{tikzcd}
            G\arrow[r,"\alpha"]\arrow[d, "\pi_I"] & H\arrow[d,"\pi_J"]\\
            G/I \arrow[r, dashrightarrow] & G/J
        \end{tikzcd}
     $$
     commutes
\end{corollary}
\begin{proof}
    "$\implies$": Suppose $\beta$ is a well-defined group homomorphism. Let $\alpha(g)\in \alpha(I)$. Then since $gI = eI$, 
    $$
        \alpha(g)J =\beta(gI) = \beta(eI)=\alpha(e)J = eJ,
    $$
    hence $\alpha(g)\in J$.\\
    "$\impliedby$": Suppose $\alpha(I)\subset J$. Then for the canonical surjection $\pi_J : H\rightarrow H/J$, 
    $$
        I \subset \ker \ \pi_J\alpha,
    $$
    hence the result follows from $G/I$ being the coequaliser of 
    $$
        \begin{tikzcd}
            I \arrow[r, hookrightarrow, shift right=0.5ex, "\iota", swap] \arrow[r, shift left=0.5ex,"0"] & G
        \end{tikzcd} 
    $$
    Uniqueness when $\beta$ is well-defined likewise follows from this fact. 
\end{proof}
\begin{definition}
    Let $\alpha: G\rightarrow H$ be a group map. We define \emph{the cokernel of $\alpha$}, denoted $\coker\ \alpha$, to be the group $H/\overline{\im \ \alpha}$
\end{definition}
\begin{corollary}
    The cokernel of a group map $\alpha : G\rightarrow H$ is the coequaliser of the diagram 
    $$
        \begin{tikzcd}
            G \arrow[r, shift right=0.5ex, "\alpha", swap] \arrow[r, shift left=0.5ex,"0"] & H 
        \end{tikzcd} 
    $$
\end{corollary}
\begin{proof}
    Note that $\pi : H\rightarrow \coker \ \alpha$ is a map such that $\pi \alpha=0$, since $\im\ \alpha \subset \overline{\im\ \alpha}$. Let $\beta: G\rightarrow K$ be a group map with $\beta\alpha = 0$. Note that then $\beta\iota = 0$ where $\iota : \overline{im\ \alpha} \hookrightarrow H$ is the canonical embedding. Since $\coker\ \alpha$ is the coequaliser of 
    $$
        \begin{tikzcd}
            \overline{\im \ \alpha} \arrow[r, shift right=0.5ex, "\iota", swap] \arrow[r, shift left=0.5ex,"0"] & H
        \end{tikzcd}
    $$
    the result follows. 
\end{proof}

\begin{lemma}\label{ConditionOnGroupBeingFinite}
    Let $G$ be a group and $N$ a normal subgroup of $G$. Then 
    $$
        \# G <\infty \iff \#N <\infty \text{ and } \#G/N < \infty.
    $$
\end{lemma}
\begin{proof}
    Suppose $G$ is finite, then obviously $H$ is finite, and since $G$ surjects onto $G/N$ it follows that $G/N$ is finite.\\
    Suppose $N$ and $G/N$ are finite. Then 
    $$
        G = \bigcup_{g\in G} gN = \bigcup_{gN \in G/N} gN
    $$ 
    and since each $gN$ is finite, it follows that $G$ is finite. 
\end{proof}

\subsubsection{The First Isomorphism Theorem of Groups}
\begin{proposition}
    Given a group map $\alpha :  G\rightarrow H$, the equaliser of the diagram
    $$
        \begin{tikzcd}
             G \arrow[r, shift right=0.5ex, "\alpha", swap] \arrow[r, shift left=0.5ex,"0"] & H 
        \end{tikzcd}    
    $$
    is ther kernel of $\alpha$
\end{proposition}
\begin{proof}
    Let $\beta: K \rightarrow G$ be a group map such that $\alpha\beta = 0$. Then $\im \ \beta \subset \ker \ \alpha$. Then 
    \begin{gather*}
        \beta' : K \rightarrow \ker \ \alpha\\
        k \mapsto \beta(k)
    \end{gather*}
    is a well-defined group map such that 
    $$
        \begin{tikzcd}
            K \arrow[rd, "\beta"] \arrow[d,dashrightarrow] \\
            \ker \ \alpha \arrow[r,"\iota"] & G
        \end{tikzcd}
    $$
    commutes. Uniqueness follows from $\iota$ being a monomorphism. 
\end{proof}
\begin{lemma}
    A group map $f : G\rightarrow H$ is an isomorphism of groups if and only if it is a bijective group map. 
\end{lemma}
\begin{proof}
    One implication is trivial. If $f$ is a bijective group map it suffices to show that its inverse is a group. Let $h,h'\in H$ be given. Then 
    $$
      f^{-1}(hh') = f^{-1}(f(f^{-1}(h))f(f^{-1}(h'))) =f^{-1}(f(f^{-1}(h)f^{-1}(h')))= f^{-1}(h)f^{-1}(h').
    $$
\end{proof}
\begin{lemma}
    A group map $\alpha: G\rightarrow H$ is injective if and only if $\ker\ \alpha = \{e\}$ 
\end{lemma}
\begin{proof}
    If $\ker\ \alpha = \{e\}$, then if $h_1,h_2 : K \rightarrow G$, such that $\alpha h_1 = \alpha h_2$. By the universal property of the kernel, we obtain maps $h_1',h_2' : K\rightarrow \ker \ \alpha$ such that 
    $$
    h_1 =0 h_1'= 0 = 0h_2' h_2,$$ 
    hence $\alpha$ is mono and therefor injective.\\
    If $\alpha$ is injective, then given $g\in \ker \ \alpha$
    $$
        \alpha(g) = e = \alpha(e) \implies g = e.
    $$ 
\end{proof}
\begin{theorem}(First Isomorphism Theorem of Groups)
    Given a group map $\alpha : G\rightarrow H$, we get a group isomorphism 
    \begin{gather*}
        \widetilde{\alpha} : G/\ker \ \alpha \rightarrow \im \ \alpha\\
        g(\ker\ \alpha) \mapsto \alpha(g)
    \end{gather*}
\end{theorem}
\begin{proof}
    By prior results presented above $\widetilde{\alpha}$ is a well-defined group map. Note that every $\alpha(g)\in \im \ \alpha$ is equal to $\widetilde{\alpha}(g(\ker\ \alpha))$. Thus $\widetilde{\alpha}$ is surjective. Let $g(\ker \ \alpha)\in \ker \ \widetilde{\alpha}$. Then $\alpha(g)=\widetilde{\alpha}(g(\ker\ \alpha))=e$, hence $g\in \ker\ \alpha$, which implies $g(\ker \ \alpha) = e(\ker \ \alpha)$.
\end{proof}
\begin{corollary}
    If $\alpha : G\rightarrow H$ is a surjective group map, $G/\ker \ \alpha \simeq H$. 
\end{corollary}
\subsection{Rings}
\subsubsection{Definition \& Basic Properties}
\begin{definition}
...
\begin{example}
    \begin{enumerate}
        ...
        \item Given a ring $(R,+,\cdot)$, we obtain the \emph{opposite ring of $R$}, denoted $R\op$ to be $(R,+,\star)$ where $(R,\star)$ is the opposite monoid of $(R,\cdot)$. Since $\cdot$ distributes over $+$, it follows that the same obviously true for $\star$ over $+$.  
        \item For a non-empty set $X$ and a ring $R$, the set $\mathrm{Set}(X,R)$ is a monoid and an additive group with respect to multiplication and addition defined earlier. One easily verifies that it is also a ring.  
    \end{enumerate}
\end{example}
...
\end{definition}
\begin{remark}
    The class of all rings together with the class of all ring homomorphisms form a subcategory of $\mathrm{Set}$ denoted $\mathrm{Ring}$. We denote the category of commutative rings by $\mathrm{ComRing}$.
\end{remark}
\begin{example}
     For rings $R,S$, if $\mathrm{Ring}(R,S)\neq \emptyset$ is never a subring of $\mathrm{Set}(R,S)$. Indeed, note that the zero map is never a ring homomorphism since it maps $1$ to $0$. The terminal object is the zero ring. We shall see that the initial object is $\Z$.
\end{example} 
\begin{lemma}
...
\begin{proposition}\label{ProductRingIsRing}
    Let $A$ be a set, $\{R_\alpha\}_{\alpha\in A}$ a family of rings. Then $\left(\prod_{\alpha\in A} R_\alpha,\cdot \right)$ is a monoid and $\left(\prod_{\alpha\in A} R_\alpha,+ \right)$ an additive group. In addition $\left(\prod_{\alpha\in A} R_\alpha , +,\cdot\right)$ is a ring. 
\end{proposition}
\begin{proof}
...
\end{proof}
\subsubsection{The Ring of Integers: $\Z$}
    \begin{definition}
        For $(a,b),(c,d)\in \N$ we define $(a,b)\sim (c,d)$ if $a+d=b+c$. One easily checks that this is an equivalence relation. We define 
        $$\Z := \N^2/\sim.$$
    \end{definition}
    \begin{proposition}
        On $\Z$ we define
        ...
        and 
        $$[(a,b)][(c,d)] := [(ac+bd,ad+bc)].$$
        Moreover we define $0:=[(0,0)]$, $-[(a,b)]:=[(b,a)]$ and $1:= [(1,0)]$. With these definitions, $\Z$ becomes a commutative ring. The set $\{[(a,0)]\in \Z :a\in \N\}$ is a sub semi-ring isomorphic to $\N$.
    \end{proposition}
    \begin{proof}
        ...
        hence
        $$
            (a+c)+(y+w)=(a+y)+(c+w)=(b+x)+(d+v)=(b+d)+(x+v).
        $$
        This implies 
        $$
            [(a+c,b+d)]=[(x+v,y+w)].   
        $$
        So addition is well-defined.  We check that $\Z$ is a group. Associativity of addition on $\Z$ readily follows from associativity of addition on $\N$. Let $[(a,b)],[(c,d)]\in\Z$ be arbitrary. Then 
        $$
            [(a,b)] + [(0,0)]= [(a+0,b+0)]=[(a,b)]
        $$
        and 
        ...
        $$
        hence $\Z$ is an additive group. It is easy to check that $\{[(a,0)]\in \Z: a\in\N\}$ is a submonoid isomorphic to $\N$. Suppose again $([(a,b)],[(c,d)])=([(x,y)],[(v,w)])$. We find that
        \begin{align*}
        ...
        \end{proof}
        One important fact about the ring of integers is that it is the initial object in the category of rings.
    \begin{lemma}
        $\Z$ is the initial object in the category of rings.
    \end{lemma}
    \begin{proof}
        Let $R$ be any ring. We define a function
        \begin{gather*}
            f_R : \Z \rightarrow R\\
            n \mapsto \sum_1^n 1
        \end{gather*}
        One readily verifies that this is a ring homomorphism. Moreover, given any ring map $f:\Z \rightarrow R$, we have for each $n\in \Z$ 
        $$
            f(n) = f\left(\sum_1^n 1\right) = \sum_1^n f(1)= \sum_1^n 1 = f_R(n).
        $$
        Thus we conclude $f= f_R$.
    \end{proof}
\subsection{Modules}
...
\begin{definition}
    ...
    A \textit{right $R$-module} is an additive group $(M,+)$ with a \textit{right scalar multiplication} $\cdot : M\times R \rightarrow M$, where $mr := m\cdot r := \cdot(m,r)$, satisfying axioms dual to ones for left scalar multiplication. To be precise a right $R$-module is a left $R\op$-module. To emphasise that a module $M$ is a right $R$-module, we may write $M_R := M$\\
    ...
    When $R$ is a commutative ring, a left (or right) $R$-module is simply called an \emph{$R$-module}.
\end{definition}
Since we can view any right module as a left module it is also sufficient to prove a result about left modules in order to obtain a dual statement on right modules. We therefore only talk about left module results. 
\begin{definition}
    Let $M$ be a left $R$-module. A left $R$-submodule is a subset $N\subset M$ such that $N$ is a subgroup of $(M,+)$ and for every $r\in R$, $n\in N$ we have that $rn\in N$ resp. $nr \in N$.
\end{definition}
\begin{remark}
    A left $R$-submodule $N\subset M$ is a left $R$-module. Indeed $(N,+)$ is group since it is a subgroup of $(M,+)$. $N$ being closed under left scalar multiplication ensures that $\cdot := \cdot\mid_{R\times N} : R\times N \rightarrow N$ respectively $\cdot := \cdot\mid_{N \times R}: N\times R\rightarrow N$ are well-defined and $M$ being a left $R$-module, these left actions respect the axioms for left scalar multiplication. If $M$ is a $(R,S)$-bimodule and $N$ is a left $R$-submodule and a right $S$-submodule of $M$ then it is also an $(R,S)$-bimodule. Indeed, if $r\in R$, $n\in N$ and $s\in S$, then since $n\in M$, $r(ns)=(rn)s$.
\end{remark}
\begin{example}
    \begin{enumerate}
        \item Let $(G,+)$ be an additive group. Then $G$ is a $\Z$-module under the left scalar multiplication
        ...
        \item A left/right $R$-submodule of $I\subset R$ is called a \textit{left ideal in $R$}. If $I$ is an $(R,R)$-bimodule, it is called a \textit{both-sided ideal in $R$}. If $R$ is commutative a left ideal is simply referred to as an \textit{ideal in $R$}. 
        \item Let $A$ be a set and $R$ a ring, then $\prod_{\alpha \in A} R$ is an $R$-module, under the left given by $a(r_\alpha) := (ar_\alpha)=(r_\alpha a)$ for $a\in R$ and $(r_\alpha)\in \prod_{\alpha \in A} R$. This is easily checked using 2. together with the fact that $(a)(r_\alpha) = a(r_\alpha)$ for every $a\in R$, $(r_\alpha)\in \prod_{\alpha \in A} R$.
        \item Let $S$ be a ring and $R$ a subring of $S$. Then $ R\times S\ni(r,s) \mapsto rs\in S $ defines a left scalar multiplication of $R$ on $S$, hence $S$ is a left $R$-module.
    \end{enumerate}
\end{example}
\begin{lemma}
...
\begin{remark}
    ...
    $$\rho(rm) = r\rho(m),$$
    which is seen by setting $m' = 0$ when $\rho: M\rightarrow N$ is a left $R$-module. This is also true from the right if $\rho$ was a right $R$-module map.
\end{remark}
\begin{remark}
    Given a ring $R$, the class of all left $R$-modules together with the class of all left $R$-module maps is a subcategory of $mathrm{ab}$ denoted $_R\mathrm{Mod}$. There are analogous categories of right $R$-modules, $\mathrm{Mod}_R$ and of $(R,S)$-bimodules $_S\mathrm{Mod}_R$. The zero object in this category is $0$.
\end{remark}
\begin{lemma}
    If $\rho : M\rightarrow  N$ is a left $R$-module homomorphism, then for a left $R$-submodule $L\subset M$, $\sigma(L)\subset N$ is a left $R$-submodule. If $M$ and $N$ are $(R,S)$-bimodules and $\rho$ is an $(R,S)$-bimodule homomorphism, then $\sigma(L)$ is an $(R,S)$-subbimodule. 
\end{lemma}
\begin{proof}
    We already know that $\sigma(L)$ is a subgroup. Let $r\in R$ and $\sigma(l)\in\sigma(L)$. Then
...
\end{proof}
\begin{lemma}
    Let $\rho : M\rightarrow N$ be a left $R$-module map. Then for any left $R$-submodule $K\subset N$, $\rho^{-1}(K)$ is a left $R$-submodule of $M$. In particular, $\ker \ \rho$ is a left $R$-submodule of $M$.
\end{lemma}
\begin{proof}
    It is sufficient to check $\rho^{-1}(K)$ is closed under left scalar multiplication, since we have already verified it to be a subgroup in an earlier result. Let $r\in R$ and $m\in \rho^{-1}(K)$. Then 
    $$
        \rho(rm)= r\rho(m)\in K, 
    $$
    hence $rm\in \rho^{-1}(K)$.
\end{proof}
\begin{lemma}
    Let $R$ be a ringz, $A$ a set and $\{M_\alpha\}_{\alpha\in A}$ be a family of left $R$-modules indexed by $A$. Then 
    $$\prod_{\alpha\in A} M_\alpha$$
    is a left $R$-module.
\end{lemma}
\begin{proof}
    Note that by Proposition~\ref{CertainSubgroupsOfProductGroup} the direct product of a family of left modules is an additive group. We check the left case. Let $r_1,r_2\in R$, $(m_\alpha),(m_\alpha')\in \prod_{\alpha \in A} M_\alpha$. Then 
    \begin{align*}
        &1. \quad (r_1+r_2)(m_\alpha)=((r_1+r_2)m_\alpha)=(r_1m_\alpha+ r_2m_\alpha)=(r_1m_\alpha)+(r_2m_\alpha)\\
        &=r_1(m_\alpha)+r_2(m_\alpha).\\
...
\end{proof}
\begin{remark}
     Suppose $\{M_\alpha\}_{\alpha\in A}$ is a family of $(R,S)$-bimodules and let $r\in R$, $s\in S$. Then 
    $$r((m_\alpha)s)= r(m_\alpha s) = (r(m_\alpha s))=((rm_\alpha)s) = (rm_\alpha)s=(r(m_\alpha))s.$$
    hence a product of $(R,S)$-modules is again also an $(R,S)$-bimodule.
\end{remark}
\begin{proposition}
    Let $A$ be a set, $R$ a ring and $\{M_\alpha\}_{\alpha \in A}$ a family of left modules. Then 
    $$\pi_\beta : \prod_{\alpha\in A} M_\alpha \rightarrow M_\beta$$
    is a left $R$-module homomorphism. Given a left $R$-module $N$ and a family of left $R$-module homomorphisms $\{f_\alpha : N \rightarrow M_\alpha\}_{\alpha\in A}$ then the unique group homomorphism $f: N\rightarrow \prod_{\alpha\in A} M_\alpha$ (cf.  Proposition~\ref{ProductGroupIsProductInCategoryOfGroups}) such that $\pi_\alpha \circ f = f_\alpha$ for every $\alpha \in A$ is a left $R$-module homomorphism. Thus we conclude that the direct product is the product in the category of left $R$-modules 
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{lemma}
    Let $R$ be a ring, $M$ a left $R$-module, $A$ a set and $\{M_\alpha\}_{\alpha\in A}$ a family of left $R$-submodules. Then 
    $$\bigcap_{\alpha\in A} M_\alpha$$
    is a left $R$-submodule.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{proposition}
    Let $A$ be a set, $R$ a ring and $\{M_\alpha\}_{\alpha \in A}$ a family of left $R$-modules. 
    Then
...
\begin{lemma}\label{QuotientModule}
    Let $R$ be a ring $M$ be a left $R$-module. Let $N\subset M$ be a left $R$-submodule. Then $M/N$ is a left submodule under the left scalar multiplication $r(m+N) := rm+N$ resp. $(m+N)r := mr+N$. If $M$ is an $(R,S)$-bimodule for some ring $S$ and $N$ is a left $R$-submodule and a right $S$-submodule. Then $M/N$ is an $(R,S)$-bimodule.
\end{lemma}
\begin{proof}
    Let $r_1,r_2\in R$, $m+N,m'+N\in M/N$. Then 
    \begin{align*}
        &1. \quad (r_1+r_2)(m+N) = (r_1+r_2)m+N = (r_1m+r_2m)+N \\ & \quad \ =(r_1m+N)+(r_2m+N)= r_1(m+N) + r_2(m+N).\\
        &2. \quad  r_1((m+m')+N)=r_1(m+m')+N = (r_1m+r_1m')+N\\ &  \quad \ = (r_1m+N)+(r_1m'+N)= r_1(m+N)+r_1(m'+N).\\ 
        &3. \quad 1(m+N) = 1m+N = m +N\\
        &4. \quad (rr')(m+N)=(rr')m+N = r(r'm)+N= r(r'm+N)=r(r'(m+N)).
    \end{align*}
    Suppose $M$ is an $(R,S)$-bimodule. Let $r\in R$, $s\in S$. Then 
    \begin{align*} 
        r((m+N)s) &= r(ms+N)=r(ms)+N = (rm)s+N = (rm+N)s\\ 
        &=(r(m+N))s.
    \end{align*}
\end{proof}
\begin{corollary}
    The canonical surjective group map $\pi : M\rightarrow M/N$ is a left module map
\end{corollary}
\begin{proof}
    Let $r\in R$, $m\in M$. Then $\pi(rm)=rm+N=r(m+N)=r\pi(m).$
\end{proof}
\begin{lemma}\label{SubmodulesOfQuotientModule}
    Let $R$ be a ring and $M$ a left $R$-module and $N\subset M$ a left $R$-submodule. Then there is one-to-one correspondence between the sets 
    $$U=\{L\subset M : L \text{ is an } R\text{-submodule of M containing }N \}$$
    and 
    $$U' = \{ K \subset M/N : K \text{ is an } R\text{-submodule of } M/N\}.$$
\end{lemma}
\begin{proof}
     This is a corollary of Proposition~\ref{ClassificationOfSubgroupsOfQuotientGroup}. Note that by Lemma~\ref{QuotientModule}, $u:U\rightarrow U'. L\mapsto L/N$ is well-defined- Note that $U\subset S$ and $U'\subset S'$. Let $K\in U'$, we check that $L(K) := \{m\in M : m+N\in K\}$ is an $R$-submodule. Let $r\in R$, $m\in L(K)$. Then $(rm+N)=r(m+N)\in M/N$, hence $rm\in L(K)$. Then $u' : U'\rightarrow U, K\mapsto L(K)$ is well-defined. One easily verifies that $u$ and $u'$ are mutual inverses.   
\end{proof}
\begin{lemma}\label{SumOfSubmodulesIsSubmodule}
    Let $R$ be a ring, $M$ a left $R$-module, $A$ a set and $\{M_\alpha\}_{\alpha\in A}$ a family of left $R$-submodules of $M$. Then 
    $$
        s : \bigoplus_{\alpha\in A} M_\alpha \rightarrow M, (m_\alpha)\mapsto \sum_{\alpha\in A} m_\alpha
    $$ hence
...
\end{proof}
\begin{theorem}
    Let $N\subset M$ be a left $R$-submodule of a left $R$-module $M$. Then the coequaliser of 
    $$
        \begin{tikzcd}
            N \arrow[r,"\iota", shift left=0.5ex] \arrow[r,"0",shift right=0.5ex, swap] & M
        \end{tikzcd}
    $$
    is $M/N$.
\end{theorem}
\begin{proof}
    Given any left $R$-module map $\alpha : M \rightarrow K$. The universal property of $M/N$ in the category of groups tells that $\beta : M/N \rightarrow K, m+N \mapsto \alpha(m)$ is the unique group map such that $\alpha = \beta \pi$. It remains to verifiy that $\beta$ is a left $R$-module map and this is trivial. 
\end{proof}
Immediately we get the following corollaries: 
\begin{corollary}\label{WellDefinednessOfQuotientModuleMaps}
    Let $\alpha : M \rightarrow N$ be a left $R$-module homomorphism and $I\subset M$ $J\subset N$ be left $R$-submodules. Then 
    \begin{gather*}
        \beta : M/I \rightarrow N/J\\
        m+I \mapsto \alpha(m)+J    
    \end{gather*}
     is a well-defined left $R$-module map if and only if $\alpha(I)\subset J$. If so $\beta$ is the unique left $R$-module map such that the square
     $$
        \begin{tikzcd}
            M\arrow[r,"\alpha"]\arrow[d, "\pi_I"] & N\arrow[d,"\pi_J"]\\
            M/I \arrow[r, dashrightarrow] & N/J
        \end{tikzcd}
     $$
     commutes
\end{corollary}
\begin{corollary}
    The cokernel of a left $R$-module map $\alpha : M\rightarrow N$ is the coequaliser of the diagram 
    $$
        \begin{tikzcd}
            M \arrow[r, shift right=0.5ex, "\alpha", swap] \arrow[r, shift left=0.5ex,"0"] & N 
        \end{tikzcd} 
    $$
\end{corollary}
\begin{corollary}\label{FirstIsomorphismTheoremForModules}(First Isomorphism Theorem for Modules)\\ 
For any left $R$-module map $\alpha : M\rightarrow N$ 
    \begin{gather*}
        \beta : M/\ker \ \alpha \rightarrow \im \ \alpha\\
        m+ \ker \ \alpha \mapsto \alpha(m)
    \end{gather*}
is an isomorphism.
\end{corollary}
Another result that one could note that is that 
\begin{lemma}
    The kernel of a left $R$-module map $\alpha : M \rightarrow N$ is the equaliser of $\alpha,0 : M\rightarrow N$.
\end{lemma}

\begin{definition}
    Let $R$ be a ring and $M$ a left $R$-module. Then $M$ is \emph{generated over by a set $X\subset M$ over $R$} if 
    $$
        M = \sum_{x\in X} Rx.
    $$
    $M$ is said to be \textit{finitely generated over $R$} if there is a finite sequence $m_1,\dots,m_n\in M$ such that $_RM= \sum_1^n Rm_i$ resp. $M_R = \sum m_i R$. 
\end{definition}
\begin{definition}
    Let $A$ be a set, $R$ a ring and $\{M_\alpha\}_{\alpha\in A}$ a family of left $R$-modules. We say that $\sum_{\alpha\in A} M_\alpha$ is \textit{direct}, if 
    for every $\beta \in A$,
    $$M_\beta \cap \sum_{\alpha\in A \setminus\{\beta\}} M_\alpha = 0.$$
\end{definition}
\begin{lemma}
    Let $A$ be a set, $R$ a ring and $\{M_\alpha\}_{\alpha\in A}$ a family of left $R$-modules such that $\sum_{\alpha\in A} M_\alpha$ is direct. Then 
...
\end{proof}
\begin{definition}
    Let $R$ be a ring and $M$ a left $R$-module. A subset $X\subset M$ is said to be \textit{left linearly independent over $R$} (or if $R$ is commutative just \textit{linearly independent}), if for every finite sequence of distinct $m_1,\dots,m_n\in X$ and every finite sequence $r_1,\dots,r_n\in R$, 
    \begin{gather*}
        \sum_1^n r_im_i = 0 \iff r_i = 0\ \forall i\in \{1,\dots,m\}
    \end{gather*}
\end{definition}
\begin{remark}
...
\end{remark}
\begin{proposition}
    Let $R$ be a ring, $M$ a left $R$-module and $X\subset M$ a subset. Then if $X$ is left linearly independent over $R$, $\sum_{x\in X} Rx$ is direct. 
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{definition}
    Let $R$ be a ring, $M$ a left $R$-module. A subset $X\subset M$ is called a \textit{basis of $M$ over $R$} if $X$ is linearly independent over $R$ and $M= RX$ respectively $M=XR$. If $X$ is finite and a basis of $M$ over $R$ it is called a \textit{finite basis}.
\end{definition}
\begin{lemma}\label{BasisOfDirectSum}
    Consider a ring $R$ and a non-empty set $X$. Then 
    $$
        \bigoplus_{x\in X} R
    $$
    has basis 
    $$
        D:=\left.\left\{ (\delta_{yx})\in \bigoplus_{x\in X} R \ \right| \ x\in X\right\}
    $$
    where 
    $$
         \delta_{yx} = 
...
\end{lemma}
\begin{proof}
    Let $(r_x)\in \bigoplus_x R$. Then for each $x\in X$,
    \begin{align*}
        (r_x)(x) &= r_x = \sum_{y\in \mathrm{supp} \ (r_x)} r_y\delta_{yx} =  \left(\sum_{y\in \mathrm{supp} \ (r_x)}r_x(\delta_{zy})(x)\right)\\ 
        &= \left(\sum_{y\in \mathrm{supp} \ (r_x)}r_x(\delta_{zy})\right)(x), 
    \end{align*}
    hence 
    $$(r_x) = \sum_{x \in X} r_x(\delta_{yx})\in \sum_{x\in X} R(\delta_{yx})$$
    Therefore $D$ generates $\bigoplus_{x\in X} R$. Suppose 
    $$
        \sum_{1}^m a_i(\delta_{yx_i}) = \sum_1^m b_i(\delta_{yx_i}).
    $$
    Then for each $j$,
    $$
        a_j = \left(\sum_{1}^m a_i(\delta_{yx_i})\right)(x_j) = \left(\sum_1^m b_i(\delta_{yx_i})\right)(x_j)=b_j.
    $$
    So $D$ is linearly independent over $R$. 
\end{proof}
The following gives an argument for why we get to define a module map by only defining a map on a basis.
\begin{lemma}
    Let $M$ and $N$ be left $R$-modules with a left basis $\pazocal{B}$ of $M$ over $R$. Consider a function $\rho : \pazocal{B}\rightarrow N$. Then there is a unique extension of $\rho$ to a left $R$-module map $\rho: M\rightarrow N$. 
\end{lemma}
\begin{proof}
    We define 
    \begin{gather*}
        \rho : \sum_{v\in \pazocal{B}} a_v v = M \rightarrow N \\
        \sum_{v} a_v v \mapsto \sum_v a_v\rho(v)
    \end{gather*}
    Note that the left linear independence of $\pazocal{B}$ over $R$, ensures that the representation of $v$ in terms of $\pazocal{B}$ is unique, so this is well-defined. Let $a,b\in R$ and $\sum_{v} a_v v,\sum_{v} b_v v\in M$. Then 
    \begin{align*}
        \rho\left(\left[a\sum_{v}a_v\right] +\left[b\sum_v b_vv \right]\right) &= \rho\left( \sum_v (aa_v+bb_v) v\right)\\ &= \sum_v (aa_v+bb_v)\rho(v)\\ &= a\sum_v a_v \rho(v) + b\sum_v b_v\rho(v)\\ 
        &= a\rho\left(\sum_v a_v v\right)+b\rho\left(\sum_v b_v v\right). 
    \end{align*}
    so this is indeed a linear map. Suppose $\rho' : M\rightarrow N$ is a linear map that restricts to $\rho: \pazocal{B} \rightarrow N$. Then 
    $$
        \rho'\left(\sum_v a_v v\right) = \sum_va_v \rho'(v)=\sum_v a_v \rho(v)= \rho\left(\sum_va_vv\right),
    $$
    which proves the uniqueness of $\rho$.
\end{proof}
We denote category of left $R$-modules with finite left bases by 
$$
    \mathrm{mod}_R^{\mathrm{fb}}
$$
\begin{lemma}
        Fix a ring $R$ and consider the set 
        $$
            \{R^n : n\in \N\}
        $$
        This together with left $R$-linear maps restricted to this set, define the skeleton of $\mathrm{mod}_R^{\mathrm{fb}}$.
    \end{lemma}
    \begin{proof}
        Let $M$ be a left $R$-module with basis $\pazocal{V}:=\{v_1,\dots,v_n\}$. Consider the left $R$-linear map 
        \begin{gather*}
            R^n \rightarrow M\\
            (a_1,\dots,a_n) \mapsto \sum_1^n a_iv_i
        \end{gather*}
        This is surjective since $\pazocal{V}$ generates $M$ over $R$ and injective since $\pazocal{V}$ is left linearly independent over $R$. 
    \end{proof}
\begin{corollary}\label{ModuleWithFiniteLeftBasisOverFiniteRingIsFinite}
    Suppose $R$ is a finite ring and $M$ a left $R$-module with finite left basis, then $M$ is finite. 
\end{corollary}
\begin{proof}
    Indeed, $M\simeq R^n$ for some $n\geq 0$, hence $M$ is finite. 
\end{proof}
\begin{proposition}
    Let $S$ be a ring and $R\subset S$ a subring. Consider $(M,\cdot,+)$, a left $S$-module, then $rm:= r\cdot m$, for $r\in R$, defines a structure of left $R$-modules. If in addition $Q$ is a subring of a ring $T$ and $M$ is an $(S,T)$-bimodule, then $M$ is an $(R,Q)$-bimodule.  
\end{proposition}
\begin{proof}
...
\end{definition}
\begin{lemma}
    Let $\sigma : R \rightarrow S$ be a ring homomorphism and $I \subset S$ be a left ideal. Then $\sigma^{-1}(I)\subset R$ is a left ideal. 
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{ImageOfSurjectiveRingHomOfIdealIsIdeal}
    Let $\sigma : R \rightarrow S$ be a surjective ring homomorphism and $I\subset R$ be a left ideal. Then $\sigma(I)\subset S$ is a left ideal. 
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{remark}
    We call $RX$ and $XR$ \textit{the left ideal generated by $X$}. \textit{The ideal generated by $X$} is the ideal $\langle X\rangle := R(XR) = (RX)R$.\\ 
...
\begin{lemma}\label{IdealIsNonProperIff1IsInIdeal}
    Let $R$ be a ring and $I\subset R$ a left ideal. Then 
    $$1\in I \iff I = R.$$
\end{lemma}
\begin{proof}
...
\begin{proposition}\label{EliminationIdeal}
    Let $S$ be a ring and $R\subset S$ a subring. Consider a left ideal $I\subset S$. Then $I\cap R\subset R$ is an ideal.
\end{proposition}
\begin{proof}
    $R$ and $I$ are both left $R$-submodules of $S$, hence so is $R\cap I$.
\end{proof}
...
\subsubsection{Quotient Rings}
    ...
    Let $R$ be a ring and $I\subset R$ a two-sided ideal. View $I$ as a subgroup of the additive group $(R,+)$. Then $(R/I,+)$ is an additive group by Corollary~\ref{QuotientGroupOfAdditiveGroupIsAdditive}. Define 
...
\end{proof}
\begin{lemma}
    Consider a ring map $\sigma: R\rightarrow S$. $(S,+)$ is an $(R,R)$-bimodule with the scalar multiplication
    $$
        rs := \sigma(r)s \quad (r\in R, s\in S)
    $$
\end{lemma}
\begin{proof}
    This follows from $S$ being an $(\im \ \sigma,\im \ \sigma)$-bimodule. 
\end{proof}
\begin{theorem}
    Let $R$ be a ring and $I$ a left ideal of $R$. Then $R/I$ is the coequaliser of the left $R$-module maps $0,\iota: I\rightarrow R$. Given a ring map $\sigma: R\rightarrow S$, considered as a $(R,R)$-bimodule map in the canonical way (cf. the prior lemma), the $(R,R)$-bimodule map 
    \begin{gather*}
        \tau : R/I \rightarrow S\\
        r+I \mapsto \sigma(r)
    \end{gather*}
    is the unique map such that $\tau\pi=\sigma$. The map $\tau$ is then the unique ring map with this property. 
\end{theorem}
\begin{proof}
    This follows readily from $\sigma$ being a ring map
\end{proof}
\begin{corollary}\label{FirstIsomorphismTheoremOfRings}(First Isomorphism Theorem of Rings)\\
    For a ring map $\sigma: R\rightarrow S$, the ring map 
    \begin{gather*}
        \tau: R/\ker\ \sigma \rightarrow \im \ \sigma
    \end{gather*}
    is an isomorphism of rings. 
\end{corollary}
\subsubsection{Noetherian and Artinian Modules}
\begin{definition}\label{NoetherianArtinianDef}
    Let $M$ be a left $R$-module and 
    $$\mathcal{M} := \left\{ N\in 2^M : N \text{ is a left/right submodule of } M \right\}$$
    be a chain. We say that $M$ is \textit{left noetherian} if every ascending chain in $\mathcal{M}$ stabilizes and \textit{left artinian} if every descending chain in $\mathcal{M}$ stabilizes. A ring $S$ is left noetherian/artinian, if it is noetherian/artinian as a left $S$-module or simply noetherian/artinian if is both left and right noetherian/artinian.  
\end{definition}
\begin{definition}
    A simple left/right $R$-module is one whose only submodules are $0$ and $M$.
\end{definition}
\begin{example}
    Any simple left $R$-module $M$ is left noetherian/artinian. A family of submodules of $M$ is of the form $\{0,M\},\{M\}$ or $\{0\}$. These are all finite sets, hence any chain $C$ is finite and thus has an upper/lower bound in $C$.
\end{example}
Assuming the axiom of choice we get the following result.
\begin{proposition}\label{NoetherianRespArtinianImpliesMaximalMinimalElement}
    If $M$ is a left $R$-module and $X$ is a non-empty set of submodules of $M$, and $M$ is noetherian/artinian then $X$ has maximal/minimal element
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{definition}
    Consider a left $R$-module $M$. A \emph{maximal left $R$-submodule of $M$} is a maximal element of the family of all proper left $R$-submodules   
\end{definition}
...
\begin{definition}
    ...
    \begin{align*}
        R[s_1,\dots,s_n] &:= \left\{ \sum_{v=(v_1,\dots,v_n)\in \N^n} a_vs_1^{v_1}\cdots s_n^{v_n} : (a_v)\in \bigoplus_{v\in\N^n} R \right\}.
    \end{align*}
...
\end{definition}
\begin{lemma}
    Let $R$ be a ring, consider $R$-algebras $A,B$ and a map $\sigma : A\rightarrow B$. Then $\sigma$ is an $R$-algebra homomorphism if and only if $\sigma$ is a multiplicative, additive map fixing $R$, i.e. for every $a_1,a_2\in A$, $\sigma(a_1a_2)=\sigma(a_1)\sigma(a_2)$ and for every $r\in R$, $\sigma(r) = r$. Here $r := r1$.
\end{lemma}
\begin{proof}
    If $\sigma$ is an $R$-algebra homomorphism, then it is by definition multiplicative. Moreover, since $\sigma$ is an $R$-module map, 
    $$
        \sigma(r)=r\sigma(1)= r1.
    $$
    If $\sigma$ is a multiplicative, additive map fixing $R$, then 
    $$
        \sigma(ra)=\sigma(r)\sigma(a)=r\sigma(a).
    $$
\end{proof}
\subsubsection{Introducing Tensor Products}
\begin{definition}
    
\end{definition}
\subsection{Defining the Polynomial Ring over a Commutative Ring}
We are interested in studying expressions formed by variables $x_1,\dots, x_n$ under addition, multiplication and an action of a ring $R$. I.e. we want to study expressions
$$
    \sum_{v\in \N^n} a_vx_1^{v_1}\cdots x_n^{v_n}
$$
where $a_v = 0$ for all but finitely many $v$. These should sit in some commutative $R$-algebra, which should be generated by these variables. It should be the minimal ring containing these variable, and we thus denote it by $R[x_1,\dots,x_n]$. Moreover, we want, for instance $x$ and $x^2$, to be different elements of the rings. Polynomial functions $R^n \rightarrow R$ will not suffice, since when $R=\Z/2\Z$, these are same. The property that captures the desired behavior is the following 
\begin{definition}
    Let $S$ be an $R$-algebra. $X\subset S$ is \emph{algebraically independent over $R$} if for each $n\geq 1$, and each $x_1,\dots,x_n\in A$,
    $$
        \sum_{v\in \N} a_v x_1^{v_1}\cdots x_n^{v_n} \neq 0 
    $$ 
    whenever $(a_v)\in \bigoplus_{v\in \N^n} R$ is non-zero. 
\end{definition}
\begin{remark}
    $X$ being algebraically independent is equivalent to 
    $$
        \left\{ x_1^{v_1}\cdots x_n^{v_n} : n\geq 1, (v_1,\dots,v_n)\in \N^n, x_1,\dots,x_n\in X \right\}
    $$
    being linearly independent over $R.$
\end{remark}
A motivation for constructing the polynomial ring is that they are an essential gadget to the study of finitely generated algebras over $R$. What we will realize is that there are always infinitely many such for any arbitrary $R$ and that these are of the form $R[x_1,\dots,x_n]/I$. In the case where $R$ is infinite, the polynomial functions will be a sufficient construction to live up this fact, but as alluded to the finite rings will need a more general construction
\subsubsection{The Free Algebra Generated by a Set}
We will introduce the notion of the free algebra generated by a set. For this we fix a set $X$ and a ring $R$.
\begin{definition}
    Let $R$ and $S$ be rings. We define \emph{the free $(S,R)$-bimodule generated by $X$} to be the $(R,S)$-bimodule
    $$
        \bigoplus_{x\in X} R \otimes_\Z S.
    $$
    Then \emph{the free $(R,R)$-module (or just $R$-module when $R$ is commutative)} is 
    $$
...
$$
    ...
    \end{cases}
$$
We thus take $x\in R[X]$ to mean $\delta_{x}$. We can thus write any element $(a_x)\in R[x]$ as a sum 
$$
    \sum_{x\in X} a_x x.
$$
The assignment of $X$ to it's free left $R$-module is functorial. Indeed, we have a functor
\begin{gather*}
    \mathrm{Set} \rightarrow \mathrm{Mod}_R\\
    X \mapsto R[X]\\
    f : X \rightarrow Y \mapsto \overline{f} : R[X]\rightarrow R[Y]
\end{gather*}
where 
\begin{gather*}
    \overline{f} : \sum_{x\in X} a_x x \mapsto \sum_{y\in Y} \left(\sum_{x \in f^{-1}(y)} a_x \right)y
\end{gather*}
We now continue under the assumption that $R$ is commutative. We want to give an analogous construction of a free algebra over a ring, which when $R=\Z$ leads to the notion of the free ring generated by a set. For this we also suppose that $X$ is a monoid $(X,e,\cdot)$. Let $\sum_{x\in X} a_x x,\sum_{x\in X} b_x x\in R[X]$. We define 
$$
    \left(\sum_{x\in X} a_x x\right)\left(\sum_{x\in X} b_x x\right) := \sum_{x\in X} \left(\sum_{yz =x} a_yb_z \right)x.
$$
One checks that this operation is associative, distributes over addition and that the function taking $e$ to $1$ and every other element to $0$ is the neutral element with respect to this operation. Moreover, one checks that the map taking $a\in R$ to the function that sends $e$ to $a$ and every other element to $0$ is an embedding. It follows that $R[X]$ is an $R$-algebra. One also checks that the functor $X\mapsto R[X]$ restricts to a functor 
$$
    \mathrm{Mon} \rightarrow \mathrm{Alg}_R
$$
\begin{definition}
    For a set $X$, \emph{The free $R$-algebra generated by $X$} is the $R$-algebra generated by $X_\N$. 
\end{definition}
Note that any element of this set can be written as a sum 
$$
    \sum_{k\in \N}\sum_{i_1,\dots,i_k\in \{1,\dots,n\}} a_{i_1,\dots,i_k} x_{i_1}\cdots x_{i_k}
$$
for some $n$ and some $x_1,\dots,x_n\in X$. In fact, any finite selection of elements in $R[X_\N]$ can be written in this way for a sufficiently large $n$ and a list of elements $x_1,\dots,x_n\in X$. Note also that $X\subset R[X_\N]$ generates $R[X_\N]$ as an algebra over $R$.\\
An interesting question that arises from our definition of the free $R$-algbebra generated by $X$, in the case where $X$ itself has some monoid structure. Is the relation of $R[X]$ and $R[X_\N]$. We let $\ast$ denote the operation of concatenation on $X_\N$. To investigate the question, consider the monoid map
\begin{gather*}
    f : X_\N \twoheadrightarrow X\\
    x_1\ast \cdots \ast x_n \mapsto \prod_{1}^n x_i
\end{gather*} 
note that this is clearly multiplicative and that the empty string maps to the empty product, which by convention is $e$. This then induces a map of $R$-algebras
\begin{gather*}
    \overline{f} : R[X_\N] \twoheadrightarrow R[X]\\
    \sum_{k,i_1,\dots,i_k} a_{i_1,\dots,i_k} x_{i_1}\ast\cdots\ast x_{i_k} \mapsto \sum_{x\in X} \left(\sum_{ x_{i_1}\ast\cdots\ast x_{i_k}\in f^{-1}(x)} a_{i_1,\dots,i_k} \right) x 
\end{gather*}
Set 
$$
    J := \langle x\ast y - xy : x,y\in X \rangle.
$$
Note that for each $x,y\in X$, 
$$
    \overline{f}(x\ast y-xy) = f(x\ast y)-f(xy) = xy-xy = 0, 
$$
hence $J\subset \ker \ \overline{f}$. Conversely, if $\sum_{k,i_1,\dots,i_k} a_{i_1,\dots,i_k} x_{i_1}\ast \cdots \ast x_{i_k}\in \ker \ \overline{f}$. Then 
\begin{align*}
    \sum_{k,i_1,\dots,i_k} a_{i_1,\dots,i_k} &x_{i_1}\ast \cdots \ast x_{i_k} + J = \sum_{k,\i_1,\dots,i_k} a_{i_1,\dots,i_k}x_{i_1}\cdots x_{i_k} + J\\ &= \sum_{x\in X} \left( \sum_{x_{i_1}\ast \cdots \ast x_{i_k}\in f^{-1}(x)}a_{i_1,\dots,i_k} \right) x +J\\ 
    &= \overline{f}\left(\sum_{k,i_1,\dots,i_k} a_{i_1,\dots,i_k}x_{i_1}\ast\cdots \ast x_{i_k} \right) + J = 0.  
\end{align*}
It thus follows that $\ker\ \overline{f} = J$. 
\subsubsection{The Free Commutative Algebra}
Note that when $X$ is a commutative monoid, so is $R[X]$.
In the general, where $X$ is any set, we consider the commutative monoid generated by $X$:
\begin{definition}
    \emph{The free commutative $R$-algebra generated by any set $X$} is the commutative $R$-algebra
    $$
        R[X_\N^\mathrm{Comm}].
    $$
\end{definition}
In the case where $X$ is an additive monoid, it is again natural to ask how $R[X]$ and $R[X_{\N}^\mathrm{Comm}]$ relate to each other. Note that we have monoid maps
\begin{gather*}
    f: X_\N \twoheadrightarrow X\\
    x_1\ast \cdots \ast x_n\mapsto \sum_1^n x_i
\end{gather*}
and
\begin{gather*}
    f' : X_\N^\mathrm{Comm} \twoheadrightarrow X\\ 
    x_1\ast \cdots \ast x_n \mapsto \sum_1^n x_i 
\end{gather*}
and 
\begin{gather*}
    g:X_\N \twoheadrightarrow X_\N^\mathrm{Comm}\\
    x_1\ast \cdots \ast x_n \mapsto x_1\ast\cdots \ast x_n  
\end{gather*}

These induce $R$-algebra maps $\overline{f} : R[X_\N] \twoheadrightarrow R[X]$, $\overline{f'}: R[X_\N^\mathrm{Comm}] \rightarrow R[X]$ and $\overline{g}:R[X_\N] \twoheadrightarrow R[X_\N^\mathrm{Comm}]$. We have already seen that 
$$
    \ker \ \overline{f} = \langle x\ast y - x+y : x,y\in X\rangle. 
$$ 
Similarly, 
$$
    \ker \ \overline{f'} = J := \langle x\ast y - x+y : x,y\in X\rangle.
$$
Moreover, it is readily verified that 
$$
    \ker \ \overline{g} = I := \langle x\ast y -y\ast x : x,y\in  X\rangle. 
$$
We summarize above results:

\begin{proposition}
    When $(X,+,e)$ is an additive/commutative monoid, $R[X]$ is a commutative $R$-algebra generated by $X$. Moreover, if $Y\subset X$ is such that each $x\in X$ can be written uniquely as a sum 
    $$
        \sum_{y\in Y} v_y y \in \sum_{y\in Y} \N y, 
    $$   
    then $Y\subset R[X]$ generates $R[X]$ and is algebraically independent over $R$.
\end{proposition}
\begin{proof}
    It remains to show that $Y$ is algebraically independent over $R$. The condition on $Y$ implies that each $x\in R[X]$ can be written uniquely as a product $y_1^{v_1}\cdots y_n^{v_n}$ for some $(v_1,\dots,v_n)\in \N^n$ for some $n$. Since $X$ forms a basis for $R[X]$ over $R$, it follows that 
    $$
        \left\{y_1^{v_1}\cdots y_n^{v_n} : n\geq 0, v\in \N^n, y_1,\dots,y_n \in Y \right\}.
    $$
    This is implies that $Y$ is algebraically independent over $R$. 
\end{proof}
In particular, we have the following result.
\begin{corollary}
    $R[X_\N^\mathrm{comm}]$ is a commutative $R$-algebra with $X$ being algebraically independent and generating it over $R$. 
\end{corollary}
\begin{remark}
    The elements of a subset $Y\subset X$ discussed above are called variables of $R[X]$ .
\end{remark}
\subsubsection{Polynomial Rings}
    We can now define, what we mean by a polynomial ring over $R$. The above discussions have given us at least two ways of doing so. The first one is the following 
    \begin{definition}
        Let $X=\{x_1,\dots,x_n\}$ be a set with $n$ elements. \emph{The polynomial ring in $n$ variables over $R$} is the commutative $R$-algebra $R[X_\N^\mathrm{Comm}]$. Since $R[X_\N^\mathrm{Comm}] = R[x_1,\dots,x_n]$, we opt to simply denote this ring by $R[x_1,\dots,x_n]$.
    \end{definition} 
    We may also define it as follows 
    \begin{definition}
        \emph{The polynomial ring in $n$ variables over $R$} is the $R$-algebra $R[\N^n]$
    \end{definition}
    Note that each $v = (v_1,\dots,v_n)\in \N^n$ is uniquely written as 
    $$
        \sum_1^n v_i e_i,
    $$
    where $e_i=(e_{i1},\dots,e_{in})$ with $e_{ij}=0$ for $j\neq i$ and $e_{ii}=1$, so indeed, taking $x_i$ to be the image of $e_i$ in $R[\N^n]$ it follows that $x_1,\dots,x_n$ are the variables of $R[\N^n]$.
    \begin{definition}
        For each $v=(v_1,\dots,v_n)\in \N^n$, the polynomial 
        $$
            x_1^{v_1}\cdots x_n^{v_n}
        $$
        is called the \emph{monomial of $v$}.
    \end{definition}
    To simplify notation, when we write something like $\mathbf{x}^v\in R[x_1,\dots,x_n]$, we take it to mean the monomial of $v$ in the variables $x_1,\dots,x_n$. In general, if we use an arbitrary letter for variables (e.g. $y_{\_}$) we will use the bold face version of that letter to denote monomials in those variables.
\subsubsection{An Example of a Non-noetherian Ring}
    The notion of a free algebra generated by a set leads an easy way of generating non-noetherian rings. Equivalently, there is an example of a module that is finitely generated for which there is a submodule that is not finitely generated. 
    \begin{definition}
        Given a commutative ring $R$, \emph{the polynomial ring in countably many variables} is the ring $R[\N_\N^{\mathrm{Comm}}]$. We denote it by $R[x_1,x_2,\dots]$. 
    \end{definition}
    \begin{proposition}
        $R[x_1,x_2,\dots]$ is not noetherian.
    \end{proposition}
    \begin{proof}
        ...
        $$
        This does not stabilize since $x_{n+1}\notin \langle x_1,\dots,x_n\rangle$ for each $n$. 
   \end{proof}
   One could also have proven that the ideal generated by the variables of $R[x_1,x_2,\dots]$ is not finitely generated. 
...
\end{proof}
\begin{corollary}
    Let $R$ be a ring $M$ a left $R$-module. Consider submodules $N,N'\subset M$ with $N'\subset N$. Then 
    $$\frac{M/N'}{N/N'}\simeq M/N.$$
\end{corollary}
\begin{proof}
    Follows from the 1st isomorphism theorem for modules: Consider the map
    \begin{gather*}
        M/N' \rightarrow M/N\\
        m + N' \mapsto m+N
    \end{gather*}
    This is a surjective left $R$-module map. Suppose $m+N = 0+N$, then $m+N'\in N/N'$, hence the kernel of this map is $N/N'$, so
    $$
        \frac{M/N'}{N/N'}\simeq M/N.
    $$ 
\end{proof}
...
\end{corollary}
\begin{proof}
    Follows from the 1st isomorphism theorem for rings and the fact that 
    \begin{gather*}
        R/J \rightarrow R/I\\
        r+ J \mapsto r+I
    \end{gather*}
...
\begin{lemma}(The Splitting Lemma)
    Let $R$ be a ring and $A,B,C$ be left $R$-modules and 
    $$
        \begin{tikzcd}
            0 \arrow[r] & A \arrow[r,"\alpha"] & B\arrow[r,"\beta"] & C \arrow[r] & 0
        \end{tikzcd}
    $$
    a short exact sequence. The following are equivalent,
    \begin{enumerate}
        \item $\beta$ is split epi.
        \item $\alpha$ is split mono.
        \item $\gamma: B\simeq A\oplus C$, $\beta\gamma=\pi_2$ and $\gamma^{-1}\alpha=\iota_1$.
    \end{enumerate}
\end{lemma}
\begin{proof}
    "3.$\implies$ 1. and 2.": There are maps 
    \begin{gather*}
        \iota: C\rightarrow B\\
        x\mapsto \gamma((0,x))
    \end{gather*}
    ...
    \begin{gather*}
        \mu : B\rightarrow A\\
        y\mapsto \pi_1(\gamma^{-1}(y))
    \end{gather*}
    Then 
    $$
        \beta\iota(x)= \beta\gamma(0,x) = \pi_1(0,x)=x = \id_C(x)
    $$
    for each $x\in C$. We also have that 
    $$
        \mu\alpha(y)=\pi_1\gamma^{-1}\alpha(y) = \pi_1\iota_1(y) = y = \id_A(y) 
    $$
    for each $y\in A$.\\ 
    "1.$\implies$ 3.": Suppose $\beta$ is split epi. Then there is a $\iota$ which is a section of $\beta$. Note that if  $b\in \ker \ \beta \cap \im \ \iota$, then $b=\iota(c)$ for some $c\in C$, hence $c=\beta(\iota(c)) =0$, implying $b=0$. Moreover 
    $$
        b= (b-\iota\beta(b))+\iota\beta(b)\in \ker\ \beta + \im\ \iota 
    $$
    for each $b\in B$, hence $B=\ker\beta \oplus \im \ \iota$.
    Note that 
    $$
        A\simeq \im\ \alpha  =\ker\ \beta 
    $$ 
    via $\alpha$, since $\alpha$ is injective (since it is in particular a monomorphism) and that 
    $$
        C\simeq \im \ \iota 
    $$
    via $\iota$ since this is a section and therefore an injective map. We thus get that 
    $$
        B = \ker \beta \oplus \im\ \iota \simeq A \oplus C.
    $$ 
    Here we use functiorality of the bifunctor $\_\oplus\_ : \mod_R \times \mod_R \rightarrow \mod_R$.\\
    By construction, 
    $$
        \beta\gamma(a,c)= \beta(\alpha(a)+\overline{\iota}(c)) = \beta\alpha(a)+\beta\iota(c)= c = \pi_2(a,c).
    $$
    Secondly 
    $$
        \gamma^{-1}\alpha(a)= (\overline{\alpha}^{-1}\alpha(a),\beta\alpha(a))= (a,0) = \iota_1(a).
    $$
    Here $\overline{\alpha}$ denotes $\alpha$ when seen as a function onto its image.\\ 
    "2.$\implies$ 3.": this is dual to 1.$\implies$ 3.   
\end{proof}
...
\begin{remark}\label{SetOfMatricesIsAnAdditiveGroup}
    Note that $M_{n\times m}(R)$ is an $(R,R)$-bimodule.
\end{remark}
\begin{example}
...
\end{lemma}
\begin{proof}
    $M_n(R)$ is trivially an additive group. Let $I:=1:=  (\delta_{ij})$ (where $\delta_{ii}=1$ for $i\in \{1,\dots,n\}$ and $\delta_{ij} = 0$ for $i,j\in\{1,\dots,n\}$ with $i \neq j$). Then for $(r_{ij})\in M_n(R)$ 
    $$1(r_{ij}) = \left(\sum_{k=1}^n \delta_{ik}r_{kj}\right) = \left(\delta_{ii}r_{ij} +\sum_{k\in\{1,\dots,n\}\setminus\{i\}}\delta_{ik}r_{kj} \right) = (1r_{ij}+0)=(r_{ij}).$$
...
\end{example}
Given a ring $R$ and matrix $(a_{ij})\in M_{n\times m}(R)$ induces a left $R$-linear map 
\begin{gather*}
    (a_{ij}) : R^m\rightarrow R^n\\
    v\mapsto (a_{ij})v
\end{gather*}
In general for left $R$-modules $M$ and $N$ with bases $\{m_1,\dots,m_k\}$ respectively $\{n_1,\dots,n_l\}$ we get a left $R$-linear map
\begin{gather*}
    (a_{ij}) : M\rightarrow N\\
\end{gather*}
defined by $m_i\mapsto \sum_{j=1}^l a_{ij} n_j$. This is the unique left $R$-module map making the diagram 
$$
    \begin{tikzcd}
        M \arrow[d] \arrow[r]& N \arrow[d]\\
        R^k \arrow[r] & R^l
    \end{tikzcd}
$$
commute, where the vertical maps denote the canonical isomorphism induced by the respective bases.
This leads to a canonical description of the morphisms of $\mathrm{Mod}_R^{\mathrm{fb}}$.
\begin{lemma}
    Let $\rho : M\rightarrow N$ be a left $R$-linear map of left $R$-modules with finite bases $\pazocal{M}=\{m_1,\dots,m_k\}$ for $M$ respectively $\pazocal{N}=\{n_1,\dots n_l\}$. Then $\rho = (a_{ij})$ for some $(a_{ij})\in M_{k\times l}(R)$.
\end{lemma}
\begin{proof}
    For each $i$ there are unique $a_{i1},\dots,a_{il}\in R$ with 
    $$
        \rho(m_i) = \sum_{j=1}^l a_{ij}n_j
    $$
    since an $R$-linear map is completely determined by its behavior on basis elements, it follows that $(a_{ij})=\rho$ as left $R$-linear maps. 
\end{proof}
...
\begin{remark}
    \begin{enumerate}[I.]
        ...
        \item $(R^\ast, \cdot)$ is a group. We check that $R^\ast$ is a submonoid of $(R,\cdot)$ and hence a monoid. Clearly $1 \in R^\ast$. Let $r,r' \in R^\ast$. Then $$\left(rr'\right)\left(r'^{-1}r^{-1}\right)=r\left(r'\left(r'^{-1}r^{-1}\right)\right)=r\left(\left(r'r'^{-1}\right)r^{-1}\right)=r\left(er^{-1}\right) = rr^{-1} = 1,$$
...
\end{definition}
\begin{definition}
    An left ideal $M\subsetneq R$ is called a \textit{left maximal} ideal, if for every left ideal $I\subset R$ with $M\subset I$, either $I=M$ or $I=R$. If $M$ is an ideal with aforementioned property it is called a \textit{maximal ideal}.  
\end{definition}
\begin{definition}
    A ring $R$ is called \textit{simple} if the only two-sided ideals in $R$ are the trivial ones, i.e. $0$ and $R$.
\end{definition}
\begin{lemma}
    Let $S$ be a simple ring and $R$ any ring. Consider a ring homomorphism $\sigma : S\rightarrow R$. Then $\sigma$ is an embedding or $R$ is the zero ring. In particular, the only field morphisms are embeddings. 
\end{lemma}
\begin{proof}
    Note that $\ker\ \sigma$ is a two-sided ideal in $S$. Since $S$ is simple, either $\ker\ \sigma=0$ or $\ker\ \sigma = S$. In the first case $\sigma$ is injective and in the second case $\sigma=0$, and the only case where $0$ can be a ring map is when $R=0$. 
\end{proof}
...
\begin{lemma}\label{MaximalIdealIffQuotientRingSimple}
    Let $R$ be a ring. Let $I\subset R$ be a two-sided ideal. Then $I$ is a maximal ideal if and only if $R/I$ is a simple.
\end{lemma}
\begin{proof}
    "$\implies$": Let $J/I\subset R/I$ be a non-zero two-sided ideal, i.e. assume $I\subsetneq J\subset R$ for some two-sided ideal $J$ in $R$. Then $J = R$, hence $J/I= R/I$, implying $R/I$ is simple.\\
    "$\impliedby$": Conversely, consider an ideal $J\subset R$ such that $I\subsetneq J$. Then $0\neq J/I\subset R/I$, implying that $J/I = R/I$ and hence that $J=R$. 
\end{proof}
\begin{lemma}\label{UnionOverChainOfSubmodulesIsSubmodule}
    Let $M$ be a left $R$-module. Consider a chain $C$ of submodules of $M$. Then 
    $$\bigcup_{N\in C} N$$
    is a submodule.
\end{lemma}
\begin{proof}
...
\end{proof} 
\begin{proposition}\label{AnyProperIdealIsContainedInMaximalIdeal}
    Assuming choice, any proper left ideal in a non-zero ring is contained in left maximal ideal. 
\end{proposition}
\begin{proof}
    Let $R$ be a non-zero ring and $I\subset R$ a proper ideal. Consider the family 
    $$
        X := \{I\subset J\subset R : J \text{ is a proper ideal}\},
    $$
    partially ordered by inclusion. Every chain in $X$ has an upper bound, namely the union of the ideals in the chain (cf. the prior lemma). Indeed note that $1$ is not an element of this union, making it a proper left ideal. By Zorn's lemma, $X$ has a maximal element, and by definition such a maximal element is a maximal ideal in $R$ containing $I$. 
\end{proof}
\begin{corollary}
...
\begin{proof}
    ...
    "$\impliedby$": Since $R/I$ is a field, it is in particular a division ring. Then by Lemma~\ref{DivisionRingsAreSimple} $R/I$ is simple. Given any ideal $J\supsetneq I$, $J/I=R/I$, hence $J=R$, meaning $I$ is maximal.  
\end{proof}
\begin{definition}
...
\end{proposition}
\begin{proof}
    Let $a\in D\setminus 0$. Then $\langle a \rangle = D$ since $D$ is simple, hence $1\in \langle a\rangle$, meaning there is some $b\in D$ such that $ba = ab = 1$. 
\end{proof}
...
\end{proof}
\begin{lemma}
    Let $R\subset S$ be a commutative subring of $S$ and $I\subset S$ be a two sided ideal. Then $R+I$ is a subring of $R$.
\end{lemma}
\begin{proof}
    Clearly $1\in R+I$. Let $x,y\in R$ and $a,b\in I$, then 
    $$
        (x+a)(y+b)= \underbrace{xy}_{\in R}+\underbrace{xb+ay+ab}_{I}\in R+I.
    $$
\end{proof}
...
\end{proposition}
\begin{proof}
    By Proposition~\ref{PrimeIdealIffQuotientRingID} $S/I$ is an integral domain. Note that $S/I\supset (R+I)/I$. Hence from Lemma~\ref{SubringOfIDIsID} $(R+I)/I$ is an integral domain. It follows from Theorem~\ref{FirstNoetherIsoTheorem} that $(R+I)/I\simeq R/(I\cap R)$, hence $I\cap R$ is prime.
\end{proof}
\begin{definition}
...
\end{proof}
\begin{proposition}\label{PreImageOfPrimeIdealUnderRingMapIsPrime}
    Given a prime ideal $\mathfrak{p}\subset S$ and a ring map $\sigma : R\rightarrow S$, then $\sigma^{-1}(\mathfrak{p})\subset R$ is a prime ideal.
\end{proposition}
\begin{proof}
    Suppose $xy\in\sigma^{-1}(\mathfrak{p})$. Then $\sigma(x)\sigma(y)=\sigma(xy)\in \mathfrak{p}$, hence $\sigma(x)\in \mathfrak{p}$ or $\sigma(y)\in\mathfrak{p}$; stated equivalently $x\in \sigma^{-1}(\mathfrak{p})$ or $y\in \sigma^{-1}(\mathfrak{p})$.   
\end{proof}
...
\end{proof}
\subsubsection{The IBN Property}
\begin{definition}
    A ring $R$ has the \emph{invariant basis number (IBN) property} if for each $n,m\geq 0$, $R^n\simeq R^m$ (as left $R$-modules) implies $n=m$.
\end{definition}
\begin{definition}
    Let $R$ be an arbitrary ring, we define $\mathbb{CFM}_\N(R)$, \emph{the column finite matrices over $\N$} to be the set 
    $$
        \left\{(a_{ij})\in \prod_{(i,j)\in \N^2} R : \text{ for each } j \text{ there exists } N \text{ such that } a_{ij}=0 \text{ for all } i\geq N\right\}.
    $$
    We define 
    $$
        (a_{ij})(b_{ij})= \left(\sum_{k\in \N}a_{ik}b_{kj}\right)
    $$
    for $(a_{ij})(b_{ij})\in \mathbb{CFM}_\N(R)$.
\end{definition}
\begin{proposition}
    The above multiplication is well-defined and with it $\mathbb{CFM}_\N(R)$ becomes a ring.
\end{proposition}
\begin{proof}
    Note that for $(a_{ij}),(b_{ij})\in \mathbb{CFM}_\N(R)$, first of all that $\sum_{k\in \N}a_{ik}b_{kj} = \sum_{1}^N a_{ik}b_{kj}$ for a suitably large $N$, since $b_{kj}=0$ for $k$ greater than a suitably large $N$. secondly when $i$ is large, $a_{ij} = 0$, hence $\sum_{k\in \N} a_{ik}b_{kj}=0$, which shows $(a_{ij})(b_{ij})\in \mathbb{CFM}_\N(R)$. Checking the ring axioms is a task completely analogous to doing the same matrix rings of $R$. 
\end{proof}
The above is a simple example of a ring without the IBN property. 
\begin{example}
    We define a $\mathbb{CFM}_\N(R)$-linear map 
    \begin{gather*}
        \mathbb{CFM}_\N(R) \rightarrow \mathbb{CFM}_\N(R)^2\\
        (a_{ij})\mapsto ((a_{i,2j}),(a_{i,2j+1}))
    \end{gather*}
    which has inverse 
    \begin{gather*}
        \mathbb{CFM}_\N(R)^2 \rightarrow \mathbb{CFM}_\N(R)
    \end{gather*}
    taking a pair $((b_{ij}),(c_{ij}))$ "weaving" them row wise, i.e. mapping it to $(a_{ij})$ with $a_{i,2j}=b_{ij}$ and $a_{i,2j+1}=c_{ij}$. Then $\mathbb{CFM}_\N(R)\simeq \mathbb{CFM}_\N(R)^2$. In fact recursively using this argument, $\mathbb{CFM}_\N(R)^m\simeq \mathbb{CFM}_\N(R)^n$ for $n,m$. 
\end{example}
\begin{lemma}
    A field $K$ has the IBN property.
\end{lemma} 
\begin{proof}
    This is an immediate consequence of dimension being an integer invariant of finite basis vector spaces, which we will see later. 
\end{proof}
\begin{proposition}
    Assuming axiom of choice, a commutative ring has the IBN property.
\end{proposition}
\begin{proof}
    Suppose $f: R^n\simeq R^m$. Let $I$ be a maximal ideal. Then $f$ restrict to a $R$-module homomorphism
    $$
        \left. f\right|_{\prod_1^n I} : \prod_1^n I\rightarrow \prod_1^n I
    $$
    We thus get an induced $R/I$-linear map
    \begin{gather*}
        g : (R/I)^n\simeq R^n/I^n \rightarrow R^m/I^m (R/I)^m\\
    \end{gather*}
    following the same construction for the inverse of $f$ we get an inverse of $g$, hence $g$ defines an isomorphim of $R/I$ vector space it follows that $n=m$, since $R/I$ has the IBN property.  
\end{proof}
\subsubsection{Reduced Rings}
    \begin{definition}
        An element $x$ in a ring $R$ is called \emph{nilpotent} if $x^n=0$ for some $n\geq 1$. 
    \end{definition}
    \begin{definition}
        An left ideal $I\subset R$ is \emph{nil}, if every element in $I$ is nilpotent. $I$ is \emph{nilpotent} if $I^n=0$ for some $n\geq 1$.
    \end{definition}
    \begin{remark}
        If $I$ is nilpotent, then it is nil. 
    \end{remark}
    \begin{lemma}
        For a commutative ring $R$ the set of nilpotent elements in $R$, $\mathrm{nil}(R)$ is a two-sided ideal
    \end{lemma}
    \begin{proof}
        Clearly, $0$ is nilpotent. Let $x,y\in \mathrm{nil}(R)$. Then $x^n=0$ and $y^m=0$ for some $n,m\geq 1$. We claim that $(x+y)^{n+m}=0$. This is a consequence of the binomial theorem. Lastly, for any $a\in R$, $(ax)^n= a^nx^n =0$.
    \end{proof}
    \begin{lemma}
        In a commutative ring $R$,  
        $$
            \mathrm{nil}(R) = \bigcap_{\mathfrak{p}\leq R : \mathfrak{p}\ \mathrm{prime}} \mathfrak{p}
        $$
    \end{lemma}
    \begin{proof}
        Indeed, using Lemma~\ref{RadicalOfIdealIsIntersectionOfAllPrimeIdealsContainingIt}, 
        $$
            \mathrm{nil}(R) = \rad(0) = \bigcap_{0\subset \mathfrak{p}\leq R : \mathfrak{p} \text{ prime}} \mathfrak{p}
        $$
    \end{proof}
    \begin{definition}
        A ring is called \emph{reduced} if it has not nilpotent elements. 
    \end{definition}
    \begin{definition}
        Given a commutative ring $R$, we form the \emph{reduced ring of $R$} defined by 
        $$
            R_\mathrm{red} := R/\mathrm{nil}(R).
        $$
    \end{definition}
    \begin{lemma}
        For a commutative ring $R$, $R_\mathrm{red}$ is reduced. 
    \end{lemma}
    \begin{proof}
        Indeed, if $x^n+\mathrm{nil}(R)=0$ for some $n\geq 1$, then $x^n\in \mathrm{nil}(R)$, hence $x^n$ is contained in every prime ideal in $R$, which implies $x$ is contained in every prime ideal in $R$, so $x\in \mathrm{nil}(R)$.
    \end{proof}
\subsubsection{The Characteristic of a Ring}
\begin{definition}
    Let $R$ be a ring. Consider the unique ring map $f_R : \Z \rightarrow R$. Note that 
    $$
        \ker \ f_R = \langle n\rangle 
    $$
    for some unique $n\in Z_{\geq 0}$. This integer is called \emph{the characteristic of $R$}, denoted 
    $$
        \Char\ R.
    $$
\end{definition}
\begin{lemma}
    Rings of characteristic $0$ are infinite.
\end{lemma}
\begin{proof}
    $\Char\ R = 0$ is equivalent to $f_R$ being injective. Then 
    $$
        \# R\geq \#\Z >\infty.  
    $$
\end{proof}
\begin{lemma}
    Suppose $R$ is a domain. Then $\Char\ R$ is $0$ or a prime. 
\end{lemma}
\begin{proof}
    We have an embedding $f:\Z/n\Z \hookrightarrow R$ induced by $f_R$. Since $\Z/n\Z$ is a domain it thus follows that $n$ is $0$ or a prime.  
\end{proof}
\begin{lemma}(Freshman's dream)
    Consider a ring commutative ring $R$ with $\Char \ R = p$. Then 
    $$
        (x+y)^{p^n}=x^{p^n}+y^{p^n}
    $$ 
    for $n\geq 0$.
\end{lemma}
\begin{proof}
    Since $p\mid {p \choose k}$ for $k\in\{1,\dots,p-1\}$, for $n=1$,
    $$
        (x+y)^p = \sum_0^p {p \choose k}x^{p-k}y^{k} = x^p + y^p. 
    $$
    By induction, 
    $$
...
\end{proof}
\begin{definition}
    Let $R$ be a commutative ring of characteristic $p$. Then we define the \emph{Frobenius map} to be the ring endomorphism
    \begin{gather*}
        F_R : R \rightarrow R\\
        r \mapsto r^p        
    \end{gather*}
\end{definition}
\begin{remark}
    When $R$ is reduced, note that $F_R$ is injective. Conversely, a ring with injective Frobenius map has no nilpotents of order less than $p$.  
\end{remark}
\begin{definition}
    A field $K$ of prime characteristic $p>0$ is \emph{perfect}, if its Frobenius map is surjective (and thus an isomorphism).  
\end{definition}
\begin{example}
    Finite fields $\mathbb{F}_{p^n}$ have characteristic $p$ and are perfect, since injective endomorphism of finite set are automatically surjective. Another example of a perfect field is the free field generated by a commutative monoid over a finite field.  
\end{example}
...
\subsubsection{Principal Ideal Domains}
    \begin{definition}
        An ideal $I\subset R$ is \textit{principal}, if $I=\langle a\rangle$ for some $a \in R$. A domain in which every ideal is principal is called a \textit{principal ideal domain} or a \textit{PID}. 
    \end{definition}
...
In our exploration of unique factorization domains and Euclidean Domains we will fix an integral domain $R$.
\begin{definition}
...
\begin{lemma}
    ...
    \end{proof}
\begin{definition}
    A \emph{Euclidean domain} is a domain $R$ such there is a function $N: R\setminus 0 \rightarrow \N$ such that for every $x,y\in R$ there are $q,r\in R$ with $r = 0$ or $N(r)<N(y)$ and 
    $$
        x = qy+r
    $$
\end{definition}
...
\end{theorem}
\begin{proof}
    Let $I\subset R$ be a non-zero ideal. Let $d\in I$ be such that
    $$
        N(d) = \min(\{N(a):a\in I\}).
    $$
    Let $x\in I$. Then there are $q,r\in R$ such that 
    $$
        x = qd+r
    $$  
    and $r = 0$ or $N(r)<N(x)$. Note that $x-r\in I$ and since $x\in I$, we have that $r\in I$. Then $N(x)\leq N(d)$, hence $r=0$. Then $x=qd$, meaning $x\in \langle d\rangle$. We thus get that $I=\langle d\rangle$ and thus that $R$ is a PID. 
\end{proof}
\begin{corollary}
    A Euclidean domain is a UFD.
\end{corollary}
\begin{proof}
    A Euclidean domain is a PID, which is a UFD. 
\end{proof}
\subsubsection{Localizations                           of Commutative Rings}
    \begin{definition}
        Given a ring $R$, a subset $X$ of $R$ is called \emph{multiplicative} if it is a submonoid of $(R,\cdot)$ 
    \end{definition}
    \begin{definition}
        Let $R$ be a commutative ring and $X$ a multiplicative set in $R$. Given $(a,x),(b,y)\in R\times X$ we write $(a,x)\sim (b,y)$ if and only if there exists a $c\in X$ such that 
        $$
            c(ay-bx)=0
        $$
    \end{definition}
    \begin{remark}
        This defines an equivalence relation on $R\times X$.
    \end{remark}
    \begin{definition}
        Let $R$ be a commutative ring and $X$ a multiplicative set. We define the \emph{localization of $R$ with respect to $X$} to be the set 
        $$
            X^{-1}R := (R\times X)/\sim. 
        $$
        an element $[(a,x)]\in X^{-1}R$ is denoted $\frac{a}{x}$.
    \end{definition}
    \begin{example}
        \begin{enumerate}
            \item  An example of a multiplicative set is $X_x:=\{x^n : n\geq 0\}$ for some $x\in R$. We define
            $$
                R_x := X_x^{-1}R.
            $$
            This construction is referred to as \emph{the localization of $R$ with respect $x$}
            \item Another example of a multiplicative set is $X_\mathfrak{p}:= R\setminus \mathfrak{p}$ where $\mathfrak{p}\subset R $ is a prime ideal. Indeed, if $a,b\in R\setminus \mathfrak{p}$, then $ab\notin \mathfrak{p}$, by the contrapositive of the definition of being a prime ideal. We define the \emph{localization of $R$ with respect to $\mathfrak{p}$} to be 
            $$
                R_\mathfrak{p} := X_\mathfrak{p}^{-1}R
            $$
            Note that $R\setminus\mathfrak{p}$ contains no zero divisors. We may therefore identify $R$ with the subring of elements of the form $\frac{a}{1}\in R_\mathfrak{p}$.
            \item Let $X$ be the set of elements in $R$ that are not zero divisors. Then $X^{-1}R$ is called the \emph{total ring of fractions of $R$}. $X$ is the largest multiplicative subset such that $\iota_X$ is injective. Indeed, if $X\subsetneq Y$ for another multiplicative set $Y$ in $R$, then $Y$ must contain a zero divisor, hence $\iota_Y$ is not injective.
        \end{enumerate}
    \end{example}
    \begin{lemma}
        Given a commutative ring $R$ and a multiplicative set $X$. Then $X^{-1}R$ is the zero ring $\{\frac{0}{1}\}$ if and only if $0\in X$.
    \end{lemma}
    \begin{proof}
        Suppose $0\in X$. Then for each $\frac{a}{x}\in X^{-1}R$, $0(a\cdot 1-x\cdot 0)=0$, hence $\frac{a}{x}=\frac{0}{1}$.\\
        Suppose $X^{-1}R=\{\frac{0}{1}\}$. Then $\frac{1}{1}=\frac{0}{1}$, hence for some $f\in X$, $f =f(1\cdot 1-1\cdot 0)=0$.     
    \end{proof}
    \begin{lemma}
        Let $R$ be a commutative ring and $X$ a multiplicative set. We define addition on $X^{-1}R$ by
        $$
            \frac{a}{x}+\frac{b}{y} := \frac{ay+bx}{xy}\quad \left(\frac{a}{x},\frac{b}{y}\in X^{-1}R\right).
        $$
        We define multiplication on $X^{-1}R$ by 
        $$
            \frac{a}{x}\frac{b}{y} := \frac{ab}{xy} \quad \left(\frac{a}{x},\frac{b}{y}\in X^{-1}R\right).
        $$
        We set $1 := 1_{X^{-1}R}:= \frac{1}{1}$ and $0 := 0_{X^{-1}R}:= \frac{0}{1}$. Then $(R,+,\cdot,0,1)$ is a commutative ring.  
    \end{lemma}
    \begin{proof}
        We first check that addition and multiplication are well-defined. Let $\frac{a}{x},\frac{b}{y},\frac{c}{z},\frac{d}{w}\in X^{-1}R$ be given with $\frac{a}{x}=\frac{c}{z}$ and $\frac{b}{y}=\frac{d}{w}$. Then there are $f,g\in R$ such that 
        $$
            f(az-cx)=0 \text{ and } g(bw-dy)=0.
        $$

        $$
            gf((ay+bx)zw-(cw+dz)xy)= wyg(f(az-cx))+zxf(g(bw-dy))= 0,
        $$
        which implies 
        $$
            \frac{a}{x}+\frac{b}{y}=\frac{ay+bx}{xy} = \frac{cw+dz}{zw} = \frac{c}{z}+\frac{d}{w}.
        $$
        Moreover,
        $$
            fg(abzw-cdxy)= fg(abzw-cdxy-bcxw+bcxw) =  gbwf(az-cx) + fcxg(bw-dy) = 0, 
        $$ 
        hence 
        $$
            \frac{a}{x}\frac{b}{y}=\frac{ab}{xy} = \frac{cd}{zw}= \frac{c}{z}\frac{d}{w}.
        $$
        Let $\frac{a_1}{x_1},\frac{a_2}{x_2},\frac{a_3}{x_3}\in X^{-1}R$ be given. Then
        \begin{align*}
            \frac{a_1}{x_1}+\left(\frac{a_2}{x_2}+\frac{a_3}{x_3}\right) &= \frac{a_1}{x_1}+\frac{a_2x_3+a_3x_2}{x_2x_3} = \frac{a_1x_2x_3+a_2x_1x_3+a_3x_1x_2}{x_1x_2x_3}\\
            &= \frac{(a_1x_2+a_2x_1)x_3+ a_3x_1x_2}{x_1x_2x_3} = \frac{a_1x_2+a_2x_1}{x_1x_2}+\frac{a_3}{x_3}\\
            &= \left(\frac{a_1}{x_1}+\frac{a_2}{x_2} \right)+\frac{a_3}{x_3},
        \end{align*}
        and 
        $$
            \frac{a}{x}+0 = \frac{a}{x}+\frac{0}{1}=\frac{a\cdot 1+0x}{1x}= \frac{a}{x}.
        $$  
        For an $\frac{a}{x}\in X^{-1}R$, we define $-\frac{a}{x}:= \frac{-a}{x}$
        $$
            \frac{a}{x}-\frac{a}{x}= \frac{a}{x}+\frac{-a}{x}= \frac{ax-ax}{x^2}=0.
        $$
        Commutativity of addition on $X^{-1}R$ follows readily from the commutativity of addition on $R$. Checking that multiplication on $X^{-1}R$ gives it the structure of a commutative monoid is also readily verifiable. multiplication being distributive over addition is also easy to verify.
    \end{proof}
    \begin{lemma}
        Let $X$ be a multiplicative set of a commutative ring $R$. The ring map 
        \begin{gather*}
            \iota_X : R \rightarrow \left\{\frac{a}{1}:a\in R\right\}\\
            a \mapsto \frac{a}{1}
        \end{gather*} 
        has image
        $$
            \left\{\frac{a}{1} : a\in R \right\}.
        $$
        Furthermore, $\iota_X$ is injective if and only if $X$ contains no zero-divisors. 
    \end{lemma}
    \begin{proof}
        The first statement is trivial. Suppose $X$ contains no zero-divisors. If $\frac{a}{1}=0$, then there is an $x\in X$ with 
        $$
            xa= 0,
        $$
        hence $a=0$. Conversely, if $X$ contains a zero divisor $x$, we may find $a\in R\setminus 0$ with $xa = 0$, implying 
        $$
            \iota_X(a)=\frac{a}{1}=0,
        $$ 
        hence $\iota_X$ cannot be injective. 
    \end{proof}
    \begin{definition}
    Let $R$ be a commutative ring. Let $X$ be a multiplicative set. We define the \textit{saturation} of $X$ to be the set
    \begin{align*}
        \widehat{X} := \left\{ r\in R : \exists r'\in R, r'r \in X \right\}.
    \end{align*}
    $X$ is \textit{saturated}, if 
    $$\widehat{X} = X.$$
\end{definition}
\begin{remark}
    Let $x\in X$, then $1x\in X$, hence $x\in \widehat{X}$. We thus have $X\subset \widehat{X}$. Let $r,s\in \widehat{X}$, then for some $r',s'\in R$, $r'r\in X$ and $s's\in X$. Then $r's'rs\in X$, hence $rs \in \widehat{X}$. Clearly $1\in \widehat{X}$. Thus $\widehat{X}$ is a multiplicative set. The saturation of $X$ is clearly saturated. Let $Y$ be a saturated multiplicative set containing $X$. Let $r \in \widehat{X}$. Then for some $r'\in R$, $r'r\in X\subset Y$. Thus $\widehat{X}\subset Y$, hence $\widehat{X}$ is the smallest saturated multiplicative set containing $X$.
\end{remark}
\begin{lemma}\label{EquivalentDescriptionOfBeingSaturated}
    Let $R$ be a commutative ring and $X$ a multiplicative set in $R$. Then $X$ is saturated if and only if for every $x,y\in R$ when $xy \in X$ then $x,y\in X$
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{UnitsOfLocalization}
     Let $R$ be a commutative ring and $X$ a multiplicative set. Consider the map 
     \begin{gather*}
         \iota_X : R \hookrightarrow X^{-1}R\\
         r \mapsto \frac{r}{1}
     \end{gather*}
     Let $\frac{r}{x}\in X^{-1}R$. Then 
     $$\frac{r}{x}\in \left(X^{-1}R\right)^\ast \iff r \in Y:= \iota_X^{-1}\left(\left(X^{-1}R\right)^\ast\right).$$
...
\end{proof}
For this subsection $\pazocal{C}$ denotes the category whose objects are pairs consisting of a commutative ring and a multiplicative set in that ring. A morphism of such objects, $(R,X)\rightarrow (S,Y)$ is ring map, mapping every element in $X$ to an element in $Y$. Let $\pazocal{D}$ be the subcategory of $\pazocal{C}$ where an object in $\pazocal{D}$ is a pair $(R,X)\in \pazocal{C}$ with $X\subset R^\ast$.
    \begin{lemma}
        Let $\sigma:(R,X)\rightarrow (S,S^\ast)$ be a morphism in $\pazocal{C}$. There is a unique ring homomorphism $\overline{\sigma}: X^{-1}R\rightarrow S$ such that 
        $$
            \begin{tikzcd}
                R \arrow[d,"\iota_X"']\arrow[rd, "\sigma"]\\
                X^{-1}R \arrow[r,"\overline{\sigma}"'] & S
            \end{tikzcd}
        $$
        commutes. 
    \end{lemma}  
    \begin{proof}
        We define 
        \begin{gather*}
            \overline{\sigma}:X^{-1}R\rightarrow S\\
            \frac{a}{x}\mapsto \sigma(a)\sigma(x)^{-1}
        \end{gather*}
        Suppose $\frac{a}{x}=\frac{b}{y}$. Then there is a $z\in X$ such that $z(ay-bx)=0$. Then $\sigma(z),\sigma(y),\sigma(x)\in S^\ast$ and 
        $$
            \sigma(z)\sigma(y)\sigma(a) = \sigma(z)\sigma(x)\sigma(b)\implies \overline{\sigma}\left(\frac{a}{x}\right)=\sigma(a)\sigma(x)^{-1}=\sigma(b)\sigma(y)^{-1}=\overline{\sigma}\left(\frac{b}{y}\right).
        $$ 
        One readily verifies that this is a ring map. Let $a\in R$. Then 
        $$
            \overline{\sigma}\iota_X(a)=\overline{\sigma}\left(\frac{a}{1}\right)=\sigma(a)\sigma(1)^{-1}=\sigma(a).
        $$
        Note that since $\sigma(x)\in S^\ast$ for every $x\in X$ we cannot have $0\in X$. Suppose $\tau: X^{-1}R\rightarrow S$ is another ring map satisfying $\sigma = \tau\iota_X$. For every $x\in X$ note that 
        $$
            1 = \tau\left(\frac{1}{1}\right) = \tau\left(\frac{x}{1}\frac{1}{x}\right)=\tau\left(\frac{x}{1}\right)\tau\left(\frac{1}{x}\right)=\tau\iota_X(x)\tau\left(\frac{1}{x}\right)=\sigma(x)\tau\left(\frac{1}{x}\right),
        $$
        hence $\tau\left(\frac{1}{x}\right)=\sigma(x)^{-1}$. It thus follows for every $\frac{a}{x}\in X^{-1}R$ that 
        $$
            \tau\left(\frac{a}{x}\right)=\tau\left(\frac{a}{1}\right)\tau\left(\frac{x}{1}\right)=\tau\iota(a)\sigma(x)^{-1}=\sigma(a)\sigma(x)^{-1}=\overline{\sigma}\left(\frac{a}{x}\right).
        $$ 
        Thus we conclude $\overline{\sigma}=\tau$.
    \end{proof}
    \begin{remark}
        The contents of this lemma is that the functor
        \begin{gather*}
            \mathrm{Loc}: \pazocal{C}\rightarrow \pazocal{D}\\
            (R,X)\mapsto (X^{-1}R,\iota_X(X))\\
            (\sigma : (R,X)\rightarrow (S,Y))\mapsto (\overline{\sigma}: X^{-1}R\rightarrow Y^{-1}S,\frac{a}{x}\mapsto \sigma(a)\sigma(x)^{-1})
        \end{gather*}
        is left adjoint to the forgetful functor $\pazocal{F}: \pazocal{D}\rightarrow \pazocal{C}$. Since for each $(R,X)\in \pazocal{C}$ and $(S,Y)\in \pazocal{D}$ 
        \begin{gather*}
            \Hom_\pazocal{C}((R,X),\pazocal{F}(S,Y))\rightarrow \Hom_\pazocal{D}((X^{-1}R,\iota_X(X)),(S,Y))\\
            \sigma \mapsto \overline{\sigma} 
        \end{gather*}
        defines a bijection. Indeed, given a morphism $\tau\in \Hom_\pazocal{D}((X^{-1}R,\iota_X(X)),(S,Y))$, and setting $\sigma:=\left.\tau\right|_{R}\in \Hom_\pazocal{C}((R,X),\pazocal{F}((S,Y)))$, we have $\overline{\sigma}=\tau$. This shows (using Yoneda) that the functor
        $$
            \left.\Hom_\pazocal{C}((R,X),\_)\right|_\pazocal{D} : \pazocal{D}\rightarrow \mathrm{Set}
        $$
        is represented by $(X^{-1}R,\iota_X(X))$, hence $X^{-1}$ is unique up to unique isomorphism among rings in $\pazocal{D}$ with this property. 
    \end{remark}
    As a gift of this functorial description of localization we get
    \begin{corollary}
        If $\sigma: R\simeq S$ and $X$ is a multiplicative set in $R$, then 
        $$
            X^{-1}R\simeq \sigma(X)^{-1}S.
        $$
    \end{corollary}
    \begin{lemma}\label{CanonicalOfLocalizations}
        Consider a multiplicative set $X$ in a commutatitive ring $R$ and a saturated multiplicative set $Y\subset X^{-1}R$ with $Y\cap R\supset X$. Then 
        $$
            (Y\cap R)^{-1}R \simeq Y^{-1}(X^{-1}R).
        $$
    \end{lemma}
    \begin{proof}
        We have the canonical injection $R\hookrightarrow X^{-1}R$ from which we obtain the injective map 
        \begin{gather*}
            \sigma : (Y\cap R)^{-1}R \rightarrow Y^{-1}(X^{-1}R)\\
            \frac{a}{y}\mapsto \frac{\frac{a}{1}}{y}
        \end{gather*}
        It suffices to show that this map is surjective. Let $\frac{\frac{a}{x}}{\frac{b}{y}}\in Y^{-1}(X^{-1}R)$. Note that since $Y$ is saturated $b\in Y$ and $\frac{1}{y}\in Y$. Since $y\frac{1}{y} = 1\in Y$ it follows that $y\in Y$. Then 
        $$
            \frac{\frac{a}{x}}{\frac{b}{y}} = \frac{a}{x}\frac{1}{\frac{b}{y}} = \frac{a}{x}\frac{y}{b} = \frac{ay}{bx} = \sigma\left(\frac{ay}{bx}\right).
        $$
    \end{proof}
    \begin{definition}
        Let $R$ be a commutatitive ring and $X$ a multiplicative subset of $R$. Consider an ideal $I\subset R$. We define the \textit{localization ideal of $I$ in $X^{-1}R$} to be the set 
        $$X^{-1}I:=\left\{\frac{a}{x}\in X^{-1}R: a\in I, x\in X\right\}$$
    \end{definition}
    \begin{lemma}
        Let $R$ be a commutatitive ring and $X$ a multiplicative subset of $R$. Consider an ideal $I\subset R$. Then $X^{-1}I$ is a proper ideal in $X^{-1}R$ if and only if $X\cap I=\emptyset$.
    \end{lemma}
    \begin{proof}
        If $X^{-1}I=X^{-1}R$, then $\frac{a}{x}=1$ for some $a\in I$ and $x\in I$. Then $\frac{a}{x}$ is a unit, hence $ab\in X$ for some $b\in R$. It follows that $\frac{ab}{xb}\in X^{-1}I$ with $ab\in X\cap I$. Conversely, if there is an $a\in X\cap I$, then $1=\frac{a}{a}\in X^{-1}I$.
    \end{proof}
\begin{lemma}\label{LocalizationIdealsDescribeIdealsGeneratedFromIdealsInSubring}
    Let $R$ be a commutative ring and $X\subset R$ a multiplicative subset of $R$. Let $I\subset R$ be an ideal. Then $(X^{-1}R)I = X^{-1}I$. Consequently, $X^{-1}I$ is an ideal.
\end{lemma}
\begin{proof}
    ...
    (In other words we just compute the sum of these fractions and see that the numerator is in $I$)
    The converse inclusion is trivial. 
\end{proof}
\begin{lemma}\label{LocalizationOfEliminationIdealGenerateOriginalIdeal}
    Let $R$ be a commutative ring and $X$ a multiplicative subset of $R$. Let $I\subset X^{-1}R$ be an ideal. Then $X^{-1}(I\cap R) = I$.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{corollary}\label{LocalizationPreservePrimeness}
    If $I\subset R$ is a prime ideal, then $X^{-1}I$ is prime. 
\end{corollary}
\begin{proof}
    Let $\frac{a}{x}\frac{b}{y}\in X^{-1}I$ then $ab\in I$, hence $a\in I$ or $b\in I$, so $\frac{a}{x}\in X^{-1}I$ or $\frac{b}{y}\in X^{-1}I$
\end{proof}
\begin{definition}
    If $R$ is a commutative ring and $X$ is a multiplicative set in $R$, then given an ideal $I\subset R$, we define $X/I := \{x+I:x\in X\}$.
\end{definition}
\begin{remark}
    One readily verifies that $X/I$ is a multiplicative set in $R/I$. 
\end{remark}
\begin{lemma}\label{QuotientRingsAndTakingLocalization}
    If $R$ is a commutative ring and $X$ is a multiplicative set in $R$, then for any ideal $I\subset R$,
    $$
        (X/I)^{-1}R/I \simeq (X^{-1}R)/(X^{-1}I)
    $$
\end{lemma}
\begin{proof}
    We have a surjective map of rings 
    \begin{gather*}
        \sigma X^{-1}R \rightarrow (X/I)^{-1}R/I\\
        \frac{a}{x}\mapsto \frac{a+I}{x+I}
    \end{gather*}
    induced by $R\twoheadrightarrow R/I$. We check that $\ker\ \sigma = X^{-1}I$. Let $\frac{a+I}{x+I}=0+I$. Then $fa\in I$ for some. Then 
    $$\frac{a}{x}=\frac{af}{ax} \in X^{-1}I.$$ 
\end{proof}
\begin{lemma}
    Let $R$ be a commutative ring and $X$ a multiplicative set. 
    \begin{enumerate}
        \item $\mathrm{nil}(X^{-1}R)=\{\frac{a}{x}: a\in \mathrm{nil}(R)\}$
        \item $R$ is reduced if and only if $X^{-1}R$ is reduced.\\
        \item Set $X_\mathrm{red}=\{x+\mathrm{nil}(R):x\in X\}$. Then 
        $$
            \left(X^{-1}R\right)_\mathrm{red}\simeq X_\mathrm{red}^{-1}R_\mathrm{red}.
        $$
    \end{enumerate} 
\end{lemma}
\begin{proof}
    1. If  $\frac{a}{x}\in X^{-1}R$ with $\frac{a^n}{x^n}=0$ for some $n\geq1$. Then $a^n=0$, hence $a\in \mathrm{nil}(R)$. If $\frac{a}{x}\in X^{-1}R$ is given such that $a^n=0$ for some $n\geq 1$, then $\frac{a^n}{x^n}=0$, hence $\frac{a}{x}\in \mathrm{nil}(X^{-1}R)$. \\
    2. If $R$ is not reduced, then trivially $X^{-1}R$ is not reduced, since $R$ is a subring of $X^{-1}R$. Suppose $X^{-1}R$ is not reduced, then there is a non-zero element $\frac{a}{x}\in \mathrm{nil}(X^{-1}R)$, hence $a$ is a non-zero element in $\mathrm{nil}(R)$, implying $R$ is not reduced.\\
    3. 
    Follows directly from Lemma~\ref{QuotientRingsAndTakingLocalization}
\end{proof}
\subsubsection{Local Rings, Localizations of Integral Domains \& Field of Fractions}
\begin{definition}\label{LocalRingDefinition}
    A ring $R$ is called \textit{local} if it has unique maximal left ideal, $\mathfrak{m}_R$. A homomorphism of local rings is a homomorphism of rings $f : R\rightarrow S$ if $f^{-1}(\mathfrak{m}_S)=\mathfrak{m}_R$. 
\end{definition}
...
\end{proof}
\begin{lemma}
    Given a prime ideal $\mathfrak{p}\subset R$ of a commutative ring $R$, the multiplicative set $X=R\setminus \mathfrak{p}$ is saturated.
\end{lemma}
\begin{proof}
    This follows from Lemma~\ref{EquivalentDescriptionOfBeingSaturated}, since if $a\in \mathfrak{p}$ or $b\in \mathfrak{p}$, then $ab\in \mathfrak{p}$.  
\end{proof}
\begin{lemma}
    Given a prime ideal $\mathfrak{p}\subset R$ of a commutative ring $R$, the ring $R_\mathfrak{p}$ is a local ring. 
\end{lemma}
\begin{proof}
    Let $\frac{a}{x},\frac{b}{y}\in R_\mathfrak{p}$ be non-units, then $a,b\in \mathfrak{p}$, hence $ay+bx\in \mathfrak{p}$, which means $\frac{a}{x}+\frac{b}{x}$ is not a unit in $R_\mathfrak{p}$ (cf. the prior lemma and Lemma~\ref{UnitsOfLocalization}). If $\frac{c}{z}\in R_\mathfrak{p}$, then $ca\in \mathfrak{p}$, hence $\frac{c}{z}\frac{a}{x}$ is a non-unit in $R_\mathfrak{p}$. We conclude that the non-units of $R_\mathfrak{p}$ is an ideal, hence $R_\mathfrak{p}$ is local.\\
    Let $\frac{a}{x},\frac{b}{y}\in R_\mathfrak{p}\setminus 0$.     
\end{proof}
\begin{lemma}
    Consider a ring map $\sigma: R\rightarrow S$ and $\mathfrak{p}\subset R$ and $\mathfrak{q}\subset S$ prime ideals with $\sigma^{-1}(\mathfrak{q})=\mathfrak{p}$. Then the localization $\sigma_\mathfrak{p}: R_\mathfrak{p}\rightarrow S_\mathfrak{q}$ is a local ring homomorphism.
\end{lemma}
\begin{proof}
    The maximal ideals $\mathfrak{m}_\mathfrak{p}\subset R$ and $\mathfrak{m}_\mathfrak{q}\subset S$ is the set of quotients with numerator in $\mathfrak{p}$ respectively $\mathfrak{q}$. If $\frac{a}{x}\in \sigma_\mathfrak{p}^{-1}(\mathfrak{m}_\mathfrak{q})$, then 
    $$
        \frac{\sigma(a)}{\sigma(x)}=\sigma_\mathfrak{p}\left(\frac{a}{x}\right)\in \mathfrak{m}_\mathfrak{q},
    $$
    hence $\sigma(a)\notin \mathfrak{q}$. This implies $a\notin \sigma^{-1}(\mathfrak{q})=\mathfrak{p}$, hence $\frac{a}{x}\in \mathfrak{m}_\mathfrak{p}$. If $\frac{a}{x}\in \mathfrak{m}_\mathfrak{p}$, then $a\notin \mathfrak{p}=\sigma^{-1}(\mathfrak{q})$, hence $\frac{a}{x}\in \sigma_\mathfrak{p}^{-1}(\mathfrak{m}_\mathfrak{q})$. We thus conclude that $\mathfrak{m}_\mathfrak{p}=\sigma_\mathfrak{p}^{-1}(\mathfrak{m}_\mathfrak{q})$. 
\end{proof}
\begin{proposition}
    When $R$ is an integral domain and $X\subset R\setminus 0$ is a multiplicative set, then for every $\frac{a}{x},\frac{b}{y}\in X^{-1}R$,
    $$
        \frac{a}{x}=\frac{b}{y} \iff ay=bx
    $$
\end{proposition}
\begin{proof}
    "$\impliedby$": trivial.\\
    "$\implies$": There is an $h\in X$, such that $h(ay-bx)=0$. Then $ay-bx=0$. 
\end{proof}
\begin{corollary}
    When $R$ is an integral domain, for a prime ideal $\mathfrak{p}\subset R$,  $R_\mathfrak{p}$ is an integral domain. 
\end{corollary}
\begin{theorem}
    When $R$ is an integral domain, and $X$ is a multiplicative set of $R$ not containing $0$, we have that $Q(R):= (R\setminus 0)^{-1}R$ is a field and $X^{-1}R$ can be identified with a subring of $Q(R)$. Moreover $Q(R)$ is the smallest field containing $R$.
\end{theorem}
\begin{proof}
    By the assumption that $R$ is an integral domain, $0$ i a prime ideal, hence $Q(R)$ is a commutative ring and $R\setminus 0$ is saturated, so 
    $$Q(R)^\ast = \left\{ \frac{r}{s}\in Q(R) : r\in R\setminus\{0\} \right\} = Q(R)\setminus\{0\}.$$
    This means $Q(R)$ is a field. Let $K$ be a field containing $R$ as a subring. Let $\frac{r}{s}\in Q(R)$. Then $r \in K$ and $\frac{1}{s}=s^{-1} \in K$, hence $\frac{r}{s} = r \frac{1}{s} \in K$. This means $Q(R) \subset K$, hence $Q(R)$ is the smallest field containing $R$ as a subring. Consider the subring 
    $$
        \left\{\frac{a}{x}\in Q(R) : x\in X \right\}\subset Q(R).
    $$
    Set $Y := R\setminus 0$ and consider the commutative diagram
    $$
        \begin{tikzcd}
            R \arrow[r,"\id"]\arrow[d,"\iota_X"]\arrow[rd,"\overline{\id}"] & R \arrow[d,"\iota_Y"]\\
            X^{-1}R \arrow[r,dashrightarrow] & Q(R)
        \end{tikzcd}
    $$
    where the dashed arrow takes $\frac{a}{x}\mapsto \frac{a}{x}$. The diagram tells us that this defines an isomorphism onto its image, which is the subring discussed earlier.
\end{proof}
\begin{remark}
    We call $Q(R)$ \emph{the field of fractions of $R$}.  From the above we conclude that if $K$ is a field then we can canonically make the identification $K = Q(K)$, and in general $Q(R) =  Q(Q(R))=Q(Q(Q(R)))...$ and so on.
\end{remark}
\begin{corollary}
    If $R\simeq S$, then 
    $$
...
\subsubsection{Discrete Valuation Rings}
    \begin{definition}
        Let $R$ be a non-field integral domain. $R$ is a \textit{discrete valuation ring (DVR)} if it is noetherian and local with principal maximal ideal. 
    \end{definition}
    ...
    \end{lemma}
    \begin{proof}
        \textbf{The case $z\in \mathfrak{m}$:} Pick $\lambda =0$. By the prior lemma for any $\mu\in K\setminus0$ 
        $$\ord(z-\mu)=\ord(\mu)=0,$$
         hence $z-\mu\notin \mathfrak{m}$.\\
        \textbf{The case $z\notin \mathfrak{m}$:} Then $z$ is a unit in $R$. Clear $\sigma$ fixes $K$, hence it is a $K$-algebra isomorphism. For some $\lambda \setminus 0$, $\lambda =\sigma(\lambda)=z$, hence $z-\lambda =0\in \mathfrak{m}$. Since $\sigma$ is injective it follows that $\lambda$ is unique. 
    \end{proof}
    ...
    \end{proof}
    For what follows we assume some easy linear algebra that will be covered later
    \begin{lemma}
    ...
    \begin{proof}
        ...
        \end{gather*}
        is a surjective $K$-algebra map. Suppose $\lambda t^n \in \mathfrak{m}^{n+1}$. Then $\ord(\lambda)>0$, hence $\lambda = 0$. We thus conclude that $\mathfrak{m}^n/\mathfrak{m}^{n+1} \simeq K$, meaning $\dim \ \mathfrak{m}^n/\mathfrak{m}^{n+1} = \dim\ K = 1.$
    \end{proof}
    ...
    \end{proof}
\subsubsection{Torsion \& Annihilators}
\begin{definition}
    An element of a ring $R$ is \emph{regular} if it is both not a left and right zero divisor.
\end{definition}
\begin{definition}
    Consider a left $R$-module $M$. $m\in M$ is a \emph{torsion element} if there is a regular element $r\in R$ such that 
    $$
        rm = 0.
    $$
\end{definition}
\begin{definition}
    We define the \emph{annihilator} of a subset $X$ of a left $R$-module $M$ to be the set 
    $$
        \mathrm{Ann}(X):=\mathrm{Ann}_R(X) := \left\{ r\in R : rx = 0 \text{ for all } x\in X \right\}
    $$ 
\end{definition}
\begin{lemma}
    The annihilator of subset $X\subset {_RM}$ is a left ideal in $R$. 
\end{lemma}
\begin{proof}
    Indeed, if $r_1,r_2\in \mathrm{Ann}(X)$ and $a\in R$, then given $x\in X$,
    $$
        (r_1+ar_2)x= r_1x+ar_2x = 0.
    $$
\end{proof}
\subsubsection{Graded Rings}
    \begin{definition}
        Let $R$ be a ring. A \emph{grading} on $R$ is pair consisting of a monoid $M$ an a family of additive subgroups $\{R_i : i\in M\}$ of $R$ such that 
        $$
            R_iR_j \subset R_{ij}
        $$
        such that 
        $$
            R = \sum_{i\in M}R_i
        $$
        with $\sum_{i\in M} R_i$ being direct. A triplet consisting of a ring $R$ with a grading $(M,\{R_i\})$ is called a \emph{graded ring with respect to $M$}. A \emph{graded ring} is a graded ring with respect to $(\N,+)$.  
    \end{definition}
    \begin{definition}
        An element $f\in R$ in a graded ring $(R,M,\{R_i\})$ is said to be \emph{$i$-homogeneous} if $f\in R_i$. When $M=(\N,+)$ we say that an element is \emph{homogeneous of degree $n$} if it is $n$-homogeneous. 
    \end{definition}
    \begin{remark}
        Note that $0$ is $i$-homogeneous for each $i\in M$.
    \end{remark}
    \begin{lemma}
        Consider a graded ring $(R,M,\{R_i\})$. Then for each $f\in R$ there is a unique decomposition, 
        $$
            f = \sum_{i\in M} f_i
        $$
        with $f_i\in R_i$. In other words, there is a unique subset $\{i_1,\dots,i_n\}\subset M$ and unique non-zero $f_{i_k}\in R_{i_k}$ with 
        $$
            f = \sum_1^n f_{i_k}
        $$
    \end{lemma}
    \begin{remark}
        We call $f_{i_1},\dots,f_{i_n}$ the homogeneous components of $f$. 
    \end{remark}
    \begin{proof}
        This is by definition. 
    \end{proof}
    \begin{lemma}
        Consider a graded ring $R$. $R_0$ is a subring of $R$.
    \end{lemma}
    \begin{proof}
        Note first that $R_0$ is an additive subgroup of $R$, hence it suffices to show that $R_0$ is a multiplicative submonoid of $R$. Clearly if $f,g\in R_0$, then $fg\in R_{0+0}=R_0$. Moreover, $1\in R_n$ for some $n$, and $1=1\cdot 1\in R_{n}R_{n}=R_{2n}$, hence $1\in R_{n}\cap R_{2n}$, implying $n=2n$, hence $n=0$.
    \end{proof}
    \begin{proposition}
        For a graded ring $R$, $R_n$ is an $(R_0,R_0)$-bimodule for any $n$.
    \end{proposition}
    \begin{proof}
        It is clear that $fg\in R_n$ for $f\in R_0$ and $g\in R_n$, hence $R_n$ is a left (and dually a right) $R_0$-submodule of $_{R_0}R_{R_0}$.         
    \end{proof}
    \begin{definition}
        An ideal $I$ is called \emph{homogeneous} of a graded ring $(R,M,\{R_i\})$ if the homogeneous if for each $f\in I$ the homogeneous components of $f$ are in $I$.
    \end{definition}
    \begin{remark}
        Note that $I$ is a left (and right) $R_0$-submodule of $(R,\N,\{R_n\})$, hence $I\cap R_n$ is a left (and right) $R_0$-submodule of $R_n$. This is called the \emph{$n$-homogeneous part of $I$}. We can write 
        $$
            I = \bigoplus_{n\in \N} (R_n\cap I).
        $$
    \end{remark}
    \begin{lemma}
        Consider a graded ring $R$ and a two-sided ideal $I\subset R$. Then $R/I$ is a graded ring with grading 
        $$
            R/I = \bigoplus_{n\in \N} (R_n+I)/I
        $$
        where $I_n$ is the homogeneous part of $I$.  
    \end{lemma}
    \begin{proof}
        Indeed, for $r_n+x+I\in (R_n+I)/I$ and $r_m+y+I\in (R_m+I)/I$, then 
        $$
            (r_n+x)(r_m+y)+I=r_nr_m+(xr_m+yr_n+xy)+I\in (R_{nm}+I)/I.
        $$ 
    \end{proof}
    \begin{remark}
        Note that $R/I\simeq \bigoplus_{n\in \N} R_n/I_n$ where $I_n$ is the $n$-homogeneous part of $I$.
    \end{remark}
    \begin{definition}
        Consider a graded ring $R$. We define $R_{+} := \sum_{d>0} R_n$.
    \end{definition}
    \begin{lemma}
        Given a graded ring $R$, $R_{+}$ defines a two-sided ideal in $R$.
    \end{lemma}
    \begin{proof}
        Indeed, if $f\in R$ and $\sum_r^n f_i\in R_{+}$, then $f\in R_l$ for some $l$, hence 
        $$
            ff_i\in R_{i+l},
        $$
        meaning 
        $$
            f\sum_r^n f_i = \sum_r ff_i\in \sum_r^n R_{i+l}\subset R_{+}.
        $$
    \end{proof}
    \begin{definition}
        In a graded ring $R$, given an element $f\in R\setminus 0$ with homogeneous components $f_i$, we define \emph{the degree of $f$} to be 
        $$
            \deg \ f := \max(\{i\in \N : f_i\neq 0\}).  
        $$
        We thus have a function 
        \begin{gather*}
            \deg : R\setminus0\rightarrow \N\\
            f\mapsto \deg \ f 
        \end{gather*} 
    \end{definition}
    \begin{remark}
        We can extend the degree function to all of $R$. Our conventions to what $\deg\ 0$ is will differ in various contexts: Sometimes we extend $\deg$ to a function $R\rightarrow \N\cup\{\infty\}$, hence $\deg\ 0 = \infty$, sometimes we take $\deg \ 0$ to be $0$, some $\deg$ is extended to a multivalued function with $\deg \ 0 = \N$. 
    \end{remark}
    \begin{lemma}\label{DegreeOfSumOfElementsInGradedRing}
        Let $R$ be a graded ring, and $f,g\in R$, then  
        $$
            \deg\ (f+g) \leq \max(\deg\ f, \deg \ g)
        $$
        holding with equality when $\deg\ f \neq \deg\ g$. 
    \end{lemma}
    \begin{proof}
        Set $d:=\max(\deg\ f, \deg \ g)$. Denote the homogeneous components of $f$ and $g$ by $f_i$ respectively $g_i$. The homogeneous components of $f+g$ are $f_i+g_i$. Then $f_i=g_i=0$ for every $i> d$ implies $f_i+g_i=0$ for every $i>d$. It thus follows that $\deg \ f+g\leq d$.\\
        If $\deg \ f < \deg \ g$, then $f_d+g_d=g_d$. It follows that $\deg \ (f+g)=d$.
    \end{proof}
    \begin{lemma}
        Let $R$ be a graded ring and a domain, and $f,g\in R\setminus 0$. Then
        $$
            \deg \ fg = \deg \ f + \deg \ g.
        $$ 
    \end{lemma}
    \begin{proof}
        Let $f_i$ and $g_i$ denote the homogeneous components of $f$ resp. $g$ and let $d$ denote $\deg \ f +\deg \ g$. The homogeneous components of $fg$ are $\sum_{i,j\in \N : i+j = k} f_ig_i$. When $i<\deg \ f$ and $i+j = d$, then $j>\deg \ g$, hence $f_ig_j=0$. Similarly, when $j<\deg \ g$ and $i+j=d$, then $f_ig_j=0$. It follows that 
        $$
            (fg)_d = f_{\deg\ f}g_{\deg \ g}\neq 0.
        $$ 
        For $k>d$, when $i+j=k$, then $i>d$ or $j>d$. It follows that $f_ig_j=0$, hence $(fg)_k =0$. We conclude that $\deg \ fg =d$.
    \end{proof}
    \begin{remark}
        When $R$ is not a domain, $\deg\ fg\leq d$ (when $fg\in R\setminus 0$ or $\deg\ 0$ is defined to be $0$), which is easily seen from the last argument of the above proof. 
    \end{remark}
    \begin{lemma}\label{ProductOfHomogeneousElements}
        Let $R$ be a graded ring. Let $f,g\in R\setminus 0$ be given. The following are equivalent
        \begin{enumerate}
            \item $fg$ is not homogeneous.
            \item $f$ has distinct homogeneous components $f_i,f_j$ for which $f_ig_n \neq 0$ and $f_jg_n\neq 0$ for some homogeneous component $g_n$ of $g$, or $g$ has distinct homogeneous components $g_k,g_l$ for which $g_kf_m\neq 0$ and $g_kf_m\neq 0$ for some homogeneous component $f_m$ of $f$.
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        "1.$\implies$ 2.": Suppose first $fg$ is not homogeneous. Then $fg\neq 0$. Hence $fg_k\neq 0$ for some $k$. Then $f_ig_k\neq 0$ for some $i$. If there is an $f_j$ distinct from $f_i$ that also satisfies $f_jg_k\neq0$, then we are done. Suppose this is note the case. Then there is an $g_h$ with $f_ig_h\neq 0$, for otherwise $fg= f_ig_k$.\\
        "2.$\implies$ 1.": Suppose then that the second condition is met. In the first case, let $k$ largest index for which there is a pair of homogeneous components $f_i$ and $f_j$ of $f$ satisfying $f_ig_k,f_jg_k\neq 0$. Assume that $i$ and $j$ is the second largest respectively largest index satisfying the condition. Then one observes that
        $$
            (fg)_{i+k}= f_ig_k \text{ and } (fg)_{j+k} = f_jg_k
        $$     
        hence $fg$ has a pair of distinct homogeneous components and is thus not homogeneous.
    \end{proof}
    \begin{corollary}
        Let $R$ be a graded ring and $x,f\in R$ with $x\neq 0$ homogenous with $x$ not being a right zero divisor. If $xf$ is homogeneous then so is $f$.  
    \end{corollary}
    \begin{corollary}
        Let $R$ be an integral domain and a graded ring. For $f,g\in R\setminus 0$, if $fg$ is homogeneous so is $f$ and $g$.
    \end{corollary}
    \begin{lemma}\label{EquivalentPrimeHomogeneousIdealCondition}
        In a commutative graded ring $R$ a homogeneous ideal $\mathfrak{p}\subset R$ is prime if and only if for every pair of homogeneous elements $a,b\in R$ if $ab\in \mathfrak{p}$, then $a\in \mathfrak{p}$ or $b\in \mathfrak{p}$. 
    \end{lemma}
    \begin{proof}
        The first statement is trivial. Suppose for every pair of homogeneous elements $a,b\in R$ if $ab\in \mathfrak{p}$, then $a\in \mathfrak{p}$ or $b\in \mathfrak{p}$. Let $f,g\in R$ with $fg\in \mathfrak{p}$. Write $f= \sum_{m}^n f_i$ and $g=\sum_{k}^l g_i$ be the homogeneous decompositions. Then $fg$ has homogeneous components $\sum_{i,j\in N : i+j=k } f_ig_j$. Note that for some $k\leq \deg \ f$ and $l\leq \deg \ g$, $f_ig_j = 0$ for $i+j>k+l$ and
        $$
            \sum_{i,j\in \N : i+j = k+ l} f_ig_j = f_kg_l\in \mathfrak{p}, 
        $$
        Then $f_k\in \mathfrak{p}$ or $g_l\in \mathfrak{p}$. Assume $f_k\in \mathfrak{p}$. Then $f_kg\in \mathfrak{p}$, hence $(f-f_k)g\in \mathfrak{p}$. By induction in the number of terms in the product, it follows that $f-f_k\in \mathfrak{p}$ or $g\in \mathfrak{p}$. In the second case we are done. In the first case $f=(f-f_k)+f_k\in \mathfrak{p}$. The case $g_l\in \mathfrak{p}$ follows from a symmetric argument.
    \end{proof}
    \begin{lemma}
        Consider a left ideal $I$ in a graded ring $R$. Then $I$ is homogeneous if and only if it is generated from the left by a set of homogeneous elements.  
    \end{lemma}
    \begin{proof}
        Indeed, if $I$ is a left ideal generated by homogeneous elements from the left, then the homogeneous decomposition of any $f\in I$ has homogeneous components that must correspond some finite subset of the generators by uniqueness of homogeneous decomposition.\\
        Conversely if $I$ is homogeneous, $I$ is generated from the left by the set
        $$
            \{a\in R : a \text{ is a homogenous component of some } f\in I\}.
        $$
    \end{proof}
    \begin{lemma}\label{HomogeneousIdealFinitelyGenerated}
        Consider a left ideal $I=\sum_1^n Ra_i$ finitely generated from the left in a graded ring $R$. $I$ is homogeneous if and only if it is generated by some finite set of homogeneous elements.   
    \end{lemma}
    \begin{proof}
        If $I$ is generated by a finite set of homogeneous elements, then it is homogeneous by the prior lemma. If $I$ is homogeneous it is generated by the set of homogeneous components of all of its generators which by assumption is a finite set.  
    \end{proof}
    \begin{proposition}
        If $I$ is a homogeneous ideal of a graded, commutative ring $R$, then $\rad(I)$ is homogeneous.  
    \end{proposition}
    \begin{proof}
        Let $f=\sum_0^d f_i \in \rad(I)$. We must prove that $f_i\in \rad(I)$. Note that $f^n = f_d^n + r\in I$ where $\deg \ r < dn$. Then $f_d^n\in I$, hence $f_d\in \rad(I)$. We thus have that $f - f_d \in \rad(I)$ by induction in degree it follows that $f\in \rad(I)$.
    \end{proof}
    \begin{lemma}
        If $\{I_\alpha\}_{\alpha\in A}$ is a family of homogeneous left ideals in a graded ring $R$, then $\sum_{\alpha\in A} I_\alpha$ and $\bigcap_{\alpha} I_\alpha$ are homogeneous left ideals in $R$.  
    \end{lemma}
    \begin{proof}
    ...
    \end{proof}
    \begin{lemma}
        Let $I,J\subset R$ be homogeneous ideals in a graded ring $R$. Then $IJ$ is homogeneous.
    \end{lemma}
    \begin{proof}
        Let $f=\sum_0^d f_i\in I$ and $g=\sum_0^e g_j\in J$. Then each $f_i\in I$ and each $g_j\in J$, meaning $f_ig_j\in IJ$ for each $0\leq i\leq d$ and $0\leq j\leq e$. Then since $fg = \sum_{k=0}^{d+e}\sum_{i,j:i+j= k} f_ig_j$ where $\sum_{i,j:i+j= k} f_ig_j$ is homogeneous of degree $k$ for each $0\leq k\leq d+e$ we get that $IJ$ is homogeneous.
    \end{proof}
    We consider a way of extending a grading to certain localizations of a graded ring 
    \begin{lemma}
        Let $R$ be a graded ring. Then 
        $$
            H_R:= \{x\in R:x \text{ homogeneous}\}
        $$
    ...
    \begin{proof}
        This is trivial. 
    \end{proof}
    \begin{lemma}\label{LocalizationOfGradedRing}
        Let $R$ be a graded ring and $X$ a multiplicative subset contained in $H_R$. We define a triple on $(X^{-1}R, \Z, \{R_n\})$ with 
        $$
            R_n := \left\{\frac{a}{x} : a\in R_i, x\in R_{j}, i-j=n \right\}
        $$
        for $n<0$. This defines a grading.
    \end{lemma}
    \begin{proof}
        We check that $R_n$ is an additive subgroup for $n<0$. Indeed, for $\frac{a}{x},\frac{b}{y}\in R_n$, we have $a\in R_{l_1}$ and $x\in R_{k_1}$ with $l_1-k_1 = n$ and $b\in R_{l_2}$ and $y\in R_{k_2}$ with $l_2-k_2=n$, then $bx\in R_{l_2+k_1}=R_{k_2+k_1+n}$ and $ay\in R_{l_1+k_2}=R_{k_1+k_2+n}$, hence $bx+ay\in R_{k_1+k_2+n}$ and $xy\in R_{k_1+k_2}$. Since $k_1+k_2+n-(k_1+k_2)=n$, it follows that $\frac{a}{x}+\frac{b}{y}\in R_n$.\\
        Let $\frac{f}{x}\in X^{-1}R$. Then
        $$
            \frac{f}{x} = \sum_{n\in \N} \frac{f_n}{x} \in \sum_{n-\deg \ x \in \N} R_k.  
        $$ 
        Suppose $\frac{a}{x}\in R_{n-\deg \ x}\cap R_{m-\deg x}$. Then $a\in R_n\cap R_m$, hence $n=m$ or $a=0$, hence $n-\deg \ x = m-\deg \ x$ or $\frac{a}{x} = 0$.
    \end{proof}
    \begin{lemma}
        Given a homogeneous prime ideal $\mathfrak{p}$ in a commutative graded ring $R$, define $R_{(\mathfrak{p})}$ to be the localization of $R$ with respect to $X:=R_0\cap R\setminus \mathfrak{p}$. Then $R_{(\mathfrak{p})}$ is a graded, local ring whose maximal ideal is homogeneous. 
    \end{lemma}
    \begin{proof}
        Suppose $\frac{a}{f}\in R_{(\mathfrak{p})}$ is not a unit in $R_\mathfrak{p}$, then $a\in \mathfrak{p}$, hence $a\notin X$ (note that $X$ is clearly saturated), hence $\frac{a}{f}$ is not a unit in $R_{(\mathfrak{p})}$. Conversely, if $\frac{a}{f}\in R_{(\mathfrak{p})}$ is a unit in $R_\mathfrak{p}$, then there is a $\frac{b}{g}\in R_\mathfrak{p}$ with 
        $$
            \sum_{n\in \N} fg_n = \sum_{n\in \N} ab_n.
        $$
        In particular, since $a,f\in R_0$, 
        $$
            fg_0  = ab_0 \implies \frac{a}{f}\frac{b_0}{g_0}=1 \implies f\in \left(R_{(\mathfrak{p})}\right)^\ast.
        $$
        It thus follows that the set of non-units in $R_{(\mathfrak{p})}$ is the ideal $R_{(\mathfrak{p})}\cap \mathfrak{m}_\mathfrak{p}$, hence $R_{(\mathfrak{p})}$ is local with maximal ideal $\mathfrak{m}_{(\mathfrak{p})}$ say. Note that $\frac{a}{f}\in \mathfrak{m}_{(\mathfrak{p})}$ implies $a\in \mathfrak{p}$, hence $a_i\in \mathfrak{p}$ for each $i$. It thus follows that $\frac{a_i}{f}\in \mathfrak{m}_{(\mathfrak{p})}$, hence $\mathfrak{m}_{(\mathfrak{p})}$ is homogeneous.  
    \end{proof}
\subsection{Vector Spaces}
\begin{definition}
    A \emph{vector space} is a module over a field $K$.
\end{definition}
For a field $K$, we define $\mathrm{Vect}_K:=\mathrm{Mod}_K$.
\subsubsection{Finite Dimensional Vector Spaces}
\begin{lemma}
    Assuming the Axiom of Choice, any vector space $V$ over a field $K$ has a basis.
\end{lemma}
\begin{proof}
    Let $X$ be the set of all linearly independent subsets of $V$. Let $C$ be a chain in $X$. Suppose 
    $$
        \sum_1^n a_i v_i=0 
    $$
    where 
    $$
        v_1,\dots,v_n\in \bigcup_{\pazocal{V}\in C} \pazocal{V}
    $$
    Then for each $i$, $v_i \in \pazocal{V}_i$ for some $\pazocal{V_i}\in C$. Then for each $i$, $\pazocal{V_i}\subset \pazocal{V}_m$ for some $m\in \{1,\dots,n\}$. It thus follows that $a_i = 0$ for every $i$. Then 
    $\bigcup_{\pazocal{V}\in C}\pazocal{V}\in X$ and is an upper bound of $C$. Then $X$ has a maximal element $\pazocal{V}$. A maximally linear independent set is a basis, hence $V$ has a basis. Indeed, given any $v\in V$ either $v\in \pazocal{V}$ or there are $a_1,\dots,a_{n+1}\in K$ with $a_{n+1}\in K\setminus0$ and $v_1,\dots,v_n$ such that 
    $$
        a_{n+1}v+ \sum_1^n a_i v_i = 0
    $$
    since otherwise $\pazocal{V}\cup \{v\}\supsetneq \pazocal{V}$ would be linearly independent over $K$, contradicting maximality. We thus have  
    $$
        v = \sum_1^n -a_{n+1}^{-1}a_iv_i.
    $$
    This means $\pazocal{V}$ generates $V$, so $\pazocal{V}$ is a basis. 
\end{proof}
\begin{lemma}
    Consider a vector space $V$ over a field $K$, if $\{v_1,\dots,v_n\}\subset V$ are linearly independent over $K$ and $\{w_1,\dots,w_m\}$ generates $V$ over $K$, then 
    $$
        n\leq m.
    $$
\end{lemma}
\begin{proof}
     ...
     $$
        v = \sum_1^m a_i w_i
     $$
     for $a_i\in K$ where wlog $a_1 \neq 0$. Then 
     $$
        w_1 = a_1^{-1}v+\sum_2^m a_1^{-1}a_i w_i 
     $$
     hence $\{v,w_2,\dots,w_m\}$.\\
     Assume that if $\{v_1,\dots,v_n\}$ are linearly independent that $n\leq m$. Suppose $\{v_1,\dots,v_{n+1}\}$ are linearly independent. Using linear independence of $\{v_1,\dots,v_n\}$ over $K$, we find that 
     $$
        \{v_1,\dots,v_n,w_{n+1},\dots,w_{m}\}
     $$
     which is a set of cardinality $m$. Then 
     $$
        v_{n+1} = \sum_1^n b_i v_i + \sum_{1}^{m-n} c_iw_{n+i}
     $$
    for suitable $b_i,c_j\in K$. Then $n<m$ and $c_k\neq 0$ for some $k$ for otherwise $\{v_1,\dots,v_{n+1}\}$ are linearly dependent. Then $\{v_1,\dots,v_{n+1},w_{n+2},\dots,w_{m}\}$, a set of cardinality $m$, generates $V$ over $K$. It thus follows that since 
    $$
        \{v_1,\dots,v_{n+1}\}\subset \{v_1,\dots,v_{n+1},w_{n+2},\dots,w_{m}\}.
    $$
    which means $n+1\leq m$.
\end{proof}
\begin{corollary}
    If $\pazocal{V},\pazocal{V}'$ are finite bases of the same vector space, then 
    $$
        \#\pazocal{V}=\#\pazocal{V}'.
    $$
\end{corollary}
\begin{definition}
    Given a vector space $V$ over a field $K$ with finite basis $\pazocal{V}$, we define the \emph{dimension of $V$ over $K$} to be 
    $$
        \dim\ V:=\dim_K\ V := \#\pazocal{V}.
    $$
    Such a vector space is called a \emph{finite dimensional vector space}. The category of such vector spaces is denoted $\mathrm{Vect}^{\mathrm{fd}}_K$. When $V$ does not have a finite basis we define
    $$
        \dim\ V := \infty
    $$
\end{definition}
\begin{remark}
    We see that $\dim$ defines an invariant of vector spaces over $K$ valued in $(\Z\cup\{\infty\})$. Indeed, if $l:V\hookrightarrow W$ and $B$ is a basis for $B$, then if 
    $$
        \sum_{v\in B} a_v l(v)=0
    $$
    we get that 
    $$
        0 = l\left(\sum_{v\in B} a_v v \right) \implies \sum_{v\in B} a_v v =0 \implies a_v=0 \text{ for all } v,
    $$
    hence $l(B)$ is linearly independent over $K$. Then if $C$ is a basis of $W$ over $K$, it follows that $\#B=\#l(B)\leq \# C$, hence 
    $$\dim \ V \leq \dim \ W.$$
\end{remark}
\begin{definition}
...
\end{remark}
\begin{lemma}
    Let $V$ be a vector space over a field $K$. Let $\pazocal{V}$ be a basis of $V$ and $l\in \mathrm{Aut}_K(V)$. Then $l(\pazocal{V})$ is basis of $V$
\end{lemma}
\begin{proof}
    Let $v\in V$. Then 
    $$
        l^{-1}(v) = \sum_1^n a_iv_i
    $$
    for some $v_1,\dots,v_n\in \pazocal{V}$ and $a_1,\dots,a_n\in K$. It follows that 
    $$
        v = \sum_1^n a_il(v_i).
    $$
    Suppose $l(v_1),\dots,l(v_n)\in l(\pazocal{V})$ are given with $a_1,\dots,a_n\in K$ such that 
    $$
        \sum_1^n a_il(v_i)=0.
    $$
    Then 
    $$
        \sum_1^n a_iv_i = l^{-1}\left(\sum_1^n a_il(v_i) \right)=0,
    $$
    hence $a_i = 0$ for each $i$.
\end{proof}
...
\begin{proof}
    ...
    $w_i = _\pazocal{V}T_\pazocal{W} v_i = _\pazocal{V}T_\pazocal{W} {_\pazocal{W}}T_\pazocal{V} w_i$, and that $v_i = _\pazocal{W}T_\pazocal{V} w_i = _\pazocal{W}T_\pazocal{V} {_\pazocal{V}}T_\pazocal{W} v_i$. Since a module homomorphism is uniquely characterized by its behavior on the basis elements, it follows that 
    $$
        _\pazocal{V}T_\pazocal{W} {_\pazocal{W}}T_\pazocal{V} = _\pazocal{W}T_\pazocal{V} {_\pazocal{V}}T_\pazocal{W} = I_n,
    $$ 
    hence ${_\pazocal{V}}T_\pazocal{W}$ is invertible with inverse ${_\pazocal{W}}T_\pazocal{V}$.\\
    "$\impliedby$": by the prior lemma, $\pazocal{W} = _\pazocal{V}T_\pazocal{W}(\pazocal{V})$ is a basis of $V$.
\end{proof}
\begin{lemma}(Rank-Nullity)
    Let $V$ be a finite dimensional vector space and $W$ any vector space. Then for a linear map $l : V\rightarrow W$,
    $$
        V \simeq \ker \ l \oplus \im \ l.
    $$
    In particular,
    $$
...
\begin{proof}
    ...
    $$
        \begin{tikzcd}
            0 \arrow[r] & \ker \ l \arrow[r,hookrightarrow] & V \arrow[r, twoheadrightarrow] & \im \ V \arrow[r] & 0
        \end{tikzcd}
    $$
    with $\id_V$ being a retraction of $\ker\ l \hookrightarrow V$. By the splitting lemma, We are done
\end{proof}
\begin{lemma}\label{DimensionsOfExactSequence}(Generalized Rank-Nullity)
    ...
    \end{lemma}
    \begin{proof}
        This follows from Lemma~\ref{ExactnessIsEquivalentToInjFirstSESSurjSecondSES} and rank-nullity. Indeed, 
        $$
            V' = \ker\ \rho' \oplus \im \ \rho' = \im \ \rho\oplus V'' \simeq V\oplus V''  
        $$
        where the isomorphism is induced by $\rho$ considered as a map onto its image.
    \end{proof}
    \begin{proposition}
    ...
    \end{proof}
    \begin{definition}
        Consider a vector space $V$ and subspace $W\subset V$. We define the \emph{codimension of $W$ in $V$} to be 
        $$
            \codim^V \ W := \codim\ W = \dim \ V - \dim \ W.
        $$
    \end{definition}
    \begin{remark}
        Given a basis $\pazocal{V}$ of $V$, there is a basis $\pazocal{W}\subset \pazocal{V}$ of $W$. It thus follows that we can express $\codim\ W$ as the cardinality of the set $\pazocal{V}\setminus \pazocal{W}$.
    \end{remark}
    
    \begin{lemma}
        Given a vector space $V$ with subspaces $W'\subset W\subset V$. Then 
        $$
            \codim\ W\leq \codim\ W'.
        $$
    \end{lemma}
    \begin{proof}
        Indeed since $\dim \ W\geq \dim \ W'$, one sees that 
        $$
            \codim \ W = \dim \ V - \dim \ W \leq \dim\ V- \dim \ W' = \codim\ W'. 
        $$
    \end{proof}
...
\subsubsection{Dual Vector Spaces}
    \begin{definition}
        Let $V$ be a vector space over a field $K$. The \emph{dual vector space of $V$ (over $K$)} is 
        $$
            \mathrm{Dual}(V):=V' := \mathrm{Vect}_K(V,K)
        $$
    \end{definition}   
    \begin{remark}
        Note that this assignment is functorial. In particular, it is a contravariant endofunctor on $\mathrm{Vect}_K$. 
    \end{remark} 
    \begin{lemma}
        Suppose $V$ is a finite dimensional vector space over $K$. Then 
        $$
            V\simeq V'
        $$
    \end{lemma}
    \begin{proof}
        WLOG $V\simeq K^n$ for some $n$, so it suffices to prove that $K^n\simeq \mathrm{Vect}_K(K^n,K)$. $K^n$ is equipped with a bilinear map 
        \begin{gather*}
            \langle \cdot,\cdot\rangle : K^n\times K^n \rightarrow K\\
            (v,w)\mapsto \sum_1^n v_iw_i
        \end{gather*} 
        For each $v\in K^n$ we thus have a $K$-linear map
        \begin{gather*}
            l_v : K^n \rightarrow K\\
            w\mapsto \langle v,w\rangle 
        \end{gather*}
        We thus define a $K$-linear map 
        \begin{gather*}
            L : V\rightarrow V'\\
            v\mapsto l_v
        \end{gather*}
        ...
        \begin{gather*}
            L^{-1}: V'\rightarrow V\\
            l\mapsto (l(e_1),\dots,l(e_n))
        \end{gather*}
        where $e_i$ is the $i$'th standard basis vector of $K^n$.
    \end{proof}
    \begin{proof}
        Pick a basis $\{v_1,\dots,v_n\}$ of $V$. Define for each $i\in \{1,\dots,n\}$,
        $$
            l_{v_i} : V\rightarrow K
        $$
        by 
        $$
            l_{v_i}(v_j) := 
            ...
            \end{cases}
        $$
        we verify that $\{l_{v_1},\dots,l_{v_n}\}$ defines a basis for $V'$ which would show that $\dim\ V' = n$, implying $V\simeq V'$. Suppose
        $$
            \sum_1^n b_il_{v_i} = 0, 
        $$
        then 
        $$
            \sum_1^n b_iv_i = \sum_1^n\left(\sum_1^n b_jl_{v_j}(v_i)\right)v_i = 0 
        $$
        hence $b_i=0$ for each $i$. Consider an arbitrary linear map $l\in V'$. Then for each $w=\sum_1^n a_iv_i\in V$
        $$
            l(v)= \sum_1^n a_il(v_i)= \sum_1^n l(v_i)l_{v_i}(w) = \left(\sum_1^n l(v_i)l_{v_i}\right)(w)
        $$
    \end{proof}
    \begin{remark}
        The basis constructed on $V'$ in the second proof, is called the \emph{dual basis on $V$ (with respect to the basis $\{v_1,\dots,v_n\}$)}.
    \end{remark}
    \begin{proposition}
        Given a field $K$ and denoting the identity functor on $\mathrm{Vect}_K^{\mathrm{fd}}$ by $\mathrm{Id}$, 
        $$
            \mathrm{Dual} \cong \mathrm{Id}
        $$
    \end{proposition}
    \begin{proof}
        Initially, we do not need the assumption of working with only finite dimensional vector spaces to define a natural transformation with injective components. We define a natural transformation $\ev:\mathrm{Id}\Rightarrow \mathrm{Dual}$ to have components
        $$
            \{ev^V : V\rightarrow V'', v\mapsto ev_v\}
        $$
        consider a linear map $L: V\rightarrow W$. Let $v\in V$ and $l\in W''$. Then 
        \begin{align*}
            \left(\left(\left(L^\ast\right)\right)^\ast \ev^V(v)\right)(l) &=\left(\left(L^\ast\right)^\ast(\ev_v)\right)(l) = \left(\ev_vL^\ast\right)(l)= \ev_v(lL)= l(L(v))\\
            &= \ev_{L(v)}(l)=\left(\ev^W L(v)\right)(l).
        \end{align*}
        In other words the square 
        $$
            \begin{tikzcd}
                V \arrow[d,"L"]\arrow[r,"\ev^V"] & V''\arrow[d,"\left(L^\ast\right)^\ast"]\\
                W \arrow[r,"\ev^W"] & W''
            \end{tikzcd}
        $$
        commutes, hence $\ev $ is a natural transformation. Suppose $V\neq 0$. Then $V$ is not the zero object in $\mathrm{Vect}_K$, hence $V'\neq 0$. Then if $v\neq 0$, then $l(v)\neq 0$ for some $l\in V'$, hence $\ev^V$ is injective for an arbitrary vector space $V$.\\
        Suppose $V$ is a finite dimensional vector space and consider a $K$-linear map $\epsilon\in V''$. When $\epsilon(l)=0$ for every $l\in V'$, then $\epsilon = \ev_0$. Suppose this is not the case. Then there is some $l\in V'\setminus 0$ for which $\epsilon(l)\neq 0$. Moreover, there is a $v\in V$  with $l(v)=\epsilon(l)\neq 0$. $\dim \ \ker \ \epsilon = \dim \ V' -1$. So if $\lambda\in V'$ is another functional on $V$ with $\epsilon(\lambda)\neq 0$, then $\lambda = \alpha l$ for some $\alpha\in K$. It thus follows that
        $$
            \epsilon(\lambda)=\epsilon(\alpha l)=\alpha\epsilon(l)=\alpha l(v)= (\alpha l)(v)=\lambda(v).
        $$      
        It thus follows that $\epsilon = \ev_v$. Then each component of $\ev$ is an isomorphism, hence $\ev$ is natural isomorphism. 
    \end{proof}
\subsubsection{Projective Space}
    ...
    \end{definition}
    \begin{remark}
        This is an equivalence relation. Indeed $v = 1\cdot v$, hence $v\sim v$. If $v\sim w$, then $w =\lambda v$, hence $v= \lambda^{-1}w$, meaning $w\sim v$. Suppose $v\sim w$ and $w\sim u$. Then $w=\lambda v$ and $u = \mu w$, hence $u = \mu\lambda v$, implying $v\sim u$. 
    \end{remark}
    \begin{definition}
    ...
    \subsubsection{The Projective Span}
        ...
        \begin{remark}
            ...
            $$
                \left[\sum_1^m \lambda_i(\alpha_i v_i)\right] = \left[\sum_1^m (\lambda_i\alpha_i)v_i\right]\in \left\{\left[\sum_1^m \lambda_iv_i\right] : (\lambda_1,\dots,\lambda_m)\in K^m\setminus 0 \right\} 
            $$
            and conversely 
            $$\left[\sum_1^m \lambda v_i\right] = \left[\sum_1^m (\lambda_i\alpha_i^{-1})\alpha_iv_i\right] \in \left\{\left[\sum_1^m \lambda_i(\alpha_iv_i)\right] : (\lambda_1,\dots,\lambda_m)\in K^m\setminus 0 \right\}.$$
        ...
        \begin{lemma}\label{EquivalentDefinitionOfLinearlyIndependent}
            $v_1,\dots,v_m\in K^n$ are linearly independent if and only if\\ 
        ...
        \begin{proof}
            ...
            $$[v_j] = \left[\lambda_j^{-1}\sum_{i\neq j} \lambda_i v_i\right] \in \Span([v_1],\dots,\widehat{[v_j]},\dots, [v_m])$$
        \end{proof}
...
In this subsection every ring will be commutative. The \emph{modulus} function on $\N^n$ will refer to 

\begin{gather*}
    \vert \cdot \vert : \N^n \rightarrow \N\\
    (v_1,\dots,v_n) \mapsto \sum_1^n v_i
\end{gather*}

Note that it is an additive function.
\subsubsection{Defining Formal Power Series} 

\begin{definition}
    \emph{The power series in $n$-variables over a ring $R$} is the set 
    $$
        R\llbracket x_1,\dots,x_n \rrbracket:=\prod_{v\in \N^n} R
    $$ 
\end{definition}

\begin{lemma}
    We define multiplication on $R\llbracket x_1,\dots,x_n \rrbracket$ by
    $$(a_v)(b_v)=\left( \sum_{v,w\in \N^n : v+w =u} a_vb_v \right)\in R\llbracket x_1,\dots,x_n \rrbracket.$$
    This multiplication makes $R\llbracket x_1,\dots,x_n \rrbracket$ a commutative ring. $R\left[x_1,\dots,x_n\right]$ is a subring of $R\llbracket x_1,\dots,x_n \rrbracket$.  
\end{lemma}
\begin{proof}
    Let $(a_v),(b_v),(c_v)\in R\llbracket x_1,\dots,x_n \rrbracket$. Then 
    \begin{align*}
    ...
    \end{align*}
    This means $R\llbracket x_1,\dots,x_n \rrbracket$ is a ring with this multiplication. Note also that
    $$(a_v)(b_v) = \left( \sum_{v,w\in \N^n : v+w =u} a_vb_w \right) =\left( \sum_{v,w\in \N^n : v+w =u} b_va_w \right)=(b_v)(a_v).$$
    Hence, $R\llbracket x_1,\dots,x_n \rrbracket$ is a commutative ring with this multiplication.\\
    To check that $R[x_1,\dots,x_n]$ is a subring of $R\llbracket x_1,\dots,x_n \rrbracket$, we need to check that $1\in R[x_1,\dots,x_n]$ and that it is closed under multiplication. Since $\delta_{\mathbf{0},v}=0$ for every $v\in \N^n \setminus \{\mathbf{0}\}$, it follows that $1\in R\left[\N^n\right]$. Let $(a_v),(b_v)\in R[\N^n]$. We note that for some $N,M\geq 0$, $a_v = 0$ for $v\in \N^n$ with $\vert v\vert \geq N$ and $b_w = 0$ for $w\in \N^n$ with $\vert w \vert \geq M$. Let $u\in \N^n$ with $\vert u\vert \geq N+M$. Consider then $v,w \in \N^n$ such that $v+w$. Then
...
\end{definition}
\begin{lemma}
    Let rings $R,S,T$ be given and consider $\sigma\in \Hom(R,S)$, $\tau\in \Hom(S,T)$. Then $\overline{\tau\circ \sigma} = \overline{\tau} \circ \overline{\sigma}$. We also have that $\overline{\text{id}_R}=\textit{id}_{R[x_1,\dots,x_n]}$. In other words 
    $$(R,\sigma)\mapsto (R[x_1,\dots,x_n],\overline{\sigma})$$
    is a covariant endofunctor on $\mathrm{CRSing}$ for every $n\geq 1$.  
\end{lemma}
\begin{proof}
...
\end{example}
\subsubsection{The Graded Ring of Polynomials}
    \begin{definition}
        For a commutative ring $R$, we define 
        $$
            V_{k,n}(R) := \sum_{v\in \N^n: \vert v\vert = k} R\mathbf{x}^{v}\subset R[x_1,\dots,x_n]\quad (n\geq 1, k\geq 0).
        $$
        An element of $V_{k,n}(R)$ is called a \emph{homogeneous polynomial (of degree $k$)}. 
    \end{definition}
    \begin{remark}
        Consider a homogeneous polynomial $f$ of degree $d>0$ in $R[x_1,\dots,x_n]$. Let $\lambda\in R$. Then 
        $$
            f(\lambda x_1,\dots,\lambda x_n) = \sum_{v\in\N^n: \vert v\vert =d} a_v(\lambda x_1)^{v_1}\cdots (\lambda x_n)^{v_n} = \sum_{v\in\N^n: \vert v\vert =d} \lambda^d\mathbf{x}^v = \lambda^d f
        $$
    \end{remark}
    \begin{lemma}
        For a commutative ring $R$ and $n\geq 1$, 
        $$
            \{V_{k,n}(R): k\geq 0\}
        $$
        defines a grading moreover $V_{k,n}(R)$ is an $R$-module and $V_{0,n}(R)=R$.
    \end{lemma}
    \begin{proof}
        Let $f=\sum_{v\in \N^n: \vert v\vert = k} a_v \mathbf{x}^v$ and $g=\sum_{v\in \N^n: \vert v\vert = l} b_v\mathbf{x}^v$. Note that whenever $\vert u\vert \neq k$ or $\vert w \vert \neq l$, 
        $$
            a_ub_w=0,
        $$
        hence 
        $$
            fg = \sum_{u,w\in \N^n : \vert u\vert = k, \vert w\vert =l} a_ub_w \mathbf{x}^{u+w} \in V_{k+l,n}(R).
        $$
        Suppose $f\in V_{k,n}(R)\cap V_{l,n}(R)$ with $k\neq l$. Then 
        $$
            \sum_{v\in \N^n:\vert v\vert = k} a_v\mathbf{x}^v = f = \sum_{w\in\N^n : \vert w \vert = l} b_w\mathbf{x}^w
        $$
        hence for $v\in \N^n$ with $\vert v\vert = k$, $a_v = b_v = 0$, hence $f=0$. Moreover, for any $g\in R[x_1,\dots,x_n]$,
        $$
            g = \sum_{v\in \N^n} a_v\mathbf{x}^v =\sum_{k\in \N} \sum_{v\in \N^n: \vert v\vert = k} a_v\mathbf{x}^v \in \sum_{k\in\N} V_{k,n}(R).
        $$
        We thus conclude that $\{V_{k,n}(R)\}$ is a grading of $R[x_1,\dots,x_n]$.\\
        For $f= \sum_{v\in\N^n : \vert v \vert = k} a_v\mathbf{x}^v\in V_{k,n}(R)$, note that $af = \sum_{v\in \N^n : \vert v\vert = k} aa_v \mathbf{x}^v\in V_{k,n}(R)$. Let $r\in R$. Then $r = r\cdot 1 \in V_{0,n}(R)$. If $r\in V_{0,n}(R)$, then $r= \sum_{v\in \N^n: \vert v\vert = 0} a_v\mathbf{x}^v = a_0\cdot 1 \in R$. 
    \end{proof}
    \begin{definition}
...
\end{proof}
\begin{corollary}\label{PropertiesOfDegreeFunction}
     The function 
    \begin{gather*}
...
\end{lemma}
\begin{proof}
    Follows from Lemma~\ref{HomogeneousIdealFinitelyGenerated}
\end{proof}
...
\end{lemma}
\begin{proof}
    This follows from Lemma~\ref{EquivalentPrimeHomogeneousIdealCondition}
\end{proof} 
\subsubsection{The Linear Subspaces of Homogeneous Polynomials over a Field}
...
\end{proposition}
\begin{proof}
    With the notation of the above lemma $\{ \mathbf{x}^v \in K[x_1,\dots,x_n] : v\in \Delta_{n,d}\}$ forms a basis of $V_K(d,n)$, hence by the prior lemma
...
\begin{proof}
    ...
    \begin{align*}
        \dim\ K_{\leq d}[x_1,\dots,x_n] &= \sum_0^d \dim\ V_K(d,n)=\sum_0^d \#\Delta_{n,j} = \sum_0^d {j+n-1 \choose n-1}\\
        &={d+n\choose n} = \frac{(d+n)!}{d!n!}={d+n\choose d}.
    \end{align*}
\end{proof}
...
\subsubsection{Evaluation \& Roots of Polynomials}
\begin{definition}
...
\begin{lemma}\label{EvaluationIsARingHomomorphism}
    Evaluation is a well-defined map. Moreover, the map $\ev_{s_1,\dots,s_n}$ is a well-defined ring homomorphism such that $\ev_{s_1,\dots,s_n}(r) = r$ for every $r\in R$, therefore it is also an $R$-module homomorphism. In other words, evaluation in an element is an $R$-algebra homomorphism. 
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{EvaluationCommutesWithTakingQuotient}
    Let $R$ be a ring and $J\subset R[y_1,\dots,y_m]$ be an ideal. Consider $\overline{f_i}:=f_i+J\in R[\mathbf{y}]/J$. Then for each $f\in R[\mathbf{x}]$
...
\begin{proof}
    ...
    \begin{align*}
        \sigma(f) &= \sigma\left(\sum_{v\in \N^n} a_v\mathbf{x}^v\right) = \sum_{v\in \N^n} \sigma(a_v) \sigma\left(x_1^{v_1}\cdots x_n^{v_n}\right) = \sum_{v\in \N^n} a_v\sigma\left(x_1^{v_1}\right)\cdots\sigma\left(x_n^{v_n}\right)\\
        &= \ev_{\sigma(x_1),\dots,\sigma(x_n)}(f)
    \end{align*}
\end{proof}
...
\end{corollary}
\begin{corollary}
    Let $I\subset R[x_1,\dots,x_n]$, $J\subset R[y_1,\dots,y_n]$ be ideals. Then an $R$-algebra map $\sigma:R[x_1,\dots,x_n]/I\rightarrow R[y_1,\dots,y_m]/J$, is induced by some $\ev_{a_1,\dots,a_n}$ with $\ev_{a_1,\dots,a_n}(I)\subset J$. 
\end{corollary}
\begin{lemma}
...
\begin{proof}
    ...
    $$
        \ev_{s_1,\dots,s_n}(f)=0 \iff f = 0.
    $$ 
    $\ev_{s_1,\dots,s_n}$ is therefor injective, implying $R[s_1,\dots,s_n]\simeq R[\mathbf{x}]$
\end{proof}
\begin{corollary}
...
\begin{definition}
    ...
    $$V_S(f).$$
    The zeroes over $R$ are denoted 
    $$
        V(f).
    $$
\end{definition}
The above definitions are central to the classical treatment of algebraic geometry, since the geometric objects considered are build from zero sets of polynomials over a field $K$. 
...
\begin{proof}
    ...
    \begin{align*} 
        f + I &= \left(\sum_{v\in \N^n} a_v \mathbf{x}^v\right) + I = \sum_{v\in \N^n} a_v(x_1+I)^{v_1}\cdots(x_n+I)^{v_n}\\
        & = \sum_{v\in \N^n} a_v(s_1+I)^{v_1}\cdots(s_n+I)^{v_n}  \\ 
...
\begin{corollary}\label{MaximalIdealsOfPolynomialRings}
    Let $(r_1,\dots,r_n)\in R^n$. Then 
    $$
        R[x_1,\dots,x_n]/\langle x_1-r_1,\dots,x_n-r_n\rangle \simeq R.
    $$ 
    Explicitly, this isomorpism is induced by $\ev_{r_1,\dots,r_n}$. In particular, $\langle x_1-r_1,\dots,x_n-r_n\rangle$ is maximal if and only if $R$ is a field. 
\end{corollary}
...
\begin{proof}
    ...
    $$f(v) = f'(v_{n+1}) \neq 0.$$
\end{proof}
\begin{lemma}\label{InfinitelyManyIrreduciblePolynomialsOverIntegralDomain}
    Given an integral domain $R$, there are infinitely many irreducible polynomials in $R[x]$ and consequently this is also the case in $R[x_1,\dots,x_n]$. 
\end{lemma}
\begin{proof}
    When $R$ is infinite, each $\alpha\in R$, uniquely corresponds to an irreducible polynomial $(x-\alpha)$.\\
    Suppose $R$ is finite. Then $R$ is a finite field. Consider the polynomial 
    $$
        f := \prod_{\alpha\in R} (x-\alpha)
    $$
    Then 
    $$
        f^n +1
    $$
    has no roots in $R$, and is therefore irreducible for each $n\geq 0$. 
\end{proof}

\subsubsection{The Hilbert Basis Theorem}
\begin{theorem}(Hilbert Basis Theorem)\label{EveryPolynomialIdealIsFinitelyGenerated}
    Let $R$ be a noetherian ring. Then $R[x]$ is  noetherian. Furthermore $R[x_1,\dots,x_n]$ is noetherian. 
\end{theorem}
\begin{proof}
    We prove the contrapositive. Suppose That $R[x]$ is not noetherian, or equivalently by Theorem~\ref{NoetherianIffEverySubmoduleFinitelyGenerated} suppose there is an ideal $I\subset R[x]$ that is not finitely generated. Let $d_1 = \min\{\deg \ f : f \in I\}$. Let $f_1 \in I$ such that $\deg \ f_1 = d_1$. We then let $I_1 = R[x]f_1$ and recursively define $I_n = \sum_1^n R[x]f_i$, where $f_n \in I\setminus I_{n-1}$ where $\deg \ f_n = d_n = \min\{ \deg \ f: f \in I \setminus I_{n-1}\}$. Note that since $I\setminus I_n \supset I\setminus I_{n+1}$, we get $d_n\leq d_{n+1}$ for each $n\geq 1$. For each $n$ we can write 
    ...
    By induction $R[x_1,\dots,x_n]\simeq \left(R[x_1,\dots,x_{n-1}]\right)[x_n]$ is noetherian.
\end{proof}
...
\end{lemma}
\begin{proof}
    Let $A:=\{\alpha_1,\dots,\alpha_n\}\subset K$. Consider the polynomial
    $$
        f:= 1+\prod_1^n (x-\alpha_i)\in K[x]  
    $$
    Then $f$ has a root $\beta \in K$. Note that $f(\alpha_i) = 1 \neq 0$, hence $\beta \notin A$. It follows that $K$ is not finite. 
\end{proof}
...
\end{lemma}
\begin{proof}
    Corollary~\ref{PolynomialRingOverFieldIsPID} (a later result), Lemma~\ref{InPIDPrimeIdealsAreMaximal} and Lemma~\ref{InPIDIrreduciblesArePrimes} shows that $I$ is maximal. Then $K[x]/I$ is a field by Proposition~\ref{InCommutativeRingMaximalIdealIffQuotientRingField}. Secondly, 
...
\begin{lemma}
    Suppose $R$ is an integral domain. Then $$R\llbracket x \rrbracket ^\ast=  \left\{\sum_{i\in \N} a_ix^i\in R\llbracket x\rrbracket : a_i \in R^\ast \right\}$$
\end{lemma}
\begin{proof}
    Note that 
    $$
        \left(\sum_{i\geq 0} \alpha_i x^i\right)\left(\sum_{i\geq 0} \beta_i x^i \right) = \sum_{i\geq 0} \sum_{j=0}^i a_{j}b_{i-j}
    $$
    Let $s = \sum_{i\in\N}a_ix^i$  be an element of the right-hand side. Set $b_0 := a_0^{-1}$ and $b_k := -a_0^{-1}\sum_{j=1}^{k} a_jb_{k-j}$ for $k\geq 1$. Define $t =  \sum_{i\in \N^n} b_ix^i$. We prove by induction that $\sum_0^n a_jb_{n-j}=0$. Indeed, 
    $$
        \sum_{j=0}^na_jb_{n-j} = -\left(a_0a_0^{-1}\sum_{j=1}^n a_{j}b_{n-j} \right)+\sum_{j=1}^n a_jb_{n-j} =0.
    $$
    It thus follows that 
    $$
        \left(\sum_{i\geq 0} a_i x^i\right)\left(\sum_{i\geq 0} b_i x^i \right) = 1.
    $$
    Conversely, if $\sum_{i\geq 0} a_ix^i\in R\llbracket x\rrbracket^ast$. Then 
    $$
         \left(\sum_{i\geq 0} a_i x^i\right)\left(\sum_{i\geq 0} b_i x^i \right)
    $$
    for some $\sum_{i\geq 0} b_i x^i \in R\llbracket x\rrbracket$. In particular, $a_0b_0 = 1$, hence $a_0\in R^\ast$. 
\end{proof}
\begin{lemma}
...
\begin{definition}
    A \textit{term order} is a total order $\leq$ on $\N^n$ such that 
    \begin{align*}
...
\end{theorem}
\begin{proof}
    We proceed by induction in $n$. For $n=1$, $S$ has a minimal element $s$ by the well ordering of the natural numbers, hence any element of $S$ can be written as $s+t$ for some $t\in \N$. Suppose Dickson's lemma is true for some $n\geq 1$. Let $S$ be some non-empty subset of $\N^{n+1}$. Consider the surjection 
    \begin{gather*}
...
\begin{proof}
    ...
    As advertised we put $\lambda_i := \lambda_i^{(N)}$ for $i\in \{1,\dots,m\}$ and $f^F := r^{(N)}$. For each $j\geq 0$ for each $i\in\{1,\dots,m\}$ for which $a_i^{(j)} = 0 $ and $a_i^{(j+1)} \neq 0$ we have that
    $$\inn\ \lambda_i^{(j+1)}f_i = \inn\left(\left(\lambda_i^{(j)}+\frac{\inn\ s^{(j)}}{\inn \ f_i}\right)f_i\right)= \inn\left(\frac{\inn\ s^{(j)}}{\inn\ f_i} f_i\right) = \inn\ s^{(j)}\leq \inn\ f$$
    It thus follows by induction in the $j$ for which $\lambda_i^{(j)}\neq 0$ that
    \begin{align*}
        \begin{cases} 
            \inn\  \lambda_i^{(j+1)}f_i = \inn\ \lambda_i^{(j)}f_i\leq \inn\ f, \\
            \text{or}\\
            \inn\ \lambda_i^{(j+1)}f_i = \inn\left(\left(\lambda_i^{(j)}+\frac{\inn\ s^{(j)}}{\inn \ f_i}\right)f_i\right)\leq 
            \max\left(\lambda_i^{(j)}f_i,\inn\ s^{(j)}\right)\leq \inn\ f.
        \end{cases}
    \end{align*}
    It thus follows that if $\lambda_i\neq 0$,
    $$\inn\ \lambda_if_i \leq \inn\ f.$$
    Note lastly that each $r^{(j)}$ is $0$ or a sum of terms not divisible by any $\inn\ f_i$, since only case where $r^{(j+1)}$ is different from $r^{(j)}$ is in case (\ref{residue}), where the no initial of an $f_i$ divides $\inn\ s^{(j)}$, thus 
    $$f^{F} = \sum_1^k \inn \ s^{(j_l)}$$
    where no initial term of an $f_i$ divides $\inn\ s^{(j_l)}$.
\end{proof}
A special case of the above is the case of $R[x]$ where our only choice of term ordering is the usual ordering on $\N$. 
\begin{corollary}
    Let $R$ be an integral domain. Let $f,g\in R[x_1,\dots,x_n]$ where the leading coefficient of $g$ is invertible. Then there are $q,r\in R[x_1,\dots,x_n]$ with $r=0$ or $\deg \ r < \deg \ g$ such that 
    $$f=qg+r.$$
    In particular, $R[x]$ is a Euclidean domain.
\end{corollary}
\begin{proof}
    For the existence, note that by the above theorem, there are $q,r\in R[x]$ with 
    $$f=qg+r$$
    and $r=0$ or $\inn\ g\nmid r$. The second condition is equivalent to $r=0$ or $\deg \ r < \deg \ g$, since $x^k\mid x^l$ if and only if $k\leq l$.\\
    To prove uniqueness, note that if $qg+r=q'g+r'$, then $(q-q')g=r-r'$, then $r-r'=0$ since otherwise 
    $$
        \deg\ g \leq \deg\ r - r' \leq \max(\deg\ r,\deg r') 
    $$
    which would imply $\deg \ g \leq \deg \ r$ or $\deg \ g \leq \deg \ r'$. Since $R$ is an integral domain, $(q-q')g=0$ implies $q=q'$. 
\end{proof}
\begin{remark}
    When there is no restrictions on the ring $R$, we get that there $q,r\in R[\mathbf{x}]$ such that 
    $$
        f = qg+r
    $$
    with $r=0$ or no term of $r$ is divisible by the leading term of $g$. In general, this is insufficient to get that $R[x]$ is a Euclidean domain, which indeed is not the case in general.  
\end{remark}
\begin{corollary}\label{PolynomialRingOverFieldIsEuclideanDomain}
    When $K$ is a field $K[x]$ is a Euclidean domain.
\end{corollary}
\begin{corollary}\label{PolynomialRingOverFieldIsPID}
    When $K$ is a field $K[x]$ is a PID.
\end{corollary}
\begin{proof}
    Since a Euclidean domain is a PID.
\end{proof}
\begin{corollary}\label{PolyonmialRingOverFieldIsUFD}
    When $K$ is a field $K[x]$ is a UFD.
\end{corollary}
\begin{proof}
    Since a PID is a UFD. 
\end{proof}
\subsubsection{Gröbner Bases and Buchbergers Algorithm}
For the exposition on Gröbner bases we fix a field $K$.
...
\end{proof}
 We now seek a way to compute a Gröbner basis for a given ideal. The introduction of $S$-polynomials and Buchberger's $S$-criterion will lead us to Buchberger's algorithm for computing Gröbner bases.
\begin{definition}
...
\end{definition}
Note that this definition does not depend on a term order, however this definition leads us to a reformulation of Proposition~\ref{GBCriterionForCheckingMembership}. Before formulating this consider the following lemmas 
...
\begin{proof}
    ...
    hence 
    $$\kappa = v \iff \inn\ \lambda_if_i\leq\inn\ f.$$
    2. Suppose $v = \kappa$. Then $v = v_i+w_i$and hence  $b_ic_i = a_v$ for some $i\in\{1,\dots,m\}$. WLOG we may then write 
    $$\inn\ f = a_v\mathbf{x}^v = \left[\sum_1^l b_ic_i\right]\mathbf{x}^{v_1}\mathbf{x}^{w_1} = \left(\left[\sum_1^l b_i\frac{c_i}{c_1}\right]\mathbf{x}^{v_1}\right)c_1\mathbf{x}^{w_1} = \left(\left[\sum_1^l b_i\frac{c_i}{c_1}\right]\mathbf{x}^{v_1}\right)\inn\ f_1, $$
    implying $\inn\ f_1 \mid \inn f$ for some $l\leq m.$
\end{proof}
...
\end{proof}
These polynomials will be the backbone of the criterion for checking that a generating set is a Gröbner basis that Lemma~\ref{CriterionForCheckingSetBeingGB} 1. provides more practical. To be precise, we can reduce this criterion to just check that a finite set of S-polynomials reduce to zero modulo $F$.
...
\begin{lemma}\label{PrimeInRingImpliesPrimePolynomialRing}
    Let $R$ be an integral domain. If $p\in R$ is prime then $p\in R[x]$ is prime. Therefor, by induction, if $p\in R$ is prime, then $p\in R[x_1,\dots,x_n]$ is prime.  
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
    1. Let $a,b\in R[x]$ such that $f\mid ab$ in $R[x]$. Using Corollary~\ref{PolyonmialRingOverFieldIsUFD} we get that $f$ is prime by Proposition~\ref{EquivalentFormulationOfUFD} in $K[x]$, hence $f \mid a$ or $f \mid b$ in $K[x]$, hence by Lemma~\ref{PrimitivePolynomialOverFractionFieldDivisorImpliesDivisorInBaseRing} $f\mid a$ or $f\mid b$ in $R[x]$.\\
...
\end{proof}
\begin{proposition}\label{MonicPolynomialOverUFDIrreducibleCriterion}
    Let $f\in R[x]$ such that $f$ is monic and $\deg\ f \in\{2,3\}$. Then $f$ is irreducible if and only if $f$ has no roots. 
\end{proposition}
\begin{proof}
...
\subsubsection{Dehomogenization \& Homogenization of Polynomials}
\begin{definition}
...
\end{definition}
\begin{remark}
    We write $f_\ast$ when it is implicitly understood with respect to what variable dehomogenization is applied. 
\end{remark}
\begin{definition}
...
\begin{remark}\label{AnAlternativeDefinitionAndPreimageOfHomogenizationOfZeroIsZero}
    An alternative definition is that $f^\ast = x_{n+1}^df\left(\frac{x_1}{x_{n+1}},\dots,\frac{x_n}{x_{n+1}}\right)$ (in e.g. $Q(R[x_{n+1}])[x_1,\dots,x_{n+1}]$). Indeed,
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    For the second statement, see that
    \begin{align*}
    ...
    4. Write $F=\sum_{v\in\N^{n+1}} a_v\mathbf{x}^v$. Note that if $x_{n+1}^j\mid F$, then $F= x_{n+1}^j Q$ for some $Q\in R[x_1,\dots,x_{n+1}]$. By Lemma~\ref{ProductOfHomogeneousElements} $Q$ is homogeneous of degree $d-j$ where $d := \deg \ F$. Then $x_{n+1}^j$ divides every term $F$. Set 
...
\begin{corollary}\label{LinearFactoringOfForms}
    Suppose $R$ is an integral domain and consider $F\in R[x_1,\dots,x_{n+1}]$ a homogeneous polynomial. Then a factorization of $F$ determines a factorization of $F_\ast$ up to a factor $x_{n+1}^r$. If $R=K$ an algebraically closed field and $F\in K[x,y]$ then $F$ factors into a product of linear factors.  
\end{corollary}
\begin{proof}
...
\end{definition}
\begin{remark}
    Note that an ideal is homogeneous if and only if it is (not necessarily finitely) generated by a set of homogeneous polynomials. We therefor note that the homogenization of an ideal is a homogeneous ideal.
\end{remark}
\begin{lemma}
...
\subsubsection{Multi- and Bihomogeneous Polynomials}
    \begin{definition}
        Let $\{x_{ij} : 1\leq i\leq m, 1\leq j \leq n_i\}$ be algebraically independent variables over a ring $R$. A polynomial in $R[\mathbf{x}]$ is called an \emph{$m$-homogeneous polynomial} or an \textit{$m$-form} of \textit{$m$-degree} $(d_1,\dots,d_m)\in \N^m$, if it is a form of degree $d_i$ when seen as an element in $R[x_{kj} : k\neq i][x_{i1},\dots,x_{in_i}]$ for each $i\in\{1,\dots,m\}$. When $m=2$, a $2$-form is called a \textit{bihomogeneous polynomial} or a \textit{biform} of \textit{bidegree} $(d_1,d_2)$.
    \end{definition}
    \begin{lemma}
        With the same notation as above a polynomial $f \in K[\mathbf{x}]\setminus 0$ of degree $d$ has a unique decomposition
        $$f = \sum_{(i_1,\dots,i_m): \sum_1^m i_j \leq d} f_{i_1,\dots,i_m},$$
        where $f_{i_1,\dots,i_m}$ is an $m$-form of $m$-degree $(i_1,\dots,i_m)$ such that for some $(i_1,\dots,i_m)$ with $\sum_1^m i_j = d$, 
        $$f_{i_1,\dots,i_m}\neq 0.$$ 
    \end{lemma}
    \begin{proof}
        Write 
        $$f = \sum_{\substack{(i_1,\dots,i_m):\sum_1^m i_j\leq d}} 
        \underbrace{\sum_{\substack{\mathbf{v}\in\prod_{k=1}^m \N^{n_k} :\\ \forall k, \vert v_k\vert = i_k}} a_\mathbf{v} \mathbf{x_1}^{v_1}\cdots \mathbf{x_m}^{v_m}}_{f_{i_1,\dots,i_m}},$$
    ...
    \end{proof}
    \begin{definition}
        For an $f\in K[\mathbf{x_1},\dots,\mathbf{x_m}]$, where $K$ is a field, we say that $([v_1],\dots,[v_m])\in \Pp^{n_1}\times \Pp^{n_m}$ is \textit{a zero of $f$} if for every $(\lambda_1,\dots,\lambda_m)\in K^m$ with $\lambda_i\neq 0$, 
        $$
            f(\lambda_1v_1,\dots,\lambda_mv_m) = 0.
        $$
        When $([v_1],\dots,[v_m])$ is a zero of $f$. We write 
        $$
            f([v_1],\dots,[v_m]) = 0,
        $$
        otherwise we write 
    ...
    \end{definition}
    \begin{remark}
        If $f$ were an $m$-form of $m$-degree $(d_1,\dots,d_m)$ note that $f(\lambda_1v_1,\dots,\lambda_mv_m)=\left(\prod_1^m \lambda_i^{d_i}\right)f(v_1,\dots,v_m)$. Hence if $(v_1,\dots,v_m)$ is a zero of $f$, then so is $([v_1],\dots,[v_m])\in \Pp^{n_1}\times\cdots \times \Pp^{n_m}$.
    \end{remark}
    \begin{definition}
    ...
    \begin{remark}\label{ImporatantRemarkForMultiprojectiveSpace}
        The above is equivalent to $I$ being a homogeneous ideal in \\$R[\mathbf{x_1},\dots, \widehat{\mathbf{x_k}},\dots,\mathbf{x_m}]$ for each $k$. Here $\widehat{\_}$ denotes removale of these variables. Any result proven about homogeneous ideals therefor naturally generalizes to $m$-homogeneous ideals. 
    \end{remark}
...
\end{definition}
\begin{remark}
    One easily checks that $D_x fg = (D_xf)g+f(D_xg)$, i.e. that the Leibniz rule is satisfied. It also satisfies the chain rule, i.e. 
    $$D_x(f(g))=(D_xg)\cdot(D_xf)(g).$$
    By definition over an integral domain of characteristic $0$, $\deg \ f' = \deg \ f -1$ when $\deg \ f\geq 1$. In positive characteristic this may not be the case. For instance, when $\Char\ R = p >0$, we get that $D_x x^p= px^{p-1}=0$. We can also come up with pathological examples over commutative rings of characteristic $0$. Indeed take $R=\Z/2\Z \times \Z$ with the usual structure of product ring. Then $D_x (1,0)x^2= (2,2)(1,0)x= (0,2)(1,0)x=0.$ 
\end{remark}
\begin{definition}
    In a polynomial ring $R[x_1,\dots,x_n]$, \emph{the partial derivative of $f$ with respect to $x_i$} is the polynomial $D_{x_i}f$, where 
    $$D_{x_i}: R[x_1,\dots, x_{i-1},\widehat{x_i},x_{i+1},x_{n}][x_i]\rightarrow R[x_1,\dots, x_{i-1},\widehat{x_i},x_{i+1},x_{n}][x_i],$$
...
\end{proof}
\begin{lemma}\label{ConditionForVanishingDerivativeForPolynomialOverPerfectField}
    Let $K$ be a characteristic $p>0$, perfect field and $f\in K[x]$. Then $f_x = 0$ if and only $f = g^p$ for some $g\in K[x]$.  
\end{lemma}
\begin{proof}
    If $f = g^p$, then 
    $$
        f_x = Dg^p = p g_x g^{p-1}=0.
    $$
    Suppose $f_x = 0$. Then by the prior lemma, $f \in K[x^p]$, hence 
    $$
        f = g(x^p) = \sum_0^d a_i x^p= \sum_0^d b_i^px^p = \left(\sum_0^d b_ix\right)^p 
    $$
    for some $g\in K[x]$ and for suitable $b_1,\dots,b_d\in K$. 
\end{proof}

\begin{lemma}
    Let $f,g\in R[x]$. If $f^{n+1}\mid g$, then $f\mid D^{(n)} g$ for each $n\geq 0$. 
\end{lemma}
\begin{proof}
    Indeed, in the case $n=0$ 
    $$
        g_x = q_xf^2+2qff_x= (q_xf+2qf_x)f \implies f \mid g_x.
    $$
    For $n>0$,
    \begin{align*}
        D^{(n+1)}g &= D^{(n+1)} qf^{n+2} = D^{(n)}\left(Dqf^{n+2}\right) = D^{(n)}\left(q_xf^{n+2}+(n+2)qf_xf^{n+1}\right)\\
        &= D^{(n)}\left(\underbrace{f^{n+1}(q_xf+(n+2)qf_x)}_{h}\right).
    \end{align*}
    By induction $f^{n+1}\mid h$, hence $f\mid D^{(n)} h = D^{(n+1)} g$. 
\end{proof}
The following is an important criterion for when a root is multiple.
\begin{lemma}\label{MultipleRootsInTermsOfDerivative}
    Let $f\in R[x]$ and $a\in R$ a root of $f$. Then $a$ is a multiple root if and only if $a$ is a root of $f_x$. 
\end{lemma}
\begin{proof}
    Suppose $a$ is a multiple root of $f$. Then by the prior lemma, $(x-a)^2\mid f$ implies $(x-a) \mid f_x$.\\
    Conversely, if $a$ is a simple root, then 
    $$
        f_x = D (x-a)g = g + (x-a)g_x,
    $$  
    and since $(x-a)\nmid g$, it follows that $(x-a)\nmid f_x$. 
\end{proof}
\subsection{Ring Extensions and Algebras over Rings} 
\subsubsection{Finiteness Conditions on Ring Maps}
\subsubsection{Finitely Generated Ring Extensions}
\begin{definition}
    An $R$-algebra $S$ is \emph{module finite (over $R$)} 
    if $S$ is finitely generated as an $R$-module.\\
    A ring map $\sigma : R \rightarrow S$ is said to be \emph{finite} if $S$ with the associated $R$-algebra structure is module finite. 
\end{definition}
\begin{definition}
    An $R$-algebra $S$ is a said to be \textit{finitely generated $R$-algebra} or of \emph{finite type} if $S=R[s_1,\dots,s_n]$ for suitable $s_1,\dots,s_n\in S$.\\ 
    If $S\supset R$ is a ring extension of finite type, we say that it is \textit{ring-finite}.\\
    A ring map $\sigma : R \rightarrow S$ is of finite type if $S$ with the associated structure of $R$-algebra is of finite type.  
\end{definition}
\begin{proposition}\label{FinitelyGeneratedAlgebrasArePolynomialQuotientRings}
    Let $S$ be an $R$-algebra of finite type. Then
    $$S\simeq R[x_1,\dots,x_n]/I,$$
    for some $n\geq 1$ and some ideal $I\subset R[x_1,\dots,x_n]$.
\end{proposition}
\begin{proof}
    For suitable $s_1,\dots,s_n\in S$, $S=R[s_1,\dots,s_n]$, hence 
    $$
        \ev_{s_1,\dots,s_n} : R[x_1,\dots,x_n]\rightarrow S
    $$ 
    is a surjective $R$-algebra homomorphism. Then by the first isomorphism theorem 
    $$
        S\simeq K[\mathbf{x}]/\ker \ \ev_{s_1,\dots,s_n}.
    $$ 
\end{proof}
\begin{proposition}\label{ModuleFiniteImpliesRingFinite}
    Let $S$ be a module finite $R$-algebra. Then $S$ is of finite type.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{example}
    The converse implication of the above proposition is clearly not true in general. Consider for instance $R[x_1,\dots,x_n]\supset R$. given $f_1,\dots,f_m\in R[\mathbf{x}]$. If these are all $0$, clearly $R[\mathbf{x}]\supsetneq \sum_1^m Rf_i$. Otherwise putting $D = \max_{i\in\{1,\dots,m\}}\ \{\deg\ f_i \}$, we see that for $r_1,\dots, r_m\in R$, 
...
\end{definition}
\begin{remark}
    Note that a $K$-algebra $L$ where $L$ is a field is automatically a field extension since the map $K\rightarrow L$ inducing the structure of $K$-algebra is automatically an embedding. 
\end{remark}
...
\begin{definition}
    A field extension $L\supset K$ is said to be \textit{a finitely generated field extension (over $K$)}, if there exist $a_1,\dots,a_n$ such that $L = K(a_1,\dots,a_n)$.
\end{definition}
\begin{definition}
    A field extension is \emph{finite} if it is module finite. 
\end{definition}
\begin{lemma}
...
\end{lemma}
\begin{proof}
    Clearly $K(\mathbf{x})\subset K(\mathbf{x},\mathbf{y})$.  To be very precise, this means $K(\mathbf{x},\mathbf{y})\supset K(\mathbf{x})$ is a ring extension. Hence $K(\mathbf{x},\mathbf{y})\supset K(\mathbf{x})[\mathbf{y}]$. This means
...
\begin{proof}
    ...
    $$\prod_1^m b_i^{v_i}\mid \prod_1^m b^{v_i}= b^{\vert v\vert } \ \mathrm{and}\ b^{\vert v\vert} \mid b^d \implies \prod_1^m b_i^{v_i}\mid b^d \implies b^d\prod_1^m \left(\frac{a_i}{b_i}\right)^{v_i} \in K[\mathbf{x}],$$
    which implies $ b^dz\in K[\mathbf{x}].$
\end{proof}
...
\end{proposition}
\begin{proof}
    The first statement is obvious as $K(x_1,\dots,x_n)$ is finitely generated as field extension over $K$ by $x_1,\dots,x_n\in K(x_1,\dots,x_n)$. To prove the second statement, let $\frac{a_1}{b_1},\dots,\frac{a_m}{b_m}\in K(\mathbf{x})$. Set $b:=\lcm(b_1,\dots,b_m)$. Then there is a $c\in K[\mathbf{x}]$ such that $c\nmid b^d$ for any $d\geq 0$, since there are infinitely many irreducible polynomials over $K$ (cf. Lemma~\ref{InfinitelyManyIrreduciblePolynomialsOverIntegralDomain}) and $K[\mathbf{x}]$ is a UFD, $b^d \frac{1}{c}\notin K[x]$ for any $d\geq 0$. By lemma~\ref{CriterionForCheckingMembershipInAlgebra}, it follows that $\frac{1}{c}\notin K\left[\frac{a_1}{b_1},\dots,\frac{a_m}{b_m}\right]$, hence $K(\mathbf{x})\supsetneq K\left[\frac{a_1}{b_1},\dots,\frac{a_m}{b_m}\right]$. This means $K(\mathbf{x})$ is not a finitely generated $K$-algebra. 
\end{proof}
\begin{lemma}\label{TransitivityOfFinitenessConditions}
    These finiteness conditions are transitive. In particular, if $T\supset S \supset R$ is a tower of ring extensions and $M\supset L\supset K$ is a tower of field extensions, then
    \begin{enumerate}
        \item If $T\supset S$ and $S\supset R$ are module finite, then so is $T\supset R$.  
        \item If $T\supset S$ and $S\supset R$ are ring-finite, then $T\supset R$ is ring-finite.
        \item If $M\supset L$ and $L\supset K$ are field finite, then $M\supset K$ is a field finite extension. 
    \end{enumerate}
\end{lemma}
\begin{proof}
    1. Suppose $T=\sum_1^n St_i$ and $S=\sum_1^m Rs_i$ for some $t_1,\dots, t_n\in T$ and $s_1,\dots,s_m\in S$. Let $t\in T$. Then 
    $$
        t = \sum_1^n a_i t_i \in \sum_1^n S t_i.
    $$
    Moreover, 
    $$
        a_i = \sum_j^m b_{ij}s_j,
    $$
    hence 
    $$
        t = \sum_{i,j} b_{ij}s_jt_i.
    $$
    We thus conclude that 
    $$
        T = \sum_{i,j} R s_jt_i.
    $$
    2. Suppose $T\supset S$ and $S\supset R$ are ring finite extensions. We can find $s_1,\dots,s_n\in S$ such that $S = R[s_1,\dots,s_n]$ and $t_1,\dots,t_m\in T$ such that $T=S[t_1,\dots,t_m]$. Let $t \in T$.  Then there are $a_v\in S$ such that 
...
\end{proof}
\begin{example}
    The converse of 2. in the above lemma is not true. Consider for instance the ring extension $R[x,y]\supset R$. And the ring extension 
    $$
        S := R[xy^i : i \geq 0].
    $$ 
    Let $f_1,\dots,f_m\in S$ (we may assume that these are non-constant). Let $S$ inherit the structure of graded ring from $R[y][x]$.
    Write 
    $$
        f_i = b_i + \sum_1^{l_i} a_{ij} xy^j
    $$
    Note that a product $f_if_j$ does not have a non-zero term of degree $1$ in $x$. Set $S' := R[f_1,\dots,f_m]$. It thus follows that $S'_1$ is finitely generated as an $R$-module by the $xy^j$ for which there is an $i$ with $a_{ij} \neq 0$. On the other hand, $S_1$ is clearly not finitely generated as an $R$-module. Then $S'\subsetneq S$.  
\end{example}
...
\begin{definition}
    ...
    Let $L\supset K$ be a field extension. An element of $L$ is \textit{algebraic (over $K$)} if it is integral over $K$. An element that is not algebraic over $K$ is \emph{transcendental} over $K$.\\ 
\end{definition}
\begin{remark}
    Equivalently $s$ is integral over $R$ if $\ker \ \ev_s \neq 0 $. 
\end{remark}
\begin{definition}
    A ring extension $S\supset R$ is \emph{integral (over $R$)}, if each element in $S$ is integral over $R$. A field extension $L\supset K$ is \emph{algebraic (over $K$)}, if each element in $L$ is algebraic over $K$.  
\end{definition}
\begin{remark}\label{MinimalPolynomialDefinitionAndTechnicalRemarks}
    In the case $L\supset K$ is a field extension and $a\in L$ is a algebraic over $K$ consider 
    $$
        d = \min (\{ k>0 : \exists f\in R[x]\setminus 0, \deg \ f = 0, f(s) =0\}).
    $$ 
    Let $f_s$ be a polynomial of degree $d$ vanishing on $s$. We may assume that this is monic. Consider another polynomial $g\in K[x]\setminus 0$ vanishing on $s$. We can write $g = qf_s+r$, where $r = 0$ or $\deg \ r < d$. Then 
    $$r(s) =g(s)- q(s)f_s(s)=0.$$
    By minimality $r = 0$, hence $f_s\mid g$. This $f_s$ is the unique irreducible monic polynomial vanishing on $s$ of minimal degree. We call this polynomial the \textit{defining polynomial of $s$ over $K$}, denoted $\Irr(a,K)$. Note that $\ker \ \ev_s = \langle \Irr(a,K)\rangle$, hence $K[s]\simeq R[x]/\langle f_s\rangle$.
    We refer to $\deg \ f_s$ as \textit{the degree of $s$ over $R$}. We can extend this result to the case where $R$ is a UFD. By the same argument as before $f_s\mid g$ in $Q(R)[x]$, hence since $f$ is primitive, $f_s\mid g$ in $R[x]$ by Lemma~\ref{PrimitivePolynomialOverFractionFieldDivisorImpliesDivisorInBaseRing}.\\
    Consider an $a\in L$  algebraic over $K$. Then $a\in K$, if and only if its defining polynomial is of degree $1$. 
\end{remark}
\begin{remark}
...
\end{example}
\begin{example}
    $\sqrt{2}\in \R$ is an algebraic number (i.e. it is algebraic over $\Q$) with defining polynomial $x^2-2$. Likewise, $i\in \C$ is algebraic over $\Q$ with defining polynomial $x^2+1$. 
\end{example}
\begin{example}
    $\C\supset \R$ is an algebraic extension. Indeed, each $\omega = a + bi \in \C$ is a root of 
    $$
        (x-\omega)(x-\overline{\omega}) = x^2 -(\omega+\overline{\omega})x +\omega\overline{\omega} = x^2-2ax+a^2+b^2\in \R[x].
    $$  
    When $\omega \in \C\setminus \R$, this is the defining polynomial of $\omega$. 
\end{example}
\begin{example}
    Consider the polynomial ring $R[x]\supset R$. Then $x$ is not integral over $R$, since $\ker \ \ev_x = 0$.\\
    For a field $K$, we thus have that $x\in K(x)$ is transcendental over $K$. 
\end{example}
...
\end{proof}
\begin{lemma}\label{TransitivityOfIntegralExtensions}
    Consider a tower of ring extensions $T\supset S\supset R$ where $T$ is a domain. $T$ is integral over $S$ and $S$ is integral over $R$ if and only if $T$ is integral over $R$. 
\end{lemma}
\begin{proof}
    Suppose $T\supset S$ and $S\supset R$ are integral. Let $t\in T$, then there is a monic $f=x^n+\sum_0^{n-1} a_ix^i\in S[x]\setminus 0$ such that $f(t)=0$. By the above lemma $R' := R[a_1,\dots,a_n,t]=R[a_1,\dots,a_n][t] \supset R[a_1,\dots,a_n]$ is module-finite. By the above lemma we also find that $R[a_1]\supset R$ is module-finite. Recursive usage of the above and the transitivity of module-finiteness thus implies that $R[a_1,\dots,a_{n-1}]\supset R$ is module-finite and hence that $R' = R[a_1,\dots,a_{n-1},t]\supset R$ is module-finite. We have thus found a subring of $T$ containing $R[t]$, which is module-finite over $R$. Hence using the above lemma yet another time it follows that $t$ is integral over $R$, hence $T$ is integral over $R$.\\
    Suppose $T\supset R$ is integral. Let $t\in T$. Then there is a monic $f\in R[x]\setminus 0 \subset S[x]\setminus $ vanishing on $t$, hence $T\supset S$ is integral. Let $s\in S\subset T$. Then there is a monic $f\in R[x]\setminus 0$ vanishing on $s$. We thus conclude $S\supset R$ is integral. 
\end{proof}
...
\end{proof}
\begin{remark}
    This ring is called \emph{the integral closure of $R$ in $S$}. 
\end{remark}
...
\end{proof}
\begin{example}
    With regards to the converse of Lemma~\ref{TransitivityOfFinitenessConditions} it certainly isn't true in general. Indeed, there are examples of the integral closure that is not finite (although I don't know any!). When $R$ is a field it is true.   
\end{example}
...
\begin{lemma}\label{TechnicalLemmaAboutFieldExtOverAlgClosedFields}
    ...
    \begin{enumerate}
        ...
        \item If $L\supset K$ is finite, then $L=K$.
    \end{enumerate}
\end{lemma}
\begin{proof}
    ...
    3. If $L\supset K$ is finite, then it is algebraic by Lemma~\ref{ModuleFiniteImpliesAlgebraic}. Let $a\in L$. Then $a$ is algebraic over $K$, hence $a\in K$ by 2. We thus get that $L=K$.
\end{proof}
...
\begin{proposition}
    ...
    $$\{a\in L: a \ \mathrm{is}\ \mathrm{algebraic}/K\}$$
    is a subfield of $L$.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{remark}
    This subfield is called \emph{the algebraic closure of $K$ in $L$}. 
\end{remark}
\begin{proposition}\label{CannotSqueezeNonFieldBetweenFiniteFieldExtension}
    Let $L\supset K$ be a finite field extension. Consider a subring $R$ of $L$ containing $K$ as a subring. Then $R$ is a field. 
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{theorem}
    Let $L\supset K$ be a ring-finite field extension generated by $a_1,\dots,a_n\in L$. Then $L\supset K$ is finite and hence also algebraic.   
\end{theorem}
\begin{proof}
    We use induction in $n$. For $n=1$, suppose $L=K[a]$ for some $a\in L$. Consider $\rho = \ev_a : K[x]\rightarrow L$. Since $K[x]$ is a PID (cf. Corollary~\ref{PolynomialRingOverFieldIsPID}), we find that $\ker\ \rho = \langle g \rangle$ for some $g\in K[x]$. Then $K[x]/\langle g\rangle \simeq K[a] = L$. We claim that $g\neq 0$\\
...
\begin{corollary}\label{ClassificationOfMaximalPolynomialIdeals}
    Let $K$ be algebraically closed and $I\subset K[x_1,\dots,x_n]$ be a maximal ideal. Then $K[x_1,\dots,x_n]/I=K$ (thinking about $K$ as the canonical embedding of $K$ in $K[\mathbf{x}]/I$). This implies $I=\langle x_1-a_1,\dots,x_n-a_n\rangle$ for some $a_1,\dots,a_n\in K$. 
\end{corollary}
\begin{proof}
    The quotient map $\pi : K[\mathbf{x}]\rightarrow K[\mathbf{x}]/I, f\mapsto f+I$ is a canonically surjective $K$-algebra homomorphism. It follows by the above corollary that $K[\mathbf{x}]/I=K$. Then for each $i$, $x_i+I = a_i+ I$ for some $a_i\in K$, this means $J:=\langle x_1-a_1,\dots,x_n-a_n\rangle\subset I$. $J$ is maximal by Corollary~\ref{MaximalIdealsOfPolynomialRings}, hence $J=I$. 
\end{proof}
\subsection{Field Extensions}
    \begin{definition}
        Let $L\supset K$ be a field extension. Consider $a,b\in L$ algebraic over $K$. $b$ is said to be \emph{conjugate of $a$ (over $K$)}, if $b$ is a root of $\Irr(a,K)$
    \end{definition}
    \begin{example}
        Consider the algebraic extension $\C\supset \R$. The conjugate of $\omega\in \C$ over $\R$ is its complex conjugate. 
    \end{example}
    \begin{example}
        The conjugate of $\sqrt{2}$ over $\Q$ is $-\sqrt{2}$. 
    \end{example} 
    \begin{remark}
        Equivalently two algebraic elements are conjugate, if 
        $$
            \Irr(a,K) = \Irr(b,K).
        $$
    \end{remark}
    \begin{lemma}
        Let $L\supset K$ be a field extension and $a\in L$. Then $L\supset K$ is finite if and only if $\dim_K \ K[a] <\infty$. If $a$ is algebraic, let $d$ denote $\deg \ \mathrm{Irr}(a,K)$. Then $\{1,a,\dots,a^{d-1}\}$ is a basis for $K[a]$. 
    \end{lemma}
    \begin{proof}
        The first statement follows immediately from Proposition~\ref{EquivalentFormulationsOfIntegralElements}. Set $I=\langle \mathrm{Irr}(a,K)\rangle$. By Lemma~\ref{DimensionOfQuotientSpace}, $\{1+ I,x+I,\dots,x^{d-1}+I\}$ is a basis of $K[x]/I \overset{\overline{\ev_a}}{\simeq} K[a]$, since $\ev_a$ is a $K$-algebra homomorphism, it is in particular a $K$-linear map, hence $\{1,a,\dots,a^{d-1}\}$ is a basis of $K[a]$
    \end{proof}
    \begin{definition}
        If $L\supset K$ is a finite field extension, we define define $[L:K]:= \dim_K\ L$ to be \textit{the degree of $L$ over $K$} 
    \end{definition}
   \begin{example}
        $[\C : \R] = 2$
   \end{example}
   \begin{example}
        $[L : K] = 1$ if and only if $ L = K$.
   \end{example}
    \begin{lemma}\label{ModuleFiniteFieldExtensions}
        Let $L\supset K$ be a field extension. Then $L\supset K$ is finite if and only if $L\supset K$ is a finite field extension generated by some $a_1,\dots,a_n\in L$ that are algebraic over $K$. 
    \end{lemma}
    \begin{proof}
        "$\implies$": By Lemma~\ref{ForRingFiniteExtensionsModuleFiniteIsTheSameAsAlgebraic} $L = K[a_1,\dots,a_n]=K(a_1,\dots,a_n)$ for suitable $a_1,\dots,a_n\in L$ that are algebraic over $K$.\\
        "$\impliedby$": Suppose $L=K(a_1,\dots,a_n)$ for some algebraic $a_i\in L$. Then $L= K[a_1,\dots,a_n]$, hence by Lemma~\ref{ForRingFiniteExtensionsModuleFiniteIsTheSameAsAlgebraic} $L\supset K$ is finite. 
    \end{proof}
     \begin{lemma}
        Consider a field extensiond $L\supset K$. If $a\in L$ is algebraic over $K$, then $[K(a) : K] = \deg \ \Irr(a, K)$. 
    \end{lemma}
    \begin{proof}
        Indeed, $\dim \ K(a) = \dim  K[x]/\langle \Irr(a,K)\rangle = \deg \ \Irr(a,K)$. 
    \end{proof}
    \begin{lemma}
        Consider field extensions $M\supset L \supset K$. $M\supset K$ is finite if and only if $M\supset L$ and $L\supset K$ are finite. Moreover, 
        $$
            [M : K] = [M : L] [L : K].
        $$ 
    \end{lemma}
    \begin{proof}
        One direction is trivial by transitivity of module finiteness. Suppose $M\supset K$ is finite. Then $L\supset K$ is a subspace of $M\supset K$, hence it is finite. A generating set for $M$ as $K$-vector space is also a generating set for $M$ as an $L$-vector space.\\
        Set $n:= [M : L]$ and $m := [L : K]$. Let $v_1,\dots,v_n$ be the basis elements of $M\supset L$ and $w_1,\dots,w_m$ the basis elements of $L\supset K$. Then 
        $$
            \{v_iw_j : i\in \{1,\dots,n\} , j\in \{1,\dots,m\}\}
        $$
        is a generating set of $M$ over $K$ (cf. the proof of Lemma~\ref{TransitivityOfFinitenessConditions}). It remains to check that this a linearly independent over $K$. Indeed,
        $$
            \sum_{i,j} a_{ij} v_iw_j = 0
        $$
        implies 
        $$
            \sum_{i} \left( \sum_j a_{ij} w_j\right)v_i = 0,
        $$
        hence 
        $$
            \sum_{j} a_{ij} w_j = 0
        $$
        for each $i$, implying $a_{ij} =0 $ for each $i$ and $j$.
        It follows that 
        $$
            [M : K] = [M : L][L : K].
        $$
        since $M \supset K$ has a basis with $nm$ elements.  
    \end{proof}
    \begin{lemma}
        Let $L\supset K$ be a finite extension and $a\in L$ algebraic over $K$. Then $\deg \ \Irr(a,K) \mid [L:K]$.
    \end{lemma}
    \begin{proof}
        Indeed, by the prior lemma,
        $$
            [L : K] = [L : K(a)][K(a) : K]. 
        $$
    \end{proof}
    \begin{lemma}
        Let $M\supset L \supset K$ be field extensions. $M\supset K$ is algebraic if and only if $M\supset L$ and $L\supset K$ are algebraic.
    \end{lemma}
    \begin{proof}
        This is just a special case of Lemma~\ref{TransitivityOfIntegralExtensions}.
    \end{proof}
    \begin{lemma}\label{AutomorphismTakesConjugatesToConjugates}
        Consider a field extension $L\supset K$ and an automorphism $\sigma\in \Aut_K(L)$. Suppose $a\in L$ is algebraic over $K$. Then $a$ and $\sigma(a)$ are conjugate over $K$.  
    \end{lemma}
    \begin{proof}
        Indeed, 
        $$
            \Irr(a,K)(\sigma(a)) = \sigma(\Irr(a,K)(a)) = \sigma(0) = 0. 
        $$
        Conversely, let $g\in \ker \ \ev_{\sigma(a)}$. Then 
        $$
            g(a) = \sigma^{-1}(\sigma(g(a))) =\sigma^{-1}(g(\sigma(a))) = 0.  
        $$ 
        Then 
        $$
            \Irr(a,K) = \Irr(\sigma(a),K).
        $$
    \end{proof}
    \begin{lemma}\label{FieldAdjoinedByConjugatesAreUniquelyIsomorphic}
        Let $L\supset K$ be a field extension and $a\in L$ algebraic over $L$. Consider $b\in L$ a conjugate of $a$ over $K$. Then there is a unique $K$-algebra isomorphism
        $$
            K(a) \rightarrow K(b)
        $$
        mapping $a$ to $b$.
    \end{lemma}
    \begin{proof}
        Since any element in $K(a)$ can be written on the form 
        $$
            \sum_0^n a_i a^i \in K[a]
        $$
        such an isomorphism is uniquely determined by the image of $a$. The assumption that $a$ maps to $b$ gives uniqueness. For existence, we take 
        $$
            K(\alpha)\simeq K[x]/\Irr(a,K) = K[x]/\Irr(b,K) \simeq K(\beta). 
        $$
    \end{proof}
\subsubsection{Splitting Fields}
    \begin{definition}
        Let $K$ be a field and $S\subset K[x]$ be a subset of non-constant polynomials. A field extension $L\supset K$ is called a \emph{splitting field of $S$ over $K$}, if each $f\in S$ splits into linear factors in $L[x]$ and for each $a\in L$ there are $a_1,\dots,a_n\in L$ such that $a_i$ is a root of some $f_i\in S$ and $a\in K(a_1,\dots,a_n)$  
    \end{definition}
    \begin{remark}
        Consider a non-constant $f\in K[x]$ with leading coefficient $b$. A \emph{splitting field of $f$} is a splitting field of $\{f\}$. Note that $L\supset K$ being a splitting field of $f$ is equivalent to the existence of $a_1,\dots,a_n\in L$ such that 
        $$
            f = b\prod_1^n (x-a_i) \text{ and } L = K(a_1,\dots,a_n).
        $$
        A splitting field of $\left\{f_1,\dots,f_k\right\}\subset K[x]$ is the same as a splitting field of $\prod_1^k f_i$. Note that if $\deg \ f = 1$, then $K$ is a splitting field for $f$. 
    \end{remark}
     \begin{example}
        The splitting field of $x^2+1$ over $\Q$ is $\Q(i)$, since 
        $$
            x^2+1=(x-i)(x+i)
        $$
    \end{example}
    \begin{example}
        Consider the polynomial $x^4 + 1\in \Q[x]$. Note that 
        \begin{align*}
            x^4 +1 &= (x^2+i)(x^2-i) \\
            &= \left(x+\frac{\sqrt{2}}{2}\left( 1-i\right)\right)\left(x-\frac{\sqrt{2}}{2}\left( 1-i\right)\right)\left(x+\frac{\sqrt{2}}{2}\left( 1+i\right)\right)\left(x-\frac{\sqrt{2}}{2}\left( 1+i\right)\right).
        \end{align*}
        The splitting field of $x^4+1$ is thus 
        $$
            \Q(\sqrt{2},i).
        $$
    \end{example}
    
    \begin{theorem}\label{ExistenceTheoremForSplittingField}(Existence Theorem for Splitting Fields of single Polynomials) 
        Let $K$ be a field and $f\in K[x]$. There is a splitting field $L$ of $f$. Moreover, $L\supset K$ is finite and $[L : K] \leq \ deg \ f !$.
    \end{theorem}
    \begin{proof}
        When $\deg \ f = 1$ the statement is trivial by taking $K=L$. If $f$ is of of degree $d+1$ for some $d\geq 1$, pick a monic irreducible factor of $f$, $g$ say. Then over $L' = K[y]/\langle g(y)\rangle$, $g =(x - (y+I))g_1 $ for some $g_1\in L'[x]$. Then $f=qg=qg_1(x-(y+I))$ for some $q\in L'[x]$. Note that $L' = K(y+I)$. By induction, there is a splitting field $L=L'(a_1,\dots,a_n)$ of $qg_1\in L'[x]$ where $a_1,\dots,a_n\in L''$ are all algebraic over $L'$ and   
        $$qg_1 = b\prod_1^n (x-a_i)$$
        for a suitable $b\in L'$. It thus follows that $L$ is a splitting field of $f$, since $L = L'(a_1,\dots,a_n,y+I)$ and  
        $$f = b (x-(y+I))\prod_1^n (x-a_i).$$ 
        Consider a splitting field $L=K(a_1,\dots,a_n)\supset K$ of $f$. Note that $\mathrm{Irr}(a_1,K) \mid f$ in $L[x]$, hence $[K(a_1) : K] \leq \deg \ f$. Note also that $L$ is a splitting field of 
        $$
            a\prod_2^n (x-a_i),
        $$
        over $K(a_1)$, hence by induction $[L:K(a_1)] \leq (\deg \ f -1)!$. Then 
        $$
            [L : K] = [L: K(a_1)][K(a_1) : K] \leq \deg \ f !.
        $$ 
    \end{proof}
  
    \begin{lemma}
        Let $K$ be a characteristic $0$ field and $f\in K[x]$ irreducible monic. Let $L$ be the splitting field of $f$ over $K$ and write $f=\prod_1^d (x-\alpha_i)$ for suitable $\alpha_i\in L$. Then the $\alpha_i$ are distinct.
    \end{lemma}
    \begin{proof}
        Suppose $L$ is a field extension over $K$, and suppose there is an $\alpha \in L$ such that $(x-\alpha^2)\mid f$. Then $g:=D f$ also has $\alpha$ as a root. Then $g\nmid f$, hence $f$ cannot be irreducible. In particular if $L$ was the splitting field, and $f$ has a multiple linear factor, then $f$ is not irreducible.  
    \end{proof}
    \begin{lemma}\label{UniquenessOfSplittingFieldsUpToIsomorphism} Let $K,K'$ be a pair of fields and $f\in K[x]$ a non-constant polynomial. If $\sigma : K\simeq K'$ and $L,L'$ are splitting fields of $f$ resp. $\overline{\sigma}(f)$, then there is an extension $\omega : L \simeq L'$ of $\sigma$.  
    \end{lemma}
    \begin{proof}
        We prove the result by induction in $[L : K]$. When $[L : K] = 1$, we have $K=L$, hence $f = a \prod_1^m (x-a_i)$ for $a_i\in K$. Then $\overline{\sigma}(f) = \sigma(a)\prod_1^m (x-\sigma(a_i))$, which implies that $K'$ is a splitting field for $\overline{\sigma}(f)$.\\
        Suppose $[L : K] > 1$. Then $\deg \ f >1$. Let $p$ be a  irreducible, monic factor of $f$ with $\deg \ p > 1$. Then $\overline{\sigma}(p)$ is a also an irreducible, monic factor of $\overline{\sigma}(f)$ and we get an isomorphism
        \begin{gather*}
            \sigma_1 : K[X]/\langle p \rangle \simeq K'[x]/\langle \overline{\sigma}(p)\rangle\\
            g + \langle p\rangle \mapsto \overline{\sigma}(g)+\langle \overline{\sigma}(p)\rangle
        \end{gather*} 
        Note that $p = \mathrm{Irr}(\alpha,K)$ and $\overline{\sigma}(p) = \mathrm{Irr}(\beta,K')$ for some $\alpha\in L$ and some $\beta\in L'$. Then we have an isomorphism
        $$
            \sigma' :  K(\alpha) \simeq K[X]/\langle p \rangle \simeq K[X]/\langle \overline{\sigma}(p) \rangle\simeq  K(\beta). 
        $$
        extending $\sigma$. We have 
        $$
            [L:K(\alpha)] = [L : K]/[K(\alpha) : K] < [L : K].
        $$
        Note that $L$ and $L'$ are splitting fields of $f$ resp. $\overline{\sigma}(f)$ over $K(\alpha)$ resp. $K(\beta)$. Then by induction there is an extension $\omega : L \rightarrow L'$ of $\sigma'$ and we are done.  
    \end{proof}

    \begin{corollary}
        If $L$ and $L'$ are splitting fields of a polynomial over $K$ resp. over $K'$, then 
        $$
            [L : K] = [L' : K'].
        $$
    \end{corollary}
\subsubsection{The Algebraic Closure of a Field}
    A natural question is, whether we can always find a field extension of field $L$ of  $K$ over which every non-constant polynomial in $K[x]$ splits into linear factors in $L[x]$. Equivalently, we seek for an extension over which all monic irreducible polynomials split. We can generalize Theorem~\ref{ExistenceTheoremForSplittingField}, if we assume the axiom of choice, {\Large (A weaker axiom can be assumed but idk)} to work for any subset $S\subset K[x]$ of non-constant polynomials, but the resulting extension need not be finite.
    \begin{theorem}
            (Existence Theorem for Splitting Field of Arbitrary Subset) Let $K$ be a field and $S\subset K[x]$ be any subset of non-constant polynomials. Assuming the axiom of choice, there is a splitting field of $L\supset K$ of $S$.
    \end{theorem}  
    \begin{proof}
        Consider the polynomial ring 
        $$
            R := K[y_{s,i} : s\in S, i\in \{1,\dots, \deg \ s\}].
        $$
        For each $s\in S$, let $b_s$ denote the leading coefficient, and set 
        $$
            f_s := s - \prod_1^{\deg \ s} (x-y_{s,i})\in R[x].
        $$
        Note that for each $s$ that
        $$
            f_s = \sum_0^{\deg \ s -1} g_{s,j}x^j
        $$
        for suitable non-constant $g_{s,j}\in R$. Then 
        $$
            I := \langle g_{s,j} : s\in S, j\in\{0,\dots, \deg\ s -1\}\rangle 
        $$
        is a proper ideal in $R$, hence it sits inside a maximal ideal $J$. Set 
        $$
            L := R/J \supset K.
        $$
        Then each $f\in S$ splits into linear factors over $L$ and its roots are exactly 
        $$
            y_{s,1} + J,\dots, y_{s,\deg\ s }+J.       
        $$
        Any element in $L$ is clearly a polynomial in some 
        $$
            y_{s_{1},i_{1}}+J,\dots, y_{s_{k},i_k} + J,
        $$
        hence $L$ is a splitting field of $S$ over $K$. 
    \end{proof}
    \begin{definition}
        Let $K$ be a field. We define the \emph{the algebraic closure of $K$} to be a splitting field of the set 
        $$
            \{f \in K[x] : f \text{ monic, irreducible}\}.
        $$
        We denote this field by $\overline{K}$. 
    \end{definition}
    \begin{remark}
        We have not justified the definite article for algebraic closures yet. We will use the existence of an algebraic closure to prove a more general result in the next subsubsection. 
    \end{remark}
    \begin{theorem}
        The algebraic closure of a field $K$ is algebraically closed. 
    \end{theorem}
    \begin{proof}
        Let $f\in \overline{K}[x]$ be non-constant. Let $L$ be the splitting field of $f$ over $\overline{K}$. Let $a\in L$. Since $L\supset K$ is algebraic, there is a non-zero $g\in K[x]$ such that $g(a)=0$. Since $g$ splits over $\overline{K}$ it follows that $a\in \overline{K}$. Then $f$ splits over $\overline{K}$.  
    \end{proof}
\subsubsection{The Isomorphism Extension Theorem}
    \begin{theorem}(The Isomorphism Extension Theorem)
        Assuming the axiom of choice, given an algebraic extension $L\supset K$ and an isomorphism of fields $\sigma : K \rightarrow K'$, there is an algebraic extension $\overline{K'}\supset L'\supset K'$ such that $\sigma$ extends to an isomorphism 
        $$
            \tau : L \rightarrow L'.
        $$
    \end{theorem}
    \begin{proof}
        Consider the family of field maps
        $$
            \pazocal{M} := \left\{ \omega : M \rightarrow M' : L\supset M \supset K, \overline{K'}\supset M'\supset K', \left.\omega\right|_K = \sigma \right\}.
        $$
        We partially order this set in the following way: Given $\omega_1 : M_1\rightarrow M_1'$ and $\omega_2 : M_2\rightarrow M_2'$ in $\pazocal{M}$, we write $\omega_1 \leq \omega_2$ if $M_1\subset M_2$, $M_1'\subset M_2'$ and $\left.\omega_2\right|_{M_1} = \omega_1$. Consider a chain $C$ in $\pazocal{M}$. This is clearly bounded above by the field map induced by taking direct limits of the domains and codomains. I.e. 
        $$
            \omega : \bigcup_{c\in C} M_c \rightarrow \bigcup_{c\in C} M_c', x_c \mapsto \omega_c(x_c).
        $$
        By Zorn's Lemma $\pazocal{M}$ contains a maximal element $\tau : \widetilde{L} \rightarrow L'$. It remains to check that $\widetilde{L} = L$. Suppose this is not the case. Pick any $a\in L\setminus \widetilde{L}$. Then the map 
        $$
            \rho : L'[x] \rightarrow \widetilde{L}[x] \rightarrow L(a)
        $$ 
        induces an isomorphism 
        $$
            L'[x]/\ker \ \rho \simeq L(a)
        $$
        which restricts to $\sigma$ under $K$, but this would contradict the maximality of $\tau$. 
    \end{proof}
    \begin{corollary}\label{UniquenessOfSplittingFieldOverArbitrarySubset}
        Consider a field extensions $L\supset K$ and $L'\supset K'$ and $\sigma : K\rightarrow K'$ an isomorphism. Let $S\subset K[x]$ be a subset of non-constant polynomials.Suppose $L$ is a splitting field of $S$ over $K$ and $L'$ is a splitting field of $\overline{\sigma}(S)$ over $K'$. Then $\sigma$ extends to an isomorphism $L\simeq L'$. 
    \end{corollary}
    \begin{proof}
        There is an extension of $\sigma$ to an isomorphism $\tau:L\rightarrow L''$, where $\overline{K'} \supset L''\supset K'$. It suffices to prove that $L''=L'$.\\ Let $a\in L'$. Then there $a_1,\dots,a_n\in L'$ with $a_i$ being a root of some $\overline{\sigma}(s_i)\in \overline{\sigma}(S)$ and $a\in K'(a_1,\dots,a_n)$. It thus suffices to prove that $a\in L''$ in the case where $a$ itself is a root some $\overline{\sigma}(s)\in \overline{\sigma}(S)$. Note that
        $$
            s = b \prod_1^d (x-b_i)
        $$
        with $b\in K$ and $b_1,\dots,b_d\in L$. The roots of $\overline{\sigma}(s)$ are thus $\tau(b_1),\dots,\tau(b_d)$, hence $a = \tau(b_i)$ for some $i$, hence $a\in L''$.\\
        Conversely, suppose $a\in L''$. $\tau^{-1}(a)\in K(b_1,\dots,b_n)$ for some $b_1,\dots,b_n\in L$ with $b_i$ a root some $s_i\in S$ for each $i$. Then $\tau(a)\in K(\tau(b_1),\dots,\tau(b_n))$ with $\tau(b_1)$ being a root of $\overline{\sigma}(s_i)\in \overline{\sigma}(S)$. Then in particular $\tau(b_i)\in L'$, hence $a\in L'$ 
    \end{proof}
...
\begin{definition}
    A field extension, $N\supset K$ is \emph{normal}, if it is algebraic, and if for each $\alpha \in N$, the polynomial $\mathrm{Irr}(\alpha,K)$ is a product of linear factors over $N$.  
\end{definition}
\begin{lemma}\label{EquivalentFormulationOfNormalExtensionForFiniteExtension}
    Let $N\supset K$ be a finite field extension. Then $N\supset K$ is normal if and only if there is a non-constant $f\in K[x]$ for which $N$ is a splitting field over $K$. 
\end{lemma}
\begin{proof}
    Suppose $N$ is normal over $K$. Since $N\supset K$ is finite, there are $a_1,\dots,a_n\in N$ algebraic over $K$ such that $N = K(a_1,\dots, a_n)$. Set $f = \prod_1^n\mathrm{Irr}(a_i,K)$. Note $\deg \ f \geq 1$. Then since $\Irr(a_i,K)$ splits into linear factors over $N$ so does $f$. I.e. $f = \prod_1^l (x-b_i)$ where $\{a_1,\dots,a_n\}\subset \{b_1,\dots,b_l\}$. In particular, $N = K(b_1,\dots,b_l)$, hence $N$ is the splitting field of $N$ over $K$.\\
    Suppose there is a non-constant $f\in K[x]$ with splitting field $N$ over $K$. Then $f= \prod (x-a_i)$ and $N=K(a_1,\dots,a_n)$ for some $a_1,\dots,a_n\in N$ algebraic over $K$. Let $b\in N$, and consider a field extension $L\supset N$ over which $\Irr(b,K)\in N[x]$ splits into linear. We need to prove that the roots of $\Irr(b,K)$ are all in $N$. Let $c$ be such a root. Then there is an isomorphism of $K$-algebras
    $$
        \sigma : K(b)\simeq K(c)
    $$
    taking $b$ to $c$. Note
    $$
        N = K(b)(a_1,\dots,a_n),
    $$ 
    hence $N$ is a splitting field of $f$ over $K(b)$. Similarly, $N(c)$ is a splitting field of\\ 
    $f = \overline{\sigma}(f)$ over $K(c)$. Then 
    $$
        [N : K(b)] = [N(c) : K(c)].
    $$
    Note that $[K(b) : K] = \deg \ \Irr(b,K) = [K(c) : K]$, hence 
    \begin{align*}
        [N : K] &= [N : K(b)][K(b) : K] = [N(c) : K(c)][K(c) : K] = [N(c) : K]\\ 
        &= [N(c) : N][N : K].
    \end{align*}
    Then $[N(c) : N]= 1$, implying $c\in N$. 
\end{proof}
    The utility of studying normal extensions is thus that they record whether it splits a polynomial in its subfield. 
    \begin{example}
        Consider the field extension $\Q(\sqrt[3]{2})\supset \Q$. The defining polynomial of $\sqrt[3]{2}$ over $\Q$ is 
        $$
            x^3-2.
        $$
        But 
        $$
            x^3 - 2 = (x-\sqrt[3]{2})(x-\xi_1\sqrt[3]{2})(x-\xi_2\sqrt[3]{2}),
        $$
        where $\xi_1,\xi_2$ are the non-trivial primitive roots of unity. The splitting field of $x^3-2$ is thus 
        $$
            \Q(\sqrt[3]{2},\xi_1\sqrt[3]{2},\xi_2\sqrt[3]{2}) \supsetneq \Q(\sqrt[3]{2}).
        $$
        Then $\Q(\sqrt[3]{2})$ is not normal over $\Q$, and thus by the prior lemma cannot be a splitting field over any non-constant $f\in \Q[x]$.  
    \end{example} 
\begin{lemma}\label{AutomorphismFixesImageOfNormalFieldExtension}
    Consider a tower of field extensions $L\supset N \supset K$ with $N\supset K$ normal. Then for each $\sigma \in \mathrm{Aut}_K(L)$, 
    $$
        \sigma(N) = N.
    $$
\end{lemma}
\begin{proof}
    Let $a\in N$. Then $\sigma(a)$ and $a$ are conjugate over $K$, in particular $\sigma(a)\in V(\Irr(a,K))\subset N$. Note that this also implies $\sigma^{-1}(N)\subset N$, since $\sigma^{-1}\in \mathrm{Aut}_K(L)$. Then $N\subset \sigma(N)$.     
\end{proof}

\begin{proposition}\label{HomomorphismOfIntermediateFieldOfNormalFiniteFieldCanBeExtentedToAutomorphim}
    Let $N\supset K$ be a normal, finite field extension. Consider an intermediate field $L$. Then each $\sigma\in \Hom_K(L,N)$ can be extended to an element in $\mathrm{Aut}_K(N)$. 
\end{proposition}
\begin{proof}
    $N$ is the splitting field of some non-constant $f\in K[x]$, hence $N$ is also the splitting field of $f$ over $L$. Then by Lemma~\ref{UniquenessOfSplittingFieldsUpToIsomorphism} $\sigma$ extends to an isomorphism $L\simeq L$. 
\end{proof}

\begin{corollary}\label{ExistAutomorphismSendingElementToAConjugate}
    Let $N\subset K$ be a normal, finite field extension. If $a,b\in N$ are conjugate over $K$, then there is a $\sigma\in \mathrm{Aut}_K(N)$ with $\sigma(a)=b$. 
\end{corollary}
\begin{proof}
    Since $a$ and $b$ are conjugate there is an isomorphism of $K$-algebras 
    $$
        \sigma: K(a)\rightarrow K(b)
    $$
    with $\sigma(a) = b$. We consider the associated map $\sigma : K(a)\rightarrow N$. By the prior proposition $\sigma$ extends to automorphism on $N$. 
\end{proof}

\begin{corollary}
    Let $N\supset K$ be a normal, finite field extension and $L$ an intermediate field. $L\supset K$ is normal if and only if $\sigma(L) = L$ for each $\sigma\in \mathrm{Aut}_K(N)$. 
\end{corollary}
\begin{proof}
    One direction is just Lemma~\ref{AutomorphismFixesImageOfNormalFieldExtension}.\\
    Suppose $\sigma(L) = L$ for each $\sigma\in \mathrm{Aut}_K(N)$. Let $a\in L$. Note that $\Irr(a,K)$ splits into linear factors over $K$. Let $b\in N$ be a root of $\Irr(a,K)$, i.e. a conjugate of $a$. Then by the prior corollary, there is a $\sigma\in \mathrm{Aut}_K(N)$ with $\sigma(a)=b$. Then by assumption $b\in L$, hence $\Irr(a,K)$ split into linear factors over $L$. We thus conclude that $L$ is normal.   
\end{proof}

\begin{lemma}\label{IntermediateFieldProperOfNormality}
    If $L\supset K$ is finite, there is a field extension $N\supset L$ such that $N\supset K$ is finite and normal.
\end{lemma}
\begin{proof}
    There are $a_1,\dots,a_n\in L$ algebraic over $K$ with $L=K(a_1,\dots,a_n)$. Let $f = \prod_1^n \Irr(a,K)$ and let $N$ be the splitting field of $f$ over $L$ and thus also a splitting field over $K$. Then $N\supset L \supset K$ is a tower of finite extensions, hence $N\supset K$ is finite and $N\supset K$ is normal by Lemma~\ref{EquivalentFormulationOfNormalExtensionForFiniteExtension}.  
\end{proof}

\begin{lemma}\label{NormalExtensionOverLargerNormalFieldIsNormal}
    Consider a tower of field extensions $M\supset L\supset K$. If $M\supset K$ is normal, so is $M\supset L$.
\end{lemma}
\begin{proof}
    Since $M\supset K$ is normal, it is in particular algebraic, hence $M\supset L$ is algebraic. Note that for each $a\in M$, $\Irr(a,L)\mid \Irr(a,K)$, hence $\Irr(a,L)$ splits into linear factors over $M$. We thus conclude that $M\supset L$ is normal.  
\end{proof}

\subsubsection{The Automorphism Group of a Field Extension}
\begin{proposition}
    Consider a field extension $L\supset K$ and let $a\in L$ be algebraic over $K$. There is a bijection 
    \begin{gather*}
        \ev_a : \Aut_K(K(a)) \rightarrow \{\text{roots of } \Irr(a,K) \text{ in } K(a)\}\\
        \sigma \mapsto \sigma(a)
    \end{gather*}
    In particular, 
    $$
        \# \Aut_K(K(a)) \leq [K(a) : K] = \deg \ \Irr(a,K).
    $$
\end{proposition}
\begin{proof}
    By Lemma~\ref{AutomorphismTakesConjugatesToConjugates}
    $\sigma(a)\in K(a)$ is conjugate to $a$ over $K$ for any 
    $\sigma\in \Aut_K(K(a))$, hence the map is well-defined. Let
    $b$ be a root of $\Irr(a,K)$. 
    By Lemma~\ref{FieldAdjoinedByConjugatesAreUniquelyIsomorphic}, there is a unique $K$-algebra isomorphism 
    $$
        \sigma_b : K(a)\rightarrow K(b)
    $$
    such that $\sigma_b(a) = b$. Note that $K(b)\subset K(a)$. Moreover, 
    $$
        [K(a) : K] = \deg \ \Irr(a,K) = [K(b) : K],
    $$
    hence 
    $$
        [K(a) : K] = [K(a) : K(b)] [K(b) : K] = [K(a) : K(b)][K(a):K],
    $$
    which implies $[K(a) : K(b)] = 1$, hence $K(a)=K(b)$. Then $\sigma_b \in \Aut_K(K(a))$. Thus, we get an inverse
    \begin{gather*}
        \{\text{roots of } \Irr(a,K) \text{ in } K(a)\} \rightarrow \Aut_K(K(a))\\
        b \mapsto \sigma_b
    \end{gather*} 
\end{proof}

\begin{lemma}
    Consider a finite extension $L\supset K$. Then 
    $$
        \#\Aut_K(L) < [L : K]
    $$
\end{lemma}
\begin{proof}
    Note that $L = K(a_1,\dots,a_n)$ for $a_1,\dots,a_n\in L$ algebraic over $K$. We have seen the $n=1$ case in the prior proposition. When $n>1$, note that 
    \begin{gather*}
        \Aut_K(L) \rightarrow \Aut_K(K(a_1,\dots,a_{n-1}))\\
        \sigma \mapsto \left. \sigma\right|_{K(a_1,\dots,a_{n-1})}
    \end{gather*}
    is a surjective group map with kernel, $\Aut_{K(a_1,\dots,a_{n-1})}(L)$. Both the image and kernel of this map are finite by induction, hence $\#\Aut_K(L)$ is finite (cf. Lemma~\ref{ConditionOnGroupBeingFinite}) and  
    \begin{align*}
        \# \Aut_K(L) &= \#\Aut_K(K(a_1,\dots,a_{n-1}))\#\Aut_{K(a_1,\dots,a_{n-1})}(L)\\
        & \leq [L:K(a_1,\dots,a_{n-1})][K(a_1,\dots,a_{n-1}):K] = [L:K],
    \end{align*}
    by Lagrange. 
\end{proof}
\subsubsection{Separable Extensions}
\begin{definition}
    Let $L\supset K$ be an algebraic extension. $a\in L$ is \emph{separable over $K$}, it is a root of multiplicity $1$ in $\Irr(a,K)$. It is \emph{inseparable} if not. $L\supset K$ is \emph{separable} if each $a\in L$ is separable over $K$. 
\end{definition}
\begin{lemma}\label{SeparabilityInTermsOfDerivatives}
    Let $L\supset K$ be an algebraic extension. Then $a\in L$ is not separable if and only if $\Irr(a,K)_x=0$. 
\end{lemma}
\begin{proof}
    Set $f:= \Irr(a,K)$. Suppose $a$ is a multiple root of $f$ and thus also a root of $f_x$ by Lemma~\ref{MultipleRootsInTermsOfDerivative}. Then the greatest common divisor of $f$ and $f_x$ in $K[x]$ is not $1$, since otherwise $gf + hf_x = 1$ for some $g,h\in K[x]$. Then since $f$ is irreducible, $f\mid f_x$, which implies $f_x = 0$. \\
    Conversely if, $a$ is separable over $K$, then $a$ is a root of $f$ but not a root of $f_x$ (cf. Lemma~\ref{MultipleRootsInTermsOfDerivative}), hence $f_x\neq 0$.  
\end{proof}
\begin{corollary}
    Let $L\supset K$ be an algebraic extension. Let $\alpha,\beta\in L$ be conjugate with $\alpha$ seperable over $K$. Then $\beta$ is seperable over $K$.
\end{corollary}
\begin{proof}
    Since $\alpha$ is separable over $K$, 
    $$
        0\neq \Irr(a,K)_x = \Irr(b,K)_x,
    $$
    which implies $b$ is separable over $K$.
\end{proof}
\begin{lemma}\label{AlgebraicExtensionOfCharacteristicZeroFieldsAreSeparable}
    Let $L\supset K$ be an algebraic extension. If $\Char\ K = 0$, then $L\supset K$ is separable. 
\end{lemma}
\begin{proof}
    Let $a\in L$. Then $a$ is a root of the non-constant polynomial $\Irr(a,K)$. The derivative of a non-constant polynomial in characteristic $0$ ring is non-zero. In particular, $\Irr(a,K)_x\neq 0$, hence $a$ is separable over $K$. 
\end{proof}

\begin{lemma}\label{AlgebraicExtensionsOverPerfectFieldsAreSeparable}
    Let $L\supset K$ be an algebraic extension with $\Char \ K = p>0$. 
    \begin{enumerate}
        \item $a\in L$ is not separable over $K$ if and only if $\Irr(a,K) \in K[x^p]$.
        \item If in addition $K$ is perfect, then $L\supset K$ is separable.  
    \end{enumerate}
\end{lemma}
\begin{proof}
    1. For this it suffices to show that $\Irr(a,K)_x = 0$ if and only if $\Irr(a,K)\in K[x^p]$. One direction is just Lemma~\ref{UsefulPartialDerivativeLemmaForPositiveCharacteristic}. The other direction is trivial.\\
    2. By Lemma~\ref{ConditionForVanishingDerivativeForPolynomialOverPerfectField}, irreducible polynomials over $K$ do not have derivative $0$, hence $\Irr(a,K)_x \neq 0$ for every $a\in L$, hence $L\supset K$ is separable by Lemma~\ref{SeparabilityInTermsOfDerivatives}.
\end{proof}
\begin{proposition}
    Let $K$ be a field of characteristic $p>0$ and $L\supset K$ a finite extension with $p\nmid [L : K]$. Then $L\supset K$ is separable. 
\end{proposition}
\begin{proof}
    Let $a\in L$. Since $\deg \ Irr(a,K) \mid [L : K]$, we have $p\nmid \deg \ \Irr(a,K)$, hence $\Irr(a,K)\notin K[x^p]$, which by 1. of the prior lemma implies $a$ is separable over $K$. 
\end{proof}
\begin{definition}
    Let $f\in K[x]$ be irreducible. $f$ is \emph{separable} if all roots of $f$ in its splitting field have multiplicity $1$.\\
    An arbitrary non-constant $f\in K[x]$ is \emph{separable}, if each irreducible factor is separable.  
\end{definition}

\begin{lemma}
    Let $f\in K[x]$ be irreducible. $f$ is separable if and only if $f_x \neq 0$. 
\end{lemma}
\begin{proof}
    Let $L$ be the splitting field of $f$ over $K$. Let $a\in L$ be a root of $f$. Note that $\Irr(a,K)\mid f$ in $K[x]$. The assumption that $f$ is irreducible thus implies that 
    $$
        f = c\Irr(a,K)
    $$ 
    for some $c\in K\setminus 0$.\\
    Suppose $f$ is separable. A $a$ root in $f$ is thus a root of multiplicity $1$ of $\Irr(a,K)$, hence $\Irr_x(a,K)\neq 0$ by Lemma~\ref{SeparabilityInTermsOfDerivatives}. Then $f_x = c\Irr(a,K)_x \neq 0$.\\
    Suppose $f_x\neq 0$. Let $a\in L$ be a root of $f$. Then $\Irr(a,K)_x\neq 0$, hence $a$ is a root of multiplicity $1$ in $\Irr(a,K)$, hence it also has multiplicity $1$ in $f$.  
\end{proof}

\begin{lemma}
    If $K$ is a field of characteristic $0$, or of characteristic $p>0$ and perfect, then every non-constant $f\in K[x]$ is separable.  
\end{lemma}
\begin{proof}
    This is a direct consequence of Lemma~\ref{AlgebraicExtensionOfCharacteristicZeroFieldsAreSeparable} resp. Lemma~\ref{AlgebraicExtensionsOverPerfectFieldsAreSeparable} 2. 
\end{proof}

\begin{lemma}\label{SeparabilityOfIntermediateFieldsOfAlgebraicSeparableFields}
    Let $M\supset L \supset K$ be field extensions. If $M\supset K$ is algebraic, separable, then $M\supset L$ and $L\supset K$ are algebraic, separable.  
\end{lemma}
\begin{proof}
    We already know that $M\supset L$ and $L\supset K$ are algebraic. Clearly $L\supset K$ is separable. Let $a\in M$. Note that $\Irr(a,L)\mid \Irr(a,K)$ in $L[x]$. Since $a$ is a root of multiplicity $1$ in $\Irr(a,K)$, then it is also a root of multiplicity $1$ in $\Irr(a,L)$. It follows that $M\supset L$ is separable.
\end{proof}
\begin{definition}
    Let $L \supset K$ be a finite extension. For each $a\in L$, we define $G_a$ to be the subgroup of $\Aut_K(L)$, 
    $$
        \Aut_{K(a)}(L).
    $$ 
\end{definition}
\begin{lemma}
    Let $N\supset K$ be a finite, normal extension. Set $G := \Aut_K(L)$. Then 
    $$
        \# G/G_a = \#V_N(\Irr(a,K))
    $$
    for each $a\in N$
...
\begin{proof}
    Consider the map,
    \begin{gather*}
        G/G_a \rightarrow V_N(\Irr(a,K))\\
        \sigma G_a \mapsto \sigma(a)
    \end{gather*}
    This is well-defined: Indeed, if $\sigma_1  G_a = \sigma_2  G_a$, then $\sigma_1\sigma_2^{-1}(a) = a$, hence $\sigma_1(a)=\sigma_2(a)$. Conversely, when $\sigma_1(a) = \sigma_2(a)$, we clearly also have that $\sigma_1G_a = \sigma_2G_a$, hence this is an injective assignment. By Corollary~\ref{ExistAutomorphismSendingElementToAConjugate}, it is also surjective. 
\end{proof}

\begin{theorem}
    Let $N\supset K$ be a finite, normal extension. Then $\#\Aut_K(N)\leq [N : K]$ and $\#\Aut_K(N) = [N : K]$ if and only if $N\supset K$ is separable. 
\end{theorem}
\begin{proof}
    We prove the statements by induction in $[N : K]$. If $[N : K] =1$, then $N=K$, hence $\Aut_K(N)=0$ and $N\supset K$ is separable.\\
    Suppose $[N : K] >1$. Let $a\in N\setminus K$. Then 
    $$
        [N : K]  = [N : K(a)][K(a) : K].
    $$  
    Since $[K(a) : K] >1$, we thus have $[N : K(a)] < [N : K]$. By Lemma~\ref{IntermediateFieldProperOfNormality}, $N \supset K(a)$ is normal, hence by induction $\Aut_{K(a)}(N) \leq [N : K(a)]$. Then setting $G := \Aut_K(N)$, by induction,
    $$
        \#G = \#G/G_a \#G_a \leq \#V(\Irr(a,K))[N : K(a)]\leq [K(a) : K][N : K(a)] = [N : K].
    $$
    Suppose $N\supset K$ is separable. This assumption shows that $\#G/G_a = [K : K(a)]$. We also have that $N\supset K(a)$ is separable. Then by induction, $\#G_a = [N : K(a)]$. Again, using Lagrange it follows that $\#G = [N : K]$.\\
    Suppose $\#G  =[N : K]$. Let $b \in N$. Then 
    $$
        [N : K] = \#G_b\#V(\Irr(b,K)) = [N : K(b)]\#V(\Irr(b,K)) \leq [N:K(b)][K(b) : K] = [N : K],
    $$ 
    hence $\#V(\Irr(b,K)) = [K(b) : K]=\deg \ \Irr(b,K)$. It thus follows that each root in $\Irr(b,K)$ has multiplicity $1$, hence $N\supset K$ is separable. 
\end{proof}

\begin{lemma}
    Let $L\supset K$ be a finite field extension and write $L=K(a_1,\dots,a_n)$ with $a_1,\dots,a_n\in L$ algebraic over $K$. If each $a_i$ is separable, then $L\supset K$ is separable. 
\end{lemma}
\begin{proof}
    Suppose first that $L\supset K$ is normal. Consider the case $n=1$. This is easy, since each conjugate of $a$ is also separable, hence 
    $$
        \#\Aut_K(K(a)) = \#(\Aut_K(K(a))/G_a) = \deg\ \Irr(a,K) = [K(a):K],
    $$
    so $K(a)\supset K$ is separable by the prior theorem. Consider now the case $n>1$. Set $K' = K(a_1)$. $L\supset K'$ is then also normal by Lemma~\ref{NormalExtensionOverLargerNormalFieldIsNormal}. Note that $\Irr(a_i,K')\mid \Irr(a_i,K)$ in $K'[x]$ for each $i>1$, hence $a_i$ is also a root of multiplicity $1$ in $Irr(a_i,K')$. Then each $a_i$ is separable over $K'$ for $i>1$, so by induction, $L\supset K'$ is separable. This implies that 
    $$
        \# G_a = [L:K'],
    $$
    by the prior theorem. 
    Moreover, since $K'\supset K$ is separable,  
    $$
        \#(\Aut_K(L)/G_{a_1}) = \#V_L(\Irr(a_1,K))=\#V_{K'}(\Irr(a_1,K)) = \# [K':K] 
    $$
    We therefore conclude that 
    $$
        \#\Aut_{K}(L) = \#(\Aut_K(L)/G_a) \#G_a = [L:K'][K':K] = [L:K],
    $$
    so again by the prior theorem, $L\supset K$ is separable.\\
    We now turn to the case where $L\supset K$ need not be normal. Let $N$ be the splitting field of 
    $$
        f := \prod_1^n \Irr(a_i,K)
    $$
    over $L$, and thus also over $K$ by ... Then $N\supset K$ is normal and $N = K(b_1,\dots,b_m)$, where 
    $\{b_1,\dots,b_m\} = V(f).$ Since each $b_i$ is conjugate to an $a_j$ it follows that all these are separable over $K$. Then by the specialised case, we considered first, $N\supset K$ is separable. It thus follows that $L\supset K$ is separable by...    
\end{proof}

\begin{corollary}
    Let $f\in K[x]$ be non-constant with splitting field $N$ over $K$. If $f$ is separable over $K$, then $N\supset K$ is separable over $K$. 
\end{corollary}
\begin{proof}
    Indeed, $N=K(a_1,\dots,a_n)$ where $a_1,\dots,a_n$ are the roots of $f$. Moreover, $N\supset K$ is normal by . Since $f$ is separable, it follows that that each $a_i$ has multiplicity $1$ as a root of $f$. Then each $a_i$ has multiplicity $1$ as a root of $\Irr(a,K)$, since $\Irr(a,K)\mid f$, hence each $a_i$ is seperable over $K$. By the prior lemma, it follows that $N\supset K $ is separable. 
\end{proof}

\begin{definition}
    Let $L\supset K$ be an algebraic extension. We define \emph{the separable closure of $K$ in $L$} to be the set
    $$
        L_s := \{a\in L : a \text{ is separable over } K\}
    $$
\end{definition}
\begin{remark}
    Let $a,b\in L_s$. Then $K(a,b)\supset K$ is separable. Then $K(a,b)\subset L_s$. In particular, $L_s$ contains $0$ and $1$ and is closed under addition, multiplicatio and taking both additive and multiplicative inverse. In other words, $L_s$ is a subfield of $L$.  
\end{remark}

\subsubsection{Theorem of the Primitive Element}
    \begin{lemma}
    Let $L\supset K$ be a separable, finite extension. 
        \begin{enumerate}
            \item There is a field extension $N\supset L$ such that $N\supset K$ is normal, separable, and finite. 
            \item For $N$ as in 1. set $G := \Aut_K(N)$ and $H := \Aut_L(N)$. Then there is an isomorphism of sets 
            $$
                f : G/H \rightarrow \Hom_K(L,N)
            $$
            \item For $N$ as in 1.
                $$
                    [L : K] = \#\Hom_K(L,N).
    ...
    \end{lemma}
    \begin{proof}
        1. Write $L = K(a_1,\dots,a_n)$ for suitable $a_1,\dots,a_n\in L$ that are algebraic over $K$. The desired normal field extension is the splitting field of 
        $$
            \prod_1^n \Irr(a_i,K).
        $$ 
        2. We define 
        $$
            f(\psi H) := \left.\psi\right|_{L} 
        $$ 
        Note that for $\psi_1H,\psi_2H\in G/H$,
        $$
            \psi_1 H = \psi_2 H \iff \psi_2^{-1}\psi_1 \in H \iff \left.\psi_2^{-1}\psi_1\right|_{L} = \id_L \iff \left. \psi_1\right|_L = \left.\psi\right|_L,
        $$ 
        so $f$ is a well-defined, injective map. Let $\eta \in \Hom_K(L,N)$. By Proposition~\ref{HomomorphismOfIntermediateFieldOfNormalFiniteFieldCanBeExtentedToAutomorphim}, we extend $\eta$ to an automorphism $\psi\in \Aut_K(N)$. Clearly,
        $$
            f(\psi H) = \left.\psi\right|_{L} = \eta,
        $$
        hence $f$ is a bijection.\\
        3. By Lemma~\ref{SeparabilityOfIntermediateFieldsOfAlgebraicSeparableFields} $N\supset L$ is separable. The same is true for $N\supset K$ by assumption. By the main theorem of the prior section, 2. and Lagrange 
        \begin{align*}
            \# G/H = \frac{\#G}{\#H} = \frac{[N : K]}{[N : L]} = \frac{[N : L][L : K]}{[N : L]} = [L : K].
        \end{align*}
    \end{proof}
    
    \begin{theorem}(Theorem of the Primitive Element)
        Let $L\supset K$ be a separable, finite extension. There is an $a\in L$ such that $L = K(a)$. 
    \end{theorem}
    \begin{proof}
        \textbf{$K$ is finite:} In this case, $L$ is a finite set (cf. Corollary~\ref{ModuleWithFiniteLeftBasisOverFiniteRingIsFinite}). Then $L^\ast = \langle \alpha\rangle$ for some $\alpha\in L^\ast$ {\Large some result about cyclotomic polynomials}. It thus follows that $L = K(\alpha)$.\\
        \textbf{$K$ is infinite:} We may write $L=K(a_1,\dots,a_n)$ for suitable $a_1,\dots,a_n\in L$. The case $n=1$ is trivial. When $n>1$, 
        $$
            K(a_1,\dots,a_{n-1}) = K(b)
        $$
        for some $b\in L$ by induction, hence $L = K(b,a_n)$. It thus suffices to prove that the theorem holds in the case where $L= K(b,c)$ for some $b,c\in L$. By the prior lemma there is an $N\supset L$ such that $N\supset K$ is normal, separable and finite. Moreover, 
        $$
            \Hom_K(L,N) = \{\psi_1,\dots,\psi_m\}
        $$ 
        where $m = [L:K]$. Set 
        $$
            f := \prod_{i<j} \left((\psi_i(b)-\psi_j(b))x+\psi_i(c)-\psi_j(c)\right)\in N[x].
        $$
        Note that when $i\neq j$, 
        $$
            \psi_i(b)\neq \psi_j(b) \text{ or } \psi_i(c) \neq \psi_j(c).
        $$
        It thus follows that $f\neq 0$. Then since $K$ is infinite, there is a $d\in K$ such that $f(d)\neq 0$. Then for $i<j$, 
        $$
            d\psi_i(b)-d\psi_j(b) \neq \psi_i(c) - \psi_j(c).
        $$ 
        Set $a = db+c$. Then 
        $$
            \psi_i(a) = b \psi_i(d)+\psi_i(c) \neq b\psi_j(d)+\psi_j(c) = \psi_j(a).
        $$
        Each $\psi_i(a) = \left.\psi_i\right|_{L}(a)$ is conjugate to $a$. It thus follows that 
        $$
            [K(a) : K] = \deg \Irr(a,K) \geq [L : K] = [L : K(a)][K(a) : K], 
        $$
        hence $[L : K(a)] = 1$. Then $ L = K(a)$. 
    \end{proof}

    \begin{theorem}
        Let $K$ be a characteristic $0$ field and $L\supset K$ a finite extension. Then there is a $c\in L$ such that $L=K(c)$.
    \end{theorem}
    \begin{proof}
        {\Large Don't need this. It's a special case of the above.}
...
\subsubsection{Galois Extensions}
    \begin{definition}
        A field extension $L\supset K$ is \emph{Galois} if it is algebraic, normal, and separable. The automorphism group of such a Galois extension is called \emph{the Galois group}, denoted $\Gal(L/K)$.  
    \end{definition}
\subsubsection{Galois Groups}
\subsection{Commutative Algebra Results}
...
\subsubsection{Perron's Theorem of Effective Algebraic Dependence of Polynomials}
    ...
    \end{proof}
    \begin{remark}
        In the above setup we can therefore given any $f\in K[\mathbf{x}]$ find a family of polynomials 
    ...
    \begin{proof}
        ...
        And let $\pazocal{V}$ be the standard basis of $\{\mathbf{z}^v : v\in \Delta_{\delta_1,\dots,\delta_n}\}$. The matrix $_\pazocal{V}T_{B(H)}$ is not invertible if and only if for suitable $a_v\in M$
        ...
        \end{align*}
        Set $G := \{g_1,\dots,g_n,f_{n+1}\}$. Then $\sigma(_BT_{B(G)}) = {_B}T_{B(F)}$. It thus follows that 
        $$\det \ {_B}T_{B(F)} = \det \ \sigma(_BT_{B(G)}) = \sigma(\det\ _BT_{B(G)})= \sigma(0)=0.$$ 
    \end{proof}
\subsubsection{Krull Dimension}
\begin{definition}
    Let $R$ be a commutative ring and $\mathfrak{p}$ a prime ideal in $R$. We define the \emph{height} of $\mathfrak{p}$ to be the supremum of natural numbers (including infinity) $n$ such that there is an ascending chain 
    $$
        \mathfrak{p}_0\subsetneq \mathfrak{p}_1\subsetneq\dots \subsetneq \mathfrak{p}_n = \mathfrak{p}.
    $$
    We denote the height of $\mathfrak{p}$ by 
    $$
        \height\ \mathfrak{p}.
    $$
\end{definition}
\begin{definition}
    Given a commutative ring $R$. The \emph{Krull dimension} of $R$, denoted $\dim \ R$ is the quantity
    $$
        \sup 
    $$
\end{definition}
\begin{lemma}
    When $R$ is an integral domain, $0$ has height $0$. 
\end{lemma}
\begin{proof}
    Indeed, the only chain of distinct primes contained in $0$ is the trivial one, hence $\height \ 0 = 0$. 
\end{proof}
\begin{corollary}
    The Krull dimension of a field is $0$. 
\end{corollary}
\begin{lemma}
    The Krull dimension of PID that is not a field is $1$
\end{lemma}
\begin{proof}
    Let $R$ be a non-field PID. Then there is a non-trivial prime ideal $\langle p\rangle$. Suppose $\langle q\rangle \subset \langle p\rangle$ is a non-zero prime. Then $q=pd$ for some $d$. Since $q$ is in particular irreducible, hence $d$ is a unit. It follows that $\langle q\rangle =\langle p\rangle$, hence the height of any non-zero prime is $1$. It thus follows that $\dim \ R = 1$. 
\end{proof}
\begin{corollary}
    $\Z$, $K[x]$ where $K$ is a field, and DVRs have Krull dimension $1$.
\end{corollary}
\begin{lemma}\label{TranscendenceDegreeAndKrullDimensionCorrespond}
    Let $R:= K[x_1,\dots,x_n]/\mathfrak{p}$ where $\mathfrak{p}\subset K[x_1,\dots,x_n]$ is a prime ideal. Then 
    $$
        \dim \ R = \trdeg\ \mathrm{Frac}(R)
    $$
\end{lemma}
\begin{proof}
    {\Large That's gonna take some work}
\end{proof}
\begin{corollary}
    The Krull dimension of $K[x_1,\dots,x_n]$ is $n$
\end{corollary}
\begin{lemma}
    The Krull dimension of $R[x_1,dots,x_n]$ where $R$ is noetherian is 
    $$
        \dim \ R + n
    $$
\end{lemma}
\begin{proof}
    {\Large Is also gonna take some work to do} 
\end{proof}
...
Fix a field $K$.
\begin{definition}
    Let $R$ be a $K$-algebra and $M$ and $R$-module. A \emph{derivation of $R$ into $M$ (over $K$)} is a $K$-linear map 
    $$
        D : R \rightarrow M
    $$
    ...
    $$ 
\end{definition}
\begin{lemma}\label{ChainRuleForPolynomialExpressionInAlgebraOverField}
    Let $D: R\rightarrow M$ be Derivation of $R$ into $M$. 
    \begin{enumerate}
        \item For each $c\in K$, 
        $$
            Dc = 0
        $$ 
        \item Let $f\in K[x_1,\dots,x_n]$ and $t=(t_1,\dots,t_n)\in R^n$. Then 
        $$
            D(f(t)) = \sum_1^n f_{x_i}(t)Dt_i
        $$
    \end{enumerate}
\end{lemma}
\begin{proof}
    1. This is obvious for $c=0$. So suppose $c\neq 0$. Then 
    $$
        D1 = D(cc^{-1})=cDc^{-1}+c^{-1}Dc= 2D(1) \implies D1=0 \implies Dc=cD1=0.
    $$
    2. 
    Since $D$ is $K$-linear, it suffices to prove the statement for a monomial $\mathbf{x}^v$. Note that the statement trivially holds for $v=0$. We prove the statement by induction in the number of variables. Let $l>1$. By induction in the degree, we have
    \begin{align*}
        Dt^l= t Dt^{l-1} + t^{l-1}Dt = (l-1)tt^{l-2}Dt + t^{l-1}Dt =  lt^{l-1}Dt = x^l_{x}(t)Dt.
    \end{align*} 
    Consider $n>1$. We may assume that $v\neq 0$ and $v_1>0$. Set $w:=(v_2,\dots,v_n)$. Then 
    \begin{align*}
        D\left(t_1^{v_1}\cdots t_n^{v_n}\right) &= t_1^{v_1}D\left(t_2^{v_2} \cdots t_n^{v_n}\right) + t_2^{v_2}\cdots t_n^{v_n}D t_1^{v_1}\\
        &= \sum_2^n t^{v_1}\mathbf{x}^w_{x_i}(t_2,\dots,t_n)D t_i + v_1t_1^{v_1-1}t_2^{v_2}\cdots t_n^{v_n}D t_1 \\
        &= \sum_2^n \mathbf{x}^v_{x_i}(t)Dt_i + v_1t_1^{v_1-1}t_2^{v_2}\cdots t_n^{v_n}D t_1 \\
        &= \sum_1^n \mathbf{x}^v_{x_i}(t)Dt_i.
    \end{align*}
\end{proof}

\begin{lemma}\label{ExtensionToFieldOfFractions}
    Let $R$ be $K$-algebra that is an integral domain with quotient field $L := Q(R)$ and $M$ an $L$-vector space. Then any derivation $D:R\rightarrow M$ extends uniquely to a derivation $\widetilde{D} : L \rightarrow M$ 
\end{lemma}
\begin{proof}
    \textbf{Existence:} Define 
    \begin{gather*}
        \widetilde{D} : L \rightarrow M\\
        \frac{x}{y} \mapsto \frac{1}{y}\left(Dx-\frac{x}{y}Dy\right)
    \end{gather*}
    We check that this is well-defined: Suppose $\frac{x_1}{y_1}=\frac{x_2}{y_2}$. Then 
    \begin{align*}
        \frac{1}{y_1}\left(Dx_1-\frac{x_1}{y_1}Dy_1 \right) &=  \frac{1}{y_1}\left(Dx_1-\frac{x_1}{y_1}Dy_1 +\frac{1}{y_2}\left(Dx_2y_1-Dx_1y_2\right) \right)\\
        &= \frac{1}{y_1}\left(Dx_1-\frac{x_1}{y_1}Dy_1 -\frac{x_1}{y_2}Dy_2-Dx_1+\frac{x_2}{y_2}Dy_1+\frac{y_1}{y_2}Dx_2 \right)\\
        &= \frac{1}{y_1}\left(\frac{y_1}{y_2}Dx_2-\frac{x_1}{y_2}Dy_2 \right) = \frac{1}{y_2}\left(Dx_2-\frac{x_1}{y_1}Dy_2\right)\\
        &= \frac{1}{y_2}\left(Dx_2-\frac{x_2}{y_2}Dy_2\right).
    \end{align*}
    We check that it is $K$-linear,
    \begin{align*}
        \widetilde{D}\left(\frac{ax_1}{y_1}+\frac{bx_2}{y_2}\right) &= \widetilde{D}\left(\frac{ax_1y_2+bx_2y_1}{y_1y_2}\right)\\
        &= \frac{1}{y_1y_2}\left((Dax_1y_2+bx_2y_1)-\frac{ax_1y_2+bx_2y_1}{y_1y_2}Dy_1y_2\right)\\
        &= \frac{1}{y_1y_2}\bigg(ax_1Dy_2+ay_2Dx_1+bx_2Dy_1+by_1Dx_2\\ 
        &-\frac{ax_1y_2+bx_2y_1}{y_1y_2}(y_1Dy_2+y_2Dy_1)\bigg) \\
        &= a\frac{1}{y_1}\left(Dx_1-\frac{x_1}{y_1}Dy_1\right) + b\frac{1}{y_2}\left(Dx_2-\frac{x_2}{y_2}Dy_2\right)\\
        &= a\widetilde{D}\left(\frac{x_1}{y_1} \right)+b\widetilde{D}\left(\frac{x_2}{y_2}\right).
    \end{align*}
    We also check the Leibniz rule,
    \begin{align*}
        \widetilde{D}\left(\frac{x_1x_2}{y_1y_2}\right) &= \frac{1}{y_1y_2}\left((Dx_1x_2)-\frac{x_1x_2}{y_1y_2}(Dy_1y_2) \right)\\ 
        &= \frac{1}{y_1y_2}\left(x_1Dx_2 + x_2Dx_1- \frac{x_1x_2}{y_2}Dy_2-\frac{x_1x_2}{y_1}Dy_1\right)\\
        &= \frac{x_1}{y_1}\left(\frac{1}{y_2}\left(Dx_2-\frac{x_2}{y_2}Dy_2 \right)\right)+\frac{x_2}{y_2}\left(\frac{1}{y_1}\left(Dx_1-\frac{x_1}{y_1}Dy_1\right) \right)\\
        &= \frac{x_1}{y_1}\widetilde{D}\left(\frac{x_2}{y_2}\right)+\frac{x_2}{y_2}\widetilde{D}\left(\frac{x_1}{y_1}\right).
    \end{align*}
    \textbf{Uniqueness} Let $z = \frac{x}{y}\in L$. Any derivation $\widetilde{D} : L\rightarrow M$ extending $D$ must satisfy 
    $$
        Dx = \widetilde{D}yz = y\widetilde{D}z + zDy \implies \widetilde{D}(z) = \frac{1}{y}(Dx-zDy). 
    $$
\end{proof}
We now define differentials on a $K$-algebra $R$. Let $F$ denote the free $R$-module generated by $R$, i.e. $F:= R[R]$. For each $x\in R$, we let $[x]$ denote $x$ seen as an element of $R[R]$. Let $N$ be the submodule of $F$ generated by 
$$
    \{[x+y]-[x]-[y] : x,y\in R\}\cup \{[ax]-a[x] : x\in R, a\in K\}\cup \{[xy]-x[y]-y[x] : x,y\in R\}
$$
\begin{definition}
    Define \emph{the $R$-module of differentials over $K$} to be 
    $$
        \Omega(R) := \Omega_K(R):= F/N.
    $$
    We define a derivation of $R$ into $\Omega(R)$, 
    \begin{gather*}
        d : R\rightarrow \Omega(R)\\
        x \mapsto [x]+N
    \end{gather*} 
\end{definition}
\begin{remark}
    \begin{enumerate}
        We need to check $d$ is indeed a derivation. It is $K$-linear, since for each $a,b\in K$, $x,y\in R$, 
        \begin{align*}
            d(ax+by) = [ax+by]+N=a([x]+N)+b([y]+N)= adx+bdy. 
        \end{align*}
        It satisfies the Leibniz rule, since
        \begin{align*}
            d(xy)=[xy]+N = x[y]+N + y[x]+N=xdy+ydx.
        \end{align*}
    \end{enumerate}
\end{remark}
Any derivation may be described uniquely in terms of a derivation into $\Omega(R)$. 
\begin{lemma}\label{ExtensionOfDerivationToFieldOfFractions}
    For any $R$-module $M$ and derivation $D: R\rightarrow M$, the diagram 
    $$
        \begin{tikzcd}
            R\arrow[d,"d"']\arrow[r,"D"] & M\\
            \Omega(R) \arrow[ur,dashrightarrow,"\exists!\rho"']
        \end{tikzcd}
    $$
    commutes
\end{lemma}
\begin{proof}
    ...
    \begin{gather*}
        \rho' : F \rightarrow M\\
        \sum_{y\in R} x_y[y] \mapsto \sum_{y\in R} x_y D 
    \end{gather*}
    One readily verifies that $\rho'(N)=0$, hence there is a unique induced map $\rho : \Omega(R)\rightarrow M$ with 
    $$
        \begin{tikzcd}
            F \arrow[dr,"\rho'"']\arrow[r,twoheadrightarrow] & \Omega(R)\arrow[d,"\rho"]\\
            & M
        \end{tikzcd}
    $$
    commuting. It follows that $\rho$ is the unique map making the desired diagram commute.
\end{proof}
\begin{lemma}
    Let $(t_1,\dots,t_n)\in R^n$. Then $\Omega(R)$ is finitely generated as a $K[t_1,\dots,t_n]$-module by $dt_1,\dots,dt_n$.  
\end{lemma}
\begin{proof}
    This follows from Lemma~\ref{ChainRuleForPolynomialExpressionInAlgebraOverField}.
\end{proof}
\begin{lemma}
    Suppose $R$ is an integral domain with field of fractions $L$. Consider the derivations $d : R\rightarrow \Omega(R)$ and $d' : L \rightarrow \Omega(L)$. Then $\widetilde{d}=d'$. Moreover, if $L=K(t_1,\dots,t_n)$, for suitable $t=(t_1,\dots,t_n)\in L^n$, then $\Omega(L)$ is an finite dimensional $L$ vector space. 
\end{lemma}
\begin{proof}
    Indeed, consider the diagram
    $$
        \begin{tikzcd}
            R \arrow[d,hookrightarrow]\arrow[r, "d"] & \Omega(R) \arrow[d,hookrightarrow, "\iota"]\\
            L \arrow[r,"d'"'] & \Omega(L)
        \end{tikzcd}
    $$
    Since $\iota d$ is derivation, it follows from Lemma~\ref{ExtensionOfDerivationToFieldOfFractions} that $\widetilde{d}=d'$. Let\\ $\frac{f(t)}{g(t)}\in L=K(t_1,\dots,t_n)$. Then 
    \begin{align*}
        d' \frac{f(t)}{g(t)} &= \frac{1}{g(t)}\left(d' f(t)-\frac{f(t)}{g(t)} dg(t) \right) = \sum_1^n \left[\frac{f_{x_i}(t)g(t)-f(t)g_{x_i}(t)}{g(t)^2}\right] dt_i.
    \end{align*}
\end{proof}