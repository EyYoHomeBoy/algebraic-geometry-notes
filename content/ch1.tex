...
\end{proof}
\subsubsection{Local Rings of Commutative Rings}
    \begin{definition}
...
\begin{lemma}\label{EquivalentDescriptionOfBeingSaturated}
    Let $R$ be a commutative ring and $X$ a multiplicative set in $R$. Then $X$ is saturated if and only if for every $x,y\in R$ s.t. $xy \in X$, $x,y\in X$
\end{lemma}
\begin{proof}
...
For this subsection $\pazocal{C}$ denotes the category whose objects are pairs consisting of a commutative ring and a multiplicative set in that ring. A morphism of such objects, $(R,X)\rightarrow (S,Y)$ is ring map, mapping every element in $X$ to an element in $Y$. Let $\pazocal{D}$ be the subcategory of $\pazocal{C}$ where an object in $\pazocal{D}$ is a pair $(R,X)\in \pazocal{C}$ with $X\subset R^\ast$.
    ...
    \begin{remark}
        ...
        $$
            \Hom_\pazocal{C}((R,X),\_) : \pazocal{D}\rightarrow \mathrm{Set}
        $$
...
\end{proof}
\begin{remark}
    From the above we conclude that if $K$ is a field then we can canonically (to be shown) make the identification $K = Q(K)$, and in general $Q(R) =  Q(Q(R))=Q(Q(Q(R)))...$ and so on.
\end{remark}
\begin{corollary}
    If $R\simeq S$, then 
    $$
