...
\begin{definition}
    A field extension $L\supset K$ is said to be \textit{a finitely field extension (over $K$)}, if there exist $a_1,\dots,a_n$ such that $L = K(a_1,\dots,a_n)$.
\end{definition}
\begin{lemma}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item $T\supset S$ and $S\supset R$ are module-finite if and only if 
        \item If $T\supset S$ and $S\supset R$ are ring-finite, then $T\supset R$ is ring-finite.
        \item $M\supset L$ and $L\supset K$ are field finite  extensions if and only if $M\supset K$ is a field finite extension. 
    \end{enumerate}
\end{lemma}
\begin{proof}
    1. This follows from {\Large It is a special case of results about finitely generated modules}.\\ 
...
\end{proof}
\begin{example}
    The converse of 2. in the above lemma is not true. Consider for instance the polynomial ring $R[x]\supset R$ which is ring finite extension. Then the ring extension of $R$ generated by 
    $$
        X := \{x^n : n\geq 2\}
    $$
    is contained in $R[x]$, but it is not ring finite over $R$. Indeed, consider a finite sequence of $f_1,\dots,f_l$ of polynomials in $R[X]$. Note that each $f_i = a_2x^2+\dots+ a_{n_i}x^{n_i}\in R[x^2,\dots,x^{n_i}]$, hence we may assume $f_i = x^{n_i}$ for some $n_i$. It thus suffieces to find an integer $k\geq 2$ such that the equation 
    $$
        \mu_1n_1+\dots+\mu_l n_l = k
    $$
    does not have any solutions in $\Z_{\geq 0}$. 
\end{example}
