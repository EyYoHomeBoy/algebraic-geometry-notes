...
\begin{definition}
    Let $R$ be a ring and $M$ a left $R$-module. Then $M$ is said to be \textit{finitely generated over $R$} if there is a finite sequence $m_1,\dots,m_n\in M$ such that $_RM= \sum_1^n Rm_i$ resp. $M_R = \sum m_i R$.
\end{definition}
\begin{definition}
...
\end{proof}
\begin{definition}
    Let $R$ be a ring and $M$ a left $R$-module. A subset $X\subset M$ is said to be \textit{left linearly independent over $R$} (or if $R$ is commutative just \textit{linearly independent}), if for every finite sequence $m_1,\dots,m_n\in M$ and every finite sequence $r_1,\dots,r_n\in R$, 
    \begin{gather*}
...
\end{definition}
\begin{lemma}
    Consider a family of left $R$-submodules of $_RM$, $X$, that is totally ordered by inclusion. Then {\Large Here!}
\end{lemma}
