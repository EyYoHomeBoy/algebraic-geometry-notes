...
Put $d = \deg \ f$ and $d' = \deg \ g$, and write $f = \sum_{v\in \N^n : \vert v\vert \leq d} a_v \mathbf{x}^v$, $g= \sum_{v\in \N^n : \vert v \vert \leq d'} b_v \mathbf{x}^v$
    ...
    4. There is a $v$ such that $\vert v\vert = d$ and $a_v\neq 0$. Likewise there is a $w$ such that $\vert w\vert = d'$ and $b_w \neq 0$. Note that if $\vert \mu \vert < d$, then $b_{\nu} > d'$ in which case $b_\nu = 0$. If $\vert \nu\vert < d'$, then $a_\mu >d$, meaning $a_\mu =0$. It follows that 
    $$
        \sum_{\mu,\nu : \mu+\nu = v+w} a_\mu b_\nu = a_v b_w \neq 0.
    $$
    It follows that $\deg \ fg = \deg\ f + \deg \ g$. 
\end{proof}
\begin{definition}
