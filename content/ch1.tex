...
We introduce set theory first via the Zermelo-Frankel axioms with an added axiom of choice which will be necessary in some cases. This will be axioms with respect to the signature $\{\in\}$ For a pair of variables $z,X$ we define $z\in X :=  \in(z,X)$. This is to be read as "$z$ is an element of $X$" or "$z$ belongs to $X$". We now introduce the axioms of Zermelos set theory, starting with the first four. 
\begin{axioms}
...
\begin{example}\label{ASetCannotHaveItselfAsAnElement}
