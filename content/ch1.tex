...
\end{remark}
\begin{definition}
    A field $K$ of prime characteristic $p$ is \emph{perfect}, if its Frobenius map is surjective (and thus an isomorphism).  
\end{definition}
\begin{example}
...
\subsection{Field Extensions}
   ...
   \end{example}
    ...
    \end{proof}
     \begin{lemma}
        Consider a field extension $L\supset K$. If $a\in L$ is algebraic over $K$, then $[K(a) : K] = \deg \ \Irr(a, K)$. 
    \end{lemma}
    \begin{proof}
        Indeed, $\dim \ K(a) = \dim  K[x]/\langle \Irr(a,K)\rangle = \deg \ \Irr(a,K)$. 
    \end{proof}
    \begin{lemma}
        Consider field extension $M\supset L \supset K$. $M\supset K$ is finite if and only if $M\supset L$ and $L\supset K$ are finite. Then 
        $$
            [M : K] = [M : L] [L : K].
        $$ 
    \end{lemma}
    \begin{proof}
        One direction is trivial by transitivity of finiteness. Suppose $M\supset K$ is finite. Then $L\supset K$ is a subspace of $M\supset K$, hence it is finite. A generating set for $M$ as $K$-vector space is also a generating set for $M$ as an $L$-vector space.\\
        ...
        $$
            [M : K] = [M : L][L : K].
        $$
        since $M \supset K$ has $nm$ basis elements.  
    \end{proof}
    \begin{lemma}
    ...
    \begin{proof}
        ...
        $$
        Conversely, let $g\in \ev_{\sigma(a)}$. Then 
        $$
    ...
    \begin{proof}
        ...
        $$
            K(\alpha)\simeq K[x]/\Irr(a,K) \simeq K(\beta). 
        $$
    \end{proof}
\subsubsection{Splitting Fields}
    ...
    \end{theorem}
    \begin{proof}
        When $\deg \ f = 1$ the statement is trivial by taking $K=L$. If $f$ is of of degree $d+1$ for some $d\geq 1$, pick a monic irreducible factor of $f$, $g$ say. Then over $L' = K[x]/\langle g\rangle$, $g =(y - (x+I))g_1 $ for some $g_1\in L'[y]$. Then $f=qg=qg_1(y-(x+I))$ for some $q\in L'[y]$ and the result follows by induction in the degree.\\ 
...
\subsubsection{The Isomorphism Extension Theorem}
    ...
    \begin{proof}
        ...
        $$
        By Zorn's Lemma $\pazocal{M}$ contains a maximal element $\tau : \widetilde{L} \rightarrow L'$. It remains to check that $\widetilde{L} = L$. Suppose this is not the case. Then pick an $a\in L\setminus \widetilde{L}$. Then the map 
        $$
...
\begin{proof}
    ...
    $$
        [N : K(b)] = [N(c) : K(c)].
    $$
    Note that $[K(b) : K] = \Irr(b,K) = [K(c) : K]$, hence 
    \begin{align*}
...
\end{proof}
    ...
    \begin{example}
        ...
        $$
        Then $\Q(\sqrt[3]{2})$ is not normal over $\Q$ and thus cannot, and by the prior lemma cannot be a splitting field over any non-constant $f\in \Q[x]$.  
    \end{example} 
\begin{lemma}\label{AutomorphismFixesImageOfNormalFieldExtension}
    Consider a tower of field extensions $L\supset N \supset K$ with $N\supset K$ normal. Then for each $\sigma \in \mathrm{Aut}_K(L)$, then 
    $$
        \im\ \sigma = N.
    $$
\end{lemma}
\begin{proof}
    Let $a\in N$. Then $\sigma(a)$ and $a$ are conjugate over $K$, in particular $\Irr(a,K)$ vanishes on $\sigma(a)$. $\Irr(a,K)$ splits over $N[x]$, hence $\sigma(a)\in N$. Then $N\subset \varphi(N)$. Note that this also implies $\sigma^{-1}(N)\subset N$, since $\sigma^{-1}\in \mathrm{Aut}_K(L)$. Then $N\subset \sigma(N)$.     
\end{proof}
...
\end{corollary}
\begin{proof}
    Since $a$ and $b$ are conjugate there is an isomorphism of $K$-algebra $\sigma: K(a)\rightarrow K(b)$ with $\sigma(a) = b$. We consider the associated map $\sigma : K(a)\rightarrow N$. By the prior proposition $\sigma$ extends to automorphism on $N$. 
\end{proof}
...
\begin{definition}
    Let $L\supset K$ be an algebraic extension. $a\in L$ is \emph{separable over $K$} if $a$ is a root of multiplicity $1$ in $\Irr(a,K)$. It is \emph{inseparable} if not. $L\supset K$ is \emph{separable} if each $a\in L$ is separable over $K$. 
\end{definition}
\begin{lemma}\label{SeparabilityInTermsOfDerivatives}
    Let $L\supset K$ be a field extension. Then $a\in L$ is not separable if and only if $\Irr(a,K)_x=0$. 
\end{lemma}
\begin{proof}
...
\begin{lemma}
    ...
    $$
        \# G/G_a = \#\{\text{roots of } \Irr(a,K) \text{ in } N\}
    $$
    for each $a\in N$
\end{lemma}
\begin{proof}
