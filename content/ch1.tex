...
\begin{corollary}
    ...
    $$
\end{corollary}
\begin{corollary}\label{FirstIsomorphismTheoremForModules}(First Isomorhism Theorem for Modules)\\ 
...
\begin{definition}\label{NoetherianArtinianDef}
    ...
    be a chain. We say that $M$ is \textit{left noetherian} if every ascending chain stabilizes and \textit{left artinian} if every descending chain stabilizes. A ring $S$ is left noetherian/artinian, if it is noetherian/artinian as a left $S$-module or simply noetherian/artinian if is both left and right noetherian/artinian.  
\end{definition}
\begin{definition}
...
\begin{lemma}
    For a non-empty set $X$, the free additive group $R[X]$, where $R$ is a ring, is an $(R,R)$-bimodule with generating set $\{\delta_{yx}: X\rightarrow R \ | \ x\in X\}$
\end{lemma}
\begin{proof}
    This is just a reformulation of Lemma~\ref{BasisOfDirectSum}.
\end{proof}
