...
\begin{proposition}
    Conside hyperplanes $V(p_1+b_i),\dots,V(p_{n-1}+c_{n-1})$, with $p_i = \sum_1^n a_{ij}x_j\in K[x_1,\dots,x_n]$, $b_i\in K$ such that the vectors 
        $$a_i:=\begin{pmatrix}
            a_{i1}\\
            \vdots\\
            a_{in}
...
\begin{proof}
    ...
    is a non-zero polynomial in one variable, implying there are every infinitely many $v_n\in K$ such that $f'(v_n)=f(v_1,\dots,v_n)\neq 0$, hence $\A^n\setminus V(f)$ is infinite.\\
...
\begin{lemma}\label{NonEmptyFamilyOfAlgebraicSetsHasMinimalElement}
    A non-empty family of algebraic elements in $\A^n$ has a minimal element.
\end{lemma}
\begin{proof}
    Indeed, consider 
    $$
        \{V(I) : I\in \pazocal{I}\}
    $$
    were $\pazocal{I}$ is a non-empty family of ideals in $K[x_1,\dots,x_n]$. Then $\pazocal{I}$ has a maximal element $J$, since $K[x_1,\dots,x_n]$ is Noetherian. Then $V(I)\subset V(J)$ where $I\in\pazocal{I}$ implies $I\supset J$, hence $I=J$, meaning $V(I)=V(J)$. We thus conclude that $V(J)$ is a minimal element of $\{V(I) : I\in \pazocal{I}\}$. 
\end{proof}
\begin{theorem}
...
\begin{proof}
    ...
    Suppose for a contradiction that $\mathcal{A} = \emptyset$. {\Huge HERE!!!!} Then by Lemma~\ref{NonEmptyFamilyOfAlgebraicSetsHasMinimalElement}, there is a minimal element $X_0 \in \mathcal{A}$. On the one hand $X_0$ is not irreducible. Hence There are $Y,Z\subsetneq X_0$ such that $X_0 = Y \cup Z$. However, by minimality $Y,Z\notin \mathcal{A}$, hence $Y= \bigcup_1^k V_i$ and $Z = \bigcup_1^l W_i$ for varieties $V_1,\dots,V_k,W_1,\dots,W_l \subset \A^n$. But then 
...
\begin{example}
   ...
   Indeed,  $I(\A^n(K)) = 0$, which is trivially prime, hence $\A^n(K)$ is irreducible by Theorem~\ref{VarietyIffCoordinateRingIsID}
\end{example}
