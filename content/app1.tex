...
We assume a notion of \emph{finiteness}, we assume the existence of finite sets or lists of element. With this notion consider symbols $\mathbf{0}$ and $\mathbf{s}$, from which we build a \emph{potentially infinite} list $\mathbf{N}$ of \emph{natural numbers} of finite strings formed by appending $\mathbf{s}$ to the left to the prior string in the list, where the list starts with $\mathbf{0}$. In other words the have a list
...
Sometimes we need to strengthen our metatheory with \emph{the law of excluded middle} and \emph{naive set theory}. The first of these notions is that a statement \textbf{in the metatheory} is either true or false. The last of which is the notion that we can consider a sufficiently small collection of objects and ask whether a given object is a member of such a collection. We will need these two notions to define the semantics of first order logic.
\subsection{The Alphabet of First Order Logic}
\subsubsection*{The Logical Symbols}
\verb|FOL| is a framework for formalizing different theories. It therefore has to flexible enough to be augmented to work in a large variety of contexts and domains. We therefor make the following distinction between the role symbols that comprising \verb|FOL| play: The \emph{logical symbols} in the language of \verb|FOL| are those symbols that are not context specific. These are 
\begin{enumerate}
    \item \textbf{Variables}: These are play the role of placeholders for objects in the domain to which we want to apply \verb|FOL|. Eg. $x$ might be a placeholder for a set in Zermol-Fraenkel set theory or $n$ might be a placeholder for a natural number in Peano arithemetic or $\mathcal{C}$ could be a placeholder for a category in Category Theory. We presuppose a potentially infinite list of such symbols. That is we may always generate sufficiently large number of distinct variables.
...
\begin{enumerate}
    ...
    \item \textbf{Function symbols:} A symbol that is a placeholder for an object that depends on finite list of objects in a domain called arguments. For instance if $F$ is a function symbol that takes $n$ arguments, then given variables $x_1,\dots, x_n$, $F(x_1,\dots,x_n)$ is to be thought of as a placeholder of some object. A function symbol has a natural number attached to them called an \emph{arity}. A function symbol that takes $n$ arguments is an $n$-ary function. Examples are $+$ in Peano Arithmetic or $\cup$ in Zermelo-fraenkel set theory. In particular, these are examples of \emph{binary} functions.  
    \item \textbf{Relation symbols:} Symbol that signify relations of objects. Like function symbols, these have a natural number arity attached to them; and given a relation symbol $R$ and objects $x_1,\dots, x_n$, $R(x_1,\dots,x_n)$ signifies that $x_1,\dots,x_n$ are in relation by $R$.  
\end{enumerate}
To actually give these syntactical meaning, we need to define rules for building well-formed formulae in $\verb|FOL|$. I.e. we need to define what strings of symbols, we are allowed to write in the language. A set of potentially infinite list of non-logical symbols is called a \textit{signature} often denoted $\mathcal{L}$. 
...
\begin{proof}
    ...
    means $\Phi \not\vdash \varphi\wedge \neg\varphi$ by the soundness theorem.\\
...
\begin{proof}
    ...
    such that for some $k\leq n$ and term constants $\tau_0,\dots,\tau_{k-1}$,
    $$
