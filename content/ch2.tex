...
\subsubsection{Equivalence of Categories}
    ...
    \begin{example}
        ...
        If $L_A = L_B$ for some $n\times m$ matrices $A,B$, we know that $A_{ij}=(L_A e_j)_i = (L_B e_j)_i = B_{ij}$, for each $i$ and $j$, hence $A=B$.\\
        If $L : V\rightarrow W$ is a linear map, we define the matrix $A = (L(e_j)_i)$.  
    \end{example}
