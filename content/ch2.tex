...
\end{lemma}
\begin{proof}
    By the Yoneda Lemma for each object $A\in \pazocal{C}$ we have a bijection 
    \begin{gather*}
        \pazocal{F}(A) \rightarrow \Hom(\Hom(\_,A),\pazocal{F})\\
        x\mapsto \Psi(x) 
    \end{gather*}
    In other words there is a one-to-one correspondence of objects $(A,x)\in\pazocal{F}(A)$, and natural transformations unique natural transformation $\Hom(\_,A,\pazocal{F})\Rightarrow \pazocal{F}$, obtained by the mapping $x\mapsto \Psi(x)$. We suppress the dependence on $A$ from the notation for simplicity and to stay consistent with the notation of the Yoneda Lemma. Consider now the functor $\Hom(\_, \pazocal{F}) : \mathrm{Set}^\pazocal{C} \rightarrow \mathrm{Set}$. We construct a category whose objects are natural transformations of the form $\Psi(x) : \Hom(\_ ,A)\rightarrow \pazocal{F}$ and where are morphism $\Psi(x)\rightarrow \Psi(x')$ is a natural transformation $f_\ast : \Hom(\_,A) \Rightarrow \Hom(\_,A')$ where $f:A\rightarrow A'$ (all natural tranformations with this domain \& codomain are of this form) such that $\Psi(x) = \Psi(x')f_\ast$. 
\end{proof}
\begin{proposition}
