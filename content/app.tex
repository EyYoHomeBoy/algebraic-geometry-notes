...
$$
    T^\ast_{j+1}:= \begin{cases}
        ...
        \text{Insert } \sigma_{i,n}[c_j] \text{ into } T_j^\ast\\ \text{ such that the resulting list is ordered with } & \text{otherwise} \\ \text{ respect to the } \#\text{-encodings}  
    \end{cases}
$$
\begin{definition}
...
\end{definition}
\begin{definition}
    Definition of height of term constant. 
\end{definition}
\begin{lemma}
    The $\mathcal{L}_c$-theory $T^\ast_{\sigma_0}(c)$ is consistent.
\end{lemma}
\begin{proof}
   We first prove that $T^\ast_{\sigma_0}$ is consistent with respect to $\mathcal{L}_c$.  
\end{proof}
