...
\begin{proof}
    ...
    $$
    hence $\left. f_U\right|_W = f_W$. Thus if $V$ is another open affine subset of $X$, then $f_U$ and $f_V$ will agree on $U\cap V$ and  We thus obtain a morphism of schemes $f: X\rightarrow \Spec \ A$ where $f^\#_U = \left.\sigma_{f^{-1}(U)}\right|_{\pazocal{O}}$. The claim is that $f^\#_{\Spec\ A} = \sigma$
\end{proof}
