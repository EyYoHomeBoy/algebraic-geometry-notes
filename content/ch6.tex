...
\begin{proof}
    ...
    \end{gather*}
    The claim now is that $(f_\sigma)^\#_{\Spec\ A} = \sigma$. Indeed, for each $s\in A$
    \begin{align*}
...
\subsubsection{The Residue Field at a Point}
    ...
    \begin{proof}
        ...
        $$ 
        in the following way: When $x\in U\subset X$, define $\pazocal{O}(U)\rightarrow \pazocal{O}_{\Spec K}(\varphi^{-1}(U))=L$ to be the composite of $\pazocal{O}(U)\rightarrow K(x)$ and $K(x)\hookrightarrow L$. When $x\notin U$, note that $\pazocal{O}_{\Spec \  K}(\varphi^{-1}(U))=\pazocal{O}_{\Spec \ K}(\emptyset)=0$, which is the terminal object in the category of rings. The map $\pazocal{O}(U)\rightarrow \varphi_\ast\pazocal{O}_{\Spec \ K}(U)$ is then obvious. It is clear that this assignment is natural.
    \end{proof}
    \begin{proposition}
