...
\end{proof}
\begin{lemma}
    Let $X$ be a topological space with a presheaf $\pazocal{O}$ of abelian groups. Then $\pazocal{O}$ is a sheaf if and only if for each open set $U\subset X$ and each open covering $\{U_i\}$ of $U$ we may identify $\pazocal{O}(U)$ with the subring of $\prod_{i} \pazocal{O}(U_i)$,
    $$
        \pazocal{O}^{\{U_i\}}(U):=\left\{(s_i)\in \prod_{i}\pazocal{O}(U_i) : \left.s_i\right|_{U_i\cap U_j} = \left.s_j\right|_{U_i\cap U_j}\right\}
    $$ 
\end{lemma}
\begin{proof}
    Let an open set $U\subset X$ and open covering $\{U_i\}$ of $U$ be given. Then 
    \begin{gather*}
        \pazocal{O}(U)\rightarrow \pazocal{O}^{\{U_i\}}(U)\\
        s \mapsto \left(\left.s\right|_{U_i}\right)
    \end{gather*}
    is injective if and only if the first sheaf property is fulfilled and Surjective if and only if the second sheaf property is fulfilled.
\end{proof}
\begin{corollary}
    If $X$ is a topological space, $\pazocal{O}$ is a sheaf valued in the category of abelian groups and $\{U_i\}$ is an open covering of affine, disjoint subsets of $X$. It follows that 
    $$
        \pazocal{O}(U)\simeq \prod_{i} \pazocal{O}(U\cap U_i)
    $$
...
\begin{definition}
    ...
    $$
        \begin{tikzcd}
            \pazocal{F}(U) \arrow[r,"\Phi_U"] \arrow[d] & \pazocal{G}(U)\arrow[d]\\
            \pazocal{F}_P \arrow[r,"\Phi_P"] & \pazocal{G}_P
        \end{tikzcd}
    $$
    for each open neighborhood $U$ of $P$.
\end{definition}
\begin{remark}
...
\end{lemma}
\begin{proof}
    We construct a sheaf
    \begin{gather*}
        X\rightarrow \mathrm{ab}
    \end{gather*}
    Given an open subset $U$ in $X$, we define $\pazocal{F}^{+}(U)$ to be the group of elements $([s_{W_P}]_P)$ in 
    $$\prod_{P\in U} \pazocal{F}_P$$
    such that for each $P\in U$ there is an open neighborhood of $P$, $V_P\subset W_P\subset U$ with an element $t\in \pazocal{F}(V_P)$ such that for each $Q\in V_P$ the germ $[t]_Q\in \pazocal{F}_Q$ of $t$ at $Q$ satisfies
    $$[s_{W_Q}] = [t]_Q.$$
    Given $U\supset V$ we define $\varphi_{UV} : \pazocal{F}^{+}(U)\rightarrow \pazocal{F}^{+}(V)$ to be given by restriction. It thus readily follows that $\pazocal{F}^{+}$ is a sheaf. We have a natural transformation
    \begin{gather*}
        \Theta : \pazocal{F}\Rightarrow \pazocal{F}^{+}\\
        \{\Theta_U : \pazocal{F}(U)\rightarrow \pazocal{F}^{+}(U), s \mapsto ([s]_P)\}.
    \end{gather*}
    Let a natural transformation of presheaves $\Phi : \pazocal{F}\Rightarrow \pazocal{G}$ be given. We define a natural transformation of sheaves 
    \begin{gather*}
        \Psi: \pazocal{F}^{+} \Rightarrow \pazocal{G}
    \end{gather*}
    defined by the collection of morphisms  
    $$
        \left.\left\{ \Psi_U: \pazocal{F}^{+}(U) \rightarrow \pazocal{G}(U), ([s]_P)\mapsto \Theta_U(s) \ \right| \   U\subset X \text{ open}\right\}
    $$
    This is well-defined since $\pazocal{G} \cong \pazocal{G}^{+}$ or in other words since $\pazocal{G}(U)\simeq \pazocal{G}^{+}(U)$ for each $U$. $\Psi$ is the unique morphism of sheaves making the diagram
    $$
        \begin{tikzcd}
        ...
        \end{tikzcd}
    $$
    commute. Note that $(\pazocal{F}^{+},\Theta)$ is thus the coequalizer of two copies of the identity transformation of $\pazocal{F}$ (cf. Example~\ref{EqualisersAndCoequalisers}) which means it is unique up to unique isomorphism.
\end{proof}
\begin{remark}
...
\begin{lemma}
    ...
    $$
        \iota^\#: \pazocal{O} \rightarrow \iota_\star\pazocal{O}_U
    $$
    given by 
    $$
...
\end{proof}
\begin{lemma}
    Let $(X,\pazocal{O}_X)$ and $(Y,\pazocal{O}_Y)$. Let open coverings $\{U_i\}$ and $\{V_i\}$ of respectively $X$ and $Y$ be given. Consider a family of morphisms of ringed spaces $\{f_i : U_i\rightarrow V_i\}$ such that 
    \begin{enumerate}
        \item $\left. f_i\right|_{U_i\cap U_j} = \left. f_j\right|_{U_i\cap U_j}$ for each $i,j$.
        \item $\left. f_i\right|_{U_i\cap U_j}^\#= \left. f_j\right|_{U_i\cap U_j}^\#$ for each $i,j$.  
    \end{enumerate} 
    Then there is a unique morphism of ringed spaces $f:X\rightarrow Y$ such that $\left. f\right|_{U_i} : U_i\rightarrow V_i$ agrees with $f_i$ for each $i$.
\end{lemma}
\begin{proof}
    It's a basic fact of topology that the map 
    $$
        f : X\rightarrow Y
    $$
    defined by $f(P)=f_i(P)$ for $P\in U_i$, is the unique continuous map that agrees with $f_i$ when restricted to $U_i$. Let $V\subset Y$ be an open set. We define 
    \begin{gather*}
        f^\#_V : \pazocal{O}_Y(V) \rightarrow f_\ast\pazocal{O}_X(V)\\
        (\left. s\right|_{V_i\cap V})\mapsto \left( (f_i^\#)_{V_i\cap V}(\left.s\right|_{V_i\cap V})\right)
    \end{gather*} 
    Note that 
    \begin{align*}
        \left.(f_i^\#)_{V_i\cap V}(\left.s\right|_{V_i\cap V})\right|_{f_i^{-1}(V_i\cap V_j\cap V)} &= \left(\left.f_i^\#\right|_{\left.\pazocal{O}_Y\right|_{V_i\cap V_j}}\right)_{V\cap V_i}(\left. s\right|_{V_i\cap V_j\cap V})\\ &= \left(\left.f_j^\#\right|_{\left.\pazocal{O}_Y\right|_{V_i\cap V_j}}\right)_{V\cap V_i}(\left. s\right|_{V_i\cap V_j\cap V})\\ 
        &= \left.(f_j^\#)_{V_i\cap V}(\left.s\right|_{V_i\cap V})\right|_{f_i^{-1}(V_i\cap V_j\cap V)},
    \end{align*}
    hence this is a well-defined ring map. Naturality follows from each $f_i^\#$ being a natural transformation for each $i$. We thus conclude that $(f,f^\#): X\rightarrow Y$ defines a morphism of ringed spaces. Note that for $V\subset V_i$ and $s=(\left. s\right|_{V\cap V_j})\in \pazocal{O}_Y(V)$, we have  
    \begin{align*}
        \left(\left.f\right|_{U_i}^\#\right)_{V}(s) &= \iota^\#_{f^{-1}(V)\cap U_i} \left. f^\#_{V}\right|_{\left.\pazocal{O}_Y\right|_{V_i}}(s) = \varphi_{f^{-1}(V)U_i}((f_j^\#)_{V\cap V_j}(\left. s\right|_{V\cap  V_j}))\\ 
        &= \left(\left.\left(f^\#_i\right)_{V\cap V_i}(\left. s\right|_{V\cap V_i})\right|_{f_i^{-1}(V_i\cap V)}\right)\\
        &= ((f_i^\#)_{V\cap V_i}(\left. s\right|_{V\cap V_i})) = \left(f_i^\#\right)_{V}(s).
    \end{align*}  
    If $(g,g^\#): X\rightarrow Y$ is another morphism of ringed spaces with $\left.(g,g^\#)\right|_{U_i} = f_i$ for each $i$, then $g=f$ and
    \begin{align*}
        g^\#_V(s) &= (\left.g^\#_{V\cap V_i}(\left.s\right|_{V\cap V_i})\right|_{U_i}) = (\varphi_{f^{-1}(V\cap V_i) U_i}g^\#_{V\cap V_i}(\left.s\right|_{V\cap V_i})) = ((f^\#_i)_{V\cap V_i}(\left.s\right|_{V_i\cap V}))\\ 
        &= f^\#_V(s).  
    \end{align*}
    for each $V\subset Y$ and $s\in \pazocal{O}_Y(V)$
\end{proof}
\begin{remark}
    When $(X,\pazocal{O}_X)$ and $(Y,\pazocal{O}_Y)$ are locally ringed spaces and $\{f_i\}$ is a family of morphisms of locally ringed spaces, it follows readily that for $P\in U_i$,
    $$
        f^\#_P = (f_i^\#)_P,
    $$
    hence the prior upgrades to one about locally ringed spaces.
\end{remark}
\begin{lemma}
    \begin{enumerate}
...
\end{definition}
\begin{lemma}
    Let $X$ be a topological and $x\in X$. Then $\overline{\{x\}}$ is irreducible. 
\end{lemma}
\begin{proof}
    Suppose $\overline{\{x\}} = Y\cup Z$ for closed sets $Y,Z$ in $X$. Then $x\in Y$ or $x\in Z$, hence $\overline{\{x\}}=Y$ or $\overline{\{x\}}=Z$.
\end{proof}
\begin{lemma}\label{IrreducibleSetsInAffineSchemesHaveGenericPoints}
    Let $A$ be a ring. Then $Y\subset \Spec \ A $ is irreducible if and only if $Y$ contains a unique point that is generic in $Y$.
\end{lemma}
\begin{proof}
    If $Y$ has a generic point, then it is clearly irreducible by the prior lemma. Suppose $Y$ is irreducible. Set 
    $$
        \mathfrak{p} = \bigcap_{\mathfrak{q}\in Y} \mathfrak{q}.
    $$ 
    We claim that this is a prime ideal. Indeed, if $xy\in \mathfrak{p}$, then $x\in \mathfrak{q}$ for every $\mathfrak{q}\in Y$ or $y\in \mathfrak{q}$ for every $\mathfrak{q}\in Y$, for if there were $\mathfrak{q}_1,\mathfrak{q}_2$ with $x\in \mathfrak{q}_1$, $y\notin \mathfrak{q}_1$, $y\in \mathfrak{q}_2$, $x\notin \mathfrak{q}_2$, then $Y\cap V(\langle x\rangle)$ and $Y\cap V(\langle y\rangle)$ would be proper closed components of $Y$. We now claim that $\overline{\{\mathfrak{p}\}}= V(\mathfrak{p})= Y$. Clearly $V(p)\supset Y$. Let $\mathfrak{q}\in Y$ and let $D(f)$ be an open neighborhood of $\mathfrak{q}$. Then $f\notin \mathfrak{q}$, hence clearly $f\notin \mathfrak{p}$. hence $\mathfrak{q}\in \overline{\{\mathfrak{p}\}}$.\\
    Uniqueness of this $\mathfrak{p}$ follows from the fact that for prime ideals $\mathfrak{p}_1,\mathfrak{p}_2\subset A$, 
    $$
        V(\mathfrak{p}_1) = V(\mathfrak{p}_2) \iff \mathfrak{p}_1 = \mathfrak{p}_2.
    $$
\end{proof}
\begin{lemma}
    Consider a topological space $X$ with an open covering $\{U_i\}$. If $X$ is irreducible, then $X=U_i$ for some $i$.
\end{lemma}
\begin{proof}
    Indeed, 
    $$
        X = X\setminus U_i \cup X\setminus \left( \bigcup_{j\neq i} U_j\right).
    $$
    Then $X= U_i$ or $X= X\setminus U_i$. Picking $U_i$ with $X\cap U_i\neq \emptyset$ it follows that $X= U_i$.
\end{proof}
\begin{proposition}
    Let $X$ be a scheme. $Y\subset X$ is irreducible if and only if it contains a unique generic point $\eta$. 
\end{proposition}
\begin{proof}
    Again if it contains a generic point it is irreducible. Suppose $Y$ is irreducible. By the prior lemma, $Y$ must be contained in an open affine set in $X$. By Lemma~\ref{IrreducibleSetsInAffineSchemesHaveGenericPoints} it follows that $Y$ has a unique generic point. 
\end{proof}
\begin{example}
...
\end{remark}
\begin{proposition}\label{SpecOfIntegersIsInitialInCategoryOfSchemes}
...
\end{proof}
\begin{example}
    The spectrum of the zero ring is the empty set since any prime ideal must be proper and $0$ has no proper ideals. 
\end{example}
\begin{proposition}
    $0$ is the initial object in the category of schemes. 
\end{proposition}
\begin{proof}
    Indeed, $0$ is the terminal object in the category of commutative rings and thus the initial object in the category of affine schemes. We extend the result to an arbitrary scheme, as in the proof of Proposition~\ref{SpecOfIntegersIsInitialInCategoryOfSchemes}, by constructing a unique family of compatible local maps on an affine cover of said scheme. 
\end{proof}
\begin{example}
...
\end{example}
\begin{proposition}
    Let $(X,\pazocal{O})$ be a scheme and $A$ a ring. The map 
    \begin{gather*}
        \alpha: \Hom(X,\Spec \ A)\rightarrow \Hom(A,\Gamma(X,\pazocal{O}))\\
        (f,f^\#) \mapsto f^\#_{\Spec \ A}
    \end{gather*}
    is bijective.
\end{proposition}
\begin{proof}
    Let $\sigma : A \rightarrow \Gamma(X,\pazocal{O})$ be a ring map. Let $U\subset X$ be any open subset of $X$. Then we define $\sigma_U : A \rightarrow \pazocal{O}(U)$ to be given by $\varphi_{XU}\sigma$. When $U$ is affine this induces a unique morphism of locally ringed spaces $(f_U,f_U^\#): U \rightarrow \Spec \ A$. Let $W\subset U$ be an open affine subset and note that 
    $$
        \sigma_W = \varphi_{XW}\sigma = \varphi_{UW}\varphi_{XU}\sigma= \left(\sigma_{U}\right)_W.
    $$
    Note also that $\left.f_U\right|_W$ is a morphism of affine schemes induced by 
    $$
        \left(\left.f_U\right|_W\right)^\#_W = \iota^\#_W \left( f_U^\#\right)_U = \varphi_{UW}\sigma_U = (\sigma_U)_W
    $$
    hence $\left. f_U\right|_W = f_W$. Thus if $V$ is another open affine subset of $X$, then $f_U$ and $f_V$ will agree on $U\cap V$. Given an open affine cover $\{U_i\}$ of $X$, we thus obtain a morphism of schemes $f_\sigma: X\rightarrow \Spec \ A$ where 
    \begin{gather*}
        (f_\sigma)^\#_U : \pazocal{O}_{\Spec\ A}(U)\rightarrow (f_\sigma)_\ast\pazocal{O}(U)\\
        s\mapsto \left((f_i^\#)_{U}(s)\right) 
    \end{gather*}
    Here $f_i$ is equal to $f_U$. The claim now is that $(f_\sigma)^\#_{\Spec\ A} = \sigma$. Indeed, for each $s\in A$
    \begin{align*}
        (f_\sigma)^\#_{\Spec \ A}(s) = \left((f^\#_i)_{\Spec \ A}(s)\right) = (\sigma_{U_i}(s)) = (\left.\sigma(s)\right|_{U_i}) = \sigma(s).
    \end{align*}
    We thus have a well-defined map
    \begin{gather*}
        \alpha^{-1} : \Hom(X,\Spec\ A) \rightarrow \Hom(A,\Gamma(X,\pazocal{O}))\\
        \sigma \mapsto f_\sigma
    \end{gather*}
    with left inverse $\alpha$. Consider a morphism of schemes $f:X\rightarrow \Spec \ A $. Let an open affine cover $\{U_i\}$ of $X$ be given. Note that $\left. f\right|_{U_i}$ is induced by $\left(\left. f\right|_{U_i}^\#\right)_{\Spec \ A} = (f_{\Spec\ A}^\#)_{U_i}$, hence $\left. f\right|_{U_i} = f_{(f^\#_{\Spec\ A})_{U_i}}$, so
    $$
        f = f_{f^\#_{\Spec\ A}}.
    $$
    It remains to prove that $f^\#=f_{f_{\Spec\ A}^\#}^\#$. Note that    
    $$
        f_{f_{\Spec\ A}^\#,i}^\# = \left.f_{f_{\Spec\ A}^\#}\right|_{U_i}^\# = \left.f\right|_{U_i}^\#.
    $$
    Let $s\in \pazocal{O}_{\Spec\ A}(U)$, then 
    \begin{align*}
        \left(f_{f_{\Spec \ A}^\#}^\#\right)_{U}(s) = \left(\left( f_{f_{\Spec \ A}^\#,i}^\#\right)_U(s)\right) = \left(\left(\left.f\right|_{U_i}^\#\right)_U(s) \right)=f^\#_U(s).
    \end{align*}
\end{proof}
...
\begin{lemma}
    ...
    $$
        \Proj\ R = \emptyset \iff \rad(0)\supset R_{+}.
    $$
    Note that the second condition is equivalent to every element in $R_{+}$ being nilpotent
\end{lemma}
\begin{proof}
    "$\implies$": Let $\mathfrak{p}\subset R$ be a homogeneous prime ideal. Then $\mathfrak{p}\supset R_{+}$. It follows that 
    $$
        R_{+}\subset \bigcap_{\mathfrak{p} \text{ homogeneous, prime }, \mathfrak{p}\supset R_{+}} \mathfrak{p} = \rad(0).
    $$
    Here we use a strengthened version of Lemma~\ref{RadicalOfIdealIsIntersectionOfAllPrimeIdealsContainingIt} for homogenous ideal in graded rings. The proof of this version is the same except for the fact that we consider 
    $$
        X := \{J\subset R: J\supset I \text{ is a homogeneous ideal}, a^n\notin J \text{ for all } n\geq 0\}
    $$
    for $a\notin \rad(I)$, to find a homogeneous prime $\mathfrak{p}$ not containing $a$.\\
    "$\impliedby$": Suppose all elements of $R_{+}$ are nilpotent. Consider an ideal $\mathfrak{a}$ not containing all of $R_{+}$. Then there is an element $a\in R_{+}\setminus\mathfrak{a}$. hence $aa^{n-1} = a^n =0\in \mathfrak{a}$ for some $n>0$. It follows that, $\mathfrak{a}$ is not prime, hence $\Proj\ R = \emptyset.$
\end{proof}
\begin{definition}
...
\end{proof}
\begin{lemma}
    Consider a map of graded rings $\sigma : R\rightarrow S$. Then $U=\{\mathfrak{p}\in \Proj \ S : \mathfrak{p}\not\subset \sigma(R_{+})\}$ is an open set
\end{lemma}
\begin{proof}
    Let $\mathfrak{p}\in U$. Then there is an $f\in R_{+}$ not in  $\sigma^{-1}(\mathfrak{p})$. Then $\sigma(f)$ is not in $\mathfrak{p}$. Let $\mathfrak{q}\in D_{+}(\sigma(f))$. Then $\sigma(f)\notin \mathfrak{q}$, hence $\mathfrak{q}\not\supset  \sigma(R_{+})$. It follows that $D_{+}(\sigma(f))\subset U$, hence $U$ is open. 
\end{proof}
The above gives rise to a morphism of schemes, 
\begin{gather*}
    f : U \rightarrow \Spec \ R
\end{gather*}
whose underlying continuous map is given by 
$$
    \mathfrak{p} \mapsto \sigma^{-1}(\mathfrak{p})
$$
and where a component of $f^\#$ takes a section $s\in \pazocal{O}_{\Proj \ R}(V)$ and maps it to the section 
$$
    \mathfrak{p}\mapsto \sigma_{(\mathfrak{p})}(s\circ f(\mathfrak{p}))
$$
where $\sigma_{(\mathfrak{p})}: S_{(f(\mathfrak{p}))} \rightarrow R_{(\mathfrak{p})}$ is the canonical map induced by $\sigma$.
\begin{proposition}
Consider a map of graded rings $\sigma: R\rightarrow S$ with $\sigma_d: R_d\rightarrow S_d$ being an isomorphism for each $d\geq d_0$ for some $d_0\geq 0$. Then $U = \Proj \ S$ and $f : \Proj\ S\rightarrow \Proj\ R$ is an isomorphism. 
\end{proposition}
\begin{proof}
...
\end{proof}
\subsubsection{The Residue Field at a Point}
    \begin{definition}
        Let $(X,\pazocal{O})$ be a scheme and $x$ a point in $X$. Then \emph{the residue field at $x$} is the field 
        $$
            K(x) := \pazocal{O}_x/\mathfrak{m}_x,
        $$
        where $\mathfrak{m}_x$ is the maximal ideal in $\pazocal{O}_x$.
    \end{definition}
    \begin{proposition}
        Let $(X,\pazocal{O})$ be a scheme and $x$ a point in $X$. Consider a field $L$. There is a 1-1 correspondence of morphisms of schemes $\Spec \ L \rightarrow X$ and pairs $(x,\iota_x)$ where $x\in X$ and $\iota_x$ is an embedding $K(x)\hookrightarrow L$.
    \end{proposition}
    \begin{proof}
        Suppose $\varphi: \Spec \ L\rightarrow X$ is a morphism of schemes. Note that $\Spec\ L = \{\ast\}$, hence $\varphi$ is given by mapping the one point of $\Spec \ L$ to some $x\in X$. Since $\varphi$ is a morphism of locally ringed spaces, it follows that $\varphi_P : \pazocal{O}_x\rightarrow L$ is a local ring map, meaning $\ker\ \varphi_P=\varphi_P^{-1}(0)=\mathfrak{m}_x$. We thus get an embedding $K(x) = \pazocal{O}_x/\mathfrak{m}_x\hookrightarrow L$. Note that if $U\subset X$ is an open neighborhood around $x$, then the diagram 
        $$
            \begin{tikzcd}
                \pazocal{O}(U)\arrow[r]\arrow[rd] & L\\
                & K(x)\arrow[u]
            \end{tikzcd}
        $$
        commutes.\\
        Conversely, suppose we are given a point $x$ and an embedding $\iota_x : K(x)\hookrightarrow L$. Let $\varphi: \Spec\ L \rightarrow X$, be given by $\ast\mapsto x$. The structure sheaf of $\Spec L$ consists of $L$ itself. We define a natural transformation 
        $$
            \pazocal{O}\Rightarrow L
        $$ 
        in the following way: When $x\in U\subset X$, define $\pazocal{O}(U)\rightarrow \pazocal{O}_{\Spec\ L}(\varphi^{-1}(U))=L$ to be the composite of $\pazocal{O}(U)\rightarrow K(x)$ and $K(x)\hookrightarrow L$. When $x\notin U$, note that $\pazocal{O}_{\Spec \  L}(\varphi^{-1}(U))=\pazocal{O}_{\Spec \ L}(\emptyset)=0$, which is the terminal object in the category of rings. The map $\pazocal{O}(U)\rightarrow \varphi_\ast\pazocal{O}_{\Spec \ L}(U)$ is then obvious. It is clear that this assignment is natural.
    \end{proof}
    \begin{proposition}
        Let $X$ be a scheme. Every closed irreducible set $Y$ in $X$ has a unique generic point $\zeta_Y\in Y$.
    \end{proposition}
    \begin{proof}
...
\end{proof}
\begin{proposition}
    Consider a projective varierty $V$ with homogeneous coordinate ring $\Gamma^\Pp(V)$. Then 
    $$
        t(V)\simeq \Gamma^\Pp(V).
    $$
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{definition}
    Given a field $K$, \emph{the dual ring of $K$} is the ring 
    $$
        K[\epsilon]/\langle \epsilon^2\rangle.
    $$
\end{definition}
\begin{remark}
    The dual ring is a finitely generated $K$-algebra generated by $\epsilon+\langle \epsilon^2\rangle$. We denote this generator by $\epsilon$ for simplicity (when this does not cause confusion). The dual ring is also a two dimensional vector space over $K$
\end{remark}
\begin{lemma}
    There is one prime ideal of the dual ring of a field $K$, namely the one generated by $\epsilon$.
\end{lemma}
\begin{proof}
    Note that the zero ideal is not prime, since $K[\epsilon]/\langle \epsilon^2\rangle$ has a nilpotent. The only other proper ideals are of the form $\langle \epsilon+\alpha\rangle$ for $\alpha\in K$. Note that for $\alpha\neg 0$,
    $$
        \alpha^{-1}\epsilon(\epsilon+\alpha)=\epsilon.
    $$
    Moreover, 
    $$
        \alpha\epsilon = \alpha\epsilon+\epsilon^2 = \epsilon(\alpha+\epsilon).
    $$
    It follows that $\langle \epsilon\rangle$ is the only prime ideal in the dual ring.  
\end{proof}
\begin{definition}
    Given a scheme $X$, \emph{Zariski tangent space at a point $x\in X$}, is the vector space over $K(x)$, 
    $$
        T_x := \mathrm{Dual}_{K(x)}(\mathfrak{m}_x/\mathfrak{m}_x^2),
    $$
    where $\mathfrak{m}_x$ is the maximal ideal of $\pazocal{O}_{X,x}$.
\end{definition}
\begin{lemma}
    Let $x$ be an element of a scheme $X$ over a field $K$ such that $K(x)\simeq K$. Then 
    $$
        \pazocal{O}_{X,x} \simeq K \oplus \mathfrak{m}_x 
    $$
\end{lemma}
\begin{proof}
    ...
    $$
        \begin{tikzcd}
            0 \arrow[r] & \mathfrak{m}_x\arrow[r] & \pazocal{O}_{X,x} \arrow[r] & K(x)=K \arrow[r] & 0
        \end{tikzcd}
    $$
    since $X$ is a scheme over $K$ there is a morphism $K\hookrightarrow \pazocal{O}_{X,x}$, so we can apply the splitting lemma to obtain an isomorphism $\pazocal{O}_{X,x}\simeq K\oplus \mathfrak{m}_x $
\end{proof}
\begin{proposition}
    Let $K$ be a ring and $X$ a scheme over $K$ and denote the dual numbers of $K$ by $A$. There is a one-to-one correspondence of $K$-morphisms from $\Spec \ A $ to $X$ and points in $X$ whose residue field is isomorphic to $K$ together with an element of $T_x$.
\end{proposition}
\begin{proof}
    Let $f : \Spec \ A \rightarrow X$ be a $K$-morphism. Note that $\Spec \ A = \{\langle \epsilon\rangle\}$. Then the map $f$ takes $\langle \epsilon \rangle$ to some point $x\in X$. Since $A_{\langle \epsilon\rangle}/\mathfrak{m}\simeq K$, in addition, we have a commutative diagram
    $$
        \begin{tikzcd}
            K(x)\arrow[rr,hookrightarrow, "\widetilde{f^\#_x}"] && K\\
            & K \arrow[ul,hookrightarrow] \arrow[ur,"\id"]
        \end{tikzcd}
    $$ 
    Since $\id$ is surjective, it follows that $K(x)\simeq K$. Note that $f^\#_x$ induces a map $\mathfrak{m}_x/\mathfrak{m}_x^2\rightarrow \langle\epsilon\rangle/\langle \epsilon\rangle^2\simeq K$ which is an element of $T_x$. 
    \\
    Conversely, consider a point $x\in X$ such that $K(x)\simeq K$ and a linear map $l: \mathfrak{m}_x/\mathfrak{m}_x^2\rightarrow K(x)=K$. We define 
    \begin{gather*}
        f : \Spec\ A\rightarrow X\\
        \epsilon \mapsto x
    \end{gather*} 
    the structure sheaf of $\Spec\ A$ consists of $A$ itself. When $U\subset X$ is an open set not containing $x$, we see that $f^{-1}(U)=\emptyset$, hence $\pazocal{O}_X(U)\rightarrow f_\star\pazocal{O}_{\Spec\ A }(U)$ has to be the zero map. Suppose $U$ is an open neighborhood of $x$. We define $K$ linear map
    \begin{gather*}
        \beta : \pazocal{O}_{X,x} = K\oplus \mathfrak{m}_x\rightarrow A = K\oplus \langle \epsilon\rangle\\
        a + b \mapsto a + l(b+\mathfrak{m}_x^2)\epsilon 
    \end{gather*}
    One readily verfies by direct computation that this is a multiplicative map and therefor a ring map. Thus upon precomposing this map with $\pazocal{O}(U)\rightarrow \pazocal{O}_{X,x}$ we obtain a natural transformation $\pazocal{O}_X\Rightarrow \pazocal{O}_{\Spec\ A}$.\\
     It remains to check that the above assignments are mutual inverses: Consider a morphim $f: \Spec \ A \rightarrow X$. Denote the point in the image of $f$ by $x$ and the associated $K$-linear map by $l$. Denote the morphism of schemes associated with this pair by $f_{x,l}$. Clearly the underlying continuous maps agree. Consider $a+b\in \pazocal{O}_{X,x}=K\oplus \mathfrak{m}_x$. Note that $f^\#_x$ is a $K$-algebra map, hence it fixes $K$. Note also that if $b\in\mathfrak{m}_x$, then $f^\#_x(b)=\beta \epsilon$ for some $\beta \in K$, hence $l(b+\mathfrak{m}_x^2)= \beta$. Then 
     $$
        f^\#_x(a+b) = a + f^\#_x(b) = a + l(b+\mathfrak{m}_x)\epsilon.
     $$
     Let $U$ be an open neighborhood of $x$. It readily follows that $f^\#_U = \left(f_{x,l}^\#\right)_U$. Checking that $f_{x,l}$ induces $x$ and $l$, for a point $x\in X$ rational over $K$ and an element $l\in T_x$, is a trivial task.    
\end{proof}
