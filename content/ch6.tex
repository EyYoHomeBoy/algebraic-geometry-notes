...
\end{proof}
\begin{lemma}
    Consider a map of graded rings $\sigma : R\rightarrow S$. Then 
    $$
        U=\{\mathfrak{p}\in \Proj \ S : \mathfrak{p}\not\supset \sigma(R_{+})\}
    $$ 
    is an open set.
\end{lemma}
\begin{proof}
...
\end{proposition}
\begin{proof}
    Let $\mathfrak{p}\in \Proj\ S$.  Then there is a homogeneous $f\in S_d$ for $d>1$ such that $f\notin \mathfrak{p}$. Then $f^{d'}\notin \mathfrak{p}$ for some $d'\geq d_0$. Note that $f^{d'}=\sigma(g)$ for some $g\in R_{dd'}$, hence $\sigma(g)\notin \mathfrak{p}$, which implies $\mathfrak{p}\in U$.
\end{proof}
...
\end{definition}
\begin{remark}
    The dual ring is a finitely generated $K$-algebra generated by $\epsilon+\langle \epsilon^2\rangle$. We denote this generator by $\epsilon$ for simplicity (when this does not cause confusion). The dual ring is also a two dimensional vector space over $K$. There is one prime ideal in the dual numbers over $K$ containing $\langle\epsilon\rangle^2$, namely the one generated by $\epsilon$. It follows that $\Spec \ K[\epsilon]/\langle\epsilon^2\rangle = \{\langle \epsilon\rangle\}$.
\end{remark}
\begin{definition}
