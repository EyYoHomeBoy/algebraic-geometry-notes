...
\begin{lemma}
    ...
    Furthermore, $(D_{+}(f),\left.\pazocal{O}\right|_{D_{+}(f)})$ is isomorphic to $\Spec\ R_{(f)}$ as locally ringed spaces. Here $R_{(f)}$ is the subring of degree $0$ elements in $R_{f}$. It follows that $(\Proj\ R,\pazocal{O}_{\Proj\ R})$ is an affine scheme. 
\end{lemma}
\begin{proof}
    ...
    $$ 
    Note that this construction is same as mentioned in Remark~\ref{ConstructionOfNaturalTransformationOfSheavesFrom} so naturally follows. We thus conclude that $D_{+}(f)\simeq \Spec\ R_{(f)}$.
\end{proof}
