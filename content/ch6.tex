...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \item $\left. f_i^\#\right|_{\left.\pazocal{O_Y}\right|_{V_i\cap V_j}} = \left. f_j^\#\right|_{\left.\pazocal{O_Y}\right|_{V_i\cap V_j}}$ for each $i,j$.  
    \end{enumerate} 
...
\begin{proof}
    ...
    \begin{gather*}
        f^\#_V : \pazocal{O}_Y(V) \rightarrow f_\ast\pazocal{O}_X(U)\\
        (\left. s\right|_{V_i\cap V})\mapsto \left( (f_i^\#)_{f_i^{-1}(V_i)\cap f^{-1}(V)}(\left.s\right|_{V_i\cap V})\right)
    \end{gather*} 
    Note that 
\end{proof}
\begin{lemma}
...
\begin{proof}
    ...
    $$
    hence $\left. f_U\right|_W = f_W$. Thus if $V$ is another open affine subset of $X$, then $f_U$ and $f_V$ will agree on $U\cap V$. {\Large check gluing condition on natural transformations}. We thus obtain a morphism of schemes $f: X\rightarrow \Spec \ A$ where $f^\#_U = \left.\sigma_{f^{-1}(U)}\right|_{\pazocal{O}}$. The claim is that $f^\#_{\Spec\ A} = \sigma$
\end{proof}
