...
\subsection{Some Topology}
    ...
    \subsubsection{Dimension of Varieties}
        ...
        \end{proposition}
        \begin{proof}
            Indeed by the primitive element theorem $K(\alpha_1,\dots,\alpha_n)=K(\alpha)$. Then\\ $K[x_1,\dots,x_{r+1}]/\ker \ \ev_{\alpha} \simeq K[x_1,\dots,x_n,\alpha]$, hence setting $V:= V(\ker\ \ev_\alpha)$, we are done.
        \end{proof}
    ...
    \end{proof}
    \begin{proposition}
        Ever $n$-dimensional variety is birationally equivalent to a hypersurface in $\A^{n+1}$ or $\Pp^{n+1}$.
    \end{proposition}
    \begin{proof}
        By {\Large ref/works in positive char?} there is an affine variety... 
    \end{proof}
    \begin{proposition}
