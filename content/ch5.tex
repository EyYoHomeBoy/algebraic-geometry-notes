...
\subsection{Some Topology}
    ...
    \end{proof}
    \begin{corollary}\label{CurveIsBirationallyEquivalentToPlaneCurve}
    ...
    \end{proof}
    \begin{proposition}\label{ThereIsAlwaysABirationalMapToAPlaneCurveWhoseInverseHasTheImageOfAFixedPointInDomain}
    ...
    \end{proof}
    \subsubsection{Dimension of Topological Spaces}
    \begin{definition}
        Let $X$ be a topological space. We define for $n\in \N$,
        $$
            \mathrm{IrrChain}_n(X)
        $$
        to be the set of strictly increasing chains $\{Z_i\}_{i=0}^n$ of irreducible closed sets in $X$.
        Then the \emph{dimension of $X$} is
        $$
            \dim\ X := \sup(\{n \in \N \cup\{\infty\}: \mathrm{IrrChain}_n(X)\neq \emptyset\}).
        $$
    \end{definition}
    \begin{definition}
        A topological space $X$ is called \emph{Noetherian} if every descending chain of closed sets in $X$ stabilizes. 
    \end{definition}
    \begin{lemma}\label{TopologicalSpaceIsNoetherianIfCoveredByNoetherianSubsets}
        Let $X$ be topological space covered by a family of subsets $\{Y_i\}_1^n$ such that each $Y_i$ with the trace topology with respect to $X$ is Noetherian. Then $X$ is Noetherian. 
    \end{lemma}
    \begin{proof}
        Consider a descending chain $Z_1\supset Z_2\supset \dots$ of closed sets in $X$. Then for each $i$, $Y_i\cap Z_1\supset Y_i\cap Z_2\supset \dots$ is a descending chain of closed sets $Y_i$, hence it stabilizes. Then since $Z_j = \bigcup Y_i\cap Z_j$, it follows that $Z_1\supset Z_2\supset \dots$ stabilizes. 
    \end{proof}
    \begin{lemma}
        Let $X$ be a Noetherian topological space, then any subspace $Y\subset X$, where $Y$ is closed in $X$ is Noetherian.
    \end{lemma}
    \begin{proof}
        Let $Z_1 \supset Z_2\supset \dots$ be a descending chain of closed sets in $Y$. This is also a descending chain of closed sets in $X$, hence it stabilizes.
    \end{proof}
    \begin{lemma}
        A variety is Noetherian. 
    \end{lemma}
    \begin{proof}
        We first prove that $\A^n$ is Noetherian for each $n$. Consider a descending chain of closed sets in $\A^n$, $Z_1\supset Z_2\supset \dots$. Then 
        $$
            I(Z_1)\subset I(Z_2)\subset \dots
        $$
        is an ascending chain of ideals in $K[x_1,\dots,x_n]$ which is a Noetherian ring, hence it stabilizes. Since $Z_i=V(I(Z_i))$, it follows that $Z_1\supset Z_2\supset \dots$ stabilizes.\\
        Any algebraic set is Noetherian by the prior lemma. In particular, an affine variety is thus Noetherian.\\  
        Consider an arbitrary variety $X$. This is covered by open affine varieties $\{U_i\}1^n$. It thus follows from Lemma~\ref{TopologicalSpaceIsNoetherianIfCoveredByNoetherianSubsets} that $X$ is Noetherian.  
    \end{proof}
    \begin{proposition}
        A noetherian topological space can be written uniquely as a finite union of irreducible subsets 
        $$
            \bigcup_1^n Y_i,
        $$
        with $Y_i\not\supset Y_j$
    \end{proposition}
    \begin{proof}
        {\Large Not really necessary but can be used to prove the decomposition of algebraic sets into irreducibles}
    \end{proof}
    \begin{theorem}
        Let $V$ be a variety. Then the dimension of $V$ is equal to the Krull dimension of $\Gamma(V)$.
    \end{theorem}
    \begin{proof}
        
    \end{proof}
    \begin{corollary}
        The topological dimension of varieties described here corresponds to the algebro geometric described earlier. 
    \end{corollary}
\subsection{The Study of Curves \& Resolution of Singularities}
    ...
    \subsubsection{Blowing up Points in $\A^2$}
        ...
        $$
            \theta := \left.\psi\right|_{C'} : C'\rightarrow C 
        $$
        This is well-defined by continuity of $\psi$.
        Then $\theta$ is a birational equivalence of $C'$ with $C$ by restricting it to $C'\setminus E$. Note $\theta$ induces an automorphism $K(x,y)=K(C)\simeq K(C')=K(x,z)$, where $y$ is identified with $xz$, i.e $z:= y/x\in K(C)$. We need to prove two things: Firstly, that $C'$ is actually an affine plane curve. Secondly: that $C'$ has nicer multiple points. The phrasing "nicer" is of course informal. One could hope for $C'$ to be non-singular. This however is not the case, but each point in preimage of $P$ under $\theta$ will have only one tangent counted with multiplicity. 
        \begin{lemma}
            Write $C=V(f)$, 
            $$f=\sum_r^n f_i$$
            where $f_i\in K[x,y]$ is a form of degree $i$, $r=m_P(C)$ and $n=\deg\ C$. Then $C'=V(f')$ is an affine plane curve, where
        ...
        \end{proof}
        We have thus produced an affine curve $C'$ birationally isomorphic to $C$. It remains for us to check that the points of $\theta^{-1}(P)$ are all nice points. To be precise, that the multiple point $P$ is blown up into $m_P(C)$ points with one tangent each possibly counted with multiplicity.\\
        WLOG $x$ is not tangent to $f$ at $P$ and 
        $$
            f_r = \prod_1^s (y-\alpha_ix)^{r_i}.
        $$
        With the above assumption we have that following lemma 
        \begin{lemma}\label{BlowupTurnsMultiplePointsToSimplePoints}
            $\theta^{-1}(P)=\{P_1,\dots,P_s\}$ where $P_i = (0,\alpha_i)$ and 
            $$
        ...
        \begin{proof}
            Indeed,
           \begin{align*}
                \theta^{-1}(P) &= \{(a,b)\in C' : a=0\}= \{(a,b)\in \A^2 : f'(0,b)=0\}\\
        ...
        \end{proof}
        \begin{lemma}\label{AffineCaseTechnicalLemma}
            There is an open affine neighborhood $W$ of $P$ in $C$ such that $W':=\theta^{-1}(W)$ is an affine open subvariety in $C'$, $\theta(W')=W$, $\Gamma(W')$ is module finite over $\Gamma(W)$ and $x^{r-1}\Gamma(W')\subset \Gamma(W)$. Here we have implicitly made the identification of $\Gamma(W)$ as a subring of $K(C)$. 
        \end{lemma}
        \begin{proof}
            Write 
            $$
            ...
            $$
                h(P)=h(0,0) = a_{0r}= 1\neq 0,
            $$
            hence $P\in C_{\overline{h}}$. Set $W := C_{\overline{h}}$. We claim that $W'=C'_{\overline{h}}$. Indeed, if $Q=(v_1,v_2)\in \theta^{-1}(W)$. $h(v_1,v_1v_2)\neq 0$. Upon thinking of $\overline{h}$ as an element of $K(C')$, it follows that $Q\in C'_{\overline{h}}$. If $Q=(v_1,v_2)\in C'_{\overline{h}}$, then $\overline{h}(v_1,v_2)=h(v_1,v_1v_2)\neq 0$, note then $\widetilde{\theta}(\overline{h})\in K(C)$ does not vanish on $Q$, hence $Q\in \theta^{-1}(W)$.\\
            Note that from the $K$-algebra map 
            \begin{gather*}
                \widetilde{\theta}: \Gamma(W')\rightarrow \Gamma(W)\\
            \end{gather*}
            ...
            $$
                \widetilde{\theta}(g)=g(x,xz) = \sum_0^d f_i(x,xz) \frac{x^iz^i}{x^i} = \sum_0^d f_i(x,y) z^i = f
            $$
        ...
        \end{proof}
        \begin{remark}
            In general, if $U$ is an open neighborhood and $U'$ is an open set in $C'$ containing $\{P_1,\dots,P_s\}$. Construct $W$ as in the above proof and take $g\in \Gamma(W)$ such that $g(P)\neq 0$ and $g(Q)=0$ whenever $Q\in W\setminus U\cup \theta(W'\setminus U')$. Then $W_g$ is an affine open neighborhood of $P$ contained in $U$ and $W'_g= \theta^{-1}(W_g)$ is an affine open set contained in $U'$. We can thus choose $W$ and $W'$ in the above lemma arbitrarily small. 
        \end{remark}
        \begin{remark}
        ...
        \end{remark}
        \begin{proposition}\label{OrderOfPartialDerivativeAtPlace}
            Suppose $P$ is an ordinary multiple point on $C$ and that $\theta^{-1}(P)=\{P_1,\dots,P_r\}$, with $P_i=(0,\alpha_i)$.
            \begin{enumerate}
        ...
        \end{proposition}
        \begin{proof}
            1. This just follows by induction and Leibniz' rule applied to $f_r$.\\
            2. Indeed,
            \begin{align*}
        ...
        \end{proof}
        \begin{proposition}\label{AffineOrderLemma}
            Let $P$ be an ordinary multiple point on $C$, $\theta^{-1}(P)=\{P_1,\dots,P_r\}$ with $P_i = (0,\alpha_i)$ and $f_r = \prod_1^r l_i$ with $l_i = y-\alpha_i x$. Let $g$ be a plane curve and denote the image of $g$ in $\Gamma(C')\subset \Gamma(C)$ by $\overline{g}$. 
            \begin{enumerate}
    ...
    We proceed to describe the process of blowing up any finite sequence of points on a projective plane curve. Fix points $P_1,\dots,P_t\in \Pp^2$. Assume that $P_i=[a_{i1},a_{i2},1]\in U_3$ for each $i$. We do not lose any generality with this assumption since given any $t$ distinct points there is a change of coordinates moving these points to $t$ distinct points in $U_3$. We set 
    $$
    ...
    \end{proof}
    Apply a change of coordinates so that $P_1 = [0,0,1]$ and the remaining $P_i$'s remain unchanged. This is possible by the two lemmas above. Fix $Q\in E_1$. Assume $Q=[\lambda,1]$ for $\lambda\in K$.  We lose no generality with this assumption, since we can always apply a suitable change of coordinates to generalize to this case.\\ 
    What we aim to do now is construct a specific neighborhood of a point in an $E_1$.  Let $\varphi_3$ denote the identification of $\A^2$ with $U_3\subset \Pp^2$. Set $V=U_3\setminus\{P_2,\dots,P_t\}$, $W:= \varphi_3^{-1}(V)$ and $W' := \psi^{-1}(W) = (\psi\circ \varphi_3)^{-1}(V)$. Consider also the morphism
    \begin{gather*}
        \varphi : W' \rightarrow \Pp^2\times \left(\Pp^1\right)^{t}\\
        (x,z) \mapsto ([x,xz,1], [z,1],f_2([x,xz,1]),\dots,f_t([x,xz,1]))
    \end{gather*}
    Note that $\psi\circ \varphi_3 =\pi \circ \varphi$. Recall that $\psi$ is the map $(a,c)\mapsto (a,ac)$ Set $V':= \varphi(W')$. We immediately see that 
    $$
        V' = B\setminus\left(\bigcup_{i\geq 2} E_i\cup V(x_3)\cup V(y_{12}) \right).
    $$
    We thus conclude that $V'$ is an open neighborhood of $Q$ in $B$ since $\varphi(0,\lambda)=Q$. This shows the case $i=1$. To produce open neighborhoods for points in the other $E_i$, we first move $P_1$ back to its original position via a change of coordinates, and follow the same constuction for $i=2$, $i=3$ and so on. We move on to prove the claim that $B$ is the closure of $G$.
    \begin{lemma}
        $$
            B = \cl(G),
        $$
    ...
    \end{lemma}
    \begin{proof}
        Let $S\supset G$ be a closed set in $\Pp^2\times (\Pp^1)^t$. Then $\varphi^{-1}(S)$ is closed in $W'$, containing $\varphi^{-1}(G)$, which is easily checked to be equal to $W'\setminus V(x)$, hence $\varphi^{-1}(G)$ is open in $W'$ and thus dense in $W'$. We then find that $\varphi^{-1}(S)=W'$, implying $S=V'$. 
    \end{proof}
    \begin{lemma}
        Consider the morphism
        \begin{gather*}
            \phi: \Pp^2\times \left(\Pp^1\right)^t\setminus V(x_3y_{12})\rightarrow \A^2\\
            ([x_1,x_2,x_3],[y_{11},y_{12}],\dots,[y_{t1},y_{t2}])\mapsto \left(\frac{x_1}{x_3},\frac{y_{12}}{y_{11}}\right)
        \end{gather*}
         This morphism restricted to $V'$ is the inverse of $\varphi$. We thus have the following diagram 
\[
\begin{tikzcd}
A^{2} \arrow[r, phantom, "\mathrel{\supset}"] \arrow[d, "\psi"'] &
  [-2em] % <--- gap between col 1 and 2 (shrinks the phantom)
W' \arrow[r, "\varphi"] \arrow[d] &
   % <--- gap between col 2 and 3 (leave this “normal”)
V' \arrow[r, phantom, "\mathrel{\subset}"] \arrow[d] &
  [-2em] % <--- gap between col 3 and 4 (shrinks the phantom)
B \arrow[d, "\pi"] \\
A^{2} \arrow[r, phantom, "\mathrel{\supset}"] &
  [-2em]
W \arrow[r, "\varphi_{3}"] &
  V \arrow[r, phantom, "\mathrel{\subset}"] &
  [-2em]
\mathbb{P}^{2}
\end{tikzcd}
\]
    $\pi$ thus locally looks like $\psi$
    \end{lemma}
    \begin{proof}
        Indeed, for $P=([x,xz,1],[1,z],f_{2}([x,xz,1]),\dots, f_t([x,xz,1]))\in V'$,
        \begin{align*}
            \varphi\phi(P)= \varphi(\frac{x}{1},\frac{z}{1})= P
        \end{align*}
        and for $(x,z)\in \A^2$,
        $$
            \phi\varphi(x,z)=\phi([x,xz,1],[1,z],f_{2}([x,xz,1]),\dots, f_t([x,xz,1]))=(x,z).
        $$
        Fix a point $Q\in B$. When $Q$ sits in some $E_i$ the commutativity of the diagram shows that in a suitable neighborhood around $Q$, $\pi$ corresponds to $\psi$. When $Q\in G\cap \pi^{-1}(U_3)$, set $W:= \varphi_3^{-1}(U\cap U_3)$ and $W'=\psi^{-1}(W)$ define 
        \begin{gather*}
            \varphi: W' \rightarrow B\\
            (x,z)\mapsto ([x,xz,1],(f_1,\dots,f_t)(x,z))
        \end{gather*}  
        Setting $V' := \varphi(W')$ we obtain local description of $\pi$ at $Q$ as something that looks like $\psi$ in an affine chart. 
    \end{proof}
    \begin{lemma}
        Let $C$ be an irreducible projective curve in $\Pp^2$. Set $C_0:= C\cap U$, $C_0':=\pi^{-1}(C_0)$ and $C':=\cl_B(C_0')$. $\pi$ restricts to a birational morphism $\Psi:C'\rightarrow C$, given by the restriction of $\pi$ to $C'$, birationality given via the isomorphism from $C_0'$ to $C_0$. Locally this looks like the map $\theta$ from the affine case. 
    \end{lemma}
    \begin{proof}
        Indeed, $U$ and $\pi^{-1}(U)$ are isomorphic via $\pi$, so this is also the case for $C_0$ and $C_0'$. Let $Q\in C_0$ in some neighborhood $V'\cap C_0$ around $Q$, $\Psi$ looks like $f$ by the prior lemma. 
    \end{proof}
    \begin{theorem}\label{WhenCurveHasOnlyOrdinaryMultiplePointsIsBirationallyEquivalnetToNonsingularCurve}
        Let $C$ be an irreducible projective plane curve with all multiple points ordinary. There is a non-singular projective curve $C'$ birationally equivalent to $C$ with the underlying morphism being onto. 
    \end{theorem}
    \begin{proof}
        Pick $P_1,\dots,P_t$ to be the multiple points of $C$ and construct $C_0,C_0'$ and $C'$ as in the above lemma. We want to investigate the behavior of $P\in \pi^{-1}(P_i)\cap C' = E_i\cap C'$.
    Note that $\varphi_3^{-1}(P_i)$ is a multiple point of $C_\ast$. Thus $\theta^{-1}\varphi_3^{-1}(P_i)$ contain only simple points by Lemma~\ref{BlowupTurnsMultiplePointsToSimplePoints} on a suitable curve $C_\ast'$. On some suitable open neighborhood $V'$ of $P$ in $C'$, $\varphi_3\theta = \pi \varphi$, where $\varphi: W'\rightarrow V'$ is an isomorphism for some open set $W'\subset C_\ast'$. Thus $\varphi^{-1}(P)$ is a simple point in $\theta^{-1}\varphi_3^{-1}(P_i)$ contained in $W'$. Now note $\pazocal{O}_{\varphi^{-1}(P)}(C_\ast') = \pazocal{O}_{\varphi^{-1}(P)}(W') \simeq\pazocal{O}_P(V')= \pazocal{O}_P(C')$, which means $P$ is simple. That it is onto is simple: $C_0$ and $C_0'$ are isomorphic, while $C\cap E_i$ maps to $P_i$.
    \end{proof}
    \begin{remark}
    ...
    \begin{definition}
        The \emph{fundamental points in $\Pp^2$} are 
        $$
            P := [0,0,1],\quad  P' := [0,1,0],\quad  P'' := [1,0,0].
        $$
        The \emph{exceptional lines in $\Pp^2$} are 
        $$
            l := V(z),\quad l' := V(y),\quad l'' := V(x)
        $$
    \end{definition}
    \begin{remark}
        $l'$ and $l''$ intersect in $P$, and $l$ passes through $P'$ and $P''$. 
    \end{remark}
    For this section, we set
    $$
        U := \Pp^2\setminus (l\cup l'\cup l'')=\Pp^2\setminus V(xyz)
    $$
    We define a morphism 
    \begin{gather*}
        Q: \Pp^2\setminus\{P,P',P''\} \rightarrow \Pp^2\\
        [v_1,v_2,v_3]\mapsto [v_2v_3,v_1v_3,v_1v_2]
    \end{gather*}
    The image of $Q$ is $U\cup\{P,P',P''\}$. Indeed, when $[v]\neq P,P',P''$, then $v_i,v_j\neq 0$ for $i\neq j$. If $v_k\neq 0$ for each $k$, then $[v]$ does not vanish on $xyz$. If $v_k=0$ for some $k$, then $v_kv_i=0$ and $v_kv_j = 0$, hence $Q([v])\in \{P,P',P''\}$. Conversely, if $[v]=[v_1,v_2,v_3]\in U$, then $v_i\neq 0$ for each $i=1,2,3$. Then  
    $$
        [v] = Q(Q([v])).
    $$
    Note that $P=Q([1,1,0])$, $P' =Q([1,0,1])$ and $P''=Q([0,1,1])$. Moreover, 
    $$
        Q^{-1}(P)=l\setminus\{P',P''\}, Q^{-1}(P') =l'\setminus\{P,P''\}, Q^{-1}(P'')=l''\setminus\{P,P'\}.
    $$
    Note that the following lemma is true
    \begin{lemma}
        $Q$ is an automorphism of $U$. It induces a birational map $Q:\Pp^2 \dottedarrow \Pp^2$.
    \end{lemma}
    \begin{remark}
        $Q$ is called the \emph{standard quadratic transformation} or the \emph{standard Cremona transformation}.
    \end{remark}
    We fix a projective plane curve $C$ that is not an exceptional line. $ C\cap U$ is an open set in $C$ and a closed in $U$. Note that $Q(C\cap U)$ is then a closed curve in $U$. Define 
    $$
        C' := \cl\left(Q^{-1}(U\cap C)\right)= \cl\left(Q(U\cap C)\right). 
    $$
    With this definition $Q$ restricts to a birational morphism $C'\setminus\{P,P',P''\}\dottedarrow C$. 
    \begin{lemma}
        $$
            C'' = C
        $$
    ...
    \begin{proof}
        First note, that 
        $C\cap U \subset \cl(Q(\cl(Q(C\cap U)))) = C''\implies C=\cl(C\cap U) \subset C''$. Conversely, by continuity, 
        $$
            C'' \subset \cl(QQ(C\cap U )) = \cl(C\cap U)=C.
        $$
    \end{proof}
    Fix an irreducible, homogeneous $f\in K[x,y,z]$ with $C=V(f)$ and set $n:= \deg \ f$. We define the \emph{algebraic transform of $f$} to be 
    $$
        f^Q = f(yz,xz,xy),
    $$ 
    which is homogeneous of degree $2n$. Set $r := m_P(f)$.
    \begin{lemma}
        $r$ is the largest natural number such that $z^r\mid f^Q$. 
    \end{lemma}
    \begin{proof}
        For suitable forms $f_r,\dots, f_n\in K[x,y]$ with $f_r\neq 0$, we may write 
        $$
            f = \sum_{i=r}^n f_i z^{n-i},
        $$
        hence 
        $$
            f^Q =\sum_{i=r}^n f_i(yz,xz)(xy)^{n-i} = z^r \sum_{i=r}^n z^{r-i}f_i(y,x)(xy)^{n-i},
        $$
        and since $z\nmid f_r(y,x)$ and $z\mid f^{r-i}f_i(y,x)$ for every other $i$ the result follows. 
    \end{proof}
    Setting $r' := m_{P'}(f)$ and $r'' := m_{P''}(f)$, we get analogous results for these quantities. I.e., $r'$ is the largest number such that $y^{r'}\mid f^Q$ and $r''$ the largest numbers such that $x^{r''}\mid f^Q$. We can thus write 
        $$
            f^Q = x^{r''}y^{r'}z^rf'
        $$
        where $f'\in K[x,y,z]$ is form not divisible by $x$, $y$ or $z$. This polynomial is called the \emph{proper transform of $f$}. 
    \begin{lemma}
        The proper transform of $f$ has degree $2n-(r+r'+r'')$, $(f')'=f$ and $f'$ is an irreducible polynomial such that $C' = V(f')$. 
    \end{lemma}
    \begin{proof}
        That $\deg\ f' = 2n-(r+r'+r'')$ is apparent from the definition of $f'$. Note that 
        $$
            (f^Q)^Q = (f(yz,xz,xy))^Q= f(xyzx,xyzx,xyzx)=(xyz)^nf,   
        $$
        hence $(f')^Q = x^{n-(r+r')}y^{n-(r+r'')}z^{n-(r'+r'')}f$, implying $(f')' = f^Q$. Consider an irreducible factor $p$ of $f'$. Then $f\mid p^Q$, hence $x^{r''}y^{r'}z^rf'\mid (xyz)^{\deg \ p} p$. It follows that $f'\mid p$, hence $f' = p$. Let $v=[v_1,v_2,v_3]\in Q^{-1}(C\cap U)$. Then $f^Q(v)= 0$, and since $x,y$ and $z$ do not vanish at $v$, we get $f'(v)=0$, hence $V(f')\supset Q^{-1}(C\cap U)=Q(C\cap U)$, so by the typical dimension argument, $V(f')=\cl(Q(C\cap U))=C'$ 
    \end{proof}
    \begin{lemma}
        $$
            m_{P}(f') = n - (r'+r''), \quad m_{P'}(f') = n-(r+r''),\quad m_{P''}(f')=n-(r+r').
        $$
    \end{lemma}
    \begin{proof}
        Note that 
        $$
            f' = \sum_{i=0}^{n-r} f_{r+i}(y,x)x^{n-(r+r'+i)}y^{n-(r+r''+i)}z^i
        $$
        The lowest degree term in the homogeneous decomposition of $f'_\ast$ is thus $f_n(y,x)x^{-r'}y^{-r''}$, which is of degree $n-(r'+r'')$, hence $m_{P}(f')=n-(r'+r'')$. The other cases follow completely analogously.  
    \end{proof}
    \begin{definition}
        A projective plane curve $C$ is at a \emph{good position} if no excepetional line is tangent to $C$ at an exceptional point. 
    \end{definition}
    \begin{lemma}
        If $C$ is in good position, so is $C'$
    \end{lemma}
    \begin{proof}
        We prove only that $L$ is not tangent to $C'$ at $P'$. The proof for $L$ through $P''$ follows by symmetry. Proofs for $L'$ with $P$ and $P''$, and $L''$ with $P$ and $P'$ can also be found by symmetry.\\
        $L$ is tangent at $P'$ if and only if $I(P',f'\cap z)>m_{P'}(f')=n-(r+r'')$ or equivalently 
        $$
            I\left(P',f_r(y,x)x^{n-(r+r')}y^{n-(r+r'')}\cap z\right) > n-(r+r'')
        $$ 
        or equivalently (since $I(P',y^{n-(r+r'')}\cap z)=0$ and $I(P',x^{n-(r+r')}\cap z)=n-(r+r')$)
        $$
            I\left(P',f_r(y,x)\cap z\right)>0,
        $$ 
        and this only happens when $P\in f_r(y,x)\cap z$, or when $f_r(1,0)=0$. Since $C$ is in good position, $y$ is not tangent to $f$ at $P$, hence by a similar string of arguments to the above, $f_r(1,0) \neq 0$, so we conclude that $z$ is not a tangent of $C'$ at $P'$.
    \end{proof}
    We now assume that $C$ is in good position and that $P\in C$. Set 
    $$
        C_0 := (C\cap U)\cup\{P\}.
    $$
    Define 
    $$
        f_\ast' := f(x,xz,1)x^{-r},\quad C_\ast' = V(f_\ast').
    $$
    Consider the morphism 
    \begin{gather*}
        \psi_\ast : \A^2\supset C_\ast'\rightarrow C_\ast\subset \A^2 \\
        (x,z)\mapsto f_\ast(x,xz)
    \end{gather*}
    We further define $\eta := \left. Q\right|_{C_0'} : C_0'\rightarrow C_0$ where $C_0' = C'\setminus V(xy)$.
    \begin{lemma}\label{StandardQuadraticTransformationLocallyLooksLikeAffineBlowUpCase}
        There is a neighborhood $W\subset C^\ast$ of $(0,0)$ and isomorphisms $\varphi: W\rightarrow C_0$ and $\varphi'  : W':= \eta^{-1}(W)\rightarrow C_0'$ such that the diagram
        $$
            \begin{tikzcd}
                C_\ast' \arrow[r, phantom, "\mathrel{\supset}"] \arrow[d, "\psi_\ast"'] &
                [-2em] % <--- gap between col 1 and 2 (shrinks the phantom)
                W' \arrow[r, "\varphi'"] \arrow[d] &
                % <--- gap between col 2 and 3 (leave this “normal”)
                C_0' \arrow[r, phantom, "\mathrel{\subset}"] \arrow[d,"\eta"] &
                [-2em] % <--- gap between col 3 and 4 (shrinks the phantom)
                C' \\
                C_\ast \arrow[r, phantom, "\mathrel{\supset}"] &
                [-2em]
                W \arrow[r, "\varphi"] &
                C_0 \arrow[r, phantom, "\mathrel{\subset}"] &
                [-2em]
                C
            \end{tikzcd}
        $$
    \end{lemma}
    \begin{proof}
        Take $W:=(C_\ast\setminus V(xy))\cup \{(0,0)\}$, 
        \begin{gather*}
            \varphi : W \rightarrow C_0\\
            (v_1,v_2)\mapsto [v_1,v_2,1]
        \end{gather*}
        whose inverse is $[v_1,v_2,v_3]\mapsto (v_1,v_2)$. Indeed, a point in $C_0$ is $[v_1,v_2,v_3]$ where all $v_i\neq 0$ or $v_1=v_2=0$ and $v_3=1$. Thus, $C_\ast\setminus V(xy)$ is in bijection with $C\setminus V(xyz)$ and $(0,0)$ is mapped to $P$. Furthermore, we define
        \begin{gather*}
            \varphi' : W' \rightarrow C_0'\\
            (v_1,v_3)\mapsto [v_3,1,v_1v_3]
        \end{gather*}
        whose inverse is $[v_1,v_2,v_3]\mapsto (v_3/v_1,v_1/v_2)$. Note that 
        \begin{align*}
            \eta\varphi'(a,c)=\eta([c,1,ac])=[ac,ac^2,c]=[a,ac,1]=\varphi(a,ac)=\varphi\psi_\ast(a,c).
        \end{align*}
    \end{proof}
    \begin{lemma}\label{TechnialQuadraticTransformLemma}
        Suppose $P_1,\dots,P_s\in C'\cap L$ are not fundamental points. Then 
        $$m_{P_i}(C')\leq I(P_i,C'\cap L) \text{ and } \sum_{i}I(P_i,f'\cap z)=r.$$
    \end{lemma}
    \begin{proof}
        The first statement is a simple fact about intersection numbers. For the second, 
        $$
            \sum_{i}I(P_i,f'\cap z) = \sum_i I(P_i,f_r(y,x)\cap z) = r.
        $$
    \end{proof}
    \begin{remark}
        In the case $P\notin C$, note that $m_{P}(C)=0$, hence $\sum_v I(v,f'\cap z) = 0$, hence in this case there are no non-fundamental points in $C'\cap L$.  
    \end{remark}
    \begin{definition}
        A curve $C$ is in \emph{excellent position}, if it is in good position, $L$ intersects $C$ in $n$ distinct points all different from the fundamental points, and $L'$ and $L''$ intersect $C$ in $n-r$ distinct points all not fundamental.  
    \end{definition}
    We proceed with a classification of the multiple points of $C'$ when $C$ is in excellent position. 
    \begin{lemma}\label{TheOrdinaryPointsOfACurveInExcellentPosition}
        Suppose $C$ is in excellent position. $C'$ has the following multiple points.
        \begin{enumerate}
            \item Those in $C'\cap U$ are in one-to-one correspondence with those in $C\cap U$ and this correspondence preserves multiplicity and ordinary multiple points. 
            \item $P,P',P''$ are multiple points of $C'$ with $m_P(C')=n$, $m_{P'}(C')=m_{P''}(C')=n-r$.
            \item $C'\cap L'$ and $C'\cap L''$ contain only non-fundamental points. Let $P_1,\dots,P_s$ be the non-fundamental points on $C'\cap L$. Then $m_P(C')\leq I(P_i,C'\cap L)$ and $\sum_1^s I(P_i, C'\cap L)=r$.
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        1. $C\cap U\simeq C'\cap U$ via $\eta$. The local rings of these varieties thus agree at each point, so their multiplicities agree. Furthermore, being an ordinary multiple point is also completely determined by properties of the associated local ring {\Large Haven't done this exercise yet! Do at some point}, hence this is also preserved by the isomorphism.\\
        2. A prior lemma shows that since $C$ is in good position so is $C'$ and that $(C')'=C$. The assumption of $C$ being in excellent position implies 
        $$
            m_P(C') = \sum_{v\neq P,P',P''} I(v,(C')'\cap L)= \sum_{v\neq P,P',P''} I(v,C\cap L)= n
        $$
        a similar computation shows that $m_{P'}(C')=m_{P''}(C')=n-r$. {\Large Why are they ordinary}\\
        3. Note that $r\leq I(P,C\cap L')$ and that $\sum_v I(v,C\cap L') = n$. By the assumption that $C$ is excellent position, we must have $I(P,C\cap L') \leq r$, hence $I(P,C\cap L')=r$. It thus follows that $I(P'',C\cap L')=0$ and hence $P''\notin C$, which implies $C'\cap L'$ contains no non-fundamental points. A symmetric argument shows that $C'\cap L''$ contains no non-fundamental points.  $m_P(C')\leq I(P_i,C'\cap L)$ and $\sum_1^s I(P_i, C'\cap L)=r$ follows from Lemma~\ref{TechnialQuadraticTransformLemma}
    \end{proof}
    \begin{definition}
        For an irreducible projective curve $C$ of degree $n$, we define 
        $$
            g^\ast(C) := \frac{(n-1)(n-2)}{2}-\sum_{P\in C}\frac{m_P(C)(m_P(C)-1)}{2}
        $$
    \end{definition}
    \begin{lemma}
        If $C$ is excellent position, then 
        $$
            g^\ast(C')= g^\ast(C)-\sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2},
        $$
        where $P_1,\dots,P_s$ are the non-fundamental points in $C'\cap L$ 
    \end{lemma}
    \begin{proof}
        Indeed,
        \begin{align*}
            g^\ast(C') &= \frac{(2n-r-1)(2n-r-2)}{2}-\sum_{v\in C'}\frac{m_v(C')(m_v(C')-1)}{2}\\
            &=  \frac{(n-1)(n-2)}{2}+\frac{3n^2-3n-4nr+3r+r^2}{2}-\sum_{v\in C'\cap U}\frac{m_v(C)(m_v(C)-1)}{2}\\
            & -\frac{n(n-1)}{2}-\frac{2(n-r)(n-r-1)}{2}-\sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2}\\
            &= \frac{(n-1)(n-2)}{2}-\sum_{v\in C\cap U}\frac{m_v(C)(m_v(C)-1)}{2}-\frac{r(r-1)}{2}-\\ 
            &\sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2}\\
            &= \frac{(n-1)(n-2)}{2} - \sum_{v\in C\cap U}\frac{m_v(C)(m_v(C)-1)}{2} \\& -\sum_{v\in C\cap V(xyz)} \frac{m_v(C)(m_v(C)-1)}{2}- \sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2}\\
            &= \frac{(n-1)(n-2)}{2} - \sum_{v\in C} \frac{m_v(C)(m_v(C)-1)}{2} - \sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2}\\
            &= g^\ast(C)-\sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C')-1)}{2}
        \end{align*}
    \end{proof}
    We now abandon $P,P',P''$ and $L,L',L''$ being notation for special objects. The whole point of considering a curve in excellent position is that this can easily be transformed into a curve with only ordinary multiple points. The upshot is that any irreducible projective plane curve is can be transformed into one in excellent position. This puts us in a position where we can resolve the singularities of any projective plane curve. 
    \begin{lemma}\label{ThereIsAlwaysAQuadraticTransformation}
        Suppose $\Char\ K =0$. Let $f$ be any irreducible projective plane curve and $P\in f$ any point. There is a change of coordinates $T=$ such that $f^T$ is in excellent position and $T([0,0,1])=P$.
    \end{lemma}
    \begin{proof}
        If we can find lines $l,l',l''$ such that 
        $L'\ic C = rP+\sum_1^{n-r} P_i'$ and $L''\ic C = rP+\sum_1^{n-r} P_i''$, $L\ic C  =\sum_1^n P_i$ with $P,P_1,\dots,P_n,P_1',\dots,P_{n-r}',P_{1}'',\dots,P_{n-r}''$ are all distinct, then by Lemma~\ref{ChangeOfCoordinatesFOrHyperplanes} we can find a change of coordinates $T$ such that $L^T = z$, $(L')^T=y$ and $(L'')^T=x$. Then since $T(P)\in x\cap y$, hence $T(P)=[0,0,1]$. By Proposition~\ref{ThereAreInfinitelyManyLinesThroughAPointOnACurvePassingIntersectingTransversallyForTheRemainingIntersections} (this is where the assumption of $\Char\ K=0$ is used) we can find $L'$ and $L''$. For $L$ just take any simple point on $C$ different from $P$ and any $P_i'$ and $P_i''$ and apply the same result. 
    \end{proof}
    \begin{definition}
        For a projective change of coordinates $T$ on $\Pp^2$ as in the above lemma, the \emph{quadratic transform} of $T$ is $QT$. Let $f$ be any irreducible projective plane curve and $P\in f$ any point, the irreducible projective plane curve 
        $$(f^T)'$$
        is the \emph{quadratic transformation of $f$}. If $f^T$ is in excellent position and $T([0,0,1])=P$, then the quadratic transformation of $f$ is said to be \emph{centered at $P$}.\\
        Given a sequence of irreducible projective plane curves, $f=f_1,\dots,f_m=g$ such that $f_i$ is a quadratic transformation of $f_{i-1}$, then we say that $f$ is \emph{transformed} $g$ via finite sequence of quadratic transformations. 
    \end{definition}
    \begin{theorem}\label{HowToTransformCurveWithNonOrdinaryMultiplePointsIntoOneWithOnlyOrdinaryMultiplePoints}
        Suppose $\Char\ K =0$. Any irreducible projective plane curve $C$ may be transformed into an irreducible projective plane curve with only ordinary multiple points via a finite sequence of quadratic transformations. 
    \end{theorem}
    \begin{proof}
        We describe an algorithm for producing such a sequence of quadratic transformations. 
        \begin{enumerate}
            \item If $C$ has only ordinary multiple points, terminate, otherwise continue to 2.
            \item Pick a non-ordinary multiple point $P$ on $C$ and change the value of $C$ to be a quadratic transformation centered at $P$ (one such exists by the prior lemma), go to 1.
        \end{enumerate}
        Each step maps $P$ to the ordinary multiple point $[0,0,1]$. If $C'$ is the quadratic transformation of $C$ centered at $P$, then if $C'$ has the same number of non-ordinary multiple points as $C$ by Lemma~\ref{TheOrdinaryPointsOfACurveInExcellentPosition} one of these must be among the non-fundamental points $P_1,\dots,P_s$ of $C'$, hence 
        $$
            g^\ast(C')=g^\ast(C)-\sum_1^s \frac{m_{P_i}(C')(m_{P_i}(C)-1)}{2} < g^\ast(C).
        $$
        It thus follows that this algorithm must terminate in most $N+g^\ast(C)$ steps. 
    \end{proof}

    \subsubsection{Non-singular Models of Curves}
    \begin{lemma}\label{TechnicalNonsingularModelLemma}
        Suppose $\Char\ K =0$. Let $f$ be any irreducible projective plane curve and $P\in f$ any point. 
        \begin{enumerate}
            \item Let $S\subset f$ be any finite set of points not containing $P$. There is a quadratic transform $T$ of $f$ centered at $P$ with $T^{-1}(S)\cap V(xyz)=\emptyset$.
            \item When $T$ is given as in 1. there is a neighborhood of $S$ on $f$ isomorphic to an open set in $(f^T)'$.
            \item If $S$ contains only simple points of $f$, then there is a curve $f'$ that only has ordinary multiple points and neighborhood $U$ of $S$ on $f$ isomorphic to an open set $U'$ on $f'$ containing only simple points on $f'$. 
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        1. In the proof of Lemma~\ref{ThereIsAlwaysAQuadraticTransformation} we may also pick $L,L',L''$ such that non of these pass through any point in $S$, since are also infinitely many such lines.\\
        2. $f\setminus(L\cup L'\cup L'')$ is an open neighborhood of $S$, and  
        \begin{align*}
            T^{-1}(f\setminus (L\cup L'\cup L'')) &= f^T\setminus V(xyz) = f^T \cap (\Pp^2\setminus V(xyz))\\
            &\simeq (f^T)' \cap (\Pp^2\setminus V(xyz))
        \end{align*} 
        where the last isomorphism is via $Q$. {\Large Check}\\ 
        3. In the algorithm described in Theorem~\ref{HowToTransformCurveWithNonOrdinaryMultiplePointsIntoOneWithOnlyOrdinaryMultiplePoints} only pick quadratic transformations of the form described in 1. i.e. at each step apply 1. on $f_{i-1}$ and $S_{i-1}$ with $S_1=S$ and $S_i=T^{-1}(S_{i-1})$ when $i>1$, and $f_1=f$ and $f_i = (f_{i-1}^T)'$.
    \end{proof}
    \begin{example}
        The goal of this example, for a given quadratic transformation $T$ centered at a point $P\in C$, is to compute $\deg \ (C^T)'$ and $g^\ast((C^T)')$ when 
        \begin{enumerate}
            \item $P$ is an ordinary multiple point on $C$.
            \item $P$ is simple on $C$
            \item $P$ is not in $C$. 
        \end{enumerate}
        1. In this case $\deg \ (C^T)' = 2n-m_P(C)$, since $[1,0,0],[0,1,0]\notin C^T$. Moreover, locally around $P$, $Q$ looks like $\theta$, and $((C^T)')_\ast\simeq (C^T_\ast)'$, hence by the affine case $\theta^{-1}((0,0))$ consist of only simple points. These correspond to the non-fundamental points of $(C^T)'\cap z$, hence 
        $$
            g^\ast((C^T)') = g^\ast(C)
        $$  
        2. Here $\deg\ (C^T)' = 2n-1$. We compute,
        \begin{align*}
            g^\ast((C^T)') = g^\ast(C^T) - \sum_{1}^s \frac{m_{P_i}((C^T)')(m_{P_i}((C^T)')-1)}{2} = g^\ast(C)
        \end{align*}
        Note for the above that $[1,0,0]$ and $[0,1,0]$ are ordinary multiple points of multiplicity $n-1$ summing to $2n-2$. There is thus at most one non-fundamental point in $(C^T)'\cap z$ and if it exists it must be simple.\\  
        3. When $P\notin C$, we get $\deg \ (C^T)' = 2n$. Moreover, 
        \begin{align*}
            g^\ast((C^T)') &= g^\ast(C^T)-\sum_1^s \frac{m_{P_{i}}((C^T)')(m_{P_{i}}((C^T)')-1)}{2} = g^\ast(C^T)=g^\ast(C)
        \end{align*}  
        The last equality follows from the fact that when $[0,0,1]\notin C^T$, there are no non-fundamental points in $(C^T)'\cap z$
    \end{example}
    The upshot of having considered this example is the following 
    \begin{lemma}\label{TechnicalNonsingularModelLemmaCanBeChosenWithLargeDegree}
        We may pick $f'$ in Lemma~\ref{TechnicalNonsingularModelLemma} 3. of arbitrarily large degree. I.e. given any $N\geq 0$ there is an $f'$ satisfying the conditions of Lemma~\ref{TechnicalNonsingularModelLemma} with $\deg \ f'\geq N$. 
    \end{lemma}
    \begin{proof}
        Given the $f'$ obtained in 3. apply a series of suitable quadratic transformation centered at any point (at this point there will be no non-ordinary multiple points) until the degree is larger than $N$. 
    \end{proof}
    \begin{definition}
        \emph{A nonsingular model} of a projective curve $C$ is a nonsingular, projective curve $X_C$ with a birational map $\Phi:X_C\dottedarrow C$.   
    \end{definition}
    \begin{theorem}
        Let $C$ be any projective curve. There is a nonsingular model of $C$, $\Phi : X_C \dottedarrow C$. The underlying morphism of any such nonsingular model is onto. If $\Phi' : Y_C \dottedarrow C$ is another such pair, then the following diagram commutes
        $$
            \begin{tikzcd}
                X_C\arrow[d, dashrightarrow,"\exists!\Psi"'] \arrow[rd,"\Phi"]\\
                Y_C \arrow[r,"\Phi'"'] & C
            \end{tikzcd}
        $$
        $X_C$ is thus the intial object in the category $C/\mathrm{NonSingCur}^\mathrm{dom}_K$ where $\mathrm{NonSingCur}^\mathrm{bir}_K$ are nonsingular curves over $K$ with dominant rational maps as morphisms. 
    \end{theorem}
    \begin{proof}
        \textbf{Uniqueness (up to unique isomorphism):} Two such birational morphisms $\Phi$ and $\Phi'$, induce isomorphisms $\widetilde{\Phi} : K(C)\rightarrow K(X_C)$, $\widetilde{\Phi'} : K(C)\rightarrow K(Y_C)$, hence $\widetilde{\Psi} := \widetilde{\Phi}\widetilde{(\Phi')}^{-1}$. This is the unique map of fields such that 
        $$
            \begin{tikzcd}
                K(X_C)\\
                K(Y_C)\arrow[u,dashrightarrow]\arrow[r,"\widetilde{\Phi'}^{-1}"'] & K(C)\arrow[ul,"\widetilde{\Phi}"'] 
            \end{tikzcd}
        $$ 
        commutes. There is a unique birational map $\Psi$ inducing $\widetilde{\Psi}$. $\Psi^{-1}$ is constructed using the same argument with $\widetilde{\Phi}^{-1}$ and $\widetilde{\Phi'}$ and noting that all simple points of $X_C$ and $Y_C$ are all contained in the domain of $\Psi$ resp. $\Psi^{-1}$.\\
        \textbf{Existence:}  By Corollary~\ref{CurveIsBirationallyEquivalentToPlaneCurve}, $C$ is birationally equivalent to a plane curve. We can pick this with only ordinary multiple points by Theorem~\ref{HowToTransformCurveWithNonOrdinaryMultiplePointsIntoOneWithOnlyOrdinaryMultiplePoints}, hence by Theorem~\ref{WhenCurveHasOnlyOrdinaryMultiplePointsIsBirationallyEquivalnetToNonsingularCurve}, $C$ is birationally equivalent to a nonsingular curve.\\ 
        \textbf{$\Phi$ is onto:} Consider first the case $C\subset \Pp^2$. Let $P\in C$ by 2. of the prior lemma, we get a quadratic transformation, with $(C^T)'\dottedarrow C^T$ having $T^{-1}(P)$ in its domain. The repeated application of this fact as in 3. of the prior lemma shows that there is $C'$ with only ordinary multiple points birationally equivalent to $C$ with a point $P'$ in the domain of this birational map such that this is mapped to $P$. Constructing $X_C$ from $C'$ by Theorem~\ref{WhenCurveHasOnlyOrdinaryMultiplePointsIsBirationallyEquivalnetToNonsingularCurve} results in a birational map $X_C\dottedarrow C'$ which is onto, hence $\Phi : X_C\rightarrow C$ maps some $P'$ to $P$. The universal property of nonsingular models shows that for any other nonsingular model $\Phi': Y_C\dottedarrow C$, there is a point $P''$, with $\Phi'(P'')=P$. Any nonsingular model must then have an underlying map that is onto.\\
        We extend this to the general case, $C\subset \Pp^n$. Let $P\in C$. We may choose a birational map $\Theta : C\rightarrow C_{\mathrm{Plane}}$ with $C_\mathrm{Plane}$ being a plane curve such that $\Theta^{-1}(\Theta(P))=\{P\}$ (cf. Proposition~\ref{ThereIsAlwaysABirationalMapToAPlaneCurveWhoseInverseHasTheImageOfAFixedPointInDomain}). Let $\Phi :X \rightarrow C_\mathrm{Plane}$ be a nonsingular model for $C_\mathrm{Plane}$. For some $P'$, $\Phi(P')=\Theta(P)$, hence $\Theta^{-1}\Phi(P')=P$.
    \end{proof}

    \begin{corollary}
        $K(\_) : \mathrm{NonSingCur}_K \rightarrow \mathrm{AlgFuncField}_K^1$ is fully faithul, here the codomain is Algebraic function fields over $K$ in one variable.     
    \end{corollary}

    \begin{remark}
        The points of a nonsingular model $\Phi:X\dottedarrow C$ of a curve $C$ are in one-to-one correspondence with discrete valuation rings of $K(C)$ (here we canonically identify $K(C)$ with $K(X)$). Note then that $\Phi(Q)=P\in C$ exactly when $\pazocal{O}_Q(X)$ dominates $\pazocal{O}_P(C)$.
    \end{remark}
    \begin{definition}
        Let $(X,\Phi)$ be a nonsingular model of curve $C$. Then a point on $X$ is called a \emph{place} on $C$ or on $K(C)$. A place $Q\in X$ is \emph{centered} at $P\in C$ if $\Phi(Q)=P$.
    \end{definition}

    \begin{lemma}\label{SecondTechnicalNonsingularModelLemma}
        Let $C$ be a projective plane curve, $(X,\Phi)$ a nonsingular model of $C$ and $P\in C$. There is an affine neighborhood open $U$ of $P$ such that 
        \begin{enumerate}
            \item $U':=\Phi^{-1}(U)$ is an affine open subvariety of $X$.
            \item $\Gamma(U')$ is finite as a $\Gamma(U)$-module.
            \item There is a non-zero $t\in \Gamma(U)$ such that $t\Gamma(U)\subset \Gamma(U')$.
            \item $\dim_K \Gamma(U')/\Gamma(U)<\infty$.
        \end{enumerate}
        $U$ may be chosen such that it excludes an arbitrary set points $S\subset C$ with $P\notin S$.
    \end{lemma}
    \begin{proof}
        We first argue that we can find a series of quadratic transformations\\ $C_1=C,\dots,C_N$ such that there are affine open sets $W_1\subset C_1,\dots, W_N\subset C_N$ such that
        \begin{enumerate}[a)]
            \item $P\in W_1$ and $S\cap W_1 = \emptyset$.
            \item  The underlying morphism of $f_i:C_{i+1}\dashrightarrow C_i$ are $f_i:W_{i+1}\rightarrow W_i$.
            \item $W_{i+1} = f^{-1}(W_i)$ are affine and $\Gamma(W_{i+1})$ is module finite over $\Gamma(W_i)$ and $t\Gamma(W_i)\subset \Gamma(W_{i+1})$ for some $t\neq 0$. 
        \end{enumerate}
        Consider first the case where $N=2$. First pick an open neighborhood $V$ of $P$ in $C$ and a quadratic transformation $(C_2,T)$ as in Lemma~\ref{TechnicalNonsingularModelLemma} 2. Note that when restricted to $(C_2)_\ast=(C^T)'_\ast$, $Q: (C_2)_\ast\rightarrow C^T_\ast$ looks like $\theta : (a,c)\mapsto (a,ac)$. By Lemma~\ref{AffineCaseTechnicalLemma} there is an affine, open neighborhood $V_0$ of $[0,0,1]$ in $C^T_\ast$ such that $W_2 := \theta^{-1}(V_0)$ an affine neighborhood with $\Gamma(W_{2})$ module finite over $\Gamma(V_0)$ and $t\Gamma(V_0)\subset \Gamma(W_{2})$ for some $t\neq 0$. In the remark following this lemma, we know that can pick $V_0\subset T^{-1}(V)$ and $W_2\subset \left.Q\right|_{C_2}^{-1}T^{-1}(V)$, hence picking $W_1:= T(V_0)$, we get the desired $C_1,C_2$, $W_1,W_2$ and $f_1: C_1\dashrightarrow C_2$ when taking into account that the Zariski topology is Frechét, which means we can pick this neighborhood away from $S$. Now suppose a), b) and c) for some sequence of quadratic transformations are satisfied, using the same argument as before we find a quadratic transformation of $C_N$ centered at $f_{N-1}\cdots f_{1}(P)$ with an open affine neighborhood $W'\subset W_{N}$ and an affine open neighborhood $W_{N+1}$ satisfying b) and c). Replace $W_{N-1}$ with $f_i^{-1}(W')$, $W_{N-2}$ with $f_{i-1}^{-1}(W_{N-1})$ and so on, and we get the desired sequence of quadratic transformations.\\ 
        We pick such a sequence $C_1,\dots, C_N$ with $C_N$ having only ordinary multiple points. Take $f_N : X\dashrightarrow C_N$ to be the nonsingular model of $C_N$. Let $W_{N+1} := f_N^{-1}(W_N)$, we shrink the preceding neighborhoods as before so that $W_{N+1}$ has the desired compatibility with the preceding $W_i$. $f_N$ looks like $(a,c)\mapsto (a,ac)$ locally, so we get the module finiteness condition for these open affine sets as well. Set $U':= W_{N+1}$ and $U = W_1$. Being module finite is a transitive property, so $\Gamma(U')$ is finite as a $\Gamma(U)$-module.\\
        write 
        $$
            \Gamma(U') := \sum_1^m \Gamma(U)z_i\quad (z_i\in \Gamma(U')).
        $$ 
        Since $Q(\Gamma(U))=Q(\Gamma(U'))$, we may find a non-zero $t\in \Gamma(U)$ such that $tz_i\in \Gamma(U)$. It thus follows that $t\Gamma(U)\subset \Gamma(U')$.\\
        We define 
        \begin{gather*}
            \rho : \Gamma(U')/\Gamma(U) \rightarrow \Gamma(U)/\langle t\rangle\\
            z+ \Gamma(U) \mapsto tz+\langle t\rangle
        \end{gather*}
        which is an injective $K$-linear map. It thus suffices to prove that $\Gamma(U)/\langle t\rangle$ is finite-dimensional by rank-nullity. Since $t$ has only finitely many zeroes (its pole set is an algebraic subset of $U$, hence it is finite) it follows from the Nullstellensatz (more specifically Corollary~\ref{DimensionOfCoordinateRing}) that $\dim \ \Gamma(U)/\langle t\rangle$.  
    \end{proof}
    
    \begin{proposition}\label{IntersectionFormula}
        Let $C=V(f)$ be an irreducible projective plane curve, $P\in C$, $\Phi: X\dashrightarrow C$ a nonsingular model of $C$. Let $g$ be a plane curve. Then 
        $$
            I(P,g\cap C) = \sum_{v\in \Phi^{-1}(P)} \ord_v(g)
        $$
    \end{proposition}
    \begin{proof}
        Let $\overline{g}$ denote the image of $g_\ast\in K(\Pp^2)$ in $K(C)$. Pick $U$ and $U'$ as in Lemma~\ref{SecondTechnicalNonsingularModelLemma} with $P\in U$ and $U\setminus \{P\}\subset \{P':\overline{g}(P')\neq 0\}$. We may do this since there are only finitely many points on which $\overline{g}$ vanish, hence for an initial choice of $U$, replace it with something contained in $U$ excluding these points. We have thus chosen $U$ such that $\overline{g}\in \pazocal{O}_{P'}(C)$ is a unit for each $P'\in U$ with $P'\neq P$. Then 
        \begin{align*}
            I(P,C\cap g)=\dim \ \pazocal{O}_P(\Pp^2)/\langle f_\ast, g_\ast\rangle = \dim\ \pazocal{O}_P(C)/\langle \overline{g}\rangle = \dim \ \Gamma(U)/\langle \overline{g}\rangle,
        \end{align*}
        where the second equality follows from Proposition~\ref{IsomorphimOfLocalRingsOfRationalFunctions}. For the last equality note that 
        $$
            \Gamma(U) \twoheadrightarrow \pazocal{O}_P(C)/\pazocal{O}_P(C)\overline{g}
        $$
        has kernel $\Gamma(U) \overline{g}$. Indeed, if $z\in \Gamma(U)$ is in  $\pazocal{O}_P(C) \overline{g}$, then $z= h\overline{g}$ for some $h\in \pazocal{O}_P(C)$. Note that $z\in \pazocal{O}_{P'}(C)$ for $P' \in U\setminus \{P\}$, hence $h = z\overline{g}^{-1} \in \pazocal{O}_{P'}$, meaning $z\in \Gamma(U)\overline{g}$. {\large Fulton claims that Corollary~\ref{DimensionOfLocalQuotientFromIdealsWithFiniteZeroes} applies here. I sort of does, I guess, but really I think we need something like Corollary~\ref{FiniteDimensionalAlgebrasIsAFiniteProductOfLocalRings}, which is the not mandatory problem 2.47. This approach is less simple than what I did above.}
        Set $V= \Gamma(U)$ and $V'=\Gamma(U')$. Consider $\rho : V\rightarrow V', z\mapsto \overline{g}z$.
        Since $V/V'$ is finite dimensional, we get 
        $$
            \dim \ V/\langle \overline{g}\rangle = \dim \ V'/\langle \overline{g}\rangle 
        $$
        By (an appropriate version of) Corollary~\ref{DimensionOfLocalQuotientFromIdealsWithFiniteZeroes} and Lemma~\ref{OrderIsDimensionOfQuotientKAlg}
        $$
            \dim \ V'/\langle \overline{g}\rangle = \sum_{v\in \Phi^{-1}(P)} \dim\ \pazocal{O}_v/\langle \overline{g}\rangle = \sum_{v\in \Phi^{-1}(P)} \ord_v(\overline{g}).
        $$
    \end{proof}

    \begin{lemma}
        Suppose $P$ is an ordinary multiple point on an irreducible projective plane curve $C$ of multiplicity $r$ with nonsingular model $\Phi:X\dashrightarrow C$. Write $\Phi^{-1}(P)=\{P_1,\dots,P_r\}$. If $z\in K(C)$ with $\ord_{P_i}(z)\geq r-1$, then $z\in \pazocal{O}_P(C)$. 
    \end{lemma}
    \begin{proof}
        Pick $U\ni P$ such that $z\in \pazocal{O}_{P'}(C)$ for each $P'\in U\setminus \{P\}$ and such that $\Phi: \Phi^{-1}(U)\rightarrow \Phi$ looks like $\theta$ in Lemma~\ref{AffineCaseTechnicalLemma}. By Lemma~\ref{BlowupTurnsMultiplePointsToSimplePoints}, $\ord_{P_i}(x)=1$, hence $zx^{1-r}\in \Gamma(U')$. Since $x^{r-1}\Gamma(U')\subset \Gamma(U)$, we get $z=x^{r-1}(zx^{1-r})\in \Gamma(U)\subset \pazocal{O}_P(C)$.
    \end{proof}

    \begin{proposition}\label{NoethersConditionInTermsOfDivisors}
        Let $f$ be an irreducible projective plane curve, $P$ an ordinary point of multiplicity $r$ on $f$. Let $P_1,\dots,P_r$ be the places centered at $P$. For plane curves $g,h$, Noether's conditions are satisfied at $P$ with respect to $f,g,h$, if $\ord_{P_i}(h)\geq \ord_{P_i}(g)+r-1$ for each $i$. 
    \end{proposition}
    \begin{proof}
        Note that $h_\ast\in \langle f_\ast,g_\ast\rangle \subset \pazocal{O}_P(\Pp^2)$ if and only if $z:= \frac{\overline{h_\ast}}{\overline{g_\ast}}\in \pazocal{O}_P(f)$. Since $\ord_{P_i}(z) = r-1$ for each $i$, it follows that $z\in \pazocal{O}_P(f)$ by the above lemma.  
    \end{proof}
    \begin{lemma}\label{OrderAtPlacesCenteredAtOrdinaryMultiplePoint}
        Let $\Phi : X\rightarrow C$ be the nonsingular model of an irreducible projective plane curve. Let $P\in C$ be an ordinary multiple point with multiplicity $r$ and places $Q_1,\dots,Q_r$ on $X$ centered at $P$. Let $G$ be a projective plane curve and $s\leq r$. $m_P(g)\geq s$ if and only if $\ord_{Q_i}(\overline{g})\geq s $ for $i=1,\dots,r$.  
    \end{lemma}
    \begin{proof}
        We pick $U\ni P$ in the usual way such that $\Phi$ locally looks like $\theta$ (from the affine case). By Lemma~\ref{AffineOrderLemma} 2. $\ord_{Q_i}(\overline{g})\geq s$ for each $i$ implies $m_P(g)\geq s$. Conversely, for any $i$, if $m_P(g)\geq s$ we get that $\ord_{Q_i}(g)\geq s$, since $\ord_{P_i}(g)\geq m_P(g)$ by 1. of Lemma~\ref{AffineOrderLemma}.
    \end{proof}
    \begin{lemma}\label{TechnicalNonSingularCurveLemma}
        Let $X$ be a nonsingular projective curve, $P_1,\dots,P_s\in X$. 
        \begin{enumerate}
            \item There is a projective plane curve with only ordinary multiple points whose nonsingular model is $\Phi : X\rightarrow C$ with $\Phi(P_i)$ simple in $C$ for each $i$.
            \item For any finite sequence $m_1,\dots,m_s\in \Z$ there is a $z\in K(X)$ such that $\ord_{P_i}(z)=m_i$. 
            \item $C$ of part 1. may be found of arbitrarily large degree. 
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        1. There is a birational map $\Psi : X \dashrightarrow Y$ to some irreducible projective plane curve $Y$. Suppose we can construct a curve $C$ and a birational map $\phi : C\dottedarrow Y$ such that $\phi^{-1}(\Psi(P_i))$ contain only simple points for each $i$. Let $\Phi : X'\dottedarrow Y$ be a non-singular model of $Y$ that factors through $C$ via some $\Phi' : X'\dottedarrow C$ and $\phi$. Then we have a commutative diagram
        $$
            \begin{tikzcd}
                X' \arrow[r,"\Phi'"] & C\arrow[d,"\phi"] \\
                X \arrow[u,dashrightarrow, "\exists! \varphi"] \arrow[ru,"\eta"] \arrow[r,"\Psi"] & Y 
            \end{tikzcd}
        $$ 
        where $\eta := \Phi\varphi$. Then 
        $$
            \eta(P) = \Phi(\varphi(P_i)) \in \phi^{-1}(\Psi(P_i)),
        $$
        since 
        $$
            \phi\eta(P_i) = \Psi(P_i).
        $$
        It thus follows that $\eta : X\dottedarrow C$ is a birational map to projective plane curve with only ordinary multiple points such that $\eta(P_i)$ is simple for each $i$.\\
        Consider first the case $n=1$ and set $P:= P_1$. When $\Psi(P)$ is simple, we can apply Lemma~\ref{TechnicalNonsingularModelLemma} with respect to $S=\{\Psi(P)\}$ to obtain the desired curve $C$ such that $\phi^{-1}(\Psi(P))$ is a simple point. Suppose $\Psi(P)$ is an ordinary multiple point. Take a quadratic transformation centered at $\Psi(P)$ and denote the resulting quadratic transformation of $Y$ by $C_1$ and the birational map $C_1\dottedarrow C$ by $\phi$. Note that $\phi^{-1}(\Psi(P)) = C_1\cap z$ contain only simple points. This is seen from Lemma~\ref{StandardQuadraticTransformationLocallyLooksLikeAffineBlowUpCase} and Lemma~\ref{BlowupTurnsMultiplePointsToSimplePoints}. More precisely, locally the standard quadratic transformation looks like $(a,c)\mapsto (a,ac)$ and the fiber of an ordinary multiple point with respect to this map consists exclusively of simple points. We obtain the desired curve $C$ by applying Lemma~\ref{TechnicalNonsingularModelLemma} to $C_1$ with respect to $S = \phi^{-1}(\Psi(P))$. Suppose $\Psi(P)$ is a non-ordinary multiple point. Apply Lemma~\ref{TechnicalNonsingularModelLemma} 3. to obtain a plane curve $C_1$ with only ordinary multiple points and a birational map $\phi_1 : C_1\dottedarrow C$. Let $\{R_1,\dots,R_t\}$ be the subset of ordinary multiple points in $\phi_1^{-1}(\Psi(P))$. Take a quadratic transformation centered at $R_t$, resulting in a birational map $\phi_2 : C_2\dottedarrow C_1$ with $\phi_2^{-1}(R_t)$ all simple. Note that $\phi_2^{-1}(\{R_1,\dots,R_t\})$ is a set containing at most $t-1$ points, by induction there is a plane curve $C$ with only ordinary multiple points and a birational map $\phi' : C\rightarrow C_1$ such that $(\phi')^{-1}(\{R_1,\dots,R_t\})$ contain only simple points. We then set $\phi := \phi'\phi_1$.\\
        2. From 1. there is an irreducible projective plane curve $C$ and a birational map $\Phi: X\rightarrow C$ such that $\Phi(P_1),\dots,\Phi(s)$ are simple in $C$. It Follows from Lemma~\ref{ExistenceOfRationalFunctionWithSpecificOrderAtSpecificPoints} that there is a $z\in K(C)\simeq K(X)$ such that 
        $$
            \ord_{P_i}(z)=\ord_{\Phi(P_i)}(z) = m_i 
        $$
        for each $i$, where we on the left-hand side mean the order of the element in $K(X)$ corresponding to $z$.\\
        3. Each time we apply Lemma~\ref{ExistenceOfRationalFunctionWithSpecificOrderAtSpecificPoints}, apply Lemma~\ref{TechnicalNonsingularModelLemmaCanBeChosenWithLargeDegree} instead. 
    \end{proof}
    \subsection{Riemann-Roch}
    \subsubsection{Divisors}
    \begin{definition}
        Let $X\rightarrow C$ be a non-singular model of an irreducible projective curve $C$. A \emph{divisor on $X$}
        is an element in $\Z[X]$.\\
        The degree of a divisor $d = \sum_{P\in X} n_P P\in \Z[X]$, is the quantity
        $$
            \deg \ d := \sum_{P} n_P
        $$
    \end{definition}
    \begin{remark}
        Note that $\deg\ (d+d') = \deg  d + \deg  d'$.
    \end{remark}
    \begin{definition}
        A divisor $d = \sum_{P\in X} n_P P $ is \emph{effective} or \emph{positive}, if $n_P\geq 0$ for each $P$. 
    \end{definition}
    \begin{definition}
        For a pair of divisors $\sum_{P\in X} n_P P$ and $\sum_{P\in X} m_P P$ we write 
        $$
            \sum_{P\in X} n_P P \geq \sum_{P\in X} m_P P
        $$
        if $n_P\geq m_P$ for each $P$. 
    \end{definition}
    \begin{definition}
        Suppose $C$ is a plane curve of degree $n$ and $g$ is a plane curve not containing $C$ as a component of degree $m$. The \emph{divisor of $G$}, is defined to be 
        $$\mathrm{div}(G) := \sum_{P\in X} \ord_P(g)P.$$  
    \end{definition}
    \begin{remark}
        By Proposition~\ref{IntersectionFormula} and Bezout,
        $$
            \deg \ \mathrm{div}(g) = \sum_{P\in X} \ord_P(g) = \sum_{P'\in C} I(P',C\cap g) = nm.
        $$
    \end{remark}
    \begin{definition}
        Let $z\in K(X)\setminus 0$. \emph{The divisor of $z$} is defined to be 
        $$
            \Div(z) := \sum_{P\in X} \ord_P(z)P.
        $$
    \end{definition}
    \begin{remark}
        Note that $\{P : z(P) = 0\}$, corresponding to the points with positive order, and $\pazocal{P}(z)$, corresponding to points with negative order, are both finite sets, hence $\Div(z)\in \Z[X]$. With this in mind 
        $$
            (z)_0 := \sum_{P\in X : \ord_P(z)>0} \ord_P(z)P
        $$
        is called the \emph{divisors of zeros of $z$}, and 
        $$
            (z)_\infty := \sum_{P\in X: \ord_P(z)<0} -\ord_P(z)P,
        $$
        the \emph{divisors of poles of $z$}. We thus have 
        $$
            \Div(z) = (z)_0-(z)_\infty.
        $$
    \end{remark}
    \begin{lemma}
        For $z,z'\in K(X)$, we have $\Div(zz')=\Div(z)+\Div(z')$ and $\Div(z^{-1})=-\Div(z)$
    \end{lemma}
    \begin{proof}
        This follows from basic properties of the order function.
    \end{proof}
    \begin{lemma}
        For any $z\in K(X)\setminus 0$,
        $$
            \deg \ \Div(z) = 0.
        $$
    \end{lemma}
    \begin{proof}
        Write $z = \frac{f+I(C)}{g+I(C)}\in \Gamma^\Pp(C)$ for suitable homogeneous polynomials $f,g$ of the same degree, $m$ say. Say $C$ has degree $n$. Then 
        $$
            \Div(z)=\Div(f)-\Div(g)= mn -mn =0.
        $$
    \end{proof}
    \begin{corollary}\label{DivisorOfRationalFunctionIsZeroIffInBaseField}
        For a $z\in K(X)\setminus 0$ the following are equivalent
        \begin{enumerate}
            \item $\Div(z)\geq 0$.
            \item $z\in K$.
            \item $\Div(z) = 0$. 
    ...
    \end{corollary}
    \begin{proof}
        Suppose $\Div(z) \geq 0$. Then $\ord_P(z)\geq 0$ for each $P$, hence $z\in \pazocal{O}_P(C)$ for each $P$. Fix $P_0\in C$. Then $z-z(P_0) = 0$ or $\deg\ \Div(z-z(P_0))=0$. Note that $z-z(P_0)\in \pazocal{O}_P(C)$ for each $P$, meaning $\ord_P(z-z(P_0))\geq 0$ for each $P$, and that $\ord_{P_0}{z-z(P_0)}>0$. Then $z-z(P_0)=0$, hence $z\in K$. 
    \end{proof}
    \begin{corollary}
        Consider $z,z'\in K(X)\setminus 0$. Then $\Div(z)=\Div(z')$ if and only if $z=\lambda z$ for some $\lambda\in K$. 
    \end{corollary}
    \begin{proof}
        $\Div(z)=\Div(z')$ if and only if $\Div(z(z')^{-1})=0$, which is equivalent to $z(z')^{-1}\in K$. This is equivalent to $z=\lambda z'$ for some $\lambda \in K$. 
    \end{proof}
    \begin{definition}
        Two divisors $d,d'\in \Z[X]$ are \emph{linearly equivalent} if $d=d+\Div(z)$ for some $z\in K(X)$. In this case we write $d\equiv d'$.
    \end{definition}
    \begin{remark}
        Being linearly equivalent is an equivalence relation. Indeed, $d= d+ \Div(1)$ for each $d\in \Z[X]$. Suppose $d = d'+ \Div(z)$ for some $z\in K(X)\setminus 0$. Then 
        $$
            d' = d-\Div(z) = d+ \Div(z^{-1}).
        $$
        Suppose $d,d',d''\in \Z[X]$ are divisors with $d=d'+\Div(z)$ and $d' = d''+\Div(z')$ for some $z,z'\in K(X)\setminus 0$. Then 
        $$
            d = d' + \Div(z) = d'' + \Div(z')+\Div(z) = d'' + \Div(zz').
        $$
    \end{remark}
    \begin{lemma}
        The following statements hold for divisors
        \begin{enumerate}
            \item For each $d\in \Z[X]$,  $d\equiv 0$ if and only if $d=\Div(z)$ for some $z\in K(X)\setminus 0$.
            \item For each $d,d'\in \Z[X]$, $d\equiv d'$ implies $\deg\ d = \deg \ d'$.
            \item For each $d_1,d_1',d_2,d_2'\in \Z[X]$, if $d_1 \equiv d_1'$ and $d_2\equiv d_2'$, then 
            $$
                d_1 + d_2 \equiv d_1' + d_2'.
            $$
            \item For $d,d'\in \Z[X]$. $d\equiv d'$ if and only if there are two curves $g,g'$ of the same degree such that $d+\Div(g) = d' +\Div(g').$
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        1. This is trivial, since $d\equiv 0$ is equivalent to $d = 0 + \Div(z) = \Div(z)$ for some $z\in K(X)$ by definition.\\
        2. If $d\equiv d'$, then for some $z\in K(X)\setminus 0$,  
        $$
            d=d'+\Div(z)
        $$
        hence 
        $$
            \deg \ d = \deg \left( d'+\Div(z)\right)= \deg \ d' + \deg \ \Div(z) = d'.
        $$
        3. Indeed, if $d_1 \equiv d_1'$ and $d_2\equiv d_2'$, then $d_1 = d_1'+\Div(z_1)$ and $d_2 = d_2'+\Div(z_2)$ for some $z_1,z_2\in K(X)\setminus 0$. Then 
        $$
            d_1+d_2 = d_1'+d_2'+\Div(z_1z_2),
        $$
        hence $d_1+d_2\equiv d_1'+d_2'$.\\
        4. If $d' = d+ \Div(z)$, then $z = \frac{\overline{g}}{\overline{g'}}$ where $g,g'$ are projective plane curves of the same degree and $\overline{g},\overline{g'}$ are images of $g$ and $g'$ in $\Gamma^h(C)$. Then $\Div(z) = \Div(g)-\Div(g')$, hence $d + \Div(g) = d' +\Div(g')$.\\
        Conversely, if $d + \Div(g) = d' + \Div(g')$ for same degree curves. Then choosing $z = \frac{g}{g'}$, we get $d = d' + \Div(z)$. 
    \end{proof}

    \begin{definition}
        Suppose $C$ has only ordinary multiple points. We define the \emph{minimal adjoint of $C$} to be the divisor
        $$
            E_C := \sum_{Q\in X} (m_{\Phi(Q)}(C)-1)Q.
        $$
        An \emph{adjoint of $C$} is any projective plane curve $g$ with 
        $$
            \Div(g) \geq E_C
        $$ 
    \end{definition}
    \begin{remark}
        Note that 
        \begin{align*}
            \deg E_C &= \sum_{Q\in X} (m_{\Phi(Q)}(C)-1) = \sum_{P\in C}\sum_{Q\in \Phi^{-1}(P)} (m_{\Phi(Q)}-1)= \sum_{P\in C} \sum_{Q\in \Phi^{-1}(P)} (m_{P}(C)-1)\\ 
            &= \sum_{P\in C} m_{P}(C)(m_P(C)-1) \geq 0,
        \end{align*}
        hence $E_C$ is effective, hence any adjoint of $C$ is effective. 
    \end{remark}
    \begin{lemma}
        A projective plane curve $G$ is adjoint to $C$ if and only if 
        $$
            m_P(g) \geq m_P(C)-1
        $$ 
        for every multiple point $P\in C$. In particular. if $C$ is non-singular, then every curve is an adjoint to $C$.
    \end{lemma}
    \begin{proof}
        For each multiple point $P$ apply Lemma~\ref{OrderAtPlacesCenteredAtOrdinaryMultiplePoint} with $s = m_P(C)-1$. This says that $m_P(g) \geq s$ is equivalent to $\ord_{Q}(g)\geq s$ for each place centered at $P$. For each multiple point $P\in C$, having $\ord_Q(g)\geq m_{P}$ for each place centered at $P$ is by definition equivalent to $g\geq E_C$. 
    \end{proof}
    \begin{theorem}
        Consider linearly equivalent divisors, $d,d'\in \Z[X]$. Let $g$ be an adjoint of $C$ of degree $m$ with
        $$
            \Div(g) = d + E_C+a
        $$ 
        for some effective divisor $a\in \Z[X]$. Then there is an adjoint $d'$ of $C$ of degree $m$ such that 
        $$
            \Div(g') = d'+E_c+a.
        $$
    \end{theorem}
    \begin{proof}
        Since $d\equiv d'$, we may find curves $h,h'$ such that $d+\Div(h) = d' + \Div(h')$. Note that 
        $$
            \Div(hg) = \Div(h)+d + E_c +a =\Div(h')+d' + E_c+a\geq \Div(h')+ E_C.
        $$
        This shows that $\ord_{Q}(hg)\geq \ord_Q(h')+m_P(C)-1$ for each $Q\in \Phi^{-1}(P)$, hence Noether's condition is satisfied for each $P\in C$ by Proposition~\ref{NoethersConditionInTermsOfDivisors}. Noether's theorem show that there are curves $f',g'$ such that 
        $$
            gh = f'f + g'h'
        $$
        with $\deg \ g' = m$. It follows that 
        \begin{align*}
            \Div(g') &= \Div(gh)-\Div(ff')-\Div(h') = \Div(gh)-\Div(h')= \Div(h')+d'+E_C+a-\Div(h')\\ 
            &= d'+E_C+a
        \end{align*}
    \end{proof}
    \subsubsection{The Vector Spaces $L_d$}
    \begin{definition}
        For a divisor $d= \sum_{P\in X} n_P P\in \Z[X]$, define 
        $$
            L_d := \left\{ f\in K(X) : \ord_P(f) \geq -n_P \text{ for every } P \in X\right\}
        $$
    \end{definition}
    \begin{lemma}
        $L_d$ is a vector space over $K$ for each $d\in \Z[X]$. 
    \end{lemma}
    \begin{proof}
        We prove that $L_d$ is $K$ subspace of $K(X)$. Write $d = \sum_{P\in X} n_PP$ and suppose $f,g\in L_d$ and $a,b\in K$. Then for each $P\in X$,
        $$
            \ord_P(af+bg) \geq \min(\ord_P(af),\ord_P(bg)) = \min(\ord_P(f),\ord_P(g))\geq -n_P,
        $$ 
        hence $af+bg\in L_d$. Trivially $\ord_P(0)=\infty\geq -n_P$ for each $P\in X$, hence $0\in L_d$. 
    \end{proof}
    \begin{remark}
        We denote the dimension of the vector space $L_d$ by $l_d$. 
    \end{remark}
    \begin{lemma}\label{BasicDivisorVectSpaceLemma}
        \begin{enumerate}
            \item For divisors $d,d'$ on $X$, if $d\leq d'$, then $L_d\subset L_{d'}$ and 
            $$
                \dim_K \ L_{d'}/L_{d} \leq \deg \ (d-d').
            $$ 
            \item $L_0 = K$ and $L_d=0$ for divisors $d$ on $X$ with $\deg \ d < 0$.
            \item For a divisor $d$ on $X$, if $\deg \ d \geq 0$, then $l_d \leq (\deg \ d)+1$. It follows that for every divisor $d$ on $X$, $\dim_K \ L_d<\infty$.
            \item For a pair of divisors $d,d'$ on $X$, if $d\equiv d'$, then $L_d = L_{d'}$.   
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        For each instance of $d$ write $d= \sum_P n_P P$ and each instance of $d'$, $d' = \sum_P n_P'P$.\\
        1. If $f\in L_d$, then $\ord_P(f) \geq -n_P\geq -n_P'$ for each $P\in X$, hence $f\in L_{d'}$. Note there is a finite sequence of points $P_1,\dots,P_s\in X$ such that 
        $$
            d' = d+\sum_1^s P_i,
        $$
        where $s = \deg \ (d'-d)$. Set $V_0 := L_d$ and $V_i := L_{d+\sum_1^i P_i}$ for $1\leq i\leq s$. Then 
        $$
            \dim \ L_{d'}/L_{d} = \sum_0^s \dim\ V_{i+1}/V_i.
        $$
        To prove that $\dim \ L_{d'}/L_d \leq s $, it thus suffices to prove that $\dim \ L_{d+P}/L_d\leq 1$ for an arbitrary point $P\in X$. Let $t$ denote the uniformizing parameter in $\pazocal{O}_P(X)$. We define a $K$-linear map 
        \begin{gather*}
            \lambda : L_{d+P}\rightarrow K\\
            f\mapsto (t^{n_P+1}f)(P)
        \end{gather*} 
        Since $\ord_P\left(t^{n_P+1}f\right) \geq n_P+1-n_P-1=0$ for each $f\in L_{d+P}$, this is well-defined. If $f\in L_d$, then 
        $$
            \ord_P\left(t^{n_P+1}f\right)\geq n_P+1-n_P=1,
        $$
        it follows that $\lambda(f)=0$. Conversely, if $f\in L_{d+P}$ is given with $(t^{n_P+1}f)(P)=0$, then $1\leq \ord_P(t^{n_P+1}f) = n_P+1+\ord_P(f)$, hence $\ord_P(f) \geq -n_P$. We thus conclude that $\ker \ \lambda = L_d$. It follows that 
        $$
            L_{d+P}/L_d \hookrightarrow K \implies \dim \ L_{d+P}/L_d = \dim \ \im \ \lambda \leq 1.
        $$
        2. Corollary~\ref{DivisorOfRationalFunctionIsZeroIffInBaseField} shows that $\Div(f)\geq 0$ if and only if $f\in K$, hence $L(0) = K$. Suppose $\deg \ d <0$ and let $P\in X$. If $f\in L_d$, then $\ord_P(f)\geq -n_P>0$, hence $f(P)=0$, meaning $f=0$.\\
        3. Let $P\in X$ and set $d' := d - (\deg \ D+1)P$. Then $\deg \ d' < 0$, hence $L_{d'}=0$. Then
        $$
            l_d = \dim \ L_d = \dim \ L_d/L_{d'} \leq \deg \ d +1.
        $$
        4. By assumption $d = d' + \Div(g)$ for a curve $g$. Set 
        \begin{gather*}
            \lambda : L_d \rightarrow L_{d'}\\
            f\mapsto fg
        \end{gather*}
        This is a $K$ linear map with inverse $h\mapsto \frac{h}{g}$, hence $l_d = l_{d'}$ 
    \end{proof}
    \begin{definition}
        Let $S\subset X$ and $d=\sum_{P\in X} n_PP\in \Z[X]$. We define the \emph{degree of $d$ in $S$} to be 
        $$
            \deg^S \ d := \sum_{P\in S} n_P.   
        $$
        We also define 
        $$
            L^S_d := \left\{ f\in K(X) : \ord_P(f) \geq -n_P \text{ for each } P \in S\right\}
        $$
    \end{definition}
    \begin{lemma}\label{DimensionLVectorSpacesWTRASubset}
        Conider divisors on $X$, $d,d'$ with $d\leq d'$ and a subset $S\subset X$. Then $L^S_d \subset L_{d'}^S$. If $S$ is finite, then 
        $$
            \dim\ L_{d'}^S/L_d^S = \left(\deg^S\ d-d'\right).
        $$
    \end{lemma}
    \begin{proof}
        The first statement is proven similarly to the proof of 
        $$
            d\leq d'\implies L_d\subset L_{d'}
        $$
        (cf. Lemma~\ref{BasicDivisorVectSpaceLemma} 1.). For the second statement it suffices to prove 
        $$
            \dim\ L_{d+P}^S/L_d^S = 1
        $$
        for an arbitrary $P\in X$. We prove that 
        \begin{gather*}
            \lambda : L^S_{d+P} \rightarrow K
        \end{gather*}
        is surjective (again cf. the proof of Lemma~\ref{BasicDivisorVectSpaceLemma}). Write $d = \sum_{Q} n_Q Q$. It suffices to prove that $\lambda$ is not the zero map. To do this it suffices to find a $z\in K(X)$ with $\ord_P(z)=-n_p'P-1$ and $\ord_Q(z)\geq -n_Q$ for all $Q\in S$. Such a rational function exists by Lemma~\ref{TechnicalNonSingularCurveLemma} 2. 
    \end{proof}
    \begin{lemma}\label{RationalFunctionInLVectorSpaceHasSingularity}
        Consider a divisor $d=\sum_{P\in X} n_P P$ on $X$. Let $z\in K(X)$ be non-constant. If $z\in L_d$ is, then $\ord_P(z)<0$ for some $P\in X$. 
    \end{lemma}
    \begin{proof}
        Suppose $\ord_P(z)\geq 0$ for each $P\in X$. Then in particular $\ord_P(z)>0$ for some $P$. Moreover, $\ord_P(z^{n+1}) >\ord_P(z^n)$ for each $n\geq 1$ and $\{z^n : n\geq 1\}$ is linearly independent over $K$. If $z\in L_d$, then $z^n\in L_d$ for each $n\geq 0$. Since $L_d$ is finite dimensional it thus follows that $z\notin L_d$.
    \end{proof}
    \begin{proposition}\label{TechnicalGenusLemma}
        Let $z\in K(X)$ that is non-constant. Set $n := [K(X):K(z)]$. Then
        \begin{enumerate}
            \item $(z)_0 = \sum_{P\in X} n_P P$ is an effective divisor of degree $m\leq n$.
            \item There is a constant integer $\tau$ such that 
            $$
                l_{r(z)_0} \geq rn-\tau
            $$
            for all $r$.
    ...
    \end{proposition}
    \begin{proof}
        1. Set $S:= \{P\in X : n_P>0\}$. For $v\in L^S_0$ let $\overline{v}$ denote the image of $v$ in $L^S(0)/L^S_{-(z)_0}$. Note that by Lemma~\ref{DimensionLVectorSpacesWTRASubset}
        $$
            \dim \ L^S_0/L^S_{-(z)_0} = m
        $$
        Pick a basis $\{\overline{v_1},\dots,\overline{v_m}\}$ of $L^{S}(0)/L^S(-(z)_0)$ over $K(z)$. Suppose for a contradiction that 
        $$
            \sum_1^m \frac{a_i}{b_i}v_i  = 0
        $$
        for some $\frac{a_i}{b_i}\in K(z)$ not all $0$. Multiplying the left-hand side by $\prod b_i$, we get 
        $$
            \sum_1^m g_iv_i = 0.
        $$
        we may pick these $g_i$ of the form 
        $$
            \lambda_i + zh_i
        $$
        with $\lambda_i\in K$ and $h_i\in K[z]$ such that $\lambda_i\neq 0$ for some $i$ or $g_i=0$ for all $i$. It follows that 
        $$
            \sum_1^m \lambda_i v_i = -z\sum_1^m h_iv_i\in L_{-(z)_0}^S. 
        $$
        But then $\sum_1^m \lambda_i \overline{v_i} = 0$, leading to a contradiction. It thus follows that $m\leq n$.\\
        2. Fix $r\geq 0$. Pick a basis $\{w_1,\dots,w_n\}$ for $K(X)$ over $K$. By Lemma~\ref{AlgebraicFuntionFieldsInOneVariable} 1. each $w_i$ is algebraic over $K(z)$. For a fixed $i$ there is a polynomial $g_i=x^{n_i}+\sum_0^{n_i-1} a_{ij}x^{j}$ in $K(z)[x]$ vanishing on $w_i$. Multiplying by a sufficiently large power of $z^{-1}$, we may assume it in $K[z^{-1}][x]$. We then have $\ord_P(w_i) \geq0$ for $P\notin S$ by Lemma~\ref{TechnicalDVRLemma}, since 
        $$
            w_i^{n_i}+\sum_0^{n_i-1} a_{ij}w_i^{j} = 0
        $$
        and $\ord_P(a_{ij}) \geq 0$, which implies 
        $$
            n_i\ord_P(w_i) = \ord_{P}(a_{ij})+(n_i-1)\ord_P(w_i) \implies \ord_P(w_i) = \ord_P(a_{ij})\geq0. 
        $$
        Set 
        $$
            t := \max_{i,P\in S} \ord_P(w_i).
        $$
        Then for each $i$, $\div(w_i) \geq -t(z)_0$. It thus follows that for each $i\in\{1,\dots,n\}$ and $j\in \{0,\dots,r\}$, that
        $$
            \div(w_iz^{-j}) \geq -(t+r)(z)_0 \implies w_iz^{-j}\in L_{(t+r)(z)_0}.
        $$
        Since $\{w_1,\dots,w_n\}$ is linearly independent over $K(x)$ and $\{1,x^{-1},\dots,x^{-r}\}$ is linearly independent over $K$, it follows that 
        $$
            \left\{w_ix^{-j} : i\in \{1,\dots,n\}, j\in\{0,\dots,r\}\right\}
        $$
        is linearly independent over $K$. Then 
        $$
            l_{(r+t)(z)_0} = \dim L_{(t+r)(z)_0} \geq n(r+1).
        $$
        Moreover, 
        $$
            l_{(r+t)(z)_0} = l_{r(z)_0} + \dim \ L_{(r+t)(z)_0}/L_{r(z)_0}\leq l(r(z)_0)+tm.
        $$
        For the first equality we use rank-nullity and for the second we use Lemma~\ref{BasicDivisorVectSpaceLemma} 1. Setting $\tau:= tm-n$ we are done, since then 
        $$l_{r(z)_0}\geq l_{(r+t)(z)_0}-tm \geq n(r+1)-tm=nr-\tau.$$
        3. By 3. of Lemma~\ref{BasicDivisorVectSpaceLemma}
        $$
            rn-\tau \leq l_{r(z)_0} \leq rm+1\implies r(n-m) \leq \tau+1.
        $$
        Since this holds for any $r$, we must have $n=m$.
    \end{proof}
    \begin{corollary}\label{EquivalentFormulationsOfBeingRational}
        The following are equivalent
        \begin{enumerate}
            \item $C$ is rational.
            \item $X$ is isomorphic to $\Pp^1$.
            \item There is a $z\in K(X)$ with $\deg\ (z)_0 = 1$. 
            \item For some $P\in X$, $l_P>1$.
        \end{enumerate}
    \end{corollary}
    \begin{proof}
        "1. $\iff$ 2.": $C$ is rational if and only if it is birationally equivalent to $\Pp^1$ which is equivalent to $X$ being birationally equivalent to $\Pp^1$. Two non-singular curves are birationally equivalent if and only if they are isomorphic.\\ 
        "2. $\implies$ 3.":  If $X$ is birationally equivalent to $\Pp^1$, then $K(X)\simeq K(\frac{x}{y})$. In particular $[K(X) : K(\frac{x}{y})]=1$, hence $\deg \ (x)_0=1$.\\
        "3. $\implies$ 2.": Let $z\in K(X)$ with $\deg \ (z)_0 = 1$. Then $[K(X): K(z)]=\deg \ (z)_0 = 1$, hence $K(X)= K(z)\simeq K(x)=Q(K[x])$, hence $X$ is birationally equivalent to $\Pp^1$.\\
        "4. $\implies$ 1.": Note that since $P\geq 0$, $l_P\leq 2$, hence $l_P=2$. Then there is a non-constant $z\in L_P$. By Lemma~\ref{RationalFunctionInLVectorSpaceHasSingularity}, $(z)_\infty = P$. Then 
        $$
            \deg \ (z)_0 = \deg\ (z)_\infty = 1. 
        $$ 
        Then $[K(X):K(z)]=1$, hence $C$ is rational.\\
        "1. $\implies$ 4.": Suppose $C$ is rational.  Then $K(X)\simeq K(\Pp^1) = K(t)$ with $t = \frac{x}{y}\in Q(K[x,y])$. We have that $\Div(t) = [0,1]-[1,0]$, hence $l_{[1,0]}=2$.     
    \end{proof}
    \begin{lemma}\label{SmallerDivisorImpliesSmallerDimension}
        For divisors $d,d'\in \Z[X]$, if $d\leq d'$, then $l_{d'} \leq l_{d}+\deg \ (d'-d)$
    \end{lemma}
    \begin{proof}
        We apply rank-nullity to the surjective $K$-linear map 
        $$
            \pi: L_{d'} \twoheadrightarrow L_{d'}/L_d
        $$
        and get 
        $$
            l_{d'} = \dim \ (\ker \ \pi) + \dim \ (\im\ \pi) = \dim \ L_d + \dim \ L_{d'}/L_{d}\leq  l_d+\deg \ (d'-d). 
        $$
    \end{proof}
    \begin{lemma}\label{LVectorSpaceNonZeroImpliesLinEquivToEffectiveDivisor}
        Consider a divisor $d\in \Z[X]$. $l_d>0$ if and only if $d$ is linearly equivalent to an effective divisor. 
    \end{lemma}
    \begin{proof}
        If $l_d >0$, then there is a non-zero $z\in L_d$. Then $\Div(z) \geq -d$, hence 
        $$
            d = (\Div(z)+d)+\Div(z^{-1}) \implies d \equiv \Div(z)+d.
        $$
        Conversely,  $d = d' + \Div(z)$ for some effective divisor $d'\in\Z[X]$ and some $z\in K(X)$. Since $d'$ is effective $\Div(z)\leq d$, hence $\Div(z)\leq d$, meaning $\Div(z^{-1}) \geq d$, implying $0\neq z^{-1} \in l_d$.
    \end{proof}
    \begin{lemma}
        Consider a divisor $d\in \Z[X]$ with $l_d>0$. Let $z\in L_d\setminus 0$. $z\notin L_{d-P}$ for all but finitely many $P\in X$. It follows that $l_{d-P} = l_d-1$ for all but finitely many $P\in X$.
    \end{lemma}
    \begin{proof}
    ...
    \begin{definition}
        Let $C$ be an irreducible projective plane curve with non-singular model $X$. The \emph{genus} of $C$, denoted $g(C)$ or $g(X)$, is the smallest integer with 
        $$
            l_d \geq \deg \ d + 1 -g
        $$
        for each $d\in \Z[X]$. 
    \end{definition}
    \begin{lemma}
        Suppose $C$ is an irreducible projective plane curve. Define 
        $$
            s(d) := \deg \ d +1 -l_d\quad (d\in \Z[X])
        $$
        Then 
        \begin{enumerate}
            \item If $C$ has genus $g$, then $s(0)=0$ and $g\geq 0$.
            \item If $d\equiv d'$, then $s(d)=s(d')$.
            \item If $d\leq d'$, then $s(d)\leq s(d')$.
        \end{enumerate}
    \end{lemma}
    \begin{proof}
        1. is trivial.\\
        2. If $d\equiv d'$, then $\deg \ d = \deg \ d'$ and $l_d = l_{d'}$, so $s(d)=s(d')$.\\
        3. By Lemma~\ref{SmallerDivisorImpliesSmallerDimension}
        $$
            l_{d'} \leq l_d + \deg \ d' -\deg \ d, 
        $$
        hence
        $$
            s(d) = \deg \ d + 1 - l_d  \leq \deg \ d' +1 - l_{d'} = s(d').
        $$
    \end{proof}
    \begin{lemma}\label{ExistenceOfGenusObservingDivisor}
        Consider a non-constant rational function $z$ on $X$. For any divisor $d\in \Z[X]$ there is a divisor $d'\in \Z[X]$ with $d\equiv d'$ and an integer $r\geq 0$ such that 
        $$
            d'\leq r(z)_0
        $$
    \end{lemma}
    \begin{proof}
        Write $(z)_0 = \sum_{P\in X} n_PP$ and $d=\sum_{P\in X} m_P P$. Set $t:= z^{-1}$ and consider the set 
        $$
            T:= \{P\in X : m_P>0 \text{ and } \ord_P(t)\geq 0\}.
        $$
        Set $f := \prod_{P\in T} (t-t(P))^{m_P}$. For $P\in X $ with $\ord_P(y)\geq 0$ note that $P$ is a zero of $(y-y(P))^{m_P}$, hence $\ord((y-y(P))^{m_P})>m_P$. For such $P$ we thus have $m_P-\ord_P(f) \leq 0 = n_P$. Set $d' = d-\Div(f)$. When $P\notin T$, $m_P\leq 0$ or $\ord_P(y) <0$. In the first case $m_P-\ord_P(f)\leq 0\leq n_P$, obviously. In the second case $n_P>0$, hence for a sufficiently large $r_P$, $m_P-\ord_P(f)\leq r_pn_P$. Since there are only finitely many $P$ adhering to the second cases, it follows that for a sufficiently large $r$, $m_P-\ord_P(f)\leq rn_P$ for each $P\in X$, hence $d\equiv d'$ and  
        $$
            d' \leq rn_P.
        $$
    \end{proof}
    \begin{theorem} (Riemann's theorem)
        Every irreducible plane curve, $C$, has a genus. Moreover, 
        $$
            g(C)\geq 0.
        $$
    \end{theorem}
    \begin{proof}
        Let $z\in K(X)$ be non-constant, set $n:= [K(X):K(z)]$ and let $\tau$ be the smallest integer such that 
        $$
            l_{r(z)_0} \geq rn-\tau 
        $$
        for every $r\geq 0$ (cf. Proposition~\ref{TechnicalGenusLemma}). Then 
        $$
            s(r(z)_0) = rn +1-l_{r(z_0)}\leq \tau + 1
        $$ 
        Since $(s(r(z)_0))_{r\geq0}$ is increasing, it follows that $s(r(z)_0)=\tau+1$ for a sufficiently large $r$. Set $g(C) = \tau +1$. Let $d\in \Z[X]$. There is a $d'\equiv d$ such that for some $r'\geq r$,
        $$
            d' \leq r'(z)_0.
        $$
        It thus follows that 
        $$
            s(d) = s(d') \leq s(r'(z)_0) = g,
        $$
        hence 
        $$
            l_d \geq \deg \ d + 1 - g,
        $$
        hence $C$ has genus $g$ by the minimality of $\tau$.
    \end{proof}

    \begin{corollary}
        If $d_0$ is a divisor on $X$ such that $l_{d_0} = \deg \ d_0 +1 -g(X)$ and $d,d'$ is a pair of divisors on $X$ such that $d\equiv d' \geq d_0$, then 
        $$
            l_d = \deg \ d + 1 -g(X).
        $$
    \end{corollary}
    \begin{proof}
        It suffices to prove that $l_{d'}\leq \deg \ d' +1 -g(X)$. Indeed,  
        \begin{align*}
            l_{d'}&\leq l_{d_0}+\deg \ (d'-d_0) = \deg \ d_0 + 1 -g(X)+\deg \ d' -\deg \ d_0\\ 
            &= \deg\ d'+1-g(X).
        \end{align*}
    \end{proof}
    
    \begin{corollary}\label{GenusViaNonConstantRationalFunction}
        Let $z$ be a non-constant rational function on $X$. Then 
        $$
            g(X) = \deg \ r(x)_0-l_{r(x)_0}+1 = s(r(x)_0)
        $$
        for all sufficiently large $r$.
    \end{corollary}
    \begin{proof}
        This is by construction of genus as the smallest $\tau+1$ where $\tau$ is an integer with $l_{r(x)_0}\geq r\deg \ r(x)_0-\tau$ for all $r$, and indeed, we have also seen that $s(r(x)_0) = \tau +1 =g(X)$ for all sufficiently large $r$.
    \end{proof}

    \begin{corollary}\label{GenusIsObservedForAllLargeDivisors}
        There is an integer $N$ such that for all divisors $d\in \Z[X]$ with $\deg \ d \geq N$, 
        $$
            l_d = \deg \ d + 1 - g(X).
        $$
    \end{corollary}
    \begin{proof}
        Pick a divisor $d_0$ on $X$ such that $l_{d_0} = \deg \ d_0 +1 -g(X)$. Set $N:= \deg \ d_0 + g(X)$. Let $d$ be a divisor on $X$ with $d\geq N$. Then 
        $$
            \deg \ (d-d_0)+1-g(X) \geq \deg \ d_0 + N-\deg \ d_0 + 1 -g(X) = 1 >0. 
        $$ 
        It follows that $l_{d-d_0}>0$, hence $d-d_0+\Div(f)\geq 0$ for some rational function $f$ on $X$ (cf. Lemma~\ref{LVectorSpaceNonZeroImpliesLinEquivToEffectiveDivisor}), hence $d\equiv d+\Div(f) \geq d_0$, hence by the first of the two above corollaries, 
        $$
            l_{d} = \deg \ d +1 -g(X).
        $$
    \end{proof}
    
    We use the above corollary to give a method for calculating the genus of a given curve. Specifically, we use the functoriality of $\mathrm{Set}\ni X \mapsto \Z[X]\in \mathrm{ab}$ and the fact that every curve is birationally equivalent to a plane curve with only ordinary multiple points. 
    \begin{proposition}\label{FormulaForGenusOfCurveWithOnlyOrdinaryMultiplePoints}
        Let $C$ be an irreducible projective plane curve of degree $n$ with only ordinary multiple points. Then 
        $$
            g(C) = g^\ast(C).
        $$ 
    \end{proposition}
    \begin{proof}
        WLOG $z$ intersects $C=V(f)$ in $n := \deg \ C$ distinct points $P_1,\dots,P_n$. Define 
        $$
            E_m := m\sum_1^n P_i - E_C \quad (m\geq 1)
        $$
        and note that 
        $$
            N:= \deg \ E_m = mn - \sum_{P\in C} m_P(C)(m_P(C)-1).
        $$
        Define 
        $$
            A_m := \{ g\in K[x,y,z] : g \text{ is a form of degree } m \text{ adjoint to } C\}.
        $$
        We want to compute the dimension of $A_m$ for large $m$. Consider a $g\in A_m$. Then $m_P(g)\geq m_P(C)-1$ for each $P\in C$. Letting $Q_1,\dots,Q_t$ be the multiple points of $C$. Then 
        \begin{align*}
            \dim& \ V(m,(r_{Q_1}(C))Q_1,\dots,r_{Q_t}(C)Q_t) \\ &= \frac{m(m+3)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2}.
        \end{align*}
        for large $m$ (cf. Theorem~\ref{DimensionOfLinearSubvarietiesOfCurvesOfLargeDegree}). Note the $V(m,(r_{Q_1}(C))Q_1,\dots,r_{Q_t}(C)Q_t)= (A_m\setminus 0)/\sim$, hence for large $m$
        \begin{align*}
            \dim \ A_m &= \frac{m(m+3)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2} +1\\ 
            &= \frac{(m+1)(m+2)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2}.
        \end{align*} 
        Consider the $K$-linear function
        \begin{gather*}
            \rho : A_m \rightarrow L_{E_m}\\
            g\mapsto \frac{\overline{g}}{\overline{z^m}}
        \end{gather*}
        Note that $\rho(g)=0$ if and only if $f\mid g$. We prove that $\rho$ is surjective. Let $t=\frac{a}{b}\in L_{E_m}$. Then 
        $$
            \Div(\frac{a}{b})\geq -E_m = E_C-m\sum_1^n P_i,
        $$ 
        hence 
        $$
            \Div(az^m)=\Div(a)+m\sum_1^n P_i \geq E+\Div(b).
        $$
        We can then by Proposition~\ref{NoethersConditionInTermsOfDivisors} write 
        $$
            az^m = \lambda b+\mu f,
        $$
        hence 
        $$
            t = \frac{a}{b} = \frac{\lambda}{z^m} = \rho(\lambda) \in K(C).
        $$  
        Define 
        \begin{gather*}
            \psi: V(m-n,3)\rightarrow A_m\\
            h\mapsto hf
        \end{gather*}
        Then we get a short exact sequence 
        $$
            \begin{tikzcd}
                0\arrow[r] & V(m-n,3) \arrow[r,"\psi"] & A_m \arrow[r,"\rho"] & L_{E_m} \arrow[r] & 0
            \end{tikzcd}
        $$
        For sufficiently large $m$, we thus have 
        \begin{align*}
            l_{E_m} &= \frac{(m+1)(m+2)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2} - \dim \ V(m-n,3)\\  &=  \frac{(m+1)(m+2)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2} - \frac{(m-n+2)(m-n+1)}{2}\\
            &= \deg \ E_m +1+\left(\frac{(n-1)(n-2)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2}\right)
        \end{align*}
        and since $(\deg \ E_m)_{m\leq 1}$ is a strictly increasing sequence it follows that for sufficiently large $m$, by Corollary~\ref{GenusIsObservedForAllLargeDivisors}, 
        $$
            g(C)= \frac{(n-1)(n-2)}{2}-\sum_{P}\frac{m_P(m_P-1)}{2}.
        $$
    \end{proof}
    \begin{lemma}
        Consider $t :=\frac{x}{y} K(\Pp^1)$. Then 
        $$
            l_{r(t)_0} = r+1.
        $$ 
    \end{lemma}
    \begin{proof}
        Set $P:=[0,1]$. Then $r(t)_0 = rP$, hence $l_{r(t)_0}\leq \deg\ r(t)_0 +1 = r +1$. Note that 
        $$
            \left\{t^{-j} : j\in\{0,\dots,r\} \right\} \subset L_{r(t)_0}
        $$ 
        is linearly independent over $K$, hence $l_{r(t)_0}=r+1$.
    \end{proof}
    \begin{proposition}
        $$
            g(C) = 0 \iff C \cong \Pp^1,
        $$
        i.e. a curve has genus $0$ if and only if it is rational.
    \end{proposition}
    \begin{proof}
        "$\implies$": If $g(C)=0$, then 
        $$
            l(P) \geq \deg \ P+1-g(C) = 1+1-g(C)=2>0
        $$
        for each $P\in X$, hence $C$ is rational by Corollary~\ref{EquivalentFormulationsOfBeingRational}.
        \\
        "$\impliedby$": Set $t := \frac{x}{y}\in K(\Pp^1)$. Then for large $r$, 
        $$
            g(C) = \deg\ r(t)_0 - l_{r(t)_0}+1 = r -(r+1)+1=0
        $$
        by Corollary~\ref{GenusViaNonConstantRationalFunction}.
    \end{proof}
    \begin{corollary}
        For any irreducible projective plane curve $C$,
        $$
            g(C) \leq g^\ast(C).
        $$
    \end{corollary}
    \begin{proof}
        Let $C'$ be a curve birationally equivalent to $C$ whose multiple points are all ordinary. Then 
        $$
            g(C) = g(C') = g^\ast(C') \leq g^\ast(C).
        $$
    \end{proof}
    \begin{corollary}
        $C$ is rational if and only if 
        $$
            \frac{(n-1)(n-2)}{2} = \sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2}.
        $$
    \end{corollary}
    \begin{corollary}\label{DegreeOfCanonicalDivisorComputation}
        With the setup of Proposition~\ref{FormulaForGenusOfCurveWithOnlyOrdinaryMultiplePoints} and its proof,
        \begin{enumerate}
            \item every $h\in L_{E_m}$ can be written as $\frac{\overline{\lambda}}{\overline{z}^m}$ where $h$ is an adjoint of $C$ of degree $m$.
            \item 
            $$
                \deg \ E_{n-3} = 2g(C)-2 \text{ and } l_{E_{n-3}}\geq g(C).
            $$
        \end{enumerate}
    \end{corollary}
    \begin{proof}
        1. This follows from $\rho$ in Proposition~\ref{FormulaForGenusOfCurveWithOnlyOrdinaryMultiplePoints} being surjective.\\
        2. For the first identity we have 
        \begin{align*}
            \deg \ E_{n-3} &= (n-3)(n)-\sum_P m_P(C)(m_P(C)-1)\\ 
            &= (n-1)(n-2)-\left[\sum_P m_P(C)(m_P(C)-1)\right]-2 = 2g(C)-2.
        \end{align*}
        Note that when $m<n$, no form $g$ of degree $m$ but $0$ is divisible by $f$, hence $L_{E_m} = A_m$, hence 
        $$
            l_{E_{n-3}} = \dim \ A_{n-3} \leq \frac{(n-1)(n-2)}{2}-\sum_{P\in C} \frac{m_P(C)(m_P(C)-1)}{2} = g(C).
        $$
    \end{proof}
    \begin{proposition}
        $C$ has genus $1$ if (here we need $\Char \ K \neq 2$) and only if it is birationally equivalent to a non-singular cubic. 
    \end{proposition}
    \begin{proof}
        Suppose $C$ is birationally equivalent to a non-singular cubic $X$. Then 
        $$
            g(C) = g(X)=g^\ast(X) = \frac{(3-1)(3-2)}{2}-\sum_{P}\frac{m_P(X)(m_P(X)-1)}{2} = 1. 
        $$
        Suppose conversely that $g(C) = 1$. In particular, note that $C$ is not rational, since this only happens when $g(C)=0$. Then $l_P= 1$ for each $P\in C$. Then 
        $$
            l_{rP} = \deg \ rP + 1-g(C) = r
        $$
        for each $r\geq 1$. It thus follows that $L(2P)$ has a basis $\{1,v\}$ for some $v\in K(\Pp^1)\setminus K$. Write $\Div(x) = \sum_Q n_Q Q$. Then $n_P\geq -2$. In particular, $n_P\in \{-1,-2\}$. If $n_P = -1$, then $(v^{-1})_0 = 1$, meaning $C$ would be rational. We thus have $n_P = -2$. Then $[K(v) : K] = (v)_\infty =2$. Moreover, there is a basis $\{1,v,w\}$ with $w\notin K(v)$, hence $K(C) = K(v,w)$. Note that there are basis elements $1,v,w,vw,v^2,w^2\in L_{6P}$. We thus have 
        \begin{equation}\label{PolynomialEquation}
            aw^2 + bvw + cw + \underbrace{\sum_0^3 \alpha_iv^i}_{-Q(v)} = 0
        \end{equation}
        For suitable $a,b,c,\alpha_i\in K$ with at least one of these different from $0$. Here $Q\in K[x]$ with $\deg \ Q \leq 3$. Note that since $\pazocal{O}_P(X)$ is a DVR for a suitable $P$,
        $$
            \ord_P(aw^2+bvw + cw)\in \{-6,-5,-3\} \text{ and } \ord_P(Q(v)) \in \{-6,-4,-2,0\},
        $$
        and $\ord_P(aw^2+bvw + cw)= \ord_P(Q(v))$, we have WLOG $a=1$ and that $\deg \ Q = 3$. Replacing $w$ by $w-\frac{1}{2}(bv-c)$, we get that $w^2 = \prod_1^3 (v-\beta_i)$ for some $\beta_i\in K$. If $\beta_1=\beta_2$, then $\left(\frac{w}{v-\beta_1}\right)^2 = v-\beta_3$, hence $v,w\in K\left(\frac{w}{v-\beta_1}\right),$ which would imply $C$ is rational. We thus conclude that $\beta_1,\beta_2,\beta_3$ are distinct. We thus see that 
        $$
            K(C) = K(y^2z-\prod_1^3 (x-\beta_iz))
        $$
        hence $C$ is birationally equivalent to the non-singular curve $y^2z-\prod_1^3 (x-\beta_iz)$.
    \end{proof}
    \begin{corollary}
        A non-singular plane curve of degree $n$ has genus 
        $$
            \frac{(n-1)(n-2)}{2}.
        $$
    \end{corollary}
    \begin{example}
        The above corollary shows that there are plane curves that are not birationally equivalent to a non-singular plane curve. For instance $y^2xz=x^4+y^4$ has a node, i.e. an ordinary double point, hence it has genus is
        $$
            \frac{(4-1)(4-2)}{2} - \frac{2(2-1)}{2}=3-1 = 2 
        $$
        and no $m$ satisfies $\frac{(m-1)(m-2)}{2}=2$, so no non-singular curve can be birationally equivalent to $y^2xz=x^4+y^4$.
    \end{example}
    \begin{corollary}
        Lines an conics are rational. No other non-singular plane curves are rational.
    \end{corollary}
    \begin{corollary}
        Singular irreducible cubics are rational.  
    \end{corollary}
    \begin{proof}
        A singular irreducible cubic has one ordinary multiple point; either a node or a cusp. When it has a node, its genus is 
        $$
            \frac{(3-1)(3-2)}{2}-2(2-1)=1- 1 = 0.
        $$ 
        When it has a cusp its genus is smaller than $0$ by the same argument, hence its genus is also equal to $0$. 
    \end{proof}
    \begin{example}
        {\Large Do some genus computations}
    \end{example}
    \subsubsection{Differentials of a Curve}
    \begin{lemma}
        Let $f\in K[x,y]$ be an affine curve. We define
        \begin{gather*}
            D : \Gamma(f) \rightarrow K(f)\\
            \overline{g} \mapsto g_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}g_y(\overline{x},\overline{y})
        \end{gather*}
        This is a well-defined derivation. 
    \end{lemma}
    \begin{proof}
        Let $\overline{g} = \overline{h}$. Then 
        \begin{align*}
            &g_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}g_y(\overline{x},\overline{y}) - \left(h_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}h_y(\overline{x},\overline{y})\right) \\
            &= (g-h)_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}(g-h)_y(\overline{x},\overline{y})\\
            &= (af)_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}(af)_y(\overline{x},\overline{y})\\
            &= a_x(\overline{x},\overline{y})f(\overline{x},\overline{y})+a(\overline{x},\overline{y})f_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}(a_y(\overline{x},\overline{y})f(\overline{x},\overline{y})+a(\overline{x},\overline{y})f_y(\overline{x},\overline{y}))\\
            &= a(\overline{x},\overline{y})f_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}a(\overline{x},\overline{y})f_y(\overline{x},\overline{y}) = 0.
        \end{align*}
        So $D$ is well-defined. It is clearly $K$-linear. Let $\overline{g},\overline{h}\in \Gamma(f)$. Then 
        \begin{align*}
            D\overline{g}\overline{h} &= (gh)_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}(gh)_y(\overline{x},\overline{y}) \\
            &= (\overline{g}h_x(\overline{x},\overline{y})+g_x(\overline{x},\overline{y})\overline{h})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}(\overline{g}h_y(\overline{x},\overline{y})+g_y(\overline{x},\overline{y})\overline{h})\\
            &= \overline{g}\left(h_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}h_y(\overline{x},\overline{y})\right) + \overline{h}\left(g_x(\overline{x},\overline{y})-\frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}g_y(\overline{x},\overline{y})\right)\\
            &= \overline{g}D\overline{h}+\overline{h}D\overline{g}.
        \end{align*}
    \end{proof}
    \begin{proposition}
        Let $L$ be an algebraic function field in one variable over $K$. 
        \begin{enumerate}
            \item $\Omega(L)$ is a one dimensional vector space over $L$. 
            \item $(\Char \ K = 0)$. If $z\in L$ and $z\notin K$, then $dz$ is a basis for $\Omega(L)$.
        \end{enumerate}
    \end{proposition}
    \begin{proof}
        1. We may assume $L=K(f)$ for some affine plane curve $f\in K[x,y]$. Indeed, $L=K(C)$ for some curve $C$ and any curve is birationally equivalent to an affine plane curve. We may assume $f_y\neq 0$, hence $f\nmid f_y$. Then $f_y(\overline{x},\overline{y})\neq 0$. Note that $d\overline{x}$ and $d\overline{y}$ generate $\Omega(L)$ over $L$. Then 
        $$
            0 = d f(\overline{x},\overline{y}) = f_{x}(\overline{x},\overline{y})d\overline{x} + f_y(\overline{x},\overline{y})d \overline{y}, 
        $$
        hence 
        $$
            d\overline{y} = \frac{-f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})}d\overline{x}.
        $$
        Note that for $D: \Gamma(f)\rightarrow K$ of the prior lemma that 
        $$
            D\overline{x} = x_x(\overline{x},\overline{y}) - \frac{f_x(\overline{x},\overline{y})}{f_y(\overline{x},\overline{y})} x_y(\overline{x},\overline{y}) = 1. 
        $$ 
        By Lemma~\ref{ExtensionToFieldOfFractions} and Lemma~\ref{ExtensionOfDerivationToFieldOfFractions} it follows that $d\neq 0$, hence $\Omega(L)$ is a one dimensional vector space generated by $d\overline{x}$.\\
        2. $\overline{x}\neq 0$, it is transcendental over $K$. Note then that $dz\neq 0$, since for some non-zero $g\in  K[\overline{x}][t]\subset K(\overline{x})[t]$ of minimal degree (in $t$), $g(\overline{x},z)=0$, hence 
        $$
            0 = dg(\overline{x},z) = g_x(\overline{x},z) d\overline{x}+ g_t(\overline{x},z)dz.
        $$
        Since $g\neq 0$, by the assumption that $\Char \ K = 0$, it follows that $g_x,g_t\neq 0$, hence $g_x(\overline{x},z),g_t(\overline{x},z)\neq 0$, implying 
        $$
            dz = \frac{-g_x(\overline{x},z)}{g_y(\overline{x},z)}d\overline{x}\neq 0.
        $$
        It follows that $dz$ is a basis for $\Omega(L)$.
    \end{proof}

    \begin{definition}
        ($\Char\ K = 0$) Let $L$ be an algebraic function field over $K$ in one variable. Let $z,t\in L$ with $t$ non-constant. There is a unique $v\in L$ such that $dz = vdt$. We call $v$ the \emph{derivate of $z$ with respect to $t$}, and denote it by 
        $$
            \frac{dz}{dt} \text{ or when no confusion arises } z'.
        $$  
    \end{definition}

    \begin{proposition}
        Let $L$ be an algebraic function field in one variable over $K$. Let $\pazocal{O}$ be a discrete valuation ring of $L$ with uniformizing parameter $t$. If $f\in\pazocal{O}$, then so is $\frac{df}{dt}$.
    \end{proposition}
    \begin{proof}
        WLOG $L=K(C)$ for some affine curve $C\in K[x,y]$, hence we may assume $\pazocal{O}=\pazocal{O}_P(f)$ with $P=(0,0)$ simple in $C$. Choose $N$ such that $\ord_P(\overline{x}'),\ord_P(\overline{y}')\geq -N$. Let $f\in \pazocal{O}$. Suppose first $f\in K[\overline{x},\overline{y}]$ Then 
        $$
            \ord_P(f') = \ord_P(f_x(\overline{x},\overline{y})\overline{x}' + f_y(\overline{x},\overline{y})\overline{y}')\geq -N.
        $$ 
        Now in general we may write $f = \frac{g}{h}$, with $g,h\in K[\overline{x},\overline{y}]$ and $h(P)\neq 0$. Then 
        \begin{align*}
            \ord_P(f') &= \ord_P\left(\frac{1}{h^2}\left(hg'-gh'\right)\right) = \min(\ord_P(g'),\ord_P(g)+\ord_P(h')) \geq -N
        \end{align*}
        Write 
        $$
            f = \sum_{i<N}\lambda_it^i + t^Nz,
        $$
        for suitably $\lambda_i\in K$ and $z\in \pazocal{O}$. Then 
        $$
            f' = \sum i\lambda_it^{i-1}+zNt^{N-1}+t^Nz'.
        $$
        Since $\ord_P(z')\geq -N$, each term in the above sum is an element of $\pazocal{O}$, hence $f'\in \pazocal{O}$.
    \end{proof}
    \subsubsection{Canonical Divisors}
    Fix a projective plane curve $C$ with nonsingular model $X$. Define $\Omega := \Omega_K(K(C))$. We call the elements of this vector space \emph{the differentials on $X$ or on $C$}. 
    \begin{definition}
        Let $\omega \in \Omega$ and $P$ a place on $X$. Let $t\in \pazocal{O}_P(X)$ be a uniformizing parameter and write $\omega = fdt$ for some $f\in K(X)$. We define the order of $\omega$ at $P$ to be 
        $$
            \ord_P(\omega) : = \ord_P(f).
        $$
    \end{definition} 
    \begin{remark}
        One has to check that this is independent of choice of uniformizing parameter. Let $s$ be another uniformizing parameter. Then $\omega = g dt$ for some $g\in K(X)$. Then 
            $$ 
                \frac{f}{g} = \frac{ds}{dt}\in \pazocal{O}_P(X) \text{ and } \frac{g}{f} =  \frac{dt}{ds}\in \pazocal{O}_P(X),
            $$
        hence $\ord_P(f)=\ord_P(g)$.
    \end{remark}
    When $C$ is a curve of degree $1$ or $2$, we know that Riemann's Theorem holds with equality for each divisor on $C$. The goal of the following lemma is to compute a divisor that will reveal the missing term that makes Riemann's Theorem hold true in the case of $C$ having degree $\geq 3$.  
     \begin{lemma}
        Suppose $C=V(f)$ is a plane curve of degree $n\geq 3$ with only ordinary multiple points. Let $g$ be a plane curve of degree $n-3$. Then 
        $$
            \Div(g)-E_C = \sum_{P\in X} \ord_P(\omega)
        $$
        for some $\omega\in \Omega$.
    \end{lemma}
    \begin{proof}
        WLOG assume $z\ic C = \sum_1^n P_i$ with distinct $P_i$, $[1,0,0]\notin C$ and no tangent at a multiple point passes through $[1,0,0]$.  Set $t := \frac{x}{z}$ and $s := \frac{y}{z}$. Set $f_1 = f_x(t,s,1)$ and $f_2 = f_y(t,s,1).$ Define $E_m := m \sum_1^n \Phi^{-1}(P_i) - E_C$. Choose $\omega := dt$. Note that $\Div(g)-E_C \equiv \Div(h)-E_C$ for any non-zero rational function $h$ on $X$. It thus suffices to prove that 
        $$
            \Div(\omega) = \Div(z^{n-3})-E_C+Div(f_2) = E_{n-3}+\Div(f_2).
        $$
        Since $f_2 = \frac{f_y}{z^{n-1}}$, this is equivalent to showing that 
        $$
            \Div(\omega) -\Div(f_y) = 2\sum_1^n \Phi^{-1}(P_i) - E_C.
        $$ 
        We check that the coefficient on the LHS matches those on the RHS locally (i.e. at each $Q\in X$). Before doing this, note that 
        $$
            0 = df(t,s,1) = f_1dt+f_2ds+f_z(t,s,1)d1 = f_1dt+f_2ds
        $$
        which implies 
        $$
            \omega = \frac{-f_2}{f_1}ds = \frac{-f_y}{f_x}ds. 
        $$
        Then for each $Q\in X$,
        $$
            \ord_Q(\omega) -\ord_Q(f_y) = \ord_Q(ds)-\ord_Q(f_x).
        $$
        We split the comparison of the LHS and RHS into two cases: 
        \begin{enumerate}
            \item Suppose $Q$ is a place centered at a $P_i$. Note that $y$ is a line not through $P_i$ and $z$ is line through $P_i$ not tangent to $C$ at $P_i$. Then $s^{-1}=\frac{z}{y}$ is a uniformizing parameter in $\pazocal{O}_Q(X)$. Moreover,
            $$
                0 = d1 = d ss^{-1} = sds^{-1} + s^{-1}ds \implies ds = -s^2 ds^{-1} \implies \ord_Q(ds) = -2.
            $$
            By Lemma~\ref{TechnicalNonVanishingLemmaOfPartialDerivativeUsedForCanonicalDivisor}, $f_x(P)\neq 0$, hence 
            $\ord_Q(\omega)-\ord_Q(f_y) = -2$, and on the RHS at $Q$, we have $-2\Phi^{-1}(P_i)-(r_Q-1) = -2\Phi^{-1}(P_i)$, hence LHS matches RHS.
            \item Suppose $Q$ is a place centered at $P= [a,b,1]\in C$, we may assume $a=b=0$, since taking differential and derivative is invariant under translation.\\
            \textbf{Suppose $y$ as tangent to $C$ at $P$:} Since $y$ passes through $[1,0,0]$, it follows that $P$ is simple. Then $t$ is a uniformizing parameter of $\pazocal{O}_P(C)$ and $f_y(P)\neq 0$, since its tangent a $P$ is $y= f_x(P)x+f_y(P)y+f_z(P)z$. It thus follows that $\ord_Q(\omega) = \ord_Q(f_y) = 0$ which matches the coefficient at $P$ on the RHS.\\
            \textbf{Suppose $y$ is not tangent to $C$ to at $P$.} Then $s$ is a uniformizing parameter (cf. Lemma~\ref{BlowupTurnsMultiplePointsToSimplePoints}) in $\pazocal{O}_Q(X)$, hence $\ord_Q(ds) = 0$ and $\ord_Q(f_x)= r_Q -1$ (cf. Proposition~\ref{OrderOfPartialDerivativeAtPlace}), which matches the RHS.   
        \end{enumerate}
    \end{proof}
    \begin{lemma}
        Let $\omega,\omega'\in \Omega\setminus 0$, then 
        $$
            \ord_P(\omega') = \ord_P(\omega)+\ord_P(f)
        $$ 
        for some $f\in K(C)\setminus 0$ for each $P\in X$. Hence, $\Div(\omega)\sum_P \ord_P(\omega) P$ is a well-defined divisor.  
    \end{lemma}
    \begin{proof}
        Indeed, $\omega' = f\omega$ for some $f$. Moreover, by the prior lemma there is a differential $\tau$ with $\sum_P \ord_P(\tau) P$ is a well-defined divisor, hence $\Div(\omega) = \Div(\tau)+ \Div(f)$ for some $f$. 
    \end{proof}
    \begin{remark}
        Note that if $\omega$ and $\omega'$ are two non-zero differentials, we have $\Div(\omega) \equiv \Div(\omega)$. We call a divisor of the form $\Div(\omega)$ a \emph{canonical divisor}. 
    \end{remark}
    \begin{corollary}
        Let $W$ be a canonical divisor on $X$. Then $\deg \ W = 2g(X)-2$ and $l_W \geq g(X)$. 
    \end{corollary}
    \begin{proof}
        Note that $E_{n-3} = \Div\left(z^{n-3}\right)-E_C$, hence the result follows from Corollary~\ref{DegreeOfCanonicalDivisorComputation}
    \end{proof}
    \subsubsection{The Riemann-Roch Theorem for Irreducible Projective Plane Curves}
    For the following suppose $C$ has degree greater than or equal to $3$. Fix a canonical divisor of $C$, $W$ say. We aim to find the missing term in Riemann's Theorem. Note that for large divisors this is just $0$, i.e. for large $d$,
    $$
        l_d = \deg \ d + 1 -g(C).
    $$
    \begin{lemma}
        Let $P\in X$ and $d\in \Z[X]$. If $l_d>0$, and $l_{W-d-P}\neq l_{W-d}$, then $l_{d+P} = l_d$. 
    \end{lemma}
    \begin{proof}
        Again WLOG we may assume $C$ has only ordinary multiple points, $P$ is simple, and $C\ic Z = \sum_1^n P_i$ with distinct $P_i$, $P\notin Z$. We may also assume $W = E_{n-3}$ and that $d$ is effective (cf. Lemma~\ref{LVectorSpaceNonZeroImpliesLinEquivToEffectiveDivisor}). Then $L_{W-d} \subset L_W$. By assumption, we may pick an $h\in L_{W-d}\setminus L_{W-d-P}$. We may write $h=\frac{g}{z^{n-3}}$ where $g$ is an adjoint of $C$ of degree $n-3$. Then 
        $$
            \Div(g) = d + E_C + \underbrace{A}_{\Div(g)-(d+E_c)},
        $$
        where $A\geq 0$, since $\Div(g)\geq d+E_C$. Moreover, $Div(g)\not\geq d+E_C+P$, hence $A\not\geq P$. We may pick a line $L$ such that $L\ic C = P+ B$ where $B$ is the sum of $n-2$ distinct simple points. Then 
        $$
            \Div(Lg) = (d+P)+E_C+(A+B).
        $$ 
        Let $t\in L_{d+P}$. Set $d' := \div(t)+d$. We aim to prove that $t\in L_{d}$ or equivalently that $d'$ is effective. Since $d+P\equiv d'+P$, by the Residue Theorem there is a curve $g'$ of degree $n-2$ such that 
        $$
            \Div(g') = (d'+P)+E_C+(A+B).
        $$ 
        Now note that $g'\cap L \geq n-2$, hence $L\subset g'$, hence $g'(P)=0$. Then $\Div(g')\geq P$, and since $P$ has coeffecient $0$ in $E_C+A+B$, hence $d'+P\geq P$, implying $d'\geq 0$. 
    \end{proof}
    \begin{theorem}(Riemann-Roch) For any divisor $d$ on $X$, 
        $$
            l_d = \deg \ d +1 -g(X) + l_{W-d}.
        $$
    \end{theorem}
    \begin{proof}
        Consider first the case where $l_{W-d} = 0$. For this case we prove the theorem by induction in $l_d$.\\ Suppose first that $l_d=0$. Then 
        $$
            0 =l_{d} \geq \deg \ d + 1 - g_C
        $$
        and 
        $$
            0 = l_{W-d} \geq \deg \ W -\deg \ d + 1 -g_C = -(-g_C+1)-\deg \ d \implies 0\leq d+1 -g_C.
        $$ 
        So $l_d = 0 = \deg \ d + 1 - g_C.$\\
        Suppose $l_d = 1$. It then suffices to prove that $\deg \ d -g_C =0$. Since $l_d>0$, we may assume $d\geq 0$. Recall that $l_W \geq g_C$. Note that $W-d \leq W$ implies 
        $$
            \deg W-\deg d -l_{W-d} \leq \deg \ W - l_W \implies l_W \leq \deg \ d,
        $$ 
        hence $g_C\leq \deg \ d$.
        Moreover, by Riemann's Theorem, 
        $$
            1 = l_d \geq \deg \ d + 1 -g_C \implies \deg \ d \leq g_C.
        $$ 
        We thus have $g_C = \deg \ d$.\\
        Suppose $l_d> 1$. We may choose a $P\in X$ such that $l_{d-P}=l_d-1$. The contrapositive of the reduction lemma implies $l_{W-(d-P)} = l_{W-d} = 0$ and by induction 
        $$
            l_d-1=l_{d-P} = \deg \ d -g_C+l_{W-(d-P)} \implies l_d = \deg \ d +1-g_C. 
        $$
        Consider now the case where $l_{W-d}>0$. Suppose first that $l_d=0$. Then in particular, $l_{W-(W-d)} = l_d = 0$, hence by the first case 
        $$
            l_{W-d} = \deg \ W-\deg \ d +1 - g_C+l_{d} = -(\deg-g_C+1)+l_d,
        $$
        which implies $l_d = \deg \ d +1 -g_C+l_{W-d}$. Assume then that $l_d>0$. Note that $\deg \ d \leq \deg \ W = 2g_C -2$ by Lemma~\ref{BasicDivisorVectSpaceLemma} 2. Among these $d$, there is one for which the identity in question is satisfied for $d+P$ each $P\in X$. It suffices to prove that the identity is satisfied for this $d$. Choose $P\in X$ such that $l_{W-(d-P)} = l_{W-d}-1$. Then, by the reduction lemma, $l_{d+P} = l_d$. It follows that 
        $$
            l_{d} = l_{d+P} = \deg \ d +2 -g_C+l_{W-(d-P)} = \deg \ d +1 -g_C + l_{W-d}.
        $$
    \end{proof}
    \begin{corollary}
        $l_W = g_C$. 
    \end{corollary}
    \begin{corollary}
        If $\deg \ d \geq 2g_C -1$, then $l_d = \deg \ d +1 -g_C$. 
    \end{corollary}
    \begin{proof}
        Indeed, in this case $\deg \ W-d <0$, hence $l_{W-d} = 0$ and the result follows from Riemann-Roch.
    \end{proof}
    \begin{corollary}
        If $\deg \ d \geq 2g_C$, then $l_{d-P}=l_{d}-1$ for all $P\in X$. 
    \end{corollary}
    \begin{proof}
        Indeed, $W-(d-P)<0$, hence $l_{W-(d-P)}=0$, hence 
        $$
            l_{d-P}= \deg d -g_C = l_d - 1.
        $$
    \end{proof}
    \begin{corollary}
        Suppose $d$ is a divisor on $X$ with $l_d,l_{W-d}>0$. Then 
        $$
            l_d \leq \frac{\deg \ d }{2}+1.
        $$ 
    \end{corollary}
    \begin{proof}
        We may assume $d,W-d\geq 0$. Set $d' = W-d$ and note that $d+d'=W$. We prove the statement by induction in $\deg \ d$. The case $\deg \ d=0$ is trivial (since in this case $d=0$). Suppose $\deg d>1$. Then we may assume $l_{d} >l_{d-P}$ for each $P\in X$, for otherwise $l_d = l_{d-P} \leq \frac{1}{2}(\deg d -1) + 1$ by induction. Pick $g\in L_d\setminus L_{d-P}$ for each $P\leq d'$. Then we get an injective $K(X)$-linear map 
        \begin{gather*}
            \rho : L_{d'}/L_0 \rightarrow L_W/L_d\\
            h+L_0\mapsto gh + L_d 
        \end{gather*}
        hence rank-nullity implies
        $$
            l_{d'}-1  = \dim \ L_{d'}/L_0 \leq \dim \ L_W/L_d = l_W-l_d = g(C) - l_d. 
        $$ 
        Then by Riemann-Roch 
        \begin{align*}
            l_d \leq g(C)-l_{d'}+1 = g(C)-\deg \ W + \deg \ d -1+g(C)-l_{d'} = \deg \ d +1 -l_{d}  
        \end{align*}
        hence 
        $$
            l_d \leq \frac{1}{2}(\deg \ d +1)\leq \frac{1}{2}\deg \ d +1.  
        $$
    \end{proof}