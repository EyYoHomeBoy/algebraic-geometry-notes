...
\subsection{Some Topology}
    ...
    More generally,
    \begin{lemma}
        if $U$ is an open subvariety of a variety $X$ and $Y$ is a closed subvariety, setting $Z:= \cl_X(Y)$,
        \begin{enumerate}
    ...
    \end{lemma}
    \begin{proof}
        $Y$ is a variety in the closure of $Y$ in $X$, which we denote by $Z$.  Consider the map 
         We already (from general topology) know that embeddings are continuous. Let $U\subset X$ be open.  Let $f\in \Gamma(U)$. This is a rational funtion defined at each $P\in U$, and in particular it is defined at each $P\in \iota^{-1}(U)=U\cap Y$. Since $\widetilde{\iota}(f)$ agrees with $f$ on each point in $Y\cap U$, it follows that $\widetilde{\iota}(f)\in \Gamma(\iota^{-1}(U))$, and consequently $\iota$ is a morphism.  
    \end{proof}
    \begin{lemma}
        The functor 
        \begin{gather*}
...
\subsection{The Study of Curves \& Resolution of Singularities}
    ...
    \subsubsection{Blowing up Points in $\A^2$}
        ...
        Then $\varphi$ is a birational morphism of $C'$ with $C$ by restricting it to $C'\setminus E$. Note $\varphi$ induces an automorphism $K(x,y)=K(C)\simeq K(C')=K(x,z)$, where $y$ is identified with $xz$, i.e $z:= \overline{y}/\overline{x}\in K(C)$
        \begin{lemma}
            Write $C=V(f)$, 
            $$f=\sum_r^n f_i$$
            where $f_i\in K[x,y]$ a form of degree $i$, $r=m_P(C)$ and $n=\deg\ C$. Then $C=V(f')$ where
            $$f' = \sum_r^n x^{r-i}f_i(1,z).$$ 
        \end{lemma}
        \begin{proof}
            If $f'$ is reducible with $f'=gh$, then 
            $$f(x,y)=\widetilde{\varphi}(f(x,xz))=\widetilde{\varphi}(x^rf')=x^rg(x,y/x)h(x,y/x)$$
            but then $f$ would be reducible, leading to contradiction. We conclude that $f'$ is irreducible. Note that if $(a,c)\in C_0'$, then $(a,ac)=\varphi(a,c)\in C$, hence $a\neq 0$ and  
            $$
                a^rf'(a,c) = f(a,c)=0.
            $$
            Therefore, $C_0'\subset V(f')$, hence $C'=V(f')$ by a dimension argument. 
        \end{proof}
        We have thus produced an affine curve $C'$ birationally isomorphic to $C$. We it remains to check that the points of $\varphi^{-1}(P)$ are all simple. In other words, that the multiple point $P$ is blown up into $m_P(C)$ simple points.\\
        ...
        \begin{lemma}
            ...
            $$
            Moreover, if $P$ is an ordinary simple point, then each $P_i$ is a simple point of $C'$ and $\ord_{P_i}^{C'}(x)=1$ (cf. Proposition~\ref{WhenPointIsSimpleOnOneCurveIntersectionEqualsOrderWRTSaidCurveAtPoint})
        \end{lemma}
        \begin{proof}
            Indeed,
           \begin{align*}
           ...
           Moreover,
           \begin{align*}
                m_{P_i}(C') &\leq I(P_i,C'\cap E) = I(P_i,\left(\prod_1^s (z-\alpha_j)^{r_j}\right) \cap x)\\ &= \sum_1^s r_j I(P_i,(z-\alpha_j)\cap x) = r_i.
           \end{align*}
