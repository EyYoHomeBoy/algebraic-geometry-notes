...
\]
    ...
    \end{gather*}
    We further define $\eta := \left. Q\right|_{C_0'} : C_0'\rightarrow C_0$.
    \begin{lemma}
        ...
        $$
            \begin{tikzcd}
                C_\ast' \arrow[r, phantom, "\mathrel{\supset}"] \arrow[d, "\psi"'] &
                [-2em] % <--- gap between col 1 and 2 (shrinks the phantom)
                W' \arrow[r, "\varphi'"] \arrow[d] &
                % <--- gap between col 2 and 3 (leave this “normal”)
    ...
    \begin{lemma}
        ...
        $$m_{P_i}(C')\leq I(P_i,C'\cap L) \text{ and } \sum_{i}I(P_i,f'\cap z) = \sum_i I(P_i,f_r(y,x)\cap z)=r.$$
    \end{lemma}
    \begin{proof}
        
    \end{proof}
    \begin{definition}
        a curve $C$ is in \emph{excellent position}, if it is in good position, $L$ intersects $C$ transversally in $n$ distinct points all different from the fundamental points, and $L'$ and $L''$ intersect $C$ transversally in $n-r$ distinct points all not fundamental.  
    \end{definition}

    \begin{definition}
        For an irreducible projective curve 
    \end{definition}
