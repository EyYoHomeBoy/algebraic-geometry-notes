...
\subsection{The Study of Curves \& Resolution of Singularities}
    ...
    \subsubsection{Blowing up Points in $\A^2$}
        ...
        \end{proof}
        \begin{remark}
            In general, if $U$ is an open neighborhood and $U'$ is an open set in $C'$ containing $\{P_1,\dots,P_s\}$. Construct $W$ as in the above proof and take $g\in \Gamma(W)$ such that $g(P)\neq 0$ and $g(Q)=0$ whenever $Q\in W\setminus U\cup \theta(W'\setminus U')$. Then $W_g$ is an affine open neihborhood of $P$ contained in $U$ and $W'_g= \theta^{-1}(W_g)$ is an affine open set contained in $U'$. We can thus choose $W$ and $W'$ in the above lemma arbitrarily small. 
        \end{remark}
        \begin{remark}
...
\]
    ...
    \begin{definition}
        ...
        Given a sequence of irreducible projective plane curves, $f=f_1,\dots,f_m=g$ such that $f_i$ is a quadratic trannsformation of $f_{i-1}$, then we say that $f$ is \emph{transformed} $g$ via finite sequence of quadratic transformations. 
    \end{definition}
    ...
    \begin{lemma}\label{SecondTechnicalNonsingularModelLemma}
        Let $C$ be a projective plane curvem, $(X,\Phi)$ a nonsingular model of $C$ and $P\in C$. There is an affine neighborhood open $U$ of $P$ such that 
        \begin{enumerate}
    ...
    \begin{corollary}
        ...
        \begin{enumerate}
            \item every $h\in L_{E_m}$ can be written as $\frac{\overline{\lambda}}{\overline{z}^m}$ where $h$ is an adjoint of $C$ of degree $C$.
            \item 
            $$
    ...
    \begin{proposition}
        Suppose $C$ is a plane curve of degree $n\geq 3$ with only ordinary multiple points. Let $g$ be a plane curve of degree $n-3$. 
    \end{proposition}
    \subsubsection{The Riemann-Roch Theorem}
    \begin{theorem}(Riemann-Roch for projective plane curves) Let $W$ be a canonical divisor on $X$. Then for any divisor $d$ on $X$, 
    $$
