...
\subsection{The Study of Curves \& Resolution of Singularities}
    ...
    \subsubsection{Blowing up Points in $\A^2$}
        ...
        \begin{proof}
            ...
            $$
            3. Since $x$ is not tangent to $C'$ at $P_i$ and since each $P_i$ is simple, $\overline{x}$ is the uniformizing parameter for the DVR $\pazocal{O}_{P_i}(C')$. In 2. we found that 
            $$
                f_y(\overline{x},\overline{y}) = \overline{x}^{r-1}u
            $$ 
            where $u$ is a unit in $\pazocal{O}_{P_i}(C')$, hence $\ord(f_y(\overline{x},\overline{y}))=r-1$. {\Large remember refs}.
        \end{proof}
