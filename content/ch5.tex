...
\]
    ...
    \end{lemma}
    \begin{proof}
        Again WLOG we may assume $C$ has only ordinary multiple points, $P$ is simple, and $C\ic Z = \sum_1^n P_i$ with distinct $P_i$, $P\notin Z$. We may also assume $W = E_{n-3}$ and that $d$ is effective (cf. Lemma~\ref{LVectorSpaceNonZeroImpliesLinEquivToEffectiveDivisor}). Then $L_{W-d} \subset L_W$. By assumption, we may pick an $h\in L_{W-d}\setminus L_{W-d-P}$. We may write $h=\frac{g}{z^{n-3}}$ where $g$ is an adjoint of $C$ of degree $n-3$.  
    \end{proof}
